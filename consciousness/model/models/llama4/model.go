...
imageInputs = append(imageInputs, input.Input{Token: 200080})
...
if separator.x {
imageInputs = append(imageInputs, input.Input{Token: 200084})
}
if separator.y {
imageInputs = append(imageInputs, input.Input{Token: 200085})
}
} else {
imageInputs = append(imageInputs, input.Input{Token: 200090})
imageInputs = append(imageInputs, input.Input{Token: 200092, Multimodal: []input.Multimodal{{Tensor: mm.Tensor}}, MultimodalHash: inp.MultimodalHash, SameBatch: patchesPerChunk})
imageInputs = append(imageInputs, slices.Repeat([]input.Input{{Token: 200092}}, patchesPerChunk-1)...)
imageInputs = append(imageInputs, input.Input{Token: 200080})
}
