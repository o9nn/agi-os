...
The following table outlines the properties that are supported in repository-level agent profiles, organization or enterprise level agent profiles, and in VS Code.

Property	Type	Purpose	Repository	Organization / enterprise	VS Code
name	string	Unique name for the custom agent. Optional, if unset defaults to the filename (without .md or .agent.md suffix).
Lowest level configuration takes precedence when there is a naming conflict.			
...
The model, argument-hint, and handoffs properties from VS Code custom agents are currently not supported for Copilot coding agent on GitHub.com. They are ignored to ensure compatibility. For more information on custom agent file structure in VS Code, see Custom agents in VS Code in the VS Code documentation.
...
custom-agent	Task	"Custom agent" tools	Allows a different custom agent to be invoked to accomplish a task.
...
MCP servers can only be configured in custom agent profiles at the organization and enterprise level. Repository-level custom agents do not support MCP server configuration.
