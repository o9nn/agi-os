...
- `tableau`: An ODE RK tableau. Load a tableau by calling a function like `constructRK4` and may be found at https:
"""
function JuMPCollocation end
"""
InfiniteOpt Collocation solver.
- `solver`: an optimization solver such as Ipopt
- `derivative_method`: the method used by InfiniteOpt to compute derivatives. The list of possible options can be found at https:
"""
