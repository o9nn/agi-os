...
@test_broken let # SDEs are currently broken with structural simplify (https:
# Creates the model
@parameters a1 [description="Parameter a1"] a2::Rational{Int64} a3=0.3 a4::Rational{Int64}=4
...
@test getdefault(coupled_rs.a4) == 4
...
@test mtk_struct[a2] == 2
@test mtk_struct[a3] == 0.3
@test mtk_struct[a4] == 4
...
let # SDEs are currently broken with structural simplify (https:
# Creates coupled reactions system.
@parameters p d k1 k2
@species X(t)
@variables A(t)
...
tspan = 500.0
ps = [:i => 0.0001, :r => 0.01, :m1 => 5000.0, :m2 => 9000
