...
#include <asm/irq.h>
#include "ide.h"
#undef DISPLAY_TRITON_TIMINGS
#undef DISPLAY_APOLLO_TIMINGS
#undef DISPLAY_ALI15X3_TIMINGS
...
return 1;
outl(virt_to_bus (HWIF(drive)->dmatable), dma_base + 4);
outb(reading, dma_base);
outb(inb(dma_base+2)|0x06, dma_base+2);
#ifdef CONFIG_BLK_DEV_IDEATAPI
if (drive->media != ide_disk)
return 0;
#endif
ide_set_handler(drive, &dma_intr, WAIT_CMD);
