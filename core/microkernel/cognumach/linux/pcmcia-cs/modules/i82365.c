...
INT_MODULE_PARM(poll_interval, 0);
INT_MODULE_PARM(cycle_time, 120);
INT_MODULE_PARM(do_scan, 1);
...
#ifdef CONFIG_ISA
INT_MODULE_PARM(i365_base, 0x3e0);
INT_MODULE_PARM(extra_sockets, 0);
INT_MODULE_PARM(ignore, -1);
INT_MODULE_PARM(cs_irq, 0);
...
p->irqmux = (p->irqmux & ~0x0f) | 0x02;
if (!(p->sysctl & TI122X_SCR_INTRTIE))
p->irqmux = (p->irqmux & ~0xf0) | 0x20;
break;
case 1:
p->devctl &= ~TI113X_DCR_IMODE_MASK;
