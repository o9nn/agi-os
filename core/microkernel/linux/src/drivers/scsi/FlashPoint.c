...
{
pAdapterInfo->ai_FlashRomSize = 64 * 1024;
}
else
{
pAdapterInfo->ai_FlashRomSize = 32 * 1024;
}
...
WR_HARPOON(ioPort+hp_xfer_pad, i);
...
if (offset & 0x1)
{
temp2.tempw = utilEERead(ioport,(u16bits)((offset - 1) / 2));
temp2.tempb[1] = *pdata;
...
scamInfo[i].state = ID_UNASSIGNED;
}
}else {
for (i=0; i < max_id; i++)
{
...
temp += 0x4242;
utilEEWrite(p_port, 0x6C46, 64/2);
temp += 0x6C46;
utilEEWrite(p_port, 0x7361, 66/2);
temp += 0x7361;
...
index++;
utilEEWrite(p_port, 0x5542, index);
temp += 0x5542;
index++;
...
temp += 0x4349;
index++;
utilEEWrite(p_port, 0x5442, index);
temp += 0x5442;
index++;
...
temp += 0x3339;
index++;
utilEEWrite(p_port, 0x2030, index);
temp += 0x2030;
index++;
