...
#define VM_PAGE_CACHE_ADAPT_INTERVAL 100
#define VM_PAGE_CACHE_MIN_SCALE 50
#define VM_PAGE_CACHE_MAX_SCALE 200
#define VM_PAGE_CACHE_PRESSURE_THRESHOLD 90
...
unsigned long min_free_pages;
unsigned long low_free_pages;
unsigned long high_free_pages;
...
unsigned long cache_scale_percent;
unsigned long adapt_counter;
unsigned long cache_hits;
unsigned long cache_misses;
unsigned long base_high_active_pages;
};
...
if (page != NULL) {
vm_page_seg_remove_inactive_page(seg, page);
vm_page_update_replacement_data(page, FALSE);
*was_active = FALSE;
seg->cache_hits++;
return page;
}
...
vm_pageout_page(page, FALSE, TRUE);
vm_object_unlock(object);
if (double_paging) {
goto restart;
}
