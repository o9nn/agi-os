...
i_stream_create_decompress(test_input, 0);
i_stream_unref(&test_input);
i_stream_seek(input, test_data->used - 14);
test_assert(i_stream_read_more(input, &data, &size) > 0);
...
"\x1F\x8B",
"\x1F\x8B\x01\x02""\xFF\xFF\x01\x01\x01\x01",
"\x1F\x8B\x01\x04""\xFF\xFF\x01\x01\x01\x01",
"\x1F\x8B\x01\x08""\x01\x01\x01\x01\x01\x01",
"\x1F\x8B\x01\x10""\x01\x01\x01\x01\x01\x01",
"\x1F\x8B\x01\x0C""\xFF\xFF\x01\x01\x01\x01",
};
struct istream *file_input, *input;
if (compression_lookup_handler("gz", &gz) <= 0 )
return;
