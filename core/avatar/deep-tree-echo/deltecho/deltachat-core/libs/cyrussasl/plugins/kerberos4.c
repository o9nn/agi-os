...
des_key_schedule enc_keysched;
des_key_schedule dec_keysched;
...
if ((data.time_sec < text->time_sec) ||
(((data.time_sec == text->time_sec) &&
...
if (text->pinst[0]) {
alen += strlen(text->pinst) + 1 ;
}
flag = 0;
if (strcmp(text->realm, text->prealm)) {
alen += strlen(text->prealm) + 1 ;
flag = 1;
}
...
&kerberosv4_common_mech_dispose,
&kerberosv4_common_mech_free,
NULL,
NULL,
NULL,
&kerberosv4_mech_avail,
NULL
}
};
#endif
