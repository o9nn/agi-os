...
iv = calloc(1, ci.blocksize); if( iv == NULL ) { goto cleanup; }
ci.set_iv(&ci, iv);
free(iv);
iv = NULL;
ci.set_crypt_key(&ci, key);
pgp_encrypt_init(&ci);
pgp_decrypt_se_ip(&ci, decr_session_key, encr_session_key, session_key_bytes);
ci.decrypt_finish(&ci);
algo = decr_session_key[0];
if( !pgp_crypt_any(&stream->decrypt, algo)
|| stream->decrypt.keysize != (session_key_bytes-1) ) {
goto cleanup;
}
iv = calloc(1, stream->decrypt.blocksize); if( iv == NULL ) { goto cleanup; }
stream->decrypt.set_iv(&stream->decrypt, iv);
stream->decrypt.set_crypt_key(&stream->decrypt, &decr_session_key[1]);
pgp_encrypt_init(&stream->decrypt);
}
else
{
