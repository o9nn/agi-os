...
def pytest_addoption(parser):
    parser.addoption('--liveconfig', action='store', default=None, help='a file with >=2 lines where each line contains NAME=VALUE config settings for one account')
def pytest_report_header(config, startdir):
    ...
    try:
        ...
        os.remove(t)
    return 'Deltachat core={} rpgp={} openssl={} sqlite={}'.format(info['deltachat_core_version'], info['rpgp_enabled'], info['openssl_version'], info['sqlite_version'])
...
def wait_configuration_progress(account, target):
    while 1:
        evt_name, data1, data2 = account._evlogger.get_matching('DC_EVENT_CONFIGURE_PROGRESS')
        if data1 >= target:
...
def wait_successful_IMAP_SMTP_connection(account):
    ...
    while not imap_ok or not smtp_ok:
        evt_name, data1, data2 = account._evlogger.get_matching('DC_EVENT_(IMAP|SMTP)_CONNECTED')
