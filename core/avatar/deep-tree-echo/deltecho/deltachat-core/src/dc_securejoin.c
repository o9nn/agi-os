...
qr_scan->auth, own_fingerprint, join_vg? qr_scan->text2 : NULL);
...
qr_scan->invitenumber, NULL, NULL);
}
while (1) {
CHECK_EXIT
usleep(300*1000);
...
dc_log_warning(context, 0, "Secure-join denied (invitenumber missing).");
goto cleanup;
}
if (dc_token_exists(context, DC_TOKEN_INVITENUMBER, invitenumber)==0) {
dc_log_warning(context, 0, "Secure-join denied (bad invitenumber).");
goto cleanup;
}
...
context->cb(context, DC_EVENT_CONTACTS_CHANGED, contact_id, 0);
...
dc_add_contact_to_chat_ex(context, group_chat_id, contact_id, DC_FROM_HANDSHAKE);
}
else {
send_handshake_msg(context, contact_chat_id, "vc-contact-confirm",
NULL, NULL, NULL);
context->cb(context, DC_EVENT_SECUREJOIN_INVITER_PROGRESS, contact_id, 1000);
}
}
