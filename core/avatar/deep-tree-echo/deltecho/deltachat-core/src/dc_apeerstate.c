...
sqlite3_bind_blob (stmt, 4, peerstate->public_key? peerstate->public_key->binary : NULL, peerstate->public_key? peerstate->public_key->bytes : 0, SQLITE_STATIC);
sqlite3_bind_int64(stmt, 5, peerstate->gossip_timestamp);
sqlite3_bind_blob (stmt, 6, peerstate->gossip_key? peerstate->gossip_key->binary : NULL, peerstate->gossip_key? peerstate->gossip_key->bytes : 0, SQLITE_STATIC);
...
sqlite3_bind_blob (stmt, 9, peerstate->verified_key? peerstate->verified_key->binary : NULL, peerstate->verified_key? peerstate->verified_key->bytes : 0, SQLITE_STATIC);
...
{
peerstate->to_save  |= DC_SAVE_ALL;
if (old_public_fingerprint && old_public_fingerprint[0]) {
...
{
peerstate->to_save  |= DC_SAVE_ALL;
if (old_gossip_fingerprint && old_gossip_fingerprint[0]) {
peerstate->degrade_event |= DC_DE_FINGERPRINT_CHANGED;
}
}
}
