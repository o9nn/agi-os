...
NULL , NULL ,
to, NULL , NULL , dc_strdup(factory->rfc724_mid), in_reply_to_list,
references_list ,
NULL );
...
mailimf_fields_add(imf_fields, mailimf_field_new_custom(strdup("Chat-Version"), strdup("1.0")));
if (factory->req_mdn) {
...
mailimf_fields_add(imf_fields, mailimf_field_new_custom(strdup("Secure-Join"), step));
...
mailimf_fields_add(imf_fields, mailimf_field_new_custom(strdup("Chat-Group-Image"), filename_as_sent));
}
dc_msg_unref(meta);
}
...
char* fwdhint = NULL;
if (afwd_email) {
fwdhint = dc_strdup("---------- Forwarded message ----------" LINEEND "From: Delta Chat" LINEEND LINEEND);
}
...
p1 = dc_stock_str(factory->context, DC_STR_ENCRYPTEDMSG);
}
else {
...
"Disposition: manual-action/MDN-sent-automatically; displayed" LINEEND,
DC_VERSION_STR,
factory->from_addr,
factory->from_addr,
