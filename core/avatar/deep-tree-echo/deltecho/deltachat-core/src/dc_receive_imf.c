...
uint32_t row_id = dc_add_or_lookup_contact(context, display_name_dec , addr_spec, origin, NULL);
...
{
if (context==NULL || context->magic!=DC_CONTEXT_MAGIC || adr_list==NULL ) {
return;
}
...
struct mailimf_mailbox* mb = adr->ad_data.ad_mailbox;
if (mb) {
add_or_lookup_contact_by_addr(context, mb->mb_display_name, mb->mb_addr_spec, origin, ids, check_self);
}
}
...
"   AND c.type=" DC_STRINGIFY(DC_CHAT_TYPE_GROUP)
"   AND cc.contact_id!=" DC_STRINGIFY(DC_CONTACT_ID_SELF)
...
&& grpid
&& grpname
&& X_MrRemoveFromGrp==NULL
&& (!group_explicitly_left || (X_MrAddToGrp&&dc_addr_cmp(self_addr,X_MrAddToGrp)==0))
)
{
...
dc_param_set(chat->param, DC_PARAM_PROFILE_IMAGE, grpimage);
...
{
uint32_t to_id = dc_array_get_id(to_ids, i);
...
{
if (dc_array_get_cnt(from_list)>=1)
{
...
{
struct mailimf_to* fld_to = field->fld_data.fld_to;
if (fld_to)
{
dc_add_or_lookup_contacts_by_address_list(context, fld_to->to_addr_list ,
...
if (chat_id==0)
{
int create_blocked = ((test_normal_chat_id&&test_normal_chat_id_blocked==DC_CHAT_NOT_BLOCKED) || incoming_origin>=DC_ORIGIN_MIN_START_NEW_NCHAT)? DC_CHAT_NOT_BLOCKED : DC_CHAT_DEADDROP_BLOCKED;
...
dc_scaleup_contact_origin(context, from_id, DC_ORIGIN_INCOMING_REPLY_TO);
...
calc_timestamps(context, chat_id, from_id, sent_timestamp, (flags&DC_IMAP_SEEN)? 0 : 1 ,
...
icnt = carray_count(mime_parser->parts);
stmt = dc_sqlite3_prepare(context->sql,
...
&& clist_count(report_root->mm_data.mm_multipart.mm_mp_list) >= 2 )
{
if (mdns_enabled )
{
