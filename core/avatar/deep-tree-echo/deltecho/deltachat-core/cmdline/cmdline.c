...
if (bits & 8) {
dc_sqlite3_execute(context->sql, "DELETE FROM contacts WHERE id>" DC_STRINGIFY(DC_CONTACT_ID_LAST_SPECIAL) ";");
...
real_spec = dc_sqlite3_get_config(context->sql, "import_spec", NULL);
if (real_spec == NULL) {
...
if (dc_poke_eml_file(context, real_spec)) {
read_cnt++;
}
}
...
if (dc_poke_eml_file(context, path_plus_name)) {
read_cnt++;
}
free(path_plus_name);
}
