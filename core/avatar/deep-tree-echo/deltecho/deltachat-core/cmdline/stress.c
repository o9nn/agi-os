...
"p2flgJL48kY32hEw2GRPri64Tv3vMPIWa
...
"uZzP941mOUV5UeTCCbjpyfI7qtIi3TH1cQmC2kG2HrvQYuM6Momp
...
dc_saxparser_parse(&saxparser, "<tag attr=val=");
dc_saxparser_parse(&saxparser, "<tag attr=\"val\"=");
}
{
...
assert( strcmp(of->fld_value, "Bar")==0 );
...
char* str = strdup("aaa");
int replacements = dc_str_replace(&str, "a", "ab");
...
clist_free(list);
free(str);
assert( strcmp("fresh=" DC_STRINGIFY(DC_STATE_IN_FRESH), "fresh=10")==0 );
...
assert( buf2[0]==0 );
free(buf1); free(buf2);
dc_replace_bad_utf8_chars(NULL);
...
assert( dc_array_get_id(arr, -1) == 0 );
assert( dc_array_get_id(arr, TEST_CNT) == 0 );
assert( dc_array_get_id(arr, TEST_CNT+1) == 0 );
dc_array_empty(arr);
assert( dc_array_get_cnt(arr) == 0 );
...
assert( dc_param_get_int(p1, 'c', 0)==0 );
...
assert( ah_ok == 1 && ah->prefer_encrypt==DC_PE_NOPREFERENCE );
...
ah_ok = dc_aheader_set_from_string(ah, " ;;");
assert( ah_ok == 0 );
ah_ok = dc_aheader_set_from_string(ah, "addr=a@t.de; unknwon=1; keydata=jau");
assert( ah_ok == 0 );
dc_aheader_unref(ah);
free(rendered);
}
...
assert( ((char*)ctext_signed)[ctext_signed_bytes-1]!=0 );
...
dc_hash_init(&valid_signatures, DC_HASH_STRING, 1);
int ok;
ok = dc_pgp_pk_decrypt(context, ctext_signed, ctext_signed_bytes, keyring, public_keyring, 1, &plain, &plain_bytes, &valid_signatures);
...
ok = dc_pgp_pk_decrypt(context, ctext_signed, ctext_signed_bytes, keyring, NULL, 1, &plain, &plain_bytes, &valid_signatures);
assert( ok && plain && plain_bytes>0 );
assert( strncmp((char*)plain, original_text, strlen(original_text))==0 );
assert( dc_hash_cnt(&valid_signatures) == 0 );
free(plain); plain = NULL;
dc_hash_clear(&valid_signatures);
ok = dc_pgp_pk_decrypt(context, ctext_signed, ctext_signed_bytes, keyring, public_keyring2, 1, &plain, &plain_bytes, &valid_signatures);
assert( ok && plain && plain_bytes>0 );
assert( strncmp((char*)plain, original_text, strlen(original_text))==0 );
assert( dc_hash_cnt(&valid_signatures) == 0 );
free(plain); plain = NULL;
...
free(plain); plain = NULL;
dc_hash_clear(&valid_signatures);
ok = dc_pgp_pk_decrypt(context, ctext_unsigned, ctext_unsigned_bytes, keyring, public_keyring, 1, &plain, &plain_bytes, &valid_signatures);
assert( ok && plain && plain_bytes>0 );
assert( strncmp((char*)plain, original_text, strlen(original_text))==0 );
free(plain); plain = NULL;
dc_hash_clear(&valid_signatures);
...
dc_keyring_add(public_keyring, public_key);
void* plain = NULL;
int ok = dc_pgp_pk_decrypt(context, ctext_signed, ctext_signed_bytes, keyring, public_keyring, 1, &plain, &plain_bytes, NULL);
assert( ok && plain && plain_bytes>0 );
