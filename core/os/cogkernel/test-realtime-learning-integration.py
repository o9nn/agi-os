...
def test_module_structure(filepath, required_exports):
    ...
    with open(filepath, 'r') as f:
        content = f.read()
    if not re.search('define-module\\s+\\(', content):
        print(f'✗ Module definition not found in {filepath}')
        return False
    export_section = re.search('#:export\\s+\\((.*?)\\)', content, re.DOTALL)
...
def test_learning_system_implementation():
    ...
    tests = [(f'{base_path}/cognitive-interface/learning-systems/realtime.scm', 'Real-time learning system module'), (f'{base_path}/test-realtime-learning.scm', 'Learning system test suite'), (f'{base_path}/cognitive-interface/learning-systems/README.md', 'Learning system documentation'), (f'{base_path}/atomspace.scm', 'AtomSpace module'), (f'{base_path}/attention.scm', 'Attention module'), (f'{base_path}/cognitive-interface.scm', 'Cognitive interface module')]
    passed = 0
    total = len(tests)
    ...
    learning_exports = ['make-learning-system', 'learning-system?', 'learn-from-experience', 'pattern-recognition', 'adapt-behavior', 'evaluate-learning-effectiveness', 'advanced-pattern-learning', 'q-learning-update', 'temporal-difference-learning', 'experience-replay', 'monitor-learning-effectiveness', 'enhanced-continuous-learning-loop', '*global-learning-system*', 'current-time']
    module_tests = [(f'{base_path}/cognitive-interface/learning-systems/realtime.scm', learning_exports)]
    module_passed = 0
...
def test_atomspace_integration():
    ...
    with open(atomspace_file, 'r') as f:
        content = f.read()
    required_types = ['EXPERIENCE', 'PATTERN', 'STATE-VALUE', 'POLICY', 'META-PATTERN']
    missing_types = []
...
def test_documentation_completeness():
    ...
    with open(doc_file, 'r') as f:
        content = f.read()
    required_sections = ['# Real-time Learning Systems', '## Overview', '## Key Features', '## Core Components', '## Usage Examples', '## Advanced Features', '## Integration with SKZ Framework', '## Learning Types and Algorithms', '## Performance and Monitoring', '## Testing']
    missing_sections = []
...
def test_demo_integration():
    ...
    learning_functions = ['create-learning-experience', 'learn-from-experience', 'pattern-recognition', 'advanced-pattern-learning', 'adapt-behavior', 'evaluate-learning-effectiveness', 'experience-replay', 'temporal-difference-learning', 'q-learning-update']
    missing_functions = []
...
def main():
    ...
    print('=' * 55)
    tests = [test_learning_system_implementation, test_atomspace_integration, test_attention_integration, test_cognitive_interface_integration, test_documentation_completeness, test_demo_integration]
    passed = 0
    total = len(tests)
    for test in tests:
        try:
