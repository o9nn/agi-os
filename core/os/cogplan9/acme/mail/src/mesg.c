...
sprint(as+strlen(as), "%.3s %.3s ", fld[1], fld[2]);
else
sprint(as+strlen(as), "%.3s %.3s ", fld[2], fld[1]);
if(thisyear(fld[5])){
if(strchr(fld[3], ':') != nil)
sprint(as+strlen(as), "%.5s ", fld[3]);
else if(strchr(fld[4], ':') != nil)
sprint(as+strlen(as), "%.5s ", fld[4]);
}else
sprint(as+strlen(as), "%.4s ", fld[5]);
}
free(s);
return as;
}
...
{
char *buf, *p, *q;
buf = emalloc(strlen(prefix)+2*strlen(s)+50);
p = buf;
*p++ = '0';
*p++ = '/';
*p++ = '^';
...
if(winsetaddr(w, buf, 1) && winsetaddr(w, "+0-#1", 1))
