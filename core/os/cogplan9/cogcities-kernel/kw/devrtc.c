...
if(t & Rtc12){
rtc.hour = bcd2dec((t & Rtchr12) >> Rtchrs) - 1;
if(t & Rtcpm)
rtc.hour += 12;
}else
rtc.hour = bcd2dec((t & Rtchr24) >> Rtchrs);
rtc.mday = bcd2dec(d & Rdmday);
rtc.mon = bcd2dec((d & Rdmon) >> Rdms);
rtc.year = bcd2dec((d & Rdyear) >> Rdys) + 2000;
return rtc2sec(&rtc);
}
long
rtctime(void)
