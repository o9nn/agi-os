...
static int aes_setupEnc(ulong rk[], const uchar cipherKey[],
int keyBits);
static int aes_setupDec(ulong rk[], const uchar cipherKey[],
int keyBits);
static int aes_setup(ulong erk[], ulong drk[],
...
p2 = q+len;
if(len == AESbsize)
mackey = s->mackey + AESbsize;
else{
mackey = s->mackey+2*AESbsize;
*p2++ = 1 << 7;
len = AESbsize - len - 1;
memset(p2, 0, len);
}
...
static int
aes_setupEnc(ulong rk[], const uchar cipherKey[], int keyBits)
{
int i = 0;
u32 temp;
...
static int
aes_setupDec(ulong rk[], const uchar cipherKey[], int keyBits)
{
int Nr, i, j;
ulong temp;
...
void
aes_encrypt(const ulong rk[], int Nr, const uchar pt[16],
uchar ct[16])
{
...
void
aes_decrypt(const ulong rk[], int Nr, const uchar ct[16],
uchar pt[16])
{
ulong s0, s1, s2, s3, t0, t1, t2, t3;
#ifndef FULL_UNROLL
...
static void
aes_encryptRound(const u32 rk[], int Nr, u8 block[16],
int rounds)
{
int r;
...
static void
aes_decryptRound(const u32 rk[], int Nr, u8 block[16],
int rounds)
{
int r;
u32 s0, s1, s2, s3, t0, t1, t2, t3;
