...
gs_wfInfo.seekable = (input_format != sf_raw);
...
if (!strcmp(inPath, "-")) {
lame_set_stream_binary_mode(musicin = stdin);
}
else {
...
double  flen = lame_get_file_size(inPath);
if (flen >= 0) {
if (input_format == sf_mp1 ||
input_format == sf_mp2 || input_format == sf_mp3) {
