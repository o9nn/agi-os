...
ioctl_parm( (unsigned int)SIxMR_SDM_NORMAL_OPERATION );
break;
case AUTOMATIC_ECHO:
D( printk("mcc2_ioctl: ioctl set AUTOMATIC_ECHO mode\n"); )
ioctl_parm( (unsigned int)SIxMR_SDM_AUTOMATIC_ECHO );
break;
case INTERNAL_LOOPBACK:
D( printk("mcc2_ioctl: ioctl set INTERNAL_LOOPBACK mode\n"); )
ioctl_parm( (unsigned int)SIxMR_SDM_INTERNAL_LOOPBACK );
break;
case LOOPBACK_CONTROL:
D( printk("mcc2_ioctl: ioctl set LOOPBACK_CONTROL mode\n"); )
ioctl_parm( (unsigned int)SIxMR_SDM_LOOPBACK_CONTROL );
break;
default:
