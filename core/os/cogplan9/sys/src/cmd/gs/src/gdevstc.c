...
# include <signal.h>
#endif
private stc_proc_dither(stc_gscmyk);
private stc_proc_dither(stc_hscmyk);
#include <stdlib.h>
...
private stc_proc_iconvert(stc_any_depth);
private stc_proc_iconvert(stc_rgb24_long);
private stc_proc_iconvert(stc_cmyk32_long);
private stc_proc_iconvert(stc_any_direct);
private stc_proc_iconvert(stc_cmyk10_byte);
private stc_proc_iconvert(stc_cmyk10_long);
private stc_proc_iconvert(stc_cmyk10_float);
private stc_proc_iconvert(stc_cmyk10_dbyte);
private stc_proc_iconvert(stc_cmyk10_dlong);
...
{
sd->stc.escp_u = (int)(3600.0 / sd->y_pixels_per_inch);
sd->stc.escp_h = (int)(3600.0 / sd->x_pixels_per_inch);
...
if(NULL != (bp = gs_malloc(sd->memory, need,1,"stcolor/init"))) {
...
if((sd->stc.flags & STCRELEASE) == 0) {
int need = 3;
byte *bp = (byte *) (sd->stc.escp_release.data);
if(need != sd->stc.escp_release.size) {
if(NULL != (bp = gs_malloc(sd->memory, need,1,"stcolor/release"))) {
...
stc_print_setup(sd);
for(i = 0; i < sd->color_info.num_components; ++i) {
if((sd->stc.sizc[i] > 1) && (sd->stc.extc[i] != NULL)) {
...
if((sd->stc.sizv[i] > 1) && (sd->stc.extv[i] != NULL)) {
for(j = 0; j < i; ++j)
...
j = j % (32*countof(sd->stc.white_run));
...
} else if((sd->stc.flags & STCMODEL) == STCSTCII) {
sd->stc.flags |= STCNWEAVE;
}
