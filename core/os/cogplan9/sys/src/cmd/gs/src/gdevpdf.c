...
struct tm tms;
time_t t;
char buf[1+2+4+2+2+2+2+2+2+1+1];
time(&t);
tms = *localtime(&t);
sprintf(buf,
...
#if 0
memcpy(pdev->fileID, "xxxxxxxxxxxxxxxx", sizeof(pdev->fileID));
#endif
return 0;
}
...
pdev->sbstack_size = count_of(pdev->vgstack);
...
if ((page == NULL && pdev->params.AutoRotatePages == arp_All) ||
(page != NULL && page->text_rotation.Rotate >= 0) ||
dsc_orientation >= 0 ) {
