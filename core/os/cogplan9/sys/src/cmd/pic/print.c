...
dotext(p);
if (ishor(m))
move(isright(m) ? x1 : x0, oy);
else
move(ox, isdown(m) ? y0 : y1);
break;
case BLOCKEND:
break;
case CIRCLE:
...
void dotbox(double x0, double y0, double x1, double y1, int ddtype, double ddval)
{
