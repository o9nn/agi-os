...
void makeoattr(int type, obj *o)
{
YYSTYPE val;
val.o = o;
makeattr(type, 0, val);
}
void makeiattr(int type, int i)
{
YYSTYPE val;
val.i = i;
makeattr(type, 0, val);
}
void maketattr(int sub, char *p)
{
YYSTYPE val;
val.p = p;
makeattr(TEXTATTR, sub, val);
}
void addtattr(int sub)
{
attr[nattr-1].a_sub |= sub;
}
void makevattr(char *p)
{
YYSTYPE val;
val.p = p;
...
obj *makebetween(double f, obj *p1, obj *p2)
{
obj *p;
dprintf("fraction = %.2f\n", f);
p = makenode(PLACE, 0);
...
double getblkvar(obj *p, char *s)
{
YYSTYPE y;
y = getblk(p, s);
return y.f;
}
obj *getblock(obj *p, char *s)
{
YYSTYPE y;
y = getblk(p, s);
return y.o;
}
YYSTYPE getblk(obj *p, char *s)
{
static YYSTYPE bug;
struct symtab *stp;
if (p->o_type != BLOCK) {
...
void extreme(double x, double y)
{
if (x > xmax)
xmax = x;
if (y > ymax)
