...
blue = allocimage(display, Rect(0,0,1,1), screen->chan, 1, 0x008888FF);
...
if(eqpt(enddate, ZP)){
enddate = r.min;
enddate.x += stringwidth(datefont, "Wed May 30 22:54");
enddate.x += Facesep;
}
r.max.x = enddate.x;
