...
ulBeginSectInfo = ulGetLong(0xca, aucHeader);
NO_DBG_HEX(ulBeginSectInfo);
tSectInfoLen = (size_t)ulGetLong(0xce, aucHeader);
NO_DBG_DEC(tSectInfoLen);
if (tSectInfoLen < 4) {
...
tHdrFtrInfoLen = (size_t)ulGetLong(0xf6, aucHeader);
...
ulBeginLfoInfo = ulGetLong(0x2ea, aucHeader);
DBG_HEX(ulBeginLfoInfo);
tLfoInfoLen = (size_t)ulGetLong(0x2ee, aucHeader);
DBG_DEC(tLfoInfoLen);
if (tLfoInfoLen == 0) {
...
aucLfoInfo = xfree(aucLfoInfo);
ulBeginLstfInfo = ulGetLong(0x2e2, aucHeader);
DBG_HEX(ulBeginLstfInfo);
tLstfInfoLen = (size_t)ulGetLong(0x2e6, aucHeader);
