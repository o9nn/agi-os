...
tDocument.ucHdrFtrSpecification = (UCHAR)(usTmp >> 8);
tDocument.usDefaultTabWidth = usGetWord(0x0a, aucBuffer);
...
ulBeginSectInfo = ulGetLong(0x7c, aucHeader);
DBG_HEX(ulBeginSectInfo);
tSectInfoLen = (size_t)usGetWord(0x80, aucHeader);
...
tHdrFtrInfoLen = (size_t)usGetWord(0x9e, aucHeader);
...
tParfInfoLen = (size_t)usGetWord(0xaa, aucHeader);
...
aucBuffer = xfree(aucBuffer);
tParfPageNum = (size_t)usGetWord(0x144, aucHeader);
DBG_DEC(tParfPageNum);
if (tLen < tParfPageNum) {
tLenOld = tLen;
usParfFirstPage = usGetWord(0x140, aucHeader);
...
tCharInfoLen = (size_t)usGetWord(0xa4, aucHeader);
...
DBG_HEX(ulGetLong(0, aucBuffer));
aucBuffer = xfree(aucBuffer);
tCharPageNum = (size_t)usGetWord(0x142, aucHeader);
DBG_DEC(tCharPageNum);
if (tLen < tCharPageNum) {
tLenOld = tLen;
usCharFirstPage = usGetWord(0x13e, aucHeader);
