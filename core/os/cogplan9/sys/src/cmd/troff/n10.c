...
static char *parse(char *s, int typeit)
{
...
} else if (k >= t.tfont.nchars) {
oputs(chname(k)+1);
} else if (t.tfont.wp[k].str == 0) {
oputs(chname(k)+1);
} else if (t.tfont.wp[k].str[0] == MBchar) {
oputs(t.tfont.wp[k].str+1);
} else {
int oj = j;
codep = t.tfont.wp[k].str+1;
while (*codep != 0) {
if (*codep & 0200) {
codep = plot(codep);
oput(' ');
