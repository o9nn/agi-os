...
void halfrange(Obj *p, int side, double val)
{
...
double setvar(Obj *p, double f)
{
if (strcmp(p->name, "pointsize") == 0) {
pointsize = f;
ps_set = 1;
}
p->type = VARNAME;
...
Point makepoint(Obj *s, double x, double y)
{
Point p;
...
Attr *makefattr(int type, double fval)
{
return makeattr(type, fval, (char *) 0, 0, 0);
}
Attr *makesattr(char *s)
{
