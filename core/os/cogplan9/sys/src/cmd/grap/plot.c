...
void plot(Attr *sl, Point pt)
{
fprintf(tfd, "%s at %s\n", slprint(sl), xyname(pt));
range(pt);
freeattr(sl);
}
void plotnum(double f, char *fmt, Point pt)
{
char buf[100];
if (fmt) {
sprintf(buf, fmt, f);
...
void drawdesc(int type, Obj *p, Attr *desc, char *s)
{
p->attr = desc;
p->attr->sval = s;
if (type == NEW) {
...
void next(Obj *p, Point pt, Attr *desc)
{
char *s;
if (p->first == 0) {
p->first++;
