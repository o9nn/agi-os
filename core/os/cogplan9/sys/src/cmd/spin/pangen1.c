...
fprintf(tc, ", (short) Air%d", i);
if (nclaims > 1)
{	fprintf(tc, "\n#ifndef NOCLAIM\n");
fprintf(tc, "	, (short) Air%d", i+1);
fprintf(tc, "\n#endif\n\t");
}
fprintf(tc, " };\n");
...
fprintf(th, "	unsigned _pid : 8; \n");
fprintf(th, "	unsigned _t   : %d; \n",
blog(nrRdy+1));
fprintf(th, "	unsigned _p   : %d; \n",
i);
fprintf(th, "	unsigned _n   : %d; \n",
blog(nclaims));
if (i <= 255)
{	fprintf(th, "	uchar c_cur[NCLAIMS]; \n");
} else if (i <= 65535)
{	fprintf(th, "	ushort c_cur[NCLAIMS]; \n");
} else
{	fprintf(th, "	uint c_cur[NCLAIMS]; \n");
}
fprintf(th, "} P%d;\n", nrRdy+1);
fprintf(th, "uchar spin_c_typ[NCLAIMS]; \n");
...
fprintf(tc, "		channels[j]->modified = 1; \n");
fprintf(tc, "	#endif\n");
...
ntimes(tc, 1, u_sync+u_async+1, R3);
...
fprintf(th, "uchar reached%d[3];  \n", i);
fprintf(th, "uchar *loopstate%d;  \n", i);
fprintf(th, "#define nstates%d	3 \n", i);
fprintf(th, "#define endstate%d	2 \n\n", i);
fprintf(th, "#define start%d	0 \n", i);
fprintf(tc, "\tcase %d:	\n", i);
if (separate == 1)
