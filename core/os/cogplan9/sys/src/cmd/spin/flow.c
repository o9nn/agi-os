...
add_seq(nn(a3, ASGN, a3, a5));
open_seq(0);
add_seq(nn(ZN, 'c', nn(a3, LE, a3, a8), ZN));
}
Lextok *
...
z3 = nn(ZN, NAME, ZN, ZN);
z3->sym = tmp_cnt;
add_seq(nn(z3, ASGN, z3, z1));
open_seq(0);
add_seq(nn(ZN, 'c', nn(z3, LT, z3, z2), ZN));
in_for = 0;
add_seq(nn(a5, 'r', a5, expand(a3, 1)));
add_seq(nn(a5, 's', a5, expand(a3, 1)));
in_for = 1;
return z3;
} else
{ if (a5->sym->isarray == 0
...
add_seq(nn(a3, ASGN, a3, a5));
open_seq(0);
add_seq(nn(ZN, 'c', nn(a3, LT, a3, a7), ZN));
return for_body(a3, 0);
}
static void
