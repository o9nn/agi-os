...
"#endif",
"",
"double OneSecond   = (double) (ONESECOND); ",
"double TenSeconds  = 10. * (ONESECOND);    ",
"",
"",
"double Delay       = ((double) SHORT_T) * (ONESECOND);	",
"double OneHour     = ((double) LONG_T) * (ONESECOND);	",
"",
...
"",
"struct SM_frame {			",
"	volatile int	m_vsize;	",
"	volatile int	m_boq;		",
"#ifdef FULL_TRAIL",
"	volatile struct Stack_Tree *m_stack;	",
"#endif",
"	volatile uchar	m_tau;",
"	volatile uchar	m_o_pm;",
"	volatile int	nr_handoffs;	",
...
"char	o_cmdline[512];		",
"",
"int	iamin[CS_NR+NCORE];		",
"",
"#if defined(WIN32) || defined(WIN64)",
"int tas(volatile LONG *);",
"",
"HANDLE		proxy_handle_snd;	",
"",
"struct sh_Allocater {			",
"	volatile char	*dc_arena;	",
"	volatile long	 pattern;	",
"	volatile long	 dc_size;	",
"	volatile void	*dc_start;	",
"	volatile void	*dc_id;		",
"	volatile sh_Allocater *nxt;	",
"};",
"DWORD		worker_pids[NCORE];	",
"HANDLE		worker_handles[NCORE];	",
"void *		shmid      [NR_QS];	",
"void *		shmid_M;		",
"",
"#ifdef SEP_STATE",
...
"	volatile long	 pattern;	",
"	volatile long	 dc_size;	",
"	volatile char	*dc_start;	",
"	volatile int	dc_id;		",
"	volatile sh_Allocater *nxt;	",
"};",
"",
"int	worker_pids[NCORE];	",
"int	shmid      [NR_QS];	",
"int	nibis = 0;		",
"int	shmid_M;		",
"#ifdef SEP_STATE",
...
"#else",
"	int	shmid_S;	",
"	volatile sh_Allocater	*first_pool;	",
...
"",
"static SM_frame cur_Root;		",
"",
"SM_frame	*m_workq   [NR_QS];	",
"char		*shared_mem[NR_QS];	",
"#ifdef SEP_HEAP",
"char		*my_heap;",
"long		 my_size;",
"#endif",
"volatile sh_Allocater	*dc_shared;	",
"",
...
"volatile int *prmax;",
"",
"volatile int	*sh_lock;	",
"volatile double *is_alive;	",
"volatile int    *grfree, *grfull, *grcnt, *grmax;	",
"volatile double *gr_readmiss, *gr_writemiss;",
"static   int	lrfree;		",
"static   int dfs_phase2;",
"",
"void mem_put(int);		",
"void mem_put_acc(void);	",
"void mem_get(void);		",
...
"	uchar	      pr;	",
"	T_ID	    t_id;	",
"	volatile struct Stack_Tree *prv; ",
"} Stack_Tree;",
"",
"struct H_el *grab_shared(int);",
"volatile Stack_Tree **stack_last; ",
"char *stack_cache = NULL;	",
"int  nr_cached = 0;		",
"",
"#ifndef CACHE_NR",
...
"		}",
"",
"		shared_mem[m] = (char *) shmat(shmid[m], (void *) 0, 0);	",
...
"	}",
"",
"	rval = (char *) shmat(shmid_S, (void *) 0, 0);	",
"	if ((char *) rval == (char *) -1)",
...
"",
"static uchar *",
"prep_state_mem(size_t n)		",
"{	char	*rval;",
"	key_t	key;",
"	static int cnt = 3;		",
"",
"	shm_prep_result = NOT_AGAIN;	",
"	if (verbose && core_id == 0)",
...
"		return NULL;",
"	}",
"	rval = (char *) shmat(shmid_M, (void *) 0, 0);	",
"",
"	if ((char *) rval == (char *) -1)",
...
"	}",
"	#ifndef BITSTATE",
"		H_tab = (struct H_el **) prep_shmid_S((size_t) n);	",
"	#endif",
...
"	#endif",
"		if (*grcnt > 0)	",
"		{	enter_critical(GQ_RD);	",
"			if (*grcnt > 0)		",
"			{	f = &m_workq[NCORE][*grfull];	",
"				if (f->m_vsize == 0)",
"				{	",
"					*gr_writemiss++;",
"					f = &m_workq[n][prfull[n]]; ",
...
"			&&  *search_terminated == 0)",
"			{	new_state();	",
"				memset((uchar *) &cur_Root, 0, sizeof(SM_frame));	",
"			} else",
"			{	pan_exit(0);",
"			}",
"		} else",
...
"",
"		if ((dsk_drained+1)%%MAX_DSK_FILE == 0)	",
"		{	(void) close(dsk_read); 	",
"			sprintf(dsk_name, \"Q%%.3d_%%.3d.tmp\", Dsk_R_Nr++, core_id);",
"			(void) unlink(dsk_name);	",
"			sprintf(dsk_name, \"Q%%.3d_%%.3d.tmp\", Dsk_R_Nr, core_id);",
...
"	}",
"",
"	rval = (char *) MapViewOfFile(shmid_S, FILE_MAP_ALL_ACCESS, 0, 0, 0);	",
"	if ((char *) rval == NULL)",
...
"		return NULL;",
"	}",
"	rval = (char *) MapViewOfFile(shmid_M, FILE_MAP_ALL_ACCESS, 0, 0, 0);	",
"",
"	if (rval == NULL)",
...
"			get_mem/(1048576.));",
"	}",
"	x = dc_mem_start = (char *) prep_state_mem((size_t) get_mem);	",
"	if (x == NULL)",
"	{	printf(\"cpu%%d: insufficient memory -- aborting\\n\", core_id);",
"		exit(1);",
"	}",
