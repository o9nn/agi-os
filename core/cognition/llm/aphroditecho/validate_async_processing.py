...
async def test_concurrency_manager():
    ...
    try:
        manager = ConcurrencyManager(max_concurrent_requests=5, max_requests_per_second=10.0, burst_limit=5)
        request_times = []
...
async def test_dtesn_processor():
    ...
    try:
        ...
        batch_results = await processor.process_batch(['input1', 'input2', 'input3'], membrane_depth=2, esn_size=64)
...
async def main():
    ...
    tests = [test_connection_pool(), test_concurrency_manager(), test_dtesn_processor()]
    sync_tests = [test_app_creation()]
