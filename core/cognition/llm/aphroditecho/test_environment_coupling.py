...
try:
    from environment_coupling import EnvironmentStateMonitor, BehaviorAdaptationEngine, ContextSensitivityManager, EnvironmentCouplingSystem, EnvironmentEvent, BehaviorAdaptation, AdaptationStrategy, CouplingType, create_default_coupling_system
    from environment.aar_bridge import AAREnvironmentBridge, AAREnvironmentAdapter, initialize_aar_environment_coupling
...
class MockAgent:
    def __init__(self, agent_id: str):
        self.id = agent_id
        self.state = {'energy': 100, 'activity': 1.0, 'position': [0.0, 0.0, 0.0], 'behaviors': {}}
...
class MockArena:
    def __init__(self, arena_id: str):
        self.id = arena_id
        self.state = {'resources': 10, 'hazards': 0, 'agent_positions': {}, 'temperature': 20.0}
        self.state_history = []
...
class TestBehaviorAdaptationEngine:
    ...
    def test_adaptation_rules(self):
        ...
        engine.active = True
        rule = {'name': 'test_rule', 'condition': {'event_type': 'parameter_changed'}, 'action': {'target': 'agent1', 'parameters': {'behavior': 'test'}}, 'priority': 5}
        engine.add_adaptation_rule(rule)
        event = EnvironmentEvent(event_id='test_event', timestamp=time.time(), event_type='parameter_changed', source='test', data={'parameter': 'temperature', 'new_value': 30.0})
    ...
    async def test_adaptation_queue_processing(self):
        ...
        adaptation = BehaviorAdaptation(adaptation_id='test_adaptation', target_agent='agent1', adaptation_type=AdaptationStrategy.IMMEDIATE, parameters={'behavior': 'test'})
...
class TestContextSensitivityManager:
    ...
    def test_context_analysis(self):
        ...
        agent_states = {'agent1': {'position': [1, 2, 3], 'energy': 80}, 'agent2': {'position': [4, 5, 6], 'energy': 60}}
    ...
    def test_sensitivity_profiles(self):
        if not IMPORTS_AVAILABLE:
            return
        manager = ContextSensitivityManager('test_manager')
        custom_profile = {'environmental': 0.9, 'social': 0.8, 'temporal': 0.7, 'spatial': 0.6}
...
class TestEnvironmentCouplingSystem:
    ...
    async def test_complete_coupling_workflow(self):
        ...
        changed_state = {'temperature': 35.0, 'resources': 2}
...
class TestAARIntegration:
    ...
    async def test_arena_state_processing(self):
        ...
        bridge.start_bridge()
        arena_state = {'resources': 5, 'hazards': 1, 'agent_positions': {'agent1': [1, 2, 3]}}
    ...
    async def test_behavior_adaptation_integration(self):
        ...
        bridge.start_bridge()
        adaptation = BehaviorAdaptation(adaptation_id='test_adaptation', target_agent='test_agent', adaptation_type=AdaptationStrategy.IMMEDIATE, parameters={'behavior': 'resource_seeking', 'intensity': 'high'})
...
class TestPerformanceAndStress:
    async def test_high_frequency_updates(self):
        ...
        while time.time() - start_time < 1.0:
            test_state = {'timestamp': time.time(), 'random_value': update_count % 100}
    ...
    async def test_many_agents_coupling(self):
        ...
        for i in range(agent_count):
            ...
            assert success
        system.start_coupling()
        large_state = {'global_temperature': 30.0, 'global_resources': 1}
