...
def sync_to_neon_via_mcp():
    ...
    try:
        result = subprocess.run(['manus-mcp-cli', 'tool', 'call', 'execute_query', '--server', 'neon', '--input', json.dumps({'query': schema_sql})], capture_output=True, text=True, timeout=60)
...
def sync_data_to_neon(insert_statements):
    ...
    for i in range(0, total_statements, batch_size):
        ...
        combined_sql = '\n'.join(batch)
        try:
            result = subprocess.run(['manus-mcp-cli', 'tool', 'call', 'execute_query', '--server', 'neon', '--input', json.dumps({'query': combined_sql})], capture_output=True, text=True, timeout=30)
            if result.returncode == 0:
