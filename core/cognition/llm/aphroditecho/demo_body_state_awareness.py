...
class DemoVirtualBody:
    def __init__(self):
        ...
        self.joints = {'base': None, 'neck': None, 'left_shoulder': None, 'left_elbow': None, 'right_shoulder': None, 'right_elbow': None, 'left_hip': None, 'left_knee': None, 'right_hip': None, 'right_knee': None}
    ...
    def get_joint_state(self, joint_id):
        if joint_id in self.joints:
            return {'angle': self.joint_angles[joint_id], 'velocity': self.joint_velocities[joint_id], 'torque': self.joint_torques[joint_id], 'position': [0, 0, 0]}
        return None
    ...
    def simulate_movement(self, dt=0.1):
        ...
        for joint_id in self.joints:
            freq = 0.5 + np.random.uniform(0, 0.3)
        ...
        self.position[2] = max(0.5, self.position[2])
        com_offset = np.array([0.1 * (self.joint_angles['left_shoulder'] - self.joint_angles['right_shoulder']), 0.0, 0.1 * np.mean([self.joint_angles[j] for j in ['left_hip', 'right_hip']])])
...
def demonstrate_joint_sensing():
    ...
    for joint_id, data in list(joint_sensing.items())[:3]:
...
def demonstrate_internal_monitoring(body_state_system):
    ...
    key_metrics = [('Balance Score', 'balance_score'), ('Stability Index', 'stability_index'), ('Coordination Level', 'coordination_level'), ('Proprioceptive Clarity', 'proprioceptive_clarity'), ('Movement Fluidity', 'movement_fluidity')]
...
def main():
    ...
    try:
        ...
        print('=' * 60)
        results = {'Joint Angle/Velocity Sensing': '✓ Operational', 'Position/Orientation Tracking': '✓ Operational', 'Internal Body State Monitoring': '✓ Operational', 'Acceptance Criteria': '✓ PASSED' if acceptance_passed else '✗ FAILED', 'Real-time Performance': f'✓ {update_rate:.1f} Hz' if update_rate >= 30 else f'⚠ {update_rate:.1f} Hz', 'DTESN Integration': '✓ Operational', 'Temporal Consistency': '✓ High' if consistency_score < 0.1 else '⚠ Moderate'}
