...
from aphrodite.worker.pooling_model_runner import ModelInputForGPUWithPoolingMetadata
...
def test_multi_step_model_runner_input():
    ...
    assert (receieved_frozen_input.input_tokens == frozen_model_input.input_tokens).all()
    assert receieved_frozen_input.input_positions is not None
    assert (receieved_frozen_input.input_positions == frozen_model_input.input_positions).all()
    ...
    for field in dataclasses.fields(AttentionMetadata):
        assert getattr(receieved_frozen_input.attn_metadata, field.name, None) == getattr(attn_metadata, field.name, None)
    assert receieved_frozen_input.sampling_metadata.selected_token_indices == sampling_metadata.selected_token_indices
