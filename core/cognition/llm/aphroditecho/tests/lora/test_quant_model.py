...
def test_quant_model_lora(tinyllama_lora_files, model):
    llm = aphrodite.LLM(model=model.model_path, enable_lora=True, max_num_seqs=16, max_loras=4, max_model_len=400, gpu_memory_utilization=0.2, quantization=model.quantization, trust_remote_code=True, enable_chunked_prefill=True)
...
def test_quant_model_tp_equality(tinyllama_lora_files, num_gpus_available, model):
    ...
    llm_tp1 = aphrodite.LLM(model=model.model_path, enable_lora=True, max_num_seqs=16, max_loras=4, gpu_memory_utilization=0.2, quantization=model.quantization, trust_remote_code=True, enable_chunked_prefill=True)
    output_tp1 = do_sample(llm_tp1, tinyllama_lora_files, lora_id=1)
    del llm_tp1
    cleanup_dist_env_and_memory()
    llm_tp2 = aphrodite.LLM(model=model.model_path, enable_lora=True, max_num_seqs=16, max_loras=4, tensor_parallel_size=2, gpu_memory_utilization=0.2, quantization=model.quantization, enable_chunked_prefill=True)
