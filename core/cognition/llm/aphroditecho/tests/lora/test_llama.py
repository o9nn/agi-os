...
def do_sample(llm: aphrodite.LLM, lora_path: str, lora_id: int) -> List[str]:
    ...
    sampling_params = aphrodite.SamplingParams(temperature=0, max_tokens=256, stop=['[/assistant]'])
...
def test_llama_lora(sql_lora_files, tp_size, num_gpus_available):
    ...
    llm = aphrodite.LLM(MODEL_PATH, enable_lora=True, max_num_seqs=16, max_loras=4, tensor_parallel_size=tp_size)
...
def test_llama_tensor_parallel_equality(sql_lora_files, num_gpus_available):
    ...
    llm_tp1 = aphrodite.LLM(MODEL_PATH, enable_lora=True, max_num_seqs=16, max_loras=4, tensor_parallel_size=1)
    output_tp1 = do_sample(llm_tp1, sql_lora_files, lora_id=1)
    del llm_tp1
    cleanup()
    llm_tp2 = aphrodite.LLM(MODEL_PATH, enable_lora=True, max_num_seqs=16, max_loras=4, tensor_parallel_size=2)
    output_tp2 = do_sample(llm_tp2, sql_lora_files, lora_id=1)
    del llm_tp2
    cleanup()
    assert output_tp1 == output_tp2
    llm_tp4 = aphrodite.LLM(MODEL_PATH, enable_lora=True, max_num_seqs=16, max_loras=4, tensor_parallel_size=4)
