...
from aphrodite.modeling.layers.rotary_embedding import LinearScalingRotaryEmbedding
from .data.long_context_test_data import prompts_and_responses
context_len_to_scaling_factor = {'16k': 4, '32k': 8}
...
def _create_lora_request(lora_id, long_context_infos):
    ...
    return LoRARequest(context_len, lora_id, long_context_infos[lora_id]['lora'], None, 4096 * scaling_factor)
def evaluate_json_response(model_response, golden_response):
    try:
        model_response = ast.literal_eval(model_response)
    except Exception as e:
        raise ValueError(f'Model response is not a valid JSON. Expected {golden_response}, got  {model_response}') from e
    positive_values = 0
    total_values = 0
        ...
        if person_attribute in model_response:
            if isinstance(person_attribute_value, dict):
                for sub_attribute, sub_attribute_value in person_attribute_value.items():
                    total_values += 1
                    if sub_attribute in model_response[person_attribute] and model_response[person_attribute][sub_attribute] == sub_attribute_value:
                        positive_values += 1
            else:
                total_values += 1
...
def generate(llm: aphrodite.LLM, inputs: Tuple[str, SamplingParams, Optional[LoRARequest]]):
    prompts, sampling_param, lora_request = inputs
    outputs = llm.generate(prompts, sampling_param, lora_request=lora_request)
    return outputs[0].outputs[0].text.strip()
def batched_generate(llm: aphrodite.LLM, inputs: List[Tuple[str, SamplingParams, Optional[LoRARequest]]]):
    for input in inputs:
        prompt, sampling_param, lora_req = input
        llm._validate_and_add_requests(prompt, sampling_param, lora_request=lora_req, prompt_adapter_request=None)
    outputs = llm._run_engine(use_tqdm=True)
    return [outputs[i].outputs[0].text.strip() for i in range(len(outputs))]
@pytest.fixture(scope='module')
def lora_llm(long_context_infos):
    scaling_factors = [context_len_to_scaling_factor[info['context_length']] for info in long_context_infos.values()]
    llm = aphrodite.LLM('meta-llama/Llama-2-13b-chat-hf', enable_lora=True, max_num_seqs=16, max_loras=2, long_lora_scaling_factors=tuple(scaling_factors), max_num_batched_tokens=4096 * 8, tensor_parallel_size=4, distributed_executor_backend='mp')
    yield llm
    del llm
def test_rotary_emb_replaced(dist_init):
    from aphrodite.engine.args_tools import EngineArgs
    from aphrodite.worker.model_runner import ModelRunner
    engine_args = EngineArgs('meta-llama/Llama-2-7b-hf', long_lora_scaling_factors=(4.0,), enable_lora=True)
    ...
    rotary_emb_count = 0
    for module_name, module in model_runner.model.named_modules(remove_duplicate=False):
...
def test_batched_rope_kernel(lora_llm, long_context_infos):
    ...
    for lora_id, info in long_context_infos.items():
        ...
        lora_prompt = (prompts_and_responses[context_len][0]['prompt'], sampling_params, _create_lora_request(lora_id, long_context_infos))
        lora_output = generate(lora_llm, lora_prompt)
        non_batched_results.append(lora_output)
    batched_prompts: List[Tuple[str, SamplingParams, Optional[LoRARequest]]] = []
    for lora_id, info in long_context_infos.items():
        context_len = info['context_length']
        batched_prompts.extend([(prompts_and_responses[context_len][0]['prompt'], sampling_params, _create_lora_request(lora_id, long_context_infos))])
...
def test_self_consistency(lora_llm, long_context_infos):
    ...
    for i in range(num_loras):
        assert batched_results[i] == permutated_batched_results[permutation[i]], f'Results should be the same:\n{batched_results[i]}\n{permutated_batched_results[permutation[i]]}'
@pytest.mark.skip_global_cleanup
def test_quality(lora_llm, long_context_infos):
    ...
    for lora_id, info in long_context_infos.items():
        ...
        for prompt_and_response in prompts_and_responses[context_len]:
            lora_prompt = (prompt_and_response['prompt'], sampling_params, _create_lora_request(lora_id, long_context_infos))
...
def test_max_len(lora_llm, long_context_infos):
    ...
    for lora_id_with_bad_inputs in long_context_infos:
        for lora_id, info in long_context_infos.items():
            context_len = info['context_length']
            batched_prompts.extend([(prompts_and_responses[context_len][0]['prompt'] * (2 if lora_id == lora_id_with_bad_inputs else 1), sampling_params, _create_lora_request(lora_id, long_context_infos))])
        with pytest.raises(ValueError):
