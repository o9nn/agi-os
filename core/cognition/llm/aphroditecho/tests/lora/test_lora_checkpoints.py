...
def test_lora_weights_mapping(baichuan_lora_files):
    ...
    for module in BAICHUAN_LORA_MODULES:
        if module in packed_modules_mapping:
            ...
            expected_lora_modules.append(module)
    hf_to_aphrodite_mapper = WeightsMapper(orig_to_new_prefix={'model.': 'language_model.model.'}, orig_to_new_substr={'.layers.': '.baichuan_layers.'})
    peft_helper = PEFTHelper.from_local_dir(baichuan_lora_files, max_position_embeddings=4096)
    lora_model = LoRAModel.from_local_checkpoint(baichuan_lora_files, expected_lora_modules, peft_helper=peft_helper, lora_model_id=1, device='cpu', embedding_modules=embedding_modules, embedding_padding_modules=embed_padding_modules, weights_mapper=hf_to_aphrodite_mapper)
