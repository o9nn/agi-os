...
def do_sample(llm: aphrodite.LLM, lora_path: str, lora_id: int) -> list[str]:
    ...
    sampling_params = aphrodite.SamplingParams(temperature=0, max_tokens=256, skip_special_tokens=False, stop=['[/assistant]'])
...
def test_llama_lora(sql_lora_files):
    llm = aphrodite.LLM(MODEL_PATH, enable_lora=True, max_num_seqs=13, max_loras=4, enable_chunked_prefill=True)
...
def test_llama_lora_tp4(sql_lora_files):
    llm = aphrodite.LLM(MODEL_PATH, enable_lora=True, max_num_seqs=16, max_loras=4, tensor_parallel_size=4, enable_chunked_prefill=True)
    generate_and_test(llm, sql_lora_files)
@multi_gpu_test(num_gpus=4)
@create_new_process_for_each_test()
def test_llama_lora_tp4_fully_sharded_loras(sql_lora_files):
    llm = aphrodite.LLM(MODEL_PATH, enable_lora=True, max_num_seqs=16, max_loras=4, tensor_parallel_size=4, fully_sharded_loras=True, enable_chunked_prefill=True)
    generate_and_test(llm, sql_lora_files)