...
def test_clip_image_processor(image_assets, mm_registry, dtype, size_factor):
    ...
    model_config = ModelConfig(model=MODEL_NAME, tokenizer=MODEL_NAME, tokenizer_mode='auto', trust_remote_code=False, seed=0, dtype=dtype, revision=None, limit_mm_per_prompt={'image': 1})
    ...
    for asset in image_assets:
        ...
        aphrodite_result = mm_registry.map_input(model_config, {'image': image})
...
@pytest.mark.parametrize('dtype', ['half', 'float'])
@pytest.mark.parametrize('size_factor', [0.25, 0.5, 1.0])
def test_llava_next_image_processor(image_assets, mm_registry, dtype, size_factor):
...
def test_mm_limits(image_assets, mm_registry, num_images, limit, is_valid):
    MODEL_NAME = 'llava-hf/llava-1.5-7b-hf'
    model_config = ModelConfig(model=MODEL_NAME, tokenizer=MODEL_NAME, tokenizer_mode='auto', trust_remote_code=False, seed=0, dtype='half', revision=None, limit_mm_per_prompt={'image': limit})
    mm_registry.init_mm_limits_per_prompt(model_config)
...
def test_image_mapper_multi(image_assets, mm_registry, num_images):
    MODEL_NAME = 'llava-hf/llava-1.5-7b-hf'
    model_config = ModelConfig(model=MODEL_NAME, tokenizer=MODEL_NAME, tokenizer_mode='auto', trust_remote_code=False, seed=0, dtype='half', revision=None, limit_mm_per_prompt={'image': num_images})
    mm_registry.init_mm_limits_per_prompt(model_config)
    image = image_assets[0].pil_image
    mm_inputs = {'image': [image] * num_images}
