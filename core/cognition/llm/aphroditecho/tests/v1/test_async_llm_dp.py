...
engine_args = AsyncEngineArgs(model='ibm-research/PowerMoE-3b', enforce_eager=True, disable_log_requests=True, tensor_parallel_size=int(os.getenv('TP_SIZE', 1)), data_parallel_size=int(os.getenv('DP_SIZE', 2)))
...
async def generate(engine: AsyncLLM, request_id: str, prompt: PromptType, output_kind: RequestOutputKind, max_tokens: int, prompt_logprobs: Optional[int]=None) -> tuple[int, str]:
