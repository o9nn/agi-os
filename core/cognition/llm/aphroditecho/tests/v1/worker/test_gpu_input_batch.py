...
from aphrodite.v1.worker.gpu_input_batch import BlockTable, CachedRequestState, InputBatch
VOCAB_SIZE = 1024
...
def _remove_requests(input_batch: InputBatch, batch_size: int, reqs: list[CachedRequestState]) -> tuple[set[str], list[int]]:
...
def _construct_cached_request_state(req_id_suffix: int):
    ...
    return CachedRequestState(req_id=f'req_id_{req_id_suffix}', prompt_token_ids=prompt_token_ids, sampling_params=_create_sampling_params(), mm_inputs=[], mm_positions=[], block_ids=[], generator=None, num_computed_tokens=len(output_token_ids), output_token_ids=output_token_ids)
@pytest.mark.parametrize('device', CUDA_DEVICES)
@pytest.mark.parametrize('batch_size', [1, 2, 32, 64])
def test_sampling_metadata_in_input_batch(device: str, batch_size: int):
    input_batch: InputBatch = InputBatch(max_num_reqs=batch_size, max_model_len=1024, max_num_blocks_per_req=10, device=torch.device(device), pin_memory=is_pin_memory_available(), vocab_size=1024)
...
def test_swap_states_in_input_batch(device: str, batch_size: int, swap_list: list):
    input_batch: InputBatch = InputBatch(max_num_reqs=batch_size, max_model_len=1024, max_num_blocks_per_req=10, device=torch.device(device), pin_memory=is_pin_memory_available(), vocab_size=1024)
    ref_input_batch: InputBatch = InputBatch(max_num_reqs=batch_size, max_model_len=1024, max_num_blocks_per_req=10, device=torch.device(device), pin_memory=is_pin_memory_available(), vocab_size=1024)
    reqs: list[CachedRequestState] = []
    req_id_reqs = {}
    req_id_output_token_ids = {}
    for req_index in range(batch_size):
