...
from aphrodite.v1.core.kv_cache_utils import NONE_HASH, BlockHashType, FreeKVCacheBlockQueue, KVCacheBlock, PrefixCachingMetrics, estimate_max_model_len, generate_block_hash_extra_keys, hash_block_tokens, hash_request_tokens, unify_kv_cache_configs
...
def make_request(request_id, prompt_token_ids, mm_positions=None, mm_hashes=None, cache_salt=None):
    ...
    return Request(request_id=request_id, prompt_token_ids=prompt_token_ids, multi_modal_inputs=multi_modal_inputs, multi_modal_hashes=mm_hashes, multi_modal_placeholders=mm_positions, sampling_params=SamplingParams(max_tokens=17), eos_token_id=100, arrival_time=0, lora_request=None, cache_salt=cache_salt)
def new_kv_cache_spec(block_size=16, num_kv_heads=2, head_size=64, dtype=torch.float32, use_mla=False):
    return FullAttentionSpec(block_size=block_size, num_kv_heads=num_kv_heads, head_size=head_size, dtype=dtype, use_mla=use_mla)
def test_none_hash():
...
def test_unify_kv_cache_configs():
    ...
    need_sort_kv_cache_config = [KVCacheConfig(num_blocks=10, tensors={'layer1': KVCacheTensor(100), 'layer2': KVCacheTensor(100)}, kv_cache_groups=[KVCacheGroupSpec(['layer1'], new_kv_cache_spec()), KVCacheGroupSpec(['layer2'], new_kv_cache_spec(num_kv_heads=4))]), KVCacheConfig(num_blocks=20, tensors={'layer1': KVCacheTensor(100), 'layer2': KVCacheTensor(100)}, kv_cache_groups=[KVCacheGroupSpec(['layer2'], new_kv_cache_spec(num_kv_heads=4)), KVCacheGroupSpec(['layer1'], new_kv_cache_spec())])]
    ...
    diff_kv_cache_config = [KVCacheConfig(num_blocks=10, tensors={'layer1': KVCacheTensor(100), 'layer2': KVCacheTensor(100)}, kv_cache_groups=[KVCacheGroupSpec(['layer1'], new_kv_cache_spec()), KVCacheGroupSpec(['layer2'], new_kv_cache_spec(num_kv_heads=4))]), KVCacheConfig(num_blocks=20, tensors={'layer1': KVCacheTensor(100), 'layer2': KVCacheTensor(100)}, kv_cache_groups=[KVCacheGroupSpec(['layer1'], new_kv_cache_spec()), KVCacheGroupSpec(['layer2'], new_kv_cache_spec(num_kv_heads=8))])]
...
def test_estimate_max_model_len(model_id, max_model_len, want_estimated_max_len):
    ...
    aphrodite_config = AphroditeConfig(model_config=model_config, scheduler_config=scheduler_config)
    kv_cache_spec = {}
    for i in range(32):
...
def test_allocate_with_lookahead():
    block_size = 4
    config = KVCacheConfig(num_blocks=10, tensors={'layer1': KVCacheTensor(100)}, kv_cache_groups=[KVCacheGroupSpec(['layer1'], new_kv_cache_spec(block_size=block_size))])
    ...
    blocks = kv_cache_manager.allocate_slots(request, num_tokens=3, num_lookahead_tokens=2)
    assert len(blocks) == 2
    kv_cache_manager = KVCacheManager(kv_cache_config=config, max_model_len=100)
    blocks = kv_cache_manager.allocate_slots(request, num_tokens=3, num_lookahead_tokens=2)
    assert len(blocks) == 2
    kv_cache_manager = KVCacheManager(kv_cache_config=config, max_model_len=100)
    blocks = kv_cache_manager.allocate_slots(request, num_tokens=3, num_lookahead_tokens=4)
    assert len(blocks) == 2