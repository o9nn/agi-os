...
def test_ragged_paged_attention():
    ...
    attn_metadata = PallasMetadata(slot_mapping=slot_mapping, block_tables=block_tables, context_lens=context_lens, query_start_loc=query_start_loc, num_seqs=num_seqs)
    with patch('torch.ops.xla.ragged_paged_attention') as mock_ragged_paged_attention:
        ...
        mock_ragged_paged_attention.assert_called_once_with(ANY, ANY, ANY, ANY, ANY, ANY, num_kv_pages_per_block=None, num_queries_per_block=None, vmem_limit_bytes=None, use_kernel=True, sm_scale=scale, sliding_window=sliding_window, soft_cap=logits_soft_cap)