...
if not envs.APHRODITE_USE_V1:
    pytest.skip('Skipping V1 tests. Rerun with `APHRODITE_USE_V1=1` to test.', allow_module_level=True)
...
async def test_basic_vision(model_name: str, base64_encoded_image: dict[str, str]):
    ...
    server_args = ['--max-model-len', '1024', '--max-num-seqs', '16', '--gpu-memory-utilization', '0.95', '--trust-remote-code', '--max-num-batched-tokens', '576', '--disable_chunked_mm_input', '--chat-template', 'examples/template_llava.jinja']
