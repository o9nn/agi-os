...
def test_basic(aphrodite_runner: type[AphroditeRunner], monkeypatch: pytest.MonkeyPatch, model: str, max_tokens: int, tensor_parallel_size: int, max_num_seqs: int) -> None:
    prompt = 'The next numbers of the sequence ' + ', '.join((str(i) for i in range(1024))) + ' are:'
    example_prompts = [prompt]
    with monkeypatch.context() as m:
        m.setenv('APHRODITE_USE_V1', '1')
        with aphrodite_runner(model, max_num_batched_tokens=1024, max_model_len=8192, gpu_memory_utilization=0.7, max_num_seqs=max_num_seqs, tensor_parallel_size=tensor_parallel_size) as aphrodite_model:
