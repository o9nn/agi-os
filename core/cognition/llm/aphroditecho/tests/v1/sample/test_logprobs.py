...
def _run_and_validate(
    ...
    for aphrodite_result, hf_logprob, hf_output, logprob_prompt_logprob in zip(
            aphrodite_results, hf_logprobs, hf_outputs,
        ...
        if temperature == 0.0:
            ...
            assert (aphrodite_result.prompt_token_ids == hf_output[0]
                    [:len(aphrodite_result.prompt_token_ids)])
        ...
        if num_top_prompt_logprobs is not None:
            ...
            #   the prompt
            assert len(aphrodite_result.prompt_logprobs) == len(
                aphrodite_result.prompt_token_ids)
            for prompt_logprobs, prompt_token_id in zip(
                    aphrodite_result.prompt_logprobs[1:],
                    aphrodite_result.prompt_token_ids[1:]):
...
def test_zero_logprobs(aphrodite_model, example_prompts,
    ...
    with monkeypatch.context() as m:
        ...
        sampling_params_logprobs_zero = SamplingParams(max_tokens=max_tokens,
                                                       ...
                                                       prompt_logprobs=0,
                                                       temperature=0.0)
        results_logprobs_zero = aphrodite_model.model.generate(
