...
def make_request(params: SamplingParams) -> EngineCoreRequest:
    return EngineCoreRequest(request_id=str(uuid.uuid4()), prompt_token_ids=PROMPT_TOKENS, mm_inputs=None, mm_hashes=None, mm_placeholders=None, sampling_params=params, eos_token_id=None, arrival_time=time.time(), lora_request=None, cache_salt=None)
...
def test_engine_core_client(monkeypatch: pytest.MonkeyPatch, multiprocessing_mode: bool):
    with monkeypatch.context() as m:
        ...
        client = EngineCoreClient.make_client(multiprocess_mode=multiprocessing_mode, asyncio_mode=False, aphrodite_config=aphrodite_config, executor_class=executor_class, log_stats=False)
        MAX_TOKENS = 20
...
async def test_engine_core_client_asyncio(monkeypatch: pytest.MonkeyPatch):
    with monkeypatch.context() as m:
        ...
        executor_class = Executor.get_class(aphrodite_config)
        client = EngineCoreClient.make_client(multiprocess_mode=True, asyncio_mode=True, aphrodite_config=aphrodite_config, executor_class=executor_class, log_stats=True)
        try:
            MAX_TOKENS = 20
            params = SamplingParams(max_tokens=MAX_TOKENS)
            'Normal Request Cycle.'
...
def test_kv_cache_events(monkeypatch: pytest.MonkeyPatch, multiprocessing_mode: bool, publisher_config):
    with monkeypatch.context() as m:
        ...
        try:
            ...
            request = EngineCoreRequest(request_id=str(uuid.uuid4()), prompt_token_ids=custom_tokens, mm_inputs=None, mm_hashes=None, mm_placeholders=None, sampling_params=SamplingParams(max_tokens=1), eos_token_id=None, arrival_time=time.time(), lora_request=None, cache_salt=None)
            client.add_request(request)
...
def test_startup_failure(monkeypatch: pytest.MonkeyPatch):
    with monkeypatch.context() as m, pytest.raises(Exception) as e_info:
        ...
        _core_client = EngineCoreClient.make_client(multiprocess_mode=True, asyncio_mode=True, aphrodite_config=aphrodite_config, executor_class=executor_class, log_stats=True)
