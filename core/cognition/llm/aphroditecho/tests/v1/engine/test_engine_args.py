...
def test_defaults_with_usage_context():
    ...
    assert aphrodite_config.scheduler_config.max_num_batched_tokens == default_llm_tokens
    engine_args = EngineArgs(model='facebook/opt-125m')
    aphrodite_config = engine_args.create_engine_config(UsageContext.OPENAI_API_SERVER)
    assert aphrodite_config.scheduler_config.max_num_seqs == default_max_num_seqs
    assert aphrodite_config.scheduler_config.max_num_batched_tokens == default_server_tokens