...
def _aphrodite_model(apc: bool, aphrodite_runner, monkeypatch):
    ...
    return aphrodite_runner(MODEL, dtype=DTYPE, max_model_len=128, enforce_eager=True, enable_prefix_caching=apc, gpu_memory_utilization=0.5)
...
def aphrodite_model_apc(aphrodite_runner, monkeypatch):
    with _aphrodite_model(True, aphrodite_runner, monkeypatch) as aphrodite_model:
        yield aphrodite_model
def _get_test_sampling_params(prompt_list: list[str], seed: Optional[int]=42) -> tuple[list[SamplingParams], list[int]]:
    ...
    return ([SamplingParams(temperature=0.95, top_p=0.95, n=n, seed=seed) for n in n_list], n_list)
