...
from aphrodite.endpoints.deep_tree_echo.serializers import SerializationConfig, SerializerFactory, OptimizedJSONSerializer, BinarySerializer, DeterministicSerializer, SerializationFormat, benchmark_serialization, serialize_dtesn_result
...
class TestSerializationConfig:
    ...
    def test_custom_config(self):
        config = SerializationConfig(format=SerializationFormat.BINARY, include_engine_integration=True, precision=4, max_array_inline_size=512)
...
class TestOptimizedJSONSerializer:
    ...
    def test_engine_integration_exclusion(self):
        config = SerializationConfig(format=SerializationFormat.JSON_OPTIMIZED, include_engine_integration=False)
        serializer = OptimizedJSONSerializer(config)
        result = MockDTESNResult()
        serialized = serializer.serialize(result)
        data = json.loads(serialized)
        if 'engine_integration' in data:
            assert len(data['engine_integration']) <= 3
    def test_engine_integration_inclusion(self):
        config = SerializationConfig(format=SerializationFormat.JSON_OPTIMIZED, include_engine_integration=True)
        serializer = OptimizedJSONSerializer(config)
        result = MockDTESNResult()
        serialized = serializer.serialize(result)
        data = json.loads(serialized)
    ...
    def test_large_structure_summarization(self):
        config = SerializationConfig(format=SerializationFormat.JSON_OPTIMIZED)
        serializer = OptimizedJSONSerializer(config)
        result = MockDTESNResult(size_multiplier=10)
        serialized = serializer.serialize(result)
    ...
    def test_deterministic_output(self):
        config = SerializationConfig(format=SerializationFormat.JSON_OPTIMIZED, sort_keys=True)
        serializer = OptimizedJSONSerializer(config)
        result = MockDTESNResult()
...
class TestDeterministicSerializer:
    ...
    def test_checksum_included(self):
        config = SerializationConfig(format=SerializationFormat.DETERMINISTIC, validate_output=True)
        serializer = DeterministicSerializer(config)
        result = MockDTESNResult()
        serialized = serializer.serialize(result)
        data = json.loads(serialized)
...
class TestPerformanceOptimization:
    ...
    def test_benchmark_all_formats(self):
        ...
        expected_formats = [SerializationFormat.JSON, SerializationFormat.JSON_OPTIMIZED, SerializationFormat.DETERMINISTIC]
    ...
    def test_large_array_handling(self):
        ...
        config = SerializationConfig(format=SerializationFormat.JSON_OPTIMIZED, max_array_inline_size=256, compress_arrays=True)
        serializer = OptimizedJSONSerializer(config)
        serialized = serializer.serialize(result)
        data = json.loads(serialized)
...
class TestConvenienceFunctions:
    def test_serialize_dtesn_result_function(self):
        ...
        json_serialized = serialize_dtesn_result(result, format=SerializationFormat.JSON_OPTIMIZED)
        data = json.loads(json_serialized)
        assert 'input' in data
    @pytest.mark.skipif(not MSGSPEC_AVAILABLE, reason='msgspec not available')
    def test_serialize_binary_format(self):
        result = MockDTESNResult()
        binary_serialized = serialize_dtesn_result(result, format=SerializationFormat.BINARY)
        assert isinstance(binary_serialized, bytes)
class TestErrorHandling:
    def test_invalid_data_handling(self):
        result = MockDTESNResult()
        result.processed_output['invalid_func'] = lambda x: x
