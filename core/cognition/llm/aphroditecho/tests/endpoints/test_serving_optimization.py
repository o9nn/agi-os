...
import torch
from aphrodite.endpoints.openai.serving_optimization import OptimizationServingMixin, OptimizationRequest, OptimizationResponse, LoadMetricsResponse, EnsembleStatusResponse, PerformanceReportResponse, OptimizationMiddleware, setup_optimization_serving, create_optimized_request_context
...
def mock_optimizer():
    ...
    optimizer.get_optimization_status = Mock(return_value={'running': True, 'configuration': {'torch_compile_enabled': True, 'dynamic_tuning_enabled': True, 'ensemble_enabled': True, 'auto_scaling_enabled': False}, 'current_strategy': {'strategy': 'balanced', 'load_score': 0.5}, 'compilation_stats': {}, 'ensemble_status': {'enabled': True, 'model_count': 2, 'models': []}, 'performance_metrics_count': 100, 'model_manager_status': {'status': 'active'}})
    optimizer.parameter_tuner = Mock()
    optimizer.parameter_tuner.collect_load_metrics = Mock(return_value=ServerLoadMetrics(timestamp=time.time(), cpu_usage_percent=65.0, memory_usage_percent=70.0, gpu_utilization=80.0, active_requests=15, queue_depth=3, throughput_tokens_per_sec=120.0, avg_latency_ms=150.0, error_rate=0.02))
    optimizer.ensemble_manager = Mock()
    optimizer.ensemble_manager.get_ensemble_status = Mock(return_value={'enabled': True, 'strategy': 'weighted_voting', 'model_count': 2, 'max_size': 3, 'models': [{'model_id': 'model1', 'weight': 1.0, 'request_count': 100, 'avg_latency_ms': 120.0, 'error_rate': 0.01, 'last_used': time.time(), 'added_timestamp': time.time() - 3600}, {'model_id': 'model2', 'weight': 0.8, 'request_count': 80, 'avg_latency_ms': 140.0, 'error_rate': 0.02, 'last_used': time.time() - 60, 'added_timestamp': time.time() - 1800}]})
    optimizer.ensemble_manager.add_model_to_ensemble = Mock()
    optimizer.export_performance_report = Mock(return_value={'report_timestamp': time.time(), 'optimization_config': optimizer.config.__dict__, 'status': optimizer.get_optimization_status(), 'performance_history': [], 'model_versions': []})
    return optimizer
@pytest.fixture
...
class TestOptimizationEndpoints:
    def test_configure_optimization_success(self, client, mock_optimizer):
        ...
        mixin.optimizer = mock_optimizer
        request_data = {'enable_torch_compile': False, 'compile_mode': 'max-autotune', 'tuning_interval_sec': 60.0, 'enable_ensemble': True, 'max_ensemble_size': 5}
    ...
    def test_configure_optimization_invalid_data(self, client):
        ...
        request_data = {'compile_mode': 'invalid_mode', 'tuning_interval_sec': -1.0, 'max_ensemble_size': 0}
        response = test_client.post('/v1/optimization/configure', json=request_data)
    ...
    def test_add_model_to_ensemble_success(self, client, mock_optimizer):
        test_client, mixin = client
        mixin.optimizer = mock_optimizer
        response = test_client.post('/v1/optimization/ensemble/add_model', params={'model_id': 'new_model', 'initial_weight': 0.9})
        assert response.status_code == 200
        data = response.json()
        assert data['success'] is True
    ...
    def test_force_recompile_success(self, client, mock_optimizer):
        ...
        response = test_client.post('/v1/optimization/force_recompile', params={'model_id': 'test_model'})
        assert response.status_code == 200
        data = response.json()
        assert data['success'] is True
...
class TestOptimizationMiddleware:
    ...
    async def test_process_request_success(self, middleware, mock_optimization_mixin):
        ...
        request_data = {'model': 'test_model', 'request_id': 'req_123', 'input': 'test input'}
        result = await middleware.process_request(original_model, request_data, mock_process_func)
    ...
    async def test_process_request_error(self, middleware, mock_optimization_mixin):
        ...
        with pytest.raises(ValueError, match='Processing error'):
            await middleware.process_request(original_model, request_data, mock_process_func)
        mock_optimization_mixin.record_request_metrics.assert_called_once()
        call_args = mock_optimization_mixin.record_request_metrics.call_args[1]
...
class TestUtilityFunctions:
    def test_create_optimized_request_context(self):
        context = create_optimized_request_context(model_name='test-model', request_id='req_123', user_preferences={'quality': 'high'}, load_info={'cpu': 65.0})
    ...
    def test_create_optimized_request_context_minimal(self):
        context = create_optimized_request_context(model_name='test-model', request_id='req_123')
        assert context['model_id'] == 'test-model'
        assert context['request_id'] == 'req_123'
        assert 'timestamp' in context
    ...
    async def test_setup_optimization_serving(self, mock_engine):
        ...
        expected_paths = ['/v1/optimization/configure', '/v1/optimization/start', '/v1/optimization/stop', '/v1/optimization/status', '/v1/optimization/metrics', '/v1/optimization/ensemble', '/v1/optimization/ensemble/add_model', '/v1/optimization/report', '/v1/optimization/force_recompile']
        for expected_path in expected_paths:
            assert expected_path in route_paths
class TestModelRequestValidation:
    def test_optimization_request_validation(self):
        valid_request = OptimizationRequest(enable_torch_compile=True, compile_mode='max-autotune', tuning_interval_sec=45.0, load_threshold_high=0.85, max_ensemble_size=4)
        ...
        with pytest.raises(ValueError):
            OptimizationRequest(tuning_interval_sec=0.5)
        with pytest.raises(ValueError):
            OptimizationRequest(load_threshold_high=1.5)
    def test_load_metrics_response_structure(self):
        response = LoadMetricsResponse(timestamp=time.time(), cpu_usage_percent=75.0, memory_usage_percent=60.0, gpu_utilization=85.0, active_requests=20, queue_depth=5, throughput_tokens_per_sec=150.0, avg_latency_ms=120.0, error_rate=0.02, overall_load_score=0.7)
    ...
    def test_ensemble_status_response_structure(self):
        models_data = [{'model_id': 'model1', 'weight': 1.0, 'request_count': 100, 'avg_latency_ms': 120.0, 'error_rate': 0.01}]
        response = EnsembleStatusResponse(enabled=True, strategy='weighted_voting', model_count=1, max_size=3, models=models_data)
