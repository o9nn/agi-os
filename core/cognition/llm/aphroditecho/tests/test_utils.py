...
from aphrodite.common.utils import CacheInfo, FlexibleArgumentParser, LRUCache, MemorySnapshot, PlaceholderModule, StoreBoolean, bind_kv_cache, deprecate_kwargs, get_open_port, make_zmq_socket, memory_profiling, merge_async_iterators, sha256, split_zmq_path, supports_kw, swap_dict_values
...
def test_cli_override_to_config(parser_with_config, cli_config_file):
    ...
    args = parser_with_config.parse_args(['run', 'mymodel', '--tensor-parallel-size', '3', '--config', cli_config_file, '--port', '666'])
    assert args.tensor_parallel_size == 3
...
def test_config_file(parser_with_config):
    ...
    with pytest.raises(ValueError):
        parser_with_config.parse_args(['run', 'mymodel', '--tensor-parallel-size', '3', '--config', '--batch-size', '32'])
...
def test_bind_kv_cache_pp():
    with patch('aphrodite.common.utils.cuda_device_count_stateless', lambda: 2):
        cfg = AphroditeConfig(parallel_config=ParallelConfig(pipeline_parallel_size=2))
...
def test_model_specification(parser_with_config, cli_config_file, cli_config_file_with_model):
    ...
    args = parser_with_config.parse_args(['run', '--config', cli_config_file_with_model])
    assert args.model == 'config-model'
    assert args.served_model_name == 'mymodel'
    with pytest.raises(ValueError, match='No model specified!'):
        parser_with_config.parse_args(['run', '--config', cli_config_file])
    with pytest.raises(ValueError, match='With `aphrodite run`, you should provide the model as a positional argument or in a config file instead of via the `--model` option.'):
