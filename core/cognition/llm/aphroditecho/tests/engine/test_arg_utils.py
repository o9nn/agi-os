...
from aphrodite.engine.args_tools import EngineArgs, contains_type, get_kwargs, get_type, is_not_builtin, is_type, literal_to_kwargs, nullable_kvs, optional_type
from aphrodite.common.utils import FlexibleArgumentParser
@pytest.mark.parametrize(('type', 'value', 'expected'), [(int, '42', 42), (int, 'None', None), (float, '3.14', 3.14), (float, 'None', None), (str, 'Hello World!', 'Hello World!'), (str, 'None', None), (json.loads, '{"foo":1,"bar":2}', {'foo': 1, 'bar': 2}), (json.loads, 'foo=1,bar=2', {'foo': 1, 'bar': 2}), (json.loads, 'None', None)])
...
@pytest.mark.parametrize(('type_hint', 'expected'), [(int, False), (DummyConfigClass, True)])
...
@pytest.mark.parametrize(('arg', 'expected'), [(None, dict()), ('image=16', {'image': 16}), ('image=16,video=2', {'image': 16, 'video': 2}), ('Image=16, Video=2', {'image': 16, 'video': 2})])
...
def test_compilation_config():
    ...
    args = parser.parse_args(['-O=3'])
    assert args.compilation_config.level == 3
    args = parser.parse_args(['--compilation-config', "{'level': 3, 'cudagraph_capture_sizes': [1, 2, 4, 8]}"])
    assert args.compilation_config.level == 3 and args.compilation_config.cudagraph_capture_sizes == [1, 2, 4, 8]
    args = parser.parse_args(["--compilation-config={'level': 3, 'cudagraph_capture_sizes': [1, 2, 4, 8]}"])
    assert args.compilation_config.level == 3 and args.compilation_config.cudagraph_capture_sizes == [1, 2, 4, 8]
...
@pytest.mark.parametrize('arg', ['image', 'image=4,image=5', 'image=video=4'])
