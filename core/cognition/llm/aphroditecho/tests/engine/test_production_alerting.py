...
from aphrodite.engine.sla_manager import SLAManager, SLAThreshold, SLAViolation, ViolationSeverity, SLAViolationType, create_production_sla_manager
from aphrodite.engine.recovery_engine import RecoveryEngine, RecoveryProcedure, RecoveryAction, RecoveryStatus, create_production_recovery_engine
from aphrodite.engine.rca_engine import RCAEngine, CorrelationType, RootCauseCategory, ConfidenceLevel, create_production_rca_engine
from aphrodite.engine.production_monitor import ProductionMonitor, MonitoringMode, create_production_monitor
class TestSLAManager:
    ...
    def test_initialization(self):
        ...
        assert len(self.sla_manager.thresholds) > 0
    ...
    def test_threshold_management(self):
        custom_threshold = SLAThreshold(metric_name='custom_metric', target_value=100.0, tolerance_percent=10.0)
    ...
    def test_threshold_violation_detection(self):
        threshold = SLAThreshold(metric_name='test_latency', target_value=200.0, tolerance_percent=25.0, measurement_window_minutes=1, violation_threshold_percent=60)
    ...
    def test_anomaly_detection(self):
        ...
        for i in range(50):
            value = 100.0 + i % 10 - 5
            self.sla_manager.statistical_analyzer.add_measurement(metric_name, value, baseline_time + i * 60)
    ...
    def test_performance_regression_detection(self):
        ...
        for i in range(30):
            value = 100.0 + i % 5
            self.sla_manager.statistical_analyzer.add_measurement(metric_name, value, current_time - (30 - i) * 60)
        for i in range(10):
            value = 130.0 + i % 3
            self.sla_manager.statistical_analyzer.add_measurement(metric_name, value, current_time - (10 - i) * 60)
        is_regression, change_percent = self.sla_manager.statistical_analyzer.detect_performance_regression(metric_name, recent_window_minutes=15)
        assert is_regression
        assert abs(change_percent) > 10
...
class TestRecoveryEngine:
    ...
    def test_initialization(self):
        assert len(self.recovery_engine.procedures) > 0
        assert len(self.recovery_engine.circuit_breakers) > 0
    ...
    def test_procedure_management(self):
        custom_procedure = RecoveryProcedure(procedure_id='test_procedure', name='Test Recovery', description='Test recovery procedure', actions=[RecoveryAction.CACHE_INVALIDATION], timeout_seconds=60)
    ...
    async def test_recovery_execution(self):
        ...
        violation = SLAViolation(violation_id='test_violation', timestamp=time.time(), violation_type=SLAViolationType.LATENCY_BREACH, severity=ViolationSeverity.MAJOR, metric_name='request_latency_p95', threshold=threshold, actual_value=280.0, expected_value=200.0, breach_percentage=40.0, measurement_window=[250.0, 260.0, 280.0, 270.0, 290.0])
...
class TestRCAEngine:
    ...
    def test_correlation_analysis(self):
        ...
        for i in range(30):
            ...
            value1 = 100.0 + i
            value2 = value1 * 2 + i % 3 - 1
            self.rca_engine.record_metric('metric1', value1, timestamp)
            self.rca_engine.record_metric('metric2', value2, timestamp)
        correlations = self.rca_engine.correlation_analyzer.find_temporal_correlations(self.rca_engine.metrics_collector, current_time + 1800, window_minutes=60)
        assert len(correlations) > 0
        correlation = correlations[0]
        assert abs(correlation.correlation_strength) > 0.8
    ...
    async def test_rca_analysis(self):
        from aphrodite.engine.sla_manager import SLAThreshold
        threshold = SLAThreshold('test_metric', 100.0, 20.0)
        violation = SLAViolation(violation_id='test_rca_violation', timestamp=time.time(), violation_type=SLAViolationType.LATENCY_BREACH, severity=ViolationSeverity.CRITICAL, metric_name='request_latency_p95', threshold=threshold, actual_value=300.0, expected_value=200.0, breach_percentage=50.0, measurement_window=[280.0, 290.0, 300.0, 310.0, 320.0])
        current_time = time.time()
        for i in range(50):
    ...
    def test_hypothesis_generation(self):
        ...
        diagnostic_data = DiagnosticData(timestamp=time.time(), incident_id='test', system_metrics={'cpu_percent': 95.0, 'memory_percent': 92.0}, process_metrics={'top_cpu_processes': [{'name': 'test_proc', 'cpu_percent': 60.0}]}, network_metrics={}, application_logs=[], error_traces=[], configuration_snapshot={'aphrodite_config': {'gpu_memory_utilization': 0.98}}, resource_usage={})
        from aphrodite.engine.sla_manager import SLAThreshold
        threshold = SLAThreshold('cpu_usage', 80.0, 10.0)
        violation = SLAViolation(violation_id='test', timestamp=time.time(), violation_type=SLAViolationType.RESOURCE_EXHAUSTION, severity=ViolationSeverity.MAJOR, metric_name='cpu_usage', threshold=threshold, actual_value=95.0, expected_value=80.0, breach_percentage=18.75, measurement_window=[])
        hypotheses = self.rca_engine.hypothesis_generator.generate_hypotheses(violation, [], diagnostic_data)
...
class TestProductionMonitor:
    ...
    def test_monitoring_mode_adjustment(self):
        ...
        assert self.monitor.alert_cooldown_seconds < 300
        self.monitor.set_monitoring_mode(MonitoringMode.EMERGENCY)
        assert self.monitor.monitoring_mode == MonitoringMode.EMERGENCY
        assert self.monitor.health_check_interval < 60
    def test_stats_recording(self):
        from aphrodite.engine.metrics_types import Stats
        stats = Stats(now=time.time(), num_running_sys=5, num_waiting_sys=2, num_swapped_sys=0, gpu_cache_usage_sys=0.75, cpu_cache_usage_sys=0.6, cpu_prefix_cache_hit_rate=0.85, gpu_prefix_cache_hit_rate=0.9, num_prompt_tokens_iter=150, num_generation_tokens_iter=300, num_tokens_iter=450, time_to_first_tokens_iter=[0.5, 0.6, 0.7], time_per_output_tokens_iter=[0.1, 0.12, 0.11], num_preemption_iter=1, time_e2e_requests=[1.2, 1.5, 1.8], time_queue_requests=[0.1, 0.2, 0.15], time_inference_requests=[1.0, 1.2, 1.5], time_prefill_requests=[0.3, 0.4, 0.5], time_decode_requests=[0.7, 0.8, 1.0], num_prompt_tokens_requests=[50, 60, 40], num_generation_tokens_requests=[100, 120, 80], n_requests=[1, 1, 1], max_num_generation_tokens_requests=[200, 250, 150], max_tokens_requests=[300, 350, 250], finished_reason_requests=['stop', 'length', 'stop'], waiting_lora_adapters=[], running_lora_adapters=[], max_lora='0')
    ...
    def test_custom_stat_logger(self):
        ...
        model_config = ModelConfig(model='test-model', tokenizer='test-tokenizer', tokenizer_mode='auto', trust_remote_code=False, dtype='auto', seed=0, max_model_len=1024, quantization=None, enforce_eager=False, max_context_len_to_capture=8192, max_seq_len_to_capture=8192)
        cache_config = CacheConfig(block_size=16, gpu_memory_utilization=0.9, swap_space=4, cache_dtype='auto')
        aphrodite_config = AphroditeConfig(model_config=model_config, cache_config=cache_config, parallel_config=None, scheduler_config=None, device_config=None, load_config=None, lora_config=None, vision_language_config=None, speculative_config=None, decoding_config=None, observability_config=None, prompt_adapter_config=None, quant_config=None)
        stat_logger = AphroditeProductionStatLogger(local_interval=10.0, production_monitor=self.monitor, aphrodite_config=aphrodite_config)
        from aphrodite.engine.metrics_types import Stats
        stats = Stats(now=time.time(), num_running_sys=3, num_waiting_sys=1, num_swapped_sys=0, gpu_cache_usage_sys=0.8, cpu_cache_usage_sys=0.5, cpu_prefix_cache_hit_rate=0.9, gpu_prefix_cache_hit_rate=0.95, num_prompt_tokens_iter=100, num_generation_tokens_iter=200, num_tokens_iter=300, time_to_first_tokens_iter=[0.4, 0.5], time_per_output_tokens_iter=[0.1, 0.1], num_preemption_iter=0, time_e2e_requests=[1.0, 1.2], time_queue_requests=[0.1, 0.1], time_inference_requests=[0.8, 1.0], time_prefill_requests=[0.2, 0.3], time_decode_requests=[0.6, 0.7], num_prompt_tokens_requests=[50, 50], num_generation_tokens_requests=[100, 100], n_requests=[1, 1], max_num_generation_tokens_requests=[200, 200], max_tokens_requests=[300, 300], finished_reason_requests=['stop', 'stop'], waiting_lora_adapters=[], running_lora_adapters=[], max_lora='0')
...
async def test_end_to_end_integration():
    ...
    try:
        ...
        assert latest_health.overall_status in ['degraded', 'critical'], f'Health status should be degraded, got: {latest_health.overall_status}'
        summary = monitor.get_production_summary()
        assert summary['current_health']['active_violations'] > 0, 'Should have active SLA violations'
        assert summary['alert_statistics']['total_24h'] > 0, 'Should have generated alerts'
...
if __name__ == '__main__':
    ...
    print('=' * 50)
    test_classes = [TestSLAManager, TestRecoveryEngine, TestRCAEngine, TestProductionMonitor]
