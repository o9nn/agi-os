...
def test_custom_executor_type_checking(model):
    ...
    with pytest.raises(TypeError):
        engine_args = AsyncEngineArgs(model=model, distributed_executor_backend=CustomGPUExecutor)
        AsyncAphrodite.from_engine_args(engine_args)
@pytest.mark.parametrize('model', ['facebook/opt-125m'])
def test_custom_executor(model, tmpdir):
    ...
    try:
        ...
        engine_args = EngineArgs(model=model, distributed_executor_backend=CustomGPUExecutor)
...
def test_custom_executor_async(model, tmpdir):
    ...
    try:
        assert not os.path.exists('.marker')
        engine_args = AsyncEngineArgs(model=model, distributed_executor_backend=CustomGPUExecutorAsync)
