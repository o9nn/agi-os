...
def add_seq_group_to_engine(engine: AphroditeEngine, seq_group: SequenceGroup):
...
def test_num_computed_tokens_update(num_scheduler_steps: int, enable_chunked_prefill: bool, enforce_eager: bool):
    ...
    runner = AphroditeRunner(model_name=MODEL, gpu_memory_utilization=0.7, num_scheduler_steps=num_scheduler_steps, enable_chunked_prefill=enable_chunked_prefill, enforce_eager=enforce_eager)
