...
@pytest.mark.parametrize('device', CUDA_DEVICES)
@torch.inference_mode()
def test_deterministic_when_seeded(k: int, vocab_size: int, batch_size: int, frac_seeded: float, n_rep: int, device: str):
    torch.set_default_device(device)
    rejection_sampler = RejectionSampler()
    rejection_sampler.init_gpu_tensors(device=device)
    draft_probs = torch.rand(batch_size, k, vocab_size, dtype=torch.float32)
...
@torch.inference_mode()
def test_rejection_sampling_approximates_target_distribution(seed: int, draft_and_target_probs_equal: bool):
    torch.set_default_device('cpu')
    set_random_seed(seed)
    helper = _CorrectnessTestHelper(vocab_size=10, rejection_sampler=RejectionSampler())
...
class _CorrectnessTestHelper:
    ...
    def _estimate_rejection_sampling_pdf(self, draft_probs: torch.Tensor, target_probs: torch.Tensor, num_samples: int) -> torch.Tensor:
        ...
        target_probs = target_probs.reshape(1, 1, self.vocab_size).repeat(num_samples, self.k, 1)
