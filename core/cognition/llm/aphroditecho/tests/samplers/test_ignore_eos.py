...
def test_ignore_eos(aphrodite_runner, example_prompts, model: str, dtype: str, max_tokens: int) -> None:
    with aphrodite_runner(model, dtype=dtype) as aphrodite_model:
        sampling_params = SamplingParams(max_tokens=max_tokens, ignore_eos=True)
        for prompt in example_prompts:
            ignore_eos_output = aphrodite_model.model.generate(prompt, sampling_params=sampling_params)
