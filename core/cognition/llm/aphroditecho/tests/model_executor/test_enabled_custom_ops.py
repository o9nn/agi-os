...
from aphrodite.modeling.layers.fused_moe.fused_moe import dispatch_fused_experts_func, dispatch_topk_func, torch_aphrodite_inplace_fused_experts, torch_aphrodite_outplace_fused_experts, aphrodite_topk_softmax
from aphrodite.modeling.layers.fused_moe.rocm_aiter_fused_moe import is_rocm_aiter_moe_enabled
from aphrodite.modeling.layers.layernorm import RMSNorm, dispatch_cuda_rmsnorm_func, fused_add_rms_norm, rms_norm, rocm_aiter_fused_add_rms_norm, rocm_aiter_rms_norm
from aphrodite.platforms import current_platform
@CustomOp.register('relu3')
class Relu3(ReLUSquaredActivation):
    pass
@pytest.mark.parametrize('env, torch_level, ops_enabled, default_on', [('', 0, [True] * 4, True), ('', 1, [True] * 4, True), ('', 2, [True] * 4, True), ('', 3, [False] * 4, False), ('', 4, [False] * 4, False), ('+rms_norm,-silu_and_mul', 0, [1, 0, 1, 1], True), ('none,-rms_norm,+relu3', 0, [0, 0, 0, 1], False), ('all,-silu_and_mul', 1, [1, 0, 1, 1], True), ('-relu3,relu2', 1, [1, 1, 1, 0], True), ('none,-relu3,+gelu_and_mul,+silu_and_mul', 2, [0, 1, 1, 0], False), ('-rms_norm', 2, [0, 1, 1, 1], True), ('-silu_and_mul,+relu3', 3, [0, 0, 0, 1], False), ('all,-rms_norm', 4, [0, 1, 1, 1], True)])
def test_enabled_ops(env: str, torch_level: int, ops_enabled: list[int], default_on: bool):
    aphrodite_config = AphroditeConfig(compilation_config=CompilationConfig(level=torch_level, custom_ops=env.split(',')))
