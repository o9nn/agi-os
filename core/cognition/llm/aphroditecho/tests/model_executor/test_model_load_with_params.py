...
def test_model_loading_with_params(aphrodite_runner):
    with aphrodite_runner(model_name=MODEL_NAME, revision=REVISION, dtype='float16', max_model_len=MAX_MODEL_LEN) as aphrodite_model:
        output = aphrodite_model.encode('Write a short story about a robot that dreams for the first time.\n')
...
def test_roberta_model_loading_with_params(aphrodite_runner):
    with aphrodite_runner(model_name=MODEL_NAME_ROBERTA, revision=REVISION_ROBERTA, dtype='float16', max_model_len=MAX_MODEL_LEN) as aphrodite_model:
        output = aphrodite_model.encode('Write a short story about a robot that dreams for the first time.\n')
        model_config = aphrodite_model.model.llm_engine.model_config
        model_tokenizer = aphrodite_model.model.llm_engine.tokenizer
        assert model_config.encoder_config['max_seq_length'] == 512
