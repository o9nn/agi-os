...
def setup_servers():
    ...
    APHRODITE_HOST_IP = subprocess.check_output("hostname -I | awk '{print $1}'", shell=True).decode().strip()
    os.environ['APHRODITE_HOST_IP'] = APHRODITE_HOST_IP
    prefill_cmd = [sys.executable, '-m', 'aphrodite.endpoints.openai.api_server', '--model', 'meta-llama/Llama-3.2-1B-Instruct', '--port', '8100', '--gpu-memory-utilization', '0.5', '--max-model-len', '1000', '--kv-transfer-config', '{"kv_connector":"PyNcclConnector","kv_role":"kv_producer","kv_rank":0,"kv_parallel_size":2}']
    prefill_env = os.environ.copy()
    prefill_env['CUDA_VISIBLE_DEVICES'] = '0'
    prefill_proc = Popen(prefill_cmd, env=prefill_env)
    decode_cmd = [sys.executable, '-m', 'aphrodite.endpoints.openai.api_server', '--model', 'meta-llama/Llama-3.2-1B-Instruct', '--port', '8200', '--gpu-memory-utilization', '0.5', '--max-model-len', '1000', '--kv-transfer-config', '{"kv_connector":"PyNcclConnector","kv_role":"kv_consumer","kv_rank":1,"kv_parallel_size":2}']
