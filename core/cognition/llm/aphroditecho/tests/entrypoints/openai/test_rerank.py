...
async def test_activation(server: RemoteOpenAIServer, model_name: str):
    async def get_outputs(activation):
        query = 'What is the capital of France?'
        documents = ['The capital of Brazil is Brasilia.', 'The capital of France is Paris.']
        response = requests.post(server.url_for('rerank'), json={'model': model_name, 'query': query, 'documents': documents, 'activation': activation})
    ...
    assert torch.allclose(default, w_activation, atol=0.01), 'Default should use activation.'
    assert not torch.allclose(w_activation, wo_activation, atol=0.01), 'wo_activation should not use activation.'
    assert torch.allclose(F.sigmoid(wo_activation), w_activation, atol=0.01), 'w_activation should be close to activation(wo_activation).'