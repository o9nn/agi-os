...
def test_empty_file():
    with tempfile.NamedTemporaryFile('w') as input_file, tempfile.NamedTemporaryFile('r') as output_file:
        input_file.write('')
        input_file.flush()
        proc = subprocess.Popen(['aphrodite', 'run-batch', '-i', input_file.name, '-o', output_file.name, '--model', 'intfloat/multilingual-e5-small'])
        proc.communicate()
        proc.wait()
...
def test_completions():
    with tempfile.NamedTemporaryFile('w') as input_file, tempfile.NamedTemporaryFile('r') as output_file:
        ...
        input_file.flush()
        proc = subprocess.Popen(['aphrodite', 'run-batch', '-i', input_file.name, '-o', output_file.name, '--model', 'NousResearch/Meta-Llama-3-8B-Instruct'])
        proc.communicate()
        proc.wait()
        assert proc.returncode == 0, f'proc={proc!r}'
        contents = output_file.read()
...
def test_score(input_batch):
    with tempfile.NamedTemporaryFile('w') as input_file, tempfile.NamedTemporaryFile('r') as output_file:
        input_file.write(input_batch)
        input_file.flush()
        proc = subprocess.Popen(['aphrodite', 'run-batch', '-i', input_file.name, '-o', output_file.name, '--model', 'BAAI/bge-reranker-v2-m3'])
        proc.communicate()
        proc.wait()
        assert proc.returncode == 0, f'proc={proc!r}'
        contents = output_file.read()
