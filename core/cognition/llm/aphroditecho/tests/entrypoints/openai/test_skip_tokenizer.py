...
MODEL_NAME = 'christian-pinto/Prithvi-EO-2.0-300M-TL-APHRODITE'
DTYPE = 'float16'
...
def server():
    args = ['--runner', 'pooling', '--dtype', DTYPE, '--enforce-eager', '--trust-remote-code', '--skip-tokenizer-init', '--max-num-seqs', '32']
...
async def test_single_request(server: RemoteOpenAIServer, model_name: str):
    ...
    prompt = {'model': model_name, 'additional_data': {'prompt_token_ids': [1]}, 'encoding_format': 'base64', 'messages': [{'role': 'user', 'content': [{'type': 'image_embeds', 'image_embeds': {'pixel_values': base64_tensor_embedding, 'location_coords': base64_coord_embedding}}]}]}
