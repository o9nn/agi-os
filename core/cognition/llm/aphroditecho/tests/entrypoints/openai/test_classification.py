...
async def test_activation(server: RemoteOpenAIServer, model_name: str):
    ...
    async def get_outputs(activation):
        response = requests.post(server.url_for('classify'), json={'model': model_name, 'input': input_text, 'activation': activation})
    ...
    assert torch.allclose(F.softmax(wo_activation, dim=-1), w_activation, atol=0.01), 'w_activation should be close to activation(wo_activation).'