...
def test_extract_tool_calls_multiple_json(parser):
    model_output = '{"name": "searchTool", "parameters": {"query": "test1"}}; {"name": "getOpenIncidentsTool", "parameters": {}}; {"name": "searchTool", "parameters": {"query": "test2"}}'
    result = parser.extract_tool_calls(model_output, None)
    assert result.tools_called is True
...
def test_extract_tool_calls_multiple_json_with_surrounding_text(parser):
    model_output = 'Here are the results: {"name": "searchTool", "parameters": {"query": "test1"}}; {"name": "getOpenIncidentsTool", "parameters": {}}; {"name": "searchTool", "parameters": {"query": "test2"}} Would you like to know more?'
    result = parser.extract_tool_calls(model_output, None)
    assert result.tools_called is True
    assert len(result.tool_calls) == 3
    assert result.tool_calls[0].function.name == 'searchTool'
