...
def api_server_args():
    sock = socket.socket()
    return {'target_server_fn': mock_run_api_server_worker, 'listen_address': 'localhost:2242', 'sock': sock, 'args': 'test_args', 'num_servers': 3, 'input_addresses': ['tcp://127.0.0.1:5001', 'tcp://127.0.0.1:5002', 'tcp://127.0.0.1:5003'], 'output_addresses': ['tcp://127.0.0.1:6001', 'tcp://127.0.0.1:6002', 'tcp://127.0.0.1:6003'], 'stats_update_address': 'tcp://127.0.0.1:7000'}
