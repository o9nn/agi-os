...
from aphrodite.spec_decode.util import get_sampled_token_logprobs, split_batch_by_proposal_len
def test_get_all_seq_ids():
    expected_seq_ids = list(range(10)) + list(range(100, 110))
    seq_group_metadata_list = [SequenceGroupMetadata(request_id=str(seq_id), is_prompt=True, seq_data={seq_id: MagicMock()}, sampling_params=MagicMock(), block_tables={seq_id: MagicMock()}, lora_request=None) for seq_id in expected_seq_ids]
...
def fake_sequence_group_metadata():
    ...
    return [SequenceGroupMetadata(request_id=str(i), is_prompt=True, seq_data={i: MagicMock()}, sampling_params=MagicMock(), block_tables={i: MagicMock()}, lora_request=None) for i in seq_ids]
def test_filter_zero_length_proposals(fake_sequence_group_metadata):
    proposal_lens = [0, 1, 0]
    _, (filtered_groups, indices) = split_batch_by_proposal_len(fake_sequence_group_metadata, proposal_lens)
...
def test_filter_non_zero_length_proposals(fake_sequence_group_metadata):
    proposal_lens = [0, 1, 2]
    (filtered_groups, indices), _ = split_batch_by_proposal_len(fake_sequence_group_metadata, proposal_lens)
...
def test_get_sampled_token_logprobs():
    logprob_tensor = torch.tensor([[[-0.1, -0.1]] * 2])
    sampled_token_tensor = torch.tensor([[1, 0]])
