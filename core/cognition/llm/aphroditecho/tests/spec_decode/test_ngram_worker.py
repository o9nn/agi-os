...
def test_ngram_algo_correctness_for_single_no_match():
    ...
    vocab_size = 32000
    device = 'cuda:0'
    ngram_worker = create_worker(NGramWorker, model_name, block_size, num_gpu_blocks, seed)
    proposer = Top1Proposer(worker=ngram_worker, device=device, vocab_size=vocab_size, max_proposal_len=20)
    ngram_worker.set_ngram_window_size(1, 3)
    prompts = [[1, 2, 3, 4, 5, 6, 7]]
    proposal_len = 5
    final_prompt_lens = [len(prompt) + proposal_len for prompt in prompts]
    seq_group_metadata_list = create_seq_group_metadata_from_prompts(prompts, num_gpu_blocks, block_size, final_prompt_lens=final_prompt_lens)
    proposals = proposer.get_spec_proposals(execute_model_req=ExecuteModelRequest(seq_group_metadata_list=seq_group_metadata_list, num_lookahead_slots=proposal_len), seq_ids_with_bonus_token_in_last_step=None)
...
def test_ngram_algo_correctness_for_batches_not_match_all():
    ...
    proposer = Top1Proposer(worker=ngram_worker, device=device, vocab_size=vocab_size, max_proposal_len=20)
    ngram_worker.set_ngram_window_size(1, 3)
    prompts = [[1, 2, 3, 4, 5, 6, 7], [11, 12, 13, 14, 15, 16, 11], [21, 21, 22, 23, 24, 25, 26, 21, 22], [31, 32, 31, 32, 33, 34, 35, 36, 37, 38, 31, 32, 33], [31, 32, 31, 32, 31, 32, 31, 32, 31, 32, 31, 32, 33, 34, 35, 36, 37, 38, 31, 32, 33]]
    proposal_len = 5
    final_prompt_lens = [len(prompt) + proposal_len for prompt in prompts]
    seq_group_metadata_list = create_seq_group_metadata_from_prompts(prompts, num_gpu_blocks, block_size, final_prompt_lens=final_prompt_lens)
    ...
    assert proposals.proposal_lens.tolist() == [0] + [proposal_len for _ in range(3)] + [0]
...
def test_ngram_algo_correctness_for_batches_match_all():
    ...
    proposer = Top1Proposer(worker=ngram_worker, device=device, vocab_size=vocab_size, max_proposal_len=20)
    ngram_worker.set_ngram_window_size(1, 3)
    prompts = [[11, 12, 13, 14, 15, 16, 11], [21, 21, 22, 23, 24, 25, 26, 21, 22], [31, 32, 31, 32, 33, 34, 35, 36, 37, 38, 31, 32, 33]]
    proposal_len = 5
    final_prompt_lens = [len(prompt) + proposal_len for prompt in prompts]
    seq_group_metadata_list = create_seq_group_metadata_from_prompts(prompts, num_gpu_blocks, block_size, final_prompt_lens=final_prompt_lens)
    for sg in seq_group_metadata_list:
