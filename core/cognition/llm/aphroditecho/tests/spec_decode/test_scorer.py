...
from aphrodite.spec_decode.interfaces import SpeculativeProposals, SpeculativeScores
...
def create_proposal(propose_lens: list[int], vocab_size: int, device: str) -> SpeculativeProposals:
    batch_size = len(propose_lens)
    max_propose_len = max(propose_lens)
    proposal_probs = torch.rand((batch_size, max_propose_len, vocab_size), device=device)
    proposal_token_ids = torch.full((batch_size, max_propose_len), fill_value=-1, device=device)
    for i in range(batch_size):
        proposal_token_ids[i][:propose_lens[i]] = torch.argmax(proposal_probs[i][:propose_lens[i]], dim=-1)
...
def assert_score_equal(score1: SpeculativeScores, score2: SpeculativeScores) -> None:
    ...
    assert torch.equal(score1.token_ids, score2.token_ids), f'{score1.token_ids}, {score2.token_ids}'
...
def test_scorer(model_name: str, batch_size: int, max_propose_len: int, mixed_propose_len: bool, device: str, prefill_chunking: bool) -> None:
    seed = 0
    block_size = 32
    num_gpu_blocks = 2048 // block_size
    scorer_worker = create_worker(Worker, model_name, block_size, num_gpu_blocks, seed)
    scorer_worker.model_runner.disable_logprobs = True
    ...
    if not mixed_propose_len:
        ...
        propose_lens = [max_propose_len] * non_zero_cnt + [0] * (batch_size - non_zero_cnt)
        random.shuffle(propose_lens)
    seq_group_metadatalist, _, _ = create_batch(batch_size, max_propose_len, block_size=block_size, num_gpu_blocks=num_gpu_blocks)
    if mixed_propose_len and prefill_chunking and (n_prefills := (batch_size - non_zero_cnt)):
        prefill, _, _ = create_batch(n_prefills, None, prefill_chunk_size=4, block_size=block_size, num_gpu_blocks=num_gpu_blocks, seq_ids=list(range(batch_size, batch_size + n_prefills)))
        target_group_metadatalist = [seq_group_metadatalist[i] for i, p in enumerate(propose_lens) if p > 0]
    ...
    requests = ExecuteModelRequest(seq_group_metadatalist, num_lookahead_slots=max_propose_len)
