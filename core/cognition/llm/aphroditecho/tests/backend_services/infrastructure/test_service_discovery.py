...
from backend_services.infrastructure.service_discovery import ServiceDiscovery, ServiceEndpoint, ServiceHealth, ServiceStatus, ServiceType
class TestServiceDiscovery:
    @pytest.fixture
    def service_discovery(self):
        return ServiceDiscovery(redis_url='redis://localhost:6379', health_check_interval=1.0, health_check_timeout=2.0, max_consecutive_failures=2, service_ttl=60)
    @pytest.fixture
    def sample_endpoint(self):
        return ServiceEndpoint(service_id='test-service-1', service_type=ServiceType.COGNITIVE_SERVICE, host='localhost', port=8080, protocol='http', path='/', weight=1.0, metadata={'version': '1.0.0', 'region': 'us-west'})
    @pytest.fixture
    def dtesn_endpoint(self):
        return ServiceEndpoint(service_id='dtesn-membrane-1', service_type=ServiceType.DTESN_MEMBRANE, host='localhost', port=8081, protocol='http', path='/membrane', weight=2.0, metadata={'membrane_type': 'memory', 'capacity': '1000'})
    @pytest.mark.asyncio
    ...
    async def test_service_cleanup(self, service_discovery, sample_endpoint):
        service_discovery.service_ttl = 1
        await service_discovery.initialize()
        await service_discovery.register_service(sample_endpoint)
    ...
    def test_service_endpoint_properties(self, sample_endpoint):
        ...
        endpoint_with_path = ServiceEndpoint(service_id='test-2', service_type=ServiceType.API_GATEWAY, host='api.example.com', port=443, protocol='https', path='/api/v1')
    ...
    def test_service_health_properties(self):
        health = ServiceHealth(service_id='test-service', status=ServiceStatus.HEALTHY, last_check=time.time(), response_time_ms=150.0, error_count=2, consecutive_failures=0)
...
class TestServiceDiscoveryIntegration:
    ...
    async def test_dtesn_service_scenario(self):
        ...
        services = [ServiceEndpoint(service_id=f'dtesn-memory-{i}', service_type=ServiceType.DTESN_MEMBRANE, host='localhost', port=8080 + i, metadata={'membrane_type': 'memory', 'instance': i}) for i in range(3)]
    ...
    async def test_load_balancer_integration_scenario(self):
        discovery = ServiceDiscovery(health_check_interval=0.5)
        await discovery.initialize()
        services = [ServiceEndpoint(service_id='cognitive-1', service_type=ServiceType.COGNITIVE_SERVICE, host='localhost', port=8001, weight=2.0), ServiceEndpoint(service_id='cognitive-2', service_type=ServiceType.COGNITIVE_SERVICE, host='localhost', port=8002, weight=1.0)]
        for service in services:
            await discovery.register_service(service)
            discovery.service_health[service.service_id].status = ServiceStatus.HEALTHY
    ...
    async def test_service_failure_recovery_scenario(self):
        discovery = ServiceDiscovery(health_check_interval=0.1, max_consecutive_failures=2)
        await discovery.initialize()
        endpoint = ServiceEndpoint(service_id='test-service', service_type=ServiceType.COGNITIVE_SERVICE, host='localhost', port=8080)
