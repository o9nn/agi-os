...
ACCURACY_CONFIGS = [GSM8KAccuracyTestConfig(model_name='neuralmagic/Meta-Llama-3.1-8B-Instruct-quantized.w8a8', excepted_value=0.76)]
@pytest.mark.parametrize('config', ACCURACY_CONFIGS)
def test_gsm8k_correctness(config: GSM8KAccuracyTestConfig):
    results = lm_eval.simple_evaluate(model='aphrodite', model_args=config.get_model_args(), tasks='gsm8k', batch_size='auto')
