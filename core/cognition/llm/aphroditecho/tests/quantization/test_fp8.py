...
def test_model_load_and_run(aphrodite_runner, model_id: str, force_marlin: bool, use_rocm_aiter: bool, monkeypatch) -> None:
    if use_rocm_aiter:
...
@pytest.mark.parametrize('force_marlin', [False, True])
@pytest.mark.parametrize('use_rocm_aiter', [True, False] if current_platform.is_rocm() else [False])
def test_load_fp16_model(aphrodite_runner, kv_cache_dtype: str, force_marlin: bool, use_rocm_aiter: bool, monkeypatch) -> None:
    if use_rocm_aiter:
        monkeypatch.setenv('APHRODITE_ROCM_USE_AITER', '1')
    monkeypatch.setenv('APHRODITE_USE_V1', '0')
    if force_marlin:
