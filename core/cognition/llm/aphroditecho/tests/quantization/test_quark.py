import pytest
from aphrodite.quantization.quark.quark import QuarkLinearMethod, QuarkW8A8Fp8, QuarkW8A8Int8
...
def test_quark_fp8_w_per_tensor_a_per_tensor(aphrodite_runner, kv_cache_dtype, tp):
    ...
    with aphrodite_runner(model_path, kv_cache_dtype=kv_cache_dtype, tensor_parallel_size=tp) as llm:
...
def test_quark_int8_w_per_tensor_a_per_tensor(aphrodite_runner, tp):
    ...
    with aphrodite_runner(model_path, tensor_parallel_size=tp) as llm:
        def check_model(model):
            layer = model.model.layers[0]
            qkv_proj = layer.self_attn.qkv_proj
            assert isinstance(qkv_proj.quant_method, QuarkLinearMethod)
