...
try:
    from aphrodite.modeling.dynamic_loader import DynamicModelLoader, ModelResourceUsage, LoadedModelInfo, AphroditeDynamicModelLoader, ResourceMonitor, RequestRouter
...
try:
    from echo_self.integration.aphrodite_adaptive import AdaptiveModelLoader, AphroditeAdaptiveIntegration
...
class TestDynamicModelLoader:
    @pytest.fixture
    def loader(self):
        return DynamicModelLoader(max_models=3, memory_limit_gb=4.0, eviction_policy='lru')
    @pytest.fixture
    def mock_model_config(self):
        return ModelConfig(model='test-model', tokenizer='test-model', tokenizer_mode='auto', trust_remote_code=False, dtype='float16', max_model_len=512, seed=42)
    @pytest.fixture
    def mock_model(self):
        model = Mock(spec=torch.nn.Module)
        model.parameters.return_value = [Mock(numel=Mock(return_value=1000), element_size=Mock(return_value=4))]
    ...
    async def test_load_model_success(self, loader, mock_model_config, mock_model):
        with patch('aphrodite.modeling.model_loader.get_model_loader') as mock_get_loader, patch('aphrodite.modeling.model_loader.utils.initialize_model', return_value=mock_model):
            mock_loader = Mock()
...
class TestModelResourceUsage:
    def test_initialization(self):
        usage = ModelResourceUsage(memory_mb=100.0, gpu_memory_mb=200.0, cpu_cores=2.0, load_time_ms=500.0, last_access_time=time.time(), request_count=10)
