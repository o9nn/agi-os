...
def generate(self, pixel_values: torch.FloatTensor, input_ids: torch.FloatTensor, attention_mask: Optional[torch.LongTensor]=None, **generate_kwargs) -> torch.LongTensor:
    ...
    input_embeds = input_embeds.reshape(B, N, C)
    outputs = self.language_model.generate(inputs_embeds=input_embeds, attention_mask=attention_mask, **generate_kwargs)
    return outputs
def run_test(hf_runner: Type[HfRunner], aphrodite_runner: Type[AphroditeRunner], inputs: List[Tuple[List[str], PromptImageInput]], model: str, *, dtype: str, max_tokens: int, num_logprobs: int, mm_limit: int, tensor_parallel_size: int, distributed_executor_backend: Optional[str]=None):
    class InternVLProcessor:
        ...
        def __call__(self, text: str, images: Union[Image, List[Image]], **kwargs):
            from aphrodite.modeling.models.internvl import IMG_CONTEXT, IMG_END, IMG_START, image_to_pixel_values
            images = [images] if isinstance(images, Image) else images
            pixel_values = [image_to_pixel_values(image, self.image_size, self.min_num, self.max_num, self.use_thumbnail).to(self.dtype) for image in images]
            ...
            return prompt
    with aphrodite_runner(model, max_model_len=4096, dtype=dtype, limit_mm_per_prompt={'image': mm_limit}, tensor_parallel_size=tensor_parallel_size, distributed_executor_backend=distributed_executor_backend, enforce_eager=True) as aphrodite_model:
    ...
    with hf_runner(model, dtype=dtype) as hf_model:
        ...
        hf_outputs_per_image = [hf_model.generate_greedy_logprobs_limit(prompts, max_tokens, num_logprobs=num_logprobs, images=hf_images, eos_token_id=eos_token_id) for prompts, hf_images in inputs]
...
def run_awq_test(aphrodite_runner: Type[AphroditeRunner], image_assets: _ImageAssets, models: Tuple[str, str], *, size_factors: List[float], dtype: str, max_tokens: int, num_logprobs: int, tensor_parallel_size: int, distributed_executor_backend: Optional[str]=None):
    ...
    with aphrodite_runner(source_model, max_model_len=4096, dtype=dtype, tensor_parallel_size=tensor_parallel_size, distributed_executor_backend=distributed_executor_backend, enforce_eager=True) as aphrodite_model:
        source_outputs_per_image = [aphrodite_model.generate_greedy_logprobs(prompts, max_tokens, num_logprobs=num_logprobs, images=images) for prompts, images in inputs_per_image]
    with aphrodite_runner(quant_model, quantization='awq', max_model_len=4096, dtype=dtype, tensor_parallel_size=tensor_parallel_size, distributed_executor_backend=distributed_executor_backend, enforce_eager=True) as aphrodite_model:
        quant_outputs_per_image = [aphrodite_model.generate_greedy_logprobs(prompts, max_tokens, num_logprobs=num_logprobs, images=images) for prompts, images in inputs_per_image]
...
@pytest.mark.parametrize('model', models)
@pytest.mark.parametrize('size_factors', [[], [1.0], [1.0, 1.0, 1.0], [0.5, 0.75, 1.0]])
@pytest.mark.parametrize('dtype', [target_dtype])
@pytest.mark.parametrize('max_tokens', [128])
@pytest.mark.parametrize('num_logprobs', [5])
@torch.inference_mode()
...
@pytest.mark.parametrize('num_logprobs', [5])
@torch.inference_mode()
def test_different_num_patches(hf_runner, aphrodite_runner, image_assets, model, size_factors, dtype: str, max_tokens: int, num_logprobs: int) -> None:
    images = [asset.pil_image.resize((896, 896)) for asset in image_assets]
    inputs_batching = [([prompt for _ in size_factors], [rescale_image_size(image, factor) for factor in size_factors]) for image, prompt in zip(images, HF_IMAGE_PROMPTS)]
    inputs_multi_images = [([HF_MULTIIMAGE_IMAGE_PROMPT for _ in size_factors], [[rescale_image_size(image, factor) for image in images] for factor in size_factors])]
...
@pytest.mark.parametrize('num_logprobs', [5])
@torch.inference_mode()
def test_awq_models(aphrodite_runner, image_assets, models, size_factors, dtype: str, max_tokens: int, num_logprobs: int) -> None:
    run_awq_test(aphrodite_runner, image_assets, models, size_factors=size_factors, dtype=dtype, max_tokens=max_tokens, num_logprobs=num_logprobs, tensor_parallel_size=1)