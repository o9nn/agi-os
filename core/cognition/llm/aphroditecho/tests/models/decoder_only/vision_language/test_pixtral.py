...
@pytest.mark.skip(reason='Model is too big, test passed on A100 locally but will OOM on CI machine.')
@pytest.mark.parametrize('model', MODELS)
@pytest.mark.parametrize('max_model_len', MAX_MODEL_LEN)
@pytest.mark.parametrize('dtype', ['bfloat16'])
def test_chat(aphrodite_runner, max_model_len: int, model: str, dtype: str) -> None:
    EXPECTED_CHAT_LOGPROBS = load_outputs_w_logprobs(FIXTURE_LOGPROBS_CHAT)
    with aphrodite_runner(model, dtype=dtype, tokenizer_mode='mistral', enable_chunked_prefill=False, max_model_len=max_model_len, limit_mm_per_prompt=LIMIT_MM_PER_PROMPT) as aphrodite_model:
        outputs = []
        for msg in MSGS:
            output = aphrodite_model.model.chat(msg, sampling_params=SAMPLING_PARAMS)
...
def test_model_engine(aphrodite_runner, model: str, dtype: str) -> None:
    ...
    args = EngineArgs(model=model, tokenizer_mode='mistral', enable_chunked_prefill=False, limit_mm_per_prompt=LIMIT_MM_PER_PROMPT, dtype=dtype)
    ...
    while True:
        ...
        if not engine.has_unfinished_requests():
            break
    logprobs = aphrodite_runner._final_steps_generate_w_logprobs(outputs)
    check_logprobs_close(outputs_0_lst=EXPECTED_ENGINE_LOGPROBS, outputs_1_lst=logprobs, name_0='h100_ref', name_1='output')