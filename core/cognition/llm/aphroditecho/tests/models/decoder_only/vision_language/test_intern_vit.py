...
models = [snapshot_download('OpenGVLab/InternViT-300M-448px', allow_patterns=DOWNLOAD_PATTERN), snapshot_download('OpenGVLab/InternViT-6B-448px-V1-5', allow_patterns=DOWNLOAD_PATTERN)]
def run_intern_vit_test(image_assets: _ImageAssets, model: str, *, dtype: str, distributed_executor_backend: Optional[str]=None):
    img_processor = CLIPImageProcessor.from_pretrained(model)
    images = [asset.pil_image for asset in image_assets]
    pixel_values = [img_processor(images, return_tensors='pt').pixel_values.to(dtype) for images in images]
...
def test_models(dist_init, image_assets, model, dtype: str) -> None:
    run_intern_vit_test(image_assets, model, dtype=dtype)