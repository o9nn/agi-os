...
HF_IMAGE_PROMPTS = IMAGE_ASSETS.prompts({'stop_sign': 'caption es', 'cherry_blossom': 'What is in the picture?'})
...
@pytest.mark.parametrize('dtype', [pytest.param('float', marks=pytest.mark.skipif(is_hip(), reason='ROCm FA does not yet fully support 32-bit precision on PaliGemma')), 'half'])
