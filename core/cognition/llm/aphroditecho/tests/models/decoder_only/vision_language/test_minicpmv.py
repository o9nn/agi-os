...
HF_IMAGE_PROMPTS = IMAGE_ASSETS.prompts({'stop_sign': "<|begin_of_text|><|start_header_id|>user<|end_header_id|>\n\n(<image>./</image>)\nWhat's the content of the image?<|eot_id|><|start_header_id|>assistant<|end_header_id|>\n\n", 'cherry_blossom': '<|begin_of_text|><|start_header_id|>user<|end_header_id|>\n\n(<image>./</image>)\nWhat is the season?<|eot_id|><|start_header_id|>assistant<|end_header_id|>\n\n'})
...
def trunc_hf_output(hf_output: Tuple[List[int], str, Optional[SampleLogprobs]]):
    output_ids, output_str, out_logprobs = hf_output
    if output_str.endswith('<|eot_id|>'):
        output_str = output_str.split('<|eot_id|>')[0]
    return (output_ids, output_str, out_logprobs)
target_dtype = 'half'
def run_test(hf_runner: Type[HfRunner], aphrodite_runner: Type[AphroditeRunner], inputs: List[Tuple[List[str], Union[List[Image.Image], List[List[Image.Image]]]]], model: str, *, dtype: str, max_tokens: int, num_logprobs: int, mm_limit: int, tensor_parallel_size: int, distributed_executor_backend: Optional[str]=None):
    with aphrodite_runner(model, max_model_len=4096, max_num_seqs=1, dtype=dtype, limit_mm_per_prompt={'image': mm_limit}, tensor_parallel_size=tensor_parallel_size, distributed_executor_backend=distributed_executor_backend, enforce_eager=True) as aphrodite_model:
        ...
        aphrodite_outputs_per_image = [aphrodite_model.generate_greedy_logprobs(prompts, max_tokens, num_logprobs=num_logprobs, images=images, stop_token_ids=stop_token_ids) for prompts, images in inputs]
    hf_model = hf_runner(model, dtype=dtype, postprocess_inputs=_wrap_inputs)
    with hf_model, torch.no_grad():
        hf_outputs_per_image = [hf_model.generate_greedy_logprobs_limit(prompts, max_tokens, num_logprobs=num_logprobs, images=images, tokenizer=tokenizer) for prompts, images in inputs]
    for hf_outputs, aphrodite_outputs in zip(hf_outputs_per_image, aphrodite_outputs_per_image):
        check_logprobs_close(outputs_0_lst=[trunc_hf_output(hf_output) for hf_output in hf_outputs], outputs_1_lst=aphrodite_outputs, name_0='hf', name_1='aphrodite')
