...
def _run_test(hf_runner: Type[HfRunner], aphrodite_runner: Type[AphroditeRunner], inputs: List[Tuple[List[str], PromptImageInput]], model: str, *, dtype: str, max_tokens: int, num_logprobs: int, tensor_parallel_size: int, distributed_executor_backend: Optional[str]=None):
    if model.startswith('TIGER-Lab/Mantis'):
        ...
        mantis_processor = MLlavaProcessor.from_pretrained(model, torch_dtype=torch_dtype)
        assert isinstance(mantis_processor, MLlavaProcessor)
    else:
        mantis_processor = None
    with aphrodite_runner(model, dtype=dtype, max_model_len=4096, tensor_parallel_size=tensor_parallel_size, distributed_executor_backend=distributed_executor_backend, enforce_eager=True, limit_mm_per_prompt={'image': _LIMIT_IMAGE_PER_PROMPT}) as aphrodite_model:
...
def test_models_multiple_image_inputs(hf_runner, aphrodite_runner, image_assets, model, dtype, max_tokens, num_logprobs) -> None:
    ...
    inputs = [(['USER: <image><image>\nDescribe 2 images.\nASSISTANT:', 'USER: <image><image>\nDescribe 2 images.\nASSISTANT:', 'USER: <image><image><image><image>\nDescribe 4 images.\nASSISTANT:', 'USER: <image>\nWhat is the season?\nASSISTANT:'], [[stop_sign, cherry_blossom], [rescale_image_size(stop_sign, 0.1), stop_sign], [stop_sign, rescale_image_size(stop_sign, 0.25), cherry_blossom.resize((183, 488)), cherry_blossom.resize((488, 183))], cherry_blossom])]
...
def test_context_length_too_short(aphrodite_runner, image_assets, model):
    ...
    with pytest.raises(ValueError, match='too long to fit into the model'):
        aphrodite_model = aphrodite_runner(model, max_model_len=128, enforce_eager=True)
