...
@pytest.mark.skipif(not is_quant_method_supported('aqlm'), reason='AQLM is not supported on this GPU type.')
@pytest.mark.parametrize('model', ['ISTA-DASLab/Llama-2-7b-AQLM-2Bit-1x16-hf'])
@pytest.mark.parametrize('dtype', ['half'])
@pytest.mark.parametrize('max_tokens', [16])
@pytest.mark.parametrize('num_logprobs', [1])
def test_models(aphrodite_runner, example_prompts, model: str, dtype: str, max_tokens: int, num_logprobs: int) -> None:
    ...
    for prompt_idx in range(len(example_prompts)):
        aphrodite_output_ids, aphrodite_output_str, aphrodite_logprobs = aphrodite_outputs[prompt_idx]
