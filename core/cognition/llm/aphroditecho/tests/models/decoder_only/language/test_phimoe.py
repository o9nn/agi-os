...
def get_gpu_memory():
    try:
        ...
        return 0
@pytest.mark.skipif(condition=is_cpu(), reason="This test takes a lot time to run on CPU, and aphrodite CI's disk space is not enough for this model.")
@pytest.mark.skipif(condition=get_gpu_memory() < 100, reason='Skip this test if GPU memory is insufficient.')
