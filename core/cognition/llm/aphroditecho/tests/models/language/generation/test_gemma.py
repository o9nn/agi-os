...
def test_dummy_loader(aphrodite_runner, monkeypatch, model: str) -> None:
    with monkeypatch.context() as m:
        ...
        with aphrodite_runner(model, load_format='dummy') as llm:
            if model == 'google/gemma-3-4b-it':
                normalizers = llm.llm.collective_rpc(lambda self: self.model_runner.model.language_model.model.normalizer.cpu().item())
