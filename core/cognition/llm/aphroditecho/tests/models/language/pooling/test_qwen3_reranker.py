...
def test_rerank_models_mteb(aphrodite_runner, model_info: RerankModelInfo) -> None:
    ...
    aphrodite_extra_kwargs: dict[str, Any] = {'hf_overrides': {'architectures': ['Qwen3ForSequenceClassification'], 'classifier_from_token': ['no', 'yes'], 'is_original_qwen3_reranker': True}}
...
def test_rerank_models_mteb_tp(aphrodite_runner, model_info: RerankModelInfo) -> None:
    assert model_info.architecture == 'Qwen3ForSequenceClassification'
    aphrodite_extra_kwargs: dict[str, Any] = {'hf_overrides': {'architectures': ['Qwen3ForSequenceClassification'], 'classifier_from_token': ['no', 'yes'], 'is_original_qwen3_reranker': True}, 'tensor_parallel_size': 2}
    mteb_test_rerank_models(Qwen3RerankerHfRunner, aphrodite_runner, model_info, aphrodite_extra_kwargs, atol=0.012)