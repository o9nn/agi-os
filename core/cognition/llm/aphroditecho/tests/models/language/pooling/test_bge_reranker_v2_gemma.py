...
from .mteb_utils import RerankModelInfo, AphroditeMtebEncoder, mteb_test_rerank_models
...
def test_rerank_models_mteb(aphrodite_runner, model_info: RerankModelInfo, monkeypatch) -> None:
    monkeypatch.setenv('APHRODITE_USE_V1', '0')
    assert model_info.architecture == 'GemmaForSequenceClassification'
    aphrodite_extra_kwargs: dict[str, Any] = {'hf_overrides': {'architectures': ['GemmaForSequenceClassification'], 'classifier_from_token': ['Yes'], 'method': 'no_post_processing'}}
    mteb_test_rerank_models(GemmaRerankerHfRunner, aphrodite_runner, model_info, aphrodite_extra_kwargs, aphrodite_mteb_encoder=GemmaMtebEncoder)