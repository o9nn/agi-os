...
def test_embed_models_correctness(hf_runner, aphrodite_runner, model_info: EmbedModelInfo, example_prompts) -> None:
    def hf_model_callback(model):
        model.encode = partial(model.encode, task='text-matching')
    correctness_test_embed_models(hf_runner, aphrodite_runner, model_info, example_prompts, hf_model_callback=hf_model_callback)
...
def test_matryoshka(hf_runner, aphrodite_runner, model_info, dtype: str, dimensions: int, example_prompts, monkeypatch) -> None:
    ...
    with aphrodite_runner(model_info.name, runner='pooling', dtype=dtype, max_model_len=None) as aphrodite_model:
        ...
        if dimensions not in matryoshka_dimensions:
            ...
            aphrodite_outputs = aphrodite_model.embed(example_prompts, pooling_params=PoolingParams(dimensions=dimensions))
