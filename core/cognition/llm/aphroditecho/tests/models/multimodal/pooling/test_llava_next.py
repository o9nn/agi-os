...
def _run_test(hf_runner: type[HfRunner], aphrodite_runner: type[AphroditeRunner], input_texts: list[str], input_images: PromptImageInput, model: str, *, dtype: str) -> None:
    with aphrodite_runner(model, runner='pooling', dtype=dtype, max_model_len=4096, enforce_eager=True) as aphrodite_model:
...
def test_models_text(hf_runner, aphrodite_runner, image_assets, model: str, dtype: str) -> None:
    ...
    _run_test(hf_runner, aphrodite_runner, input_texts, input_images, model, dtype=dtype)
