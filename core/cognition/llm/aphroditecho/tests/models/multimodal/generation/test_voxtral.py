...
def test_models_with_multiple_audios(aphrodite_runner, audio_assets: AudioTestAssets, dtype: str, max_tokens: int, num_logprobs: int) -> None:
    aphrodite_prompt = _get_prompt(audio_assets, MULTI_AUDIO_PROMPT)
    run_multi_audio_test(aphrodite_runner, [(aphrodite_prompt, [audio.audio_and_sample_rate for audio in audio_assets])], MODEL_NAME, dtype=dtype, max_tokens=max_tokens, num_logprobs=num_logprobs, tokenizer_mode='mistral')
@pytest.mark.asyncio
