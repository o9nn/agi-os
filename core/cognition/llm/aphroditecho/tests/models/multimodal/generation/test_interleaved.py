...
def test_models(aphrodite_runner, model, dtype: str, max_tokens: int) -> None:
    ...
    with aphrodite_runner(model, runner='generate', dtype=dtype, limit_mm_per_prompt={'image': 2}, max_model_len=32768, max_num_seqs=2, tensor_parallel_size=1, enforce_eager=True) as aphrodite_model:
        aphrodite_outputs_per_case = [aphrodite_model.generate_greedy(prompts, max_tokens, images=images, videos=videos) for prompts, images, videos in inputs]
