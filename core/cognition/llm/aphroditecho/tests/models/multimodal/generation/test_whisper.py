...
def run_test(aphrodite_runner: type[AphroditeRunner], model: str, *, tensor_parallel_size: int, distributed_executor_backend: Optional[str]=None) -> None:
    prompt_list = PROMPTS * 10
    expected_list = EXPECTED[model] * 10
    with aphrodite_runner(model, dtype='half', max_model_len=448, tensor_parallel_size=tensor_parallel_size, distributed_executor_backend=distributed_executor_backend) as aphrodite_model:
...
def test_models(aphrodite_runner, model) -> None:
    run_test(aphrodite_runner, model, tensor_parallel_size=1)
@multi_gpu_test(num_gpus=2)
@pytest.mark.core_model
@pytest.mark.parametrize('model', ['openai/whisper-large-v3-turbo'])
@pytest.mark.parametrize('distributed_executor_backend', ['ray', 'mp'])
@create_new_process_for_each_test()
def test_models_distributed(aphrodite_runner, model, distributed_executor_backend) -> None:
    run_test(aphrodite_runner, model, tensor_parallel_size=2, distributed_executor_backend=distributed_executor_backend)