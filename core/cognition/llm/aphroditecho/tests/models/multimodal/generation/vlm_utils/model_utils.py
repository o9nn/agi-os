...
def qwen_aphrodite_to_hf_output(aphrodite_output: RunnerOutput, model: str) -> tuple[list[int], str, Optional[SampleLogprobs]]:
    output_ids, output_str, out_logprobs = aphrodite_output
    hf_output_str = output_str + '<|endoftext|>'
    return (output_ids, hf_output_str, out_logprobs)
def qwen2_aphrodite_to_hf_output(aphrodite_output: RunnerOutput, model: str) -> tuple[list[int], str, Optional[SampleLogprobs]]:
    output_ids, output_str, out_logprobs = aphrodite_output
    hf_output_str = output_str + '<|im_end|>'
    return (output_ids, hf_output_str, out_logprobs)
def kimiv_vl_aphrodite_to_hf_output(aphrodite_output: RunnerOutput, model: str) -> tuple[list[int], str, Optional[SampleLogprobs]]:
    output_ids, output_str, out_logprobs = aphrodite_output
...
def llava_video_aphrodite_to_hf_output(aphrodite_output: RunnerOutput, model: str) -> tuple[list[int], str, Optional[SampleLogprobs]]:
    config = AutoConfig.from_pretrained(model)
...
def h2ovl_patch_hf_runner(hf_model: HfRunner) -> HfRunner:
    class H2OVLProcessor:
        ...
        def __call__(self, text: str, images: Union[Image, list[Image]], **kwargs):
            from aphrodite.model_executor.models.h2ovl import IMG_CONTEXT, IMG_END, IMG_START, image_to_pixel_values_h2ovl
            images = [images] if isinstance(images, Image) else images
...
def skyworkr1v_patch_hf_runner(hf_model: HfRunner) -> HfRunner:
    class SkyworkR1VProcessor:
        def __init__(self, hf_runner: HfRunner):
            ...
            self.image_size = self.vision_config.image_size
        def __call__(self, text: str, images: Union[Image, list[Image]], **kwargs):
            from aphrodite.model_executor.models.skyworkr1v import IMG_CONTEXT, IMG_END, IMG_START, image_to_pixel_values_skyworkr1v
            images = [images] if isinstance(images, Image) else images
...
def internvl_patch_hf_runner(hf_model: HfRunner) -> HfRunner:
    class InternVLProcessor:
        ...
        def __call__(self, text: str, images: Union[Image, list[Image]]=None, videos: Union[npt.NDArray, list[npt.NDArray]]=None, **kwargs):
            from aphrodite.model_executor.models.internvl import IMG_CONTEXT, IMG_END, IMG_START, image_to_pixel_values_internvl, video_to_pixel_values_internvl
            images = [images] if isinstance(images, Image) else images
