...
def run_test(*, hf_runner: type[HfRunner], aphrodite_runner: type[AphroditeRunner], inputs: list[PromptWithMultiModalInput], model: str, dtype: str, max_tokens: int, num_logprobs: int, enforce_eager: bool, max_model_len: int, max_num_seqs: int, hf_output_post_proc: Optional[Callable[[RunnerOutput, str], Any]], aphrodite_output_post_proc: Optional[Callable[[RunnerOutput, str], Any]], auto_cls: type[_BaseAutoModelClass], use_tokenizer_eos: bool, comparator: Callable[..., None], get_stop_token_ids: Optional[Callable[[AnyTokenizer], list[int]]], stop_str: Optional[list[str]], limit_mm_per_prompt: dict[str, int], aphrodite_runner_kwargs: Optional[dict[str, Any]], hf_model_kwargs: Optional[dict[str, Any]], patch_hf_runner: Optional[Callable[[HfRunner], HfRunner]], runner: RunnerOption='auto', distributed_executor_backend: Optional[str]=None, tensor_parallel_size: int=1, aphrodite_embeddings: Optional[torch.Tensor]=None):
    ...
    hf_outputs_per_mm = []
    aphrodite_runner_kwargs_: dict[str, Any] = {'disable_mm_preprocessor_cache': True}
    ...
    with aphrodite_runner(model, max_model_len=max_model_len, max_num_seqs=max_num_seqs, dtype=dtype, limit_mm_per_prompt=limit_mm_per_prompt, tensor_parallel_size=tensor_parallel_size, distributed_executor_backend=distributed_executor_backend, enforce_eager=enforce_eager, runner=runner, **aphrodite_runner_kwargs_) as aphrodite_model:
        ...
        for prompts, image_data, video_data, audio_data in aphrodite_inputs:
            ...
            aphrodite_output = aphrodite_model.generate_greedy_logprobs(prompts, max_tokens, num_logprobs=num_logprobs, **aphrodite_kwargs_with_mm_data)
    ...
    hf_outputs_per_mm, aphrodite_outputs_per_mm = process_runner_outputs(model, first_runner_outputs=hf_outputs_per_mm, second_runner_outputs=aphrodite_outputs_per_mm, first_runner_processor=hf_output_post_proc, second_runner_processor=aphrodite_output_post_proc)
    for hf_outputs, aphrodite_outputs in zip(hf_outputs_per_mm, aphrodite_outputs_per_mm):
        comparator(outputs_0_lst=hf_outputs, outputs_1_lst=aphrodite_outputs, name_0='hf', name_1='aphrodite')
