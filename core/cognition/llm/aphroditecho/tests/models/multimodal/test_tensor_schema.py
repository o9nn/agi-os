...
ARCH_TO_SKIP = {'MolmoForCausalLM': 'incompatible requirements', 'MiniMaxVL01ForConditionalGeneration': 'broken model'}
def create_batched_mm_kwargs(model_config: ModelConfig, processor: BaseMultiModalProcessor) -> MultiModalKwargs:
    ...
    processor_inputs = dummy_inputs.get_dummy_processor_inputs(seq_len=model_config.max_model_len, mm_counts=mm_counts)
    mm_kwargs = processor.apply(prompt=processor_inputs.prompt, mm_data=processor_inputs.mm_data, hf_processor_mm_kwargs=processor_inputs.hf_processor_mm_kwargs, tokenization_kwargs=processor_inputs.tokenization_kwargs)['mm_kwargs']
    mm_kwargs = MultiModalKwargs.batch([mm_kwargs])
    return mm_kwargs
@pytest.mark.core_model
@pytest.mark.parametrize('model_arch', list(_MULTIMODAL_EXAMPLE_MODELS.keys()))
def test_model_tensor_schema(model_arch: str, aphrodite_runner: type[AphroditeRunner], monkeypatch):
    ...
    hf_overrides_fn = partial(dummy_hf_overrides, model_arch=model_arch, exist_overrides=model_info.hf_overrides)
    ...
    if not any((hasattr(model_cls, f'_parse_and_validate_{m}_input') for m in ['image', 'video', 'audio'])):
    ...
    def _initialize_kv_caches_v1(self, aphrodite_config):
        ...
        scheduler_kv_cache_config = get_kv_cache_config(aphrodite_config, kv_cache_specs[0], 10 * GiB_bytes)
    ...
    with patch.object(V0LLMEngine, '_initialize_kv_caches', _initialize_kv_caches_v0), patch.object(V1EngineCore, '_initialize_kv_caches', _initialize_kv_caches_v1), monkeypatch.context() as m:
        ...
        with set_default_torch_num_threads(1), aphrodite_runner(model_id, tokenizer_name=model_info.tokenizer, tokenizer_mode=model_info.tokenizer_mode, revision=model_info.revision, trust_remote_code=model_info.trust_remote_code, max_model_len=model_info.max_model_len, load_format='dummy', hf_overrides=hf_overrides_fn, limit_mm_per_prompt=limit_mm_per_prompt, enforce_eager=True) as aphrodite_model:
