...
NUM_EXPERTS = [40, 64]
TOP_KS = [6, 8]
MNK_FACTORS = [(2, 1024, 1024), (2, 1024, 1536), (2, 3072, 1024), (2, 3072, 1536), (64, 1024, 1024), (64, 1024, 1536), (64, 3072, 1024), (64, 3072, 1536), (224, 1024, 1024), (224, 1024, 1536), (224, 3072, 1024), (224, 3072, 1536)]
...
class MOETensors8Bit(MOETensors):
    ...
    def make_moe_tensors_8bit(m: int, k: int, n: int, e: int, per_act_token: bool, per_out_channel: bool) -> 'MOETensors8Bit':
        ...
        _, a_scale = ops.scaled_fp8_quant(moe_tensors_fp16.a, use_per_token_if_dynamic=per_act_token)
        a_q, _ = ops.scaled_fp8_quant(moe_tensors_fp16.a, a_scale, use_per_token_if_dynamic=per_act_token)
...
def run_with_expert_maps(num_experts: int, num_local_experts: int, **cutlass_moe_kwargs):
    ...
    return out_tensor
def run_8_bit(moe_tensors: MOETensors8Bit, topk_weights: torch.Tensor, topk_ids: torch.Tensor, num_local_experts: Optional[int]=None) -> torch.Tensor:
    ...
    kwargs = {'a': moe_tensors.a, 'w1_q': moe_tensors.w1_q.transpose(1, 2), 'w2_q': moe_tensors.w2_q.transpose(1, 2), 'topk_weights': topk_weights, 'topk_ids_': topk_ids, 'ab_strides1': moe_tensors.ab_strides1, 'c_strides1': moe_tensors.c_strides1, 'ab_strides2': moe_tensors.ab_strides2, 'c_strides2': moe_tensors.c_strides2, 'w1_scale': moe_tensors.w1_scale, 'w2_scale': moe_tensors.w2_scale, 'a1_scale': moe_tensors.a_scale}
...
def test_cutlass_moe_8_bit_no_graph(m: int, n: int, k: int, e: int, topk: int, per_act_token: bool, per_out_ch: bool):
    ...
    with set_current_aphrodite_config(AphroditeConfig(parallel_config=ParallelConfig(pipeline_parallel_size=1))):
        ...
        torch.testing.assert_close(triton_output, cutlass_output, atol=0.05, rtol=0.01)
@pytest.mark.parametrize('m,n,k', MNK_FACTORS)
@pytest.mark.parametrize('e', NUM_EXPERTS)
@pytest.mark.parametrize('topk', TOP_KS)
@pytest.mark.parametrize('per_act_token', [True, False])
@pytest.mark.parametrize('per_out_ch', [True, False])
@pytest.mark.skipif((lambda x: x is None or not ops.cutlass_group_gemm_supported(x.to_int()))(current_platform.get_device_capability()), reason='Grouped gemm is not supported on this GPU type.')
def test_cutlass_moe_8_bit_cuda_graph(m: int, n: int, k: int, e: int, topk: int, per_act_token: bool, per_out_ch: bool):
    current_platform.seed_everything(7)
    with set_current_aphrodite_config(AphroditeConfig(parallel_config=ParallelConfig(pipeline_parallel_size=1))):
        dtype = torch.half
        mt = MOETensors8Bit.make_moe_tensors_8bit(m, k, n, e, per_act_token, per_out_ch)
...
@pytest.mark.skipif((lambda x: x is None or not ops.cutlass_group_gemm_supported(x.to_int()))(current_platform.get_device_capability()), reason='Grouped gemm is not supported on this GPU type.')
def test_cutlass_moe_8_bit_EP(m: int, n: int, k: int, e: int, topk: int, per_act_token: bool, per_out_channel: bool, ep_size: int):
    current_platform.seed_everything(7)
    with set_current_aphrodite_config(AphroditeConfig(parallel_config=ParallelConfig(pipeline_parallel_size=1))):
        ...
        cutlass_output = run_8_bit(mt, topk_weights, topk_ids, num_local_experts=e // ep_size)
        torch.testing.assert_close(triton_output, cutlass_output, atol=0.05, rtol=0.01)