...
def baseline_scaled_mm(a: torch.Tensor, b: torch.Tensor, scale_a: torch.Tensor, scale_b: torch.Tensor, out_dtype: Type[torch.dtype], bias: Optional[torch.Tensor]=None) -> torch.Tensor:
    output = (scale_a * (scale_b * torch.mm(a.to(dtype=torch.float32), b.to(dtype=torch.float32)))).to(out_dtype)
    if bias is not None:
        output = output + bias
    return output
def cutlass_fp8_gemm_helper(m: int, n: int, k: int, per_token_act_quant: bool, per_out_channel_weight_quant: bool, use_bias: bool, out_dtype: Type[torch.dtype]=torch.bfloat16, device: str='cuda'):
    ...
    torch.testing.assert_close(out, baseline, rtol=0.01, atol=0.05)
def cutlass_int8_gemm_helper(m: int, n: int, k: int, per_token_act_quant: bool, per_out_channel_weight_quant: bool, use_bias: bool, out_dtype: Type[torch.dtype]=torch.bfloat16, device: str='cuda'):
...
def test_cutlass_fp8_gemm(m: int, n: int, k: int, per_act_token: bool, per_out_ch: bool, use_bias: bool):
...
@pytest.mark.parametrize('per_out_ch', [True, False])
@pytest.mark.parametrize('use_bias', [True, False])
def test_cutlass_int8_gemm(m: int, n: int, k: int, per_act_token: bool, per_out_ch: bool, use_bias: bool):
    cutlass_int8_gemm_helper(m, n, k, per_act_token, per_out_ch, use_bias)
@pytest.mark.parametrize('per_act_token', [True, False])
@pytest.mark.parametrize('per_out_ch', [True, False])
@pytest.mark.parametrize('out_dtype', [torch.bfloat16, torch.float16])
@pytest.mark.parametrize('use_bias', [True, False])
def test_cutlass_int8_gemm_output_dtype(per_act_token: bool, per_out_ch: bool, out_dtype: Type[torch.dtype], use_bias: bool):
    cutlass_int8_gemm_helper(512, 512, 512, per_act_token, per_out_ch, use_bias, out_dtype=out_dtype)
@pytest.mark.parametrize('per_act_token', [True, False])
@pytest.mark.parametrize('per_out_ch', [True, False])
@pytest.mark.parametrize('out_dtype', [torch.bfloat16, torch.float16])
@pytest.mark.parametrize('use_bias', [True, False])
@pytest.mark.skipif(capability < 89, reason='FP8 is not supported on this GPU type.')
def test_cutlass_fp8_gemm_output_dtype(per_act_token: bool, per_out_ch: bool, out_dtype: Type[torch.dtype], use_bias: bool):
    cutlass_fp8_gemm_helper(512, 512, 512, per_act_token, per_out_ch, use_bias, out_dtype=out_dtype)
@pytest.mark.parametrize('per_act_token', [True, False])
@pytest.mark.parametrize('per_out_ch', [True, False])
@pytest.mark.parametrize('use_bias', [True, False])
@pytest.mark.parametrize('device', CUDA_DEVICES)
@pytest.mark.skipif(capability < 89, reason='FP8 is not supported on this GPU type.')
def test_cutlass_fp8_gemm_devices(per_act_token: bool, per_out_ch: bool, use_bias: bool, device: str):
    cutlass_fp8_gemm_helper(512, 512, 512, per_act_token, per_out_ch, use_bias, torch.bfloat16, device)
@pytest.mark.parametrize('per_act_token', [True, False])
@pytest.mark.parametrize('per_out_ch', [True, False])
@pytest.mark.parametrize('use_bias', [True, False])
@pytest.mark.parametrize('device', CUDA_DEVICES)
def test_cutlass_int8_gemm_devices(per_act_token: bool, per_out_ch: bool, use_bias: bool, device: str):
    cutlass_int8_gemm_helper(512, 512, 512, per_act_token, per_out_ch, use_bias, out_dtype=torch.bfloat16, device=device)
@pytest.mark.parametrize('per_act_token', [True, False])
@pytest.mark.parametrize('per_out_ch', [True, False])
@pytest.mark.parametrize('use_bias', [True, False])
@pytest.mark.skipif(capability < 89, reason='FP8 is not supported on this GPU type.')
def test_cutlass_fp8_gemm_m_sweep(per_act_token: bool, per_out_ch: bool, use_bias: bool):
    for nk in range(32, 128, 32):
        for m in range(1, 128):
            cutlass_fp8_gemm_helper(m, nk, nk, per_act_token, per_out_ch, use_bias)
@pytest.mark.parametrize('per_act_token', [True, False])
@pytest.mark.parametrize('per_out_ch', [True, False])
@pytest.mark.parametrize('use_bias', [True, False])
def test_cutlass_int8_gemm_m_sweep(per_act_token: bool, per_out_ch: bool, use_bias: bool):
    for nk in range(32, 128, 32):
        for m in range(1, 128):
            cutlass_int8_gemm_helper(m, nk, nk, per_act_token, per_out_ch, use_bias)
