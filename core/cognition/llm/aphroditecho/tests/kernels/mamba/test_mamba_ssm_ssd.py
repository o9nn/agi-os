...
from aphrodite.modeling.layers.mamba.mamba2_metadata import _seq_idx_to_chunk_indices_offsets
...
def generate_random_inputs(batch_size, seqlen, n_heads, d_head, itype, device='cuda'):
    ...
    return (A, dt, X, B, C)
def generate_continous_batched_examples(example_lens_by_batch, num_examples, full_length, last_taken, exhausted, n_heads, d_head, itype, device='cuda'):
...
def test_mamba_chunk_scan_cont_batch(d_head, n_heads, seq_len_chunk_size_cases, itype):
    ...
    exhausted: dict = {}
    states = None
    for Y_min, cu_seqlens, seq_idx, (A, dt, X, B, C) in generate_continous_batched_examples(cases, num_examples, seqlen, last_taken, exhausted, n_heads, d_head, itype):
        chunk_indices, chunk_offsets = _seq_idx_to_chunk_indices_offsets(seq_idx, chunk_size)
