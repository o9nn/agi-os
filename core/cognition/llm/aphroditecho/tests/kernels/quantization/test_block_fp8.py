...
from aphrodite.modeling.layers.fused_moe.deep_gemm_moe import deep_gemm_moe_fp8
...
from aphrodite.quantization.utils.fp8_utils import per_token_group_quant_fp8, w8a8_block_fp8_matmul
...
@pytest.mark.parametrize('M,N,K,E,topk,block_size,dtype,seed', itertools.product(M_moe, N_moe, K_moe, E, TOP_KS, BLOCK_SIZE, DTYPES, SEEDS))
@torch.inference_mode()
...
def per_block_cast_to_fp8(x: torch.Tensor, block_size_n: int=128) -> tuple[torch.Tensor, torch.Tensor]:
    assert x.dim() == 2
    m, n = x.shape
    x_padded = torch.zeros((deep_gemm.ceil_div(m, 128) * 128, deep_gemm.ceil_div(n, block_size_n) * block_size_n), dtype=x.dtype, device=x.device)
    x_padded[:m, :n] = x
...
def _moe_permute(a, a_s, topk_ids, num_groups, topk, block_m):
    M, K = a.shape
    sorted_token_ids, m_indices, num_pad = moe_align_block_size(topk_ids, block_m, num_groups, None, pad_sorted_ids=True)
    num_tokens = topk * M
...
def deep_gemm_w8a8_block_fp8_moe(M, K, a, w1, w2, w1_s, w2_s, score, topk, block_shape):
    ...
    a_q, a_s, m_indices, inv_perm = _moe_permute(a_q, a_s, topk_ids, num_groups, topk, block_m)
    inter_out = torch.zeros((a_q.shape[0], N * 2), dtype=torch.bfloat16, device=a.device)
    deep_gemm.m_grouped_gemm_fp8_fp8_bf16_nt_contiguous((a_q, a_s), (w1, w1_s), inter_out, m_indices)
    act_out = SiluAndMul().forward_native(inter_out)
    ...
    deep_gemm.m_grouped_gemm_fp8_fp8_bf16_nt_contiguous((act_out_q, act_out_s), (w2, w2_s), out, m_indices)
    final_out = _moe_unpermute(out, inv_perm, topk, K, topk_weight)
    return final_out
@pytest.mark.parametrize('M,N,K,E,topk,seed', itertools.product(M_moe_dg, N_moe, K_moe, E, TOP_KS, SEEDS))
...
def test_w8a8_block_fp8_deep_gemm_fused_moe(M, N, K, E, topk, seed):
    ...
    fp8_max, fp8_min = (fp8_info.max, fp8_info.min)
    a = torch.randn((M, K), dtype=dtype) / 10
    w1_bf16 = ((torch.rand((E, 2 * N, K), dtype=torch.bfloat16) - 0.5) * 2 * fp8_max).clamp(min=fp8_min, max=fp8_max)
    w2_bf16 = ((torch.rand((E, K, N), dtype=torch.bfloat16) - 0.5) * 2 * fp8_max).clamp(min=fp8_min, max=fp8_max)
    score = torch.randn((M, E), dtype=dtype)
    block_n, block_k = (block_size[0], block_size[1])
    n_tiles_w1 = (2 * N + block_n - 1) // block_n
    k_tiles_w1 = (K + block_k - 1) // block_k
    ...
    with set_current_aphrodite_config(aphrodite_config):
        if M >= 128:
            ref_out = deep_gemm_w8a8_block_fp8_moe(M, K, a, w1, w2, w1_s, w2_s, score, topk, block_size)
        else:
            ref_out = torch_w8a8_block_fp8_moe(a, w1, w2, w1_s, w2_s, score, topk, block_size)
