...
device = 'cuda'
def scaled_mm_torch(a: torch.Tensor, b: torch.Tensor, scale_a: torch.Tensor, scale_b: torch.Tensor, out_dtype: type[torch.dtype], bias: Optional[torch.Tensor]=None) -> torch.Tensor:
...
def test_scaled_mm(M, N, K, in_dtype, out_dtype, use_scalar_scale_a, use_scalar_scale_b, use_bias):
    ...
    c_actual = scaled_mm_torch(a_cpu, b_cpu, scale_a_cpu, scale_b_cpu, out_dtype, bias_cpu)
    c_check_cpu = c_check.cpu()
    torch.testing.assert_close(c_check_cpu, c_actual, rtol=0.1, atol=0.1)