...
def test_gateway_lists_agents_and_functions(tmp_path: Path):
    ...
    gw.register_agent({'id': 'agent.demo', 'version': 'v1.0.0', 'capabilities': ['chat'], 'tools_allow': [], 'policies': [], 'prompt_profile': 'default'})
    def ping():
        return 'pong'
    gw.functions.register(ping, name='sys.ping', description='Ping function', params_schema={'type': 'object', 'properties': {}}, safety_class='low', cost_unit=0.0)
    app = FastAPI()
