...
class TestSetting:
    ...
    method: str
    fullgraph: bool
@pytest.mark.parametrize('test_setting', [TestSetting(model='meta-llama/Llama-3.2-1B-Instruct', model_args=[], pp_size=2, tp_size=2, attn_backend='FLASHINFER', method='generate', fullgraph=True), TestSetting(model='TheBloke/TinyLlama-1.1B-Chat-v0.3-GPTQ', model_args=['--quantization', 'gptq'], pp_size=1, tp_size=1, attn_backend='FLASH_ATTN', method='generate', fullgraph=True), TestSetting(model='ibm/PowerMoE-3b', model_args=[], pp_size=1, tp_size=2, attn_backend='FLASH_ATTN', method='generate', fullgraph=True), TestSetting(model='BAAI/bge-multilingual-gemma2', model_args=['--task', 'embed', '--dtype', 'bfloat16'], pp_size=1, tp_size=1, attn_backend='FLASH_ATTN', method='encode', fullgraph=True), TestSetting(model='BAAI/bge-base-en-v1.5', model_args=['--task', 'embed'], pp_size=1, tp_size=1, attn_backend='XFORMERS', method='encode', fullgraph=True), TestSetting(model='microsoft/Phi-3.5-vision-instruct', model_args=['--trust-remote-code', '--max-model-len', '2048'], pp_size=2, tp_size=1, attn_backend='FLASH_ATTN', method='generate_with_image', fullgraph=False)])
    ...
    with monkeypatch.context() as m:
        ...
        for level in [CompilationLevel.NO_COMPILATION, CompilationLevel.DYNAMO_AS_IS, CompilationLevel.DYNAMO_ONCE]:
            ...
            if level != CompilationLevel.DYNAMO_ONCE and (not fullgraph):
                all_envs[-1]['APHRODITE_TEST_DYNAMO_FULLGRAPH_CAPTURE'] = '0'
