...
async def test_evil_forward(tmp_socket):
    with RemoteMQLLMEngine(engine_args=ENGINE_ARGS, ipc_path=tmp_socket, run_fn=run_with_evil_forward) as engine:
        ...
        with pytest.raises(MQEngineDeadError):
            async for _ in client.generate(prompt='Hello my name is', sampling_params=SamplingParams(), request_id=uuid.uuid4()):
                pass
...
async def test_failed_abort(tmp_socket):
    with RemoteMQLLMEngine(engine_args=ENGINE_ARGS, ipc_path=tmp_socket, run_fn=run_with_evil_abort) as engine:
        ...
        with pytest.raises(MQEngineDeadError) as execinfo:
            async for _ in client.generate(prompt='Hello my name is', sampling_params=SamplingParams(max_tokens=10), request_id=uuid.uuid4()):
                pass
...
async def test_batch_error(tmp_socket):
    with RemoteMQLLMEngine(engine_args=ENGINE_ARGS, ipc_path=tmp_socket, run_fn=run_with_evil_abort) as engine:
        ...
        async def do_generate(client):
            ...
            async for _ in client.generate(prompt='Hello my name is', sampling_params=params, request_id=uuid.uuid4()):
                pass
...
async def test_mp_crash_detection(monkeypatch: pytest.MonkeyPatch):
    with monkeypatch.context() as m:
        ...
        assert end - start < 60, 'Expected Aphrodite to gracefully shutdown in <60s if there is an error in the startup.'
@pytest.mark.asyncio
