...
def demo_content_negotiation():
    ...
    try:
        ...
        return
    sample_data = {'service': 'Deep Tree Echo API', 'version': '1.0.0', 'status': 'operational', 'features': {'membrane_computing': True, 'echo_state_networks': True, 'batch_processing': True}, 'endpoints': ['/process', '/status', '/membrane_info'], 'performance_metrics': {'response_time_ms': 45.2, 'throughput_rps': 1250.8, 'memory_usage_mb': 128.5}}
    test_cases = [('application/json', 'Standard JSON request'), ('application/xml', 'XML API consumption'), ('text/html', 'Browser request'), ('application/xml;q=0.9,application/json;q=0.8', 'Prefer XML over JSON'), ('text/html;q=1.0,application/xml;q=0.9,*/*;q=0.1', 'Browser with fallbacks'), ('application/pdf,image/jpeg', 'Unsupported types (fallback to JSON)'), ('*/*', 'Accept anything (default to JSON)')]
    ...
    complex_data = {'dtesn_status': {'membrane_hierarchy': 'active', 'processing_stats': {'current_jobs': 15, 'completed_today': 1247, 'error_rate': 0.002}, 'supported_operations': ['evolution', 'communication', 'enumeration'], 'server_info': {'uptime_hours': 168.5, 'cpu_usage': 23.4, 'memory_gb': 16.2}}}
