...
class EnvironmentCouplingDemo:
    def __init__(self):
        self.environment_state = {'temperature': 20.0, 'humidity': 50.0, 'resources': 100, 'hazards': 0, 'time_of_day': 'morning'}
        self.agents = {'explorer_agent': {'name': 'Explorer', 'behavior': 'exploring', 'energy': 100, 'position': [0, 0], 'adaptations_applied': []}, 'gatherer_agent': {'name': 'Gatherer', 'behavior': 'gathering', 'energy': 100, 'position': [5, 5], 'adaptations_applied': []}, 'guard_agent': {'name': 'Guard', 'behavior': 'patrolling', 'energy': 100, 'position': [10, 10], 'adaptations_applied': []}}
            ...
            if key in self.environment_state:
                ...
                if isinstance(old_value, (int, float)) and isinstance(new_value, (int, float)):
                    ...
                    if change_percent > 10:
                        changes.append({'parameter': key, 'old_value': old_value, 'new_value': new_value, 'change_percent': change_percent, 'change_type': 'increase' if new_value > old_value else 'decrease'})
                elif old_value != new_value:
                    changes.append({'parameter': key, 'old_value': old_value, 'new_value': new_value, 'change_type': 'value_change'})
        for key, value in new_state.items():
            if key not in self.environment_state:
                changes.append({'parameter': key, 'old_value': None, 'new_value': value, 'change_type': 'parameter_added'})
        return changes
    def determine_behavior_adaptations(self, changes: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for change in changes:
            ...
            if parameter == 'temperature':
                if new_value > 35.0:
                    adaptations.append({'trigger': f'High temperature ({new_value}°C)', 'adaptation_type': 'environmental_stress_response', 'target_agents': ['all'], 'behavior_changes': {'movement_speed': 0.7, 'energy_consumption': 1.3, 'shelter_seeking': True, 'priority': 'temperature_regulation'}})
                elif new_value < 5.0:
                    adaptations.append({'trigger': f'Low temperature ({new_value}°C)', 'adaptation_type': 'cold_adaptation', 'target_agents': ['all'], 'behavior_changes': {'movement_speed': 0.8, 'energy_consumption': 1.2, 'warmth_seeking': True, 'priority': 'warmth_preservation'}})
            elif parameter == 'resources':
                if change_type == 'decrease' and new_value < 30:
                    adaptations.append({'trigger': f'Resource depletion (only {new_value} remaining)', 'adaptation_type': 'resource_scarcity_response', 'target_agents': ['gatherer_agent', 'explorer_agent'], 'behavior_changes': {'priority': 'resource_gathering', 'search_radius': 2.0, 'cooperation_level': 1.5, 'energy_conservation': True}})
            elif parameter == 'hazards':
                if new_value > 0:
                    adaptations.append({'trigger': f'Hazards detected ({new_value} hazards)', 'adaptation_type': 'danger_avoidance', 'target_agents': ['all'], 'behavior_changes': {'caution_level': 0.9, 'group_formation': True, 'escape_route_planning': True, 'priority': 'safety'}})
            elif parameter == 'humidity' and new_value > 80.0:
                adaptations.append({'trigger': f'High humidity ({new_value}%)', 'adaptation_type': 'humidity_adaptation', 'target_agents': ['all'], 'behavior_changes': {'ventilation_seeking': True, 'activity_reduction': 0.8, 'hydration_priority': 1.2}})
        return adaptations
                ...
                if agent_id in self.agents:
                    ...
                    for change_key, change_value in behavior_changes.items():
                        if change_key == 'priority':
                            ...
                            agent[change_key] = change_value
                    adaptation_record = {'timestamp': time.time(), 'trigger': trigger, 'adaptation_type': adaptation['adaptation_type'], 'changes_applied': behavior_changes}
    ...
    def analyze_context_sensitivity(self) -> Dict[str, float]:
        ...
        temp_sensitivity = min(1.0, abs(temp - 20.0) / 30.0)
        humidity_sensitivity = min(1.0, abs(humidity - 50.0) / 50.0)
        ...
        hazard_sensitivity = min(1.0, hazards / 5.0)
        agent_count = len(self.agents)
        social_sensitivity = min(1.0, agent_count / 10.0)
        sensitivity_scores = {'environmental': (temp_sensitivity + humidity_sensitivity) / 2.0, 'resource': resource_sensitivity, 'danger': hazard_sensitivity, 'social': social_sensitivity, 'overall': sum([temp_sensitivity, humidity_sensitivity, resource_sensitivity, hazard_sensitivity, social_sensitivity]) / 5.0}
    ...
    async def update_environment_state(self, new_state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if changes:
            ...
            for change in changes:
                if change['change_type'] in ['increase', 'decrease']:
                    print(f"  • {change['parameter']}: {change['old_value']} → {change['new_value']} ({change['change_percent']:.1f}% {change['change_type']})")
                else:
        ...
        coupling_event = {'timestamp': time.time(), 'changes': changes, 'environment_state': self.environment_state.copy()}
        ...
        return {'status': 'success', 'changes_detected': len(changes), 'adaptations_applied': len(adaptations), 'agents_affected': len(application_results), 'context_sensitivity': sensitivity['overall'], 'timestamp': time.time()}
    ...
    def print_system_summary(self):
        ...
        for agent_id, agent in self.agents.items():
            ...
            if agent['adaptations_applied']:
                ...
                for adaptation in agent['adaptations_applied'][-3:]:
...
async def run_environment_coupling_demonstration():
    ...
    await demo.update_environment_state({'temperature': 38.0})
    await asyncio.sleep(1)
    print('\n\nSCENARIO 2: Resource Depletion')
    await demo.update_environment_state({'resources': 20, 'humidity': 85.0})
    await asyncio.sleep(1)
    print('\n\nSCENARIO 3: Hazard Detection')
    await demo.update_environment_state({'hazards': 3, 'temperature': 42.0, 'time_of_day': 'noon'})
    await asyncio.sleep(1)
    print('\n\nSCENARIO 4: Environmental Recovery')
    await demo.update_environment_state({'temperature': 25.0, 'resources': 80, 'hazards': 0, 'humidity': 60.0, 'time_of_day': 'evening'})
    demo.print_system_summary()
    print(f"\n{'=' * 60}")
