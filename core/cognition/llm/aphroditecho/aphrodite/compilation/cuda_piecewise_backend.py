...
class CUDAPiecewiseBackend:
    def __init__(self, graph: fx.GraphModule, aphrodite_config: AphroditeConfig, graph_pool: Any, piecewise_compile_index: int, total_piecewise_compiles: int, sym_shape_indices: list[int], compiled_graph_for_general_shape: Callable, aphrodite_backend: AphroditeBackend):
        self.graph = graph
    ...
    def __call__(self, *args) -> Any:
        ...
        if entry.need_to_compile and (not entry.compiled):
            ...
            entry.runnable = self.aphrodite_backend.compiler_manager.compile(self.graph, args, self.compilation_config.inductor_compile_config, self.compilation_config, graph_index=self.piecewise_compile_index, num_graphs=self.total_piecewise_compiles, runtime_shape=runtime_shape)
            if self.is_last_graph and (not self.to_be_compiled_sizes):
                self.check_for_ending_compilation()
        ...
        if entry.cudagraph is None:
            if entry.num_finished_warmup < self.compilation_config.cudagraph_num_of_warmups:
                ...
                if self.is_first_graph:
                    logger.debug('Warming up {}/{} for shape {}', entry.num_finished_warmup, self.compilation_config.cudagraph_num_of_warmups, runtime_shape)
                return entry.runnable(*args)
            if self.is_first_graph:
                logger.debug('Capturing a cudagraph for shape {}', runtime_shape)
