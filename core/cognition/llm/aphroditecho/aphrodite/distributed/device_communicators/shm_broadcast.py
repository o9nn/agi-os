...
from aphrodite.utils import get_ip, get_open_port, get_open_zmq_ipc_path, is_valid_ipv6_address
...
class MessageQueue:
    def __init__(self, n_reader, n_local_reader, local_reader_ranks: Optional[list[int]]=None, max_chunk_bytes: int=1024 * 1024 * 10, max_chunks: int=10, connect_ip: Optional[str]=None):
        ...
        logger.info('Aphrodite message queue communication handle: {}', self.handle)
        ...
        while True:
            with self.buffer.get_metadata(self.current_idx) as metadata_buffer:
                ...
                if written_flag and read_count != self.buffer.n_reader:
                    sched_yield()
                    if time.monotonic() - start_time > APHRODITE_RINGBUFFER_WARNING_INTERVAL * n_warning:
                        logger.debug('No available shared memory broadcast block found in {} second.', APHRODITE_RINGBUFFER_WARNING_INTERVAL)
                        n_warning += 1
