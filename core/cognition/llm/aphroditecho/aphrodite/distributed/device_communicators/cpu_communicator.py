...
class CpuCommunicator(DeviceCommunicatorBase):
    def __init__(self, cpu_group: ProcessGroup, device: Optional[torch.device]=None, device_group: Optional[ProcessGroup]=None, unique_name: str=''):
        ...
        if current_platform.get_cpu_architecture() == CpuArchEnum.X86 and hasattr(torch.ops._C, 'init_shm_manager') and (unique_name.startswith('tp') or unique_name.startswith('pp')):
    ...
    def all_gather(self, input_: torch.Tensor, dim: int=-1) -> torch.Tensor:
        ...
        return output_tensor
    def send_tensor_dict(self, tensor_dict: dict[str, Union[torch.Tensor, Any]], dst: int) -> None:
        return self.dist_module.send_tensor_dict(tensor_dict, dst)
    def recv_tensor_dict(self, src: int) -> dict[str, Union[torch.Tensor, Any]]:
