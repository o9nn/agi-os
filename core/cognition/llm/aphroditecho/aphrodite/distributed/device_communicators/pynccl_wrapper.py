...
class NCCLLibrary:
    ...
    def __init__(self, so_file: Optional[str]=None):
        ...
        try:
            ...
            logger.error('Failed to load NCCL library from {}. It is expected if you are not running on NVIDIA/AMD GPUs.Otherwise, the nccl library might not exist, be corrupted or it does not support the current platform {}. If you already have the library, please set the environment variable APHRODITE_NCCL_SO_PATH to point to the correct nccl library path.', so_file, platform.platform())
            raise e
        if so_file not in NCCLLibrary.path_to_dict_mapping:
            _funcs: dict[str, Any] = {}
