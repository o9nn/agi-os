...
from aphrodite.utils import direct_register_custom_op, get_distributed_init_method, resolve_obj_by_qualname, supports_custom_op
...
def init_distributed_environment(world_size: int=-1, rank: int=-1, distributed_init_method: str='env://', local_rank: int=-1, backend: str='nccl'):
    logger.debug('world_size={} rank={} local_rank={} distributed_init_method={} backend={}', world_size, rank, local_rank, distributed_init_method, backend)
    ...
    if config is not None and config.parallel_config.data_parallel_size > 1:
        ...
        logger.info('Adjusting world_size={} rank={} distributed_init_method={} for DP', world_size, rank, distributed_init_method)
    if not torch.distributed.is_initialized():
        ...
        if not torch.distributed.is_backend_available(backend):
            logger.warning('Distributed backend {} is not available; falling back to gloo.', backend)
    ...
    if _WORLD is None:
        ...
        logger.debug('Detected {} nodes in the distributed environment', _NODE_COUNT)
    else:
...
def initialize_model_parallel(tensor_model_parallel_size: int=1, pipeline_model_parallel_size: int=1, backend: Optional[str]=None) -> None:
    ...
    logger.info('rank {} in world size {} is assigned as DP rank {}, PP rank {}, TP rank {}, EP rank {}', rank, world_size, _DP.rank_in_group, _PP.rank_in_group, _TP.rank_in_group, _EP.rank_in_group)
def ensure_model_parallel_initialized(tensor_model_parallel_size: int, pipeline_model_parallel_size: int, backend: Optional[str]=None) -> None:
    ...
    if not model_parallel_is_initialized():
        ...
        return
    assert get_tensor_model_parallel_world_size() == tensor_model_parallel_size, f'tensor parallel group already initialized, but of unexpected size. got: get_tensor_model_parallel_world_size()={get_tensor_model_parallel_world_size()!r} vs. wanted: tensor_model_parallel_size={tensor_model_parallel_size!r}'
    pp_world_size = get_pp_group().world_size
    assert pp_world_size == pipeline_model_parallel_size, f'pipeline parallel group already initialized, but of unexpected size. got: pp_world_size={pp_world_size!r} vs. wanted: pipeline_model_parallel_size={pipeline_model_parallel_size!r}'
