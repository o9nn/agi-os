...
from aphrodite.utils import FlexibleArgumentParser, decorate_logs, get_tcp_uri, set_process_title
...
from aphrodite.endpoints.utils import APHRODITE_SUBCMD_PARSER_EPILOG, show_filtered_argument_or_group_from_help
...
class ServeSubcommand(CLISubcommand):
    ...
    def subparser_init(self, subparsers: argparse._SubParsersAction) -> FlexibleArgumentParser:
        serve_parser = subparsers.add_parser('run', help='Start the Aphrodite OpenAI Compatible API server.', description='Start the Aphrodite OpenAI Compatible API server.', usage='aphrodite run [model_tag] [options]')
...
def run_headless(args: argparse.Namespace):
    ...
    logger.info('Launching {} data parallel engine(s) in headless mode, with head node address {}.', local_engine_count, handshake_address)
    engine_manager = CoreEngineProcManager(target_fn=EngineCoreProc.run_engine_core, local_engine_count=local_engine_count, start_index=aphrodite_config.parallel_config.data_parallel_rank, local_start_index=0, aphrodite_config=aphrodite_config, local_client=False, handshake_address=handshake_address, executor_class=Executor.get_class(aphrodite_config), log_stats=not engine_args.disable_log_stats)
    try:
...
def run_multi_api_server(args: argparse.Namespace):
    ...
    if num_api_servers > 1:
        ...
        if model_config.is_multimodal_model and (not orig_disable_mm_preprocessor_cache):
            logger.warning('Multi-modal preprocessor cache is not compatible with api_server_count > 1, so the cache will be disabled.')
    ...
    with launch_core_engines(aphrodite_config, executor_class, log_stats, num_api_servers) as (local_engine_manager, coordinator, addresses):
