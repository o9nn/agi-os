...
class Glm4MoeModelToolParser(ToolParser):
    def __init__(self, tokenizer: AnyTokenizer):
        ...
        self.func_call_regex = re.compile('<tool_call>.*?</tool_call>', re.DOTALL)
        self.func_detail_regex = re.compile('<tool_call>([^\\n]*)\\n(.*)</tool_call>', re.DOTALL)
        self.func_arg_regex = re.compile('<arg_key>(.*?)</arg_key>\\s*<arg_value>(.*?)</arg_value>', re.DOTALL)
    ...
    def extract_tool_calls(self, model_output: str, request: ChatCompletionRequest) -> ExtractedToolCallInformation:
        def _is_string_type(tool_name: str, arg_name: str, tools: Optional[list[ChatCompletionToolsParam]]) -> bool:
            ...
            logger.warning("No tool named '{}'.", tool_name)
            return False
        ...
        logger.debug('model_output: {}', model_output)
        try:
            tool_calls = []
            for match in matched_tool_calls:
                ...
                for key, value in pairs:
                    ...
                    logger.debug('arg_key = {}, arg_val = {}', arg_key, arg_val)
    ...
    def extract_tool_calls_streaming(self, previous_text: str, current_text: str, delta_text: str, previous_token_ids: Sequence[int], current_token_ids: Sequence[int], delta_token_ids: Sequence[int], request: ChatCompletionRequest) -> Union[DeltaMessage, None]:
        ...
        logger.debug('cur_text = {}', cur_text)
        ...
        if end_idx != -1:
            ...
            self.prev_tool_call_arr[self.current_tool_id] = {'name': tool_call.function.name, 'arguments': json.loads(tool_call.function.arguments)}
            self.streamed_args_for_tool[self.current_tool_id] = tool_call.function.arguments
            delta = DeltaMessage(content=extracted_tool_calls.content, tool_calls=[DeltaToolCall(index=self.current_tool_id, id=tool_call.id, type=tool_call.type, function=DeltaFunctionCall(name=tool_call.function.name, arguments=tool_call.function.arguments))])
            self.current_tool_id += 1
