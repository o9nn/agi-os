...
class KimiK2ToolParser(ToolParser):
    def __init__(self, tokenizer: AnyTokenizer):
        ...
        self.tool_call_regex = re.compile('<\\|tool_call_begin\\|>\\s*(?P<tool_call_id>.+:\\d+)\\s*<\\|tool_call_argument_begin\\|>\\s*(?P<function_arguments>.*?)\\s*<\\|tool_call_end\\|>')
        self.stream_tool_call_portion_regex = re.compile('(?P<tool_call_id>.+:\\d+)\\s*<\\|tool_call_argument_begin\\|>\\s*(?P<function_arguments>.*)')
        self.stream_tool_call_name_regex = re.compile('(?P<tool_call_id>.+:\\d+)\\s*')
    ...
    def extract_tool_calls(self, model_output: str, request: ChatCompletionRequest) -> ExtractedToolCallInformation:
        if self.tool_calls_start_token not in model_output:
            ...
            try:
                ...
                logger.debug('function_call_tuples: {}', function_call_tuples)
                tool_calls = []
        ...
        try:
            ...
            if cur_tool_start_count > cur_tool_end_count and cur_tool_start_count > prev_tool_start_count:
                ...
                if diff:
                    ...
                    logger.debug('Finishing tool and found diff that had not been streamed yet: {}', diff)
            ...
            if tool_call_portion is None:
                ...
                return delta
            logger.debug('Trying to parse current tool call with ID {}', self.current_tool_id)
