...
class Step3ToolParser(ToolParser):
    ...
    def adjust_request(self, request: ChatCompletionRequest) -> ChatCompletionRequest:
        ...
        return request
    @staticmethod
    def _parse_steptml_invoke(action_text: str) -> tuple[Optional[str], Optional[dict[str, str]]]:
        func_name_match = re.search('<steptml:invoke name="([^"]+)">', action_text)
        if not func_name_match:
            return (None, None)
        func_name = func_name_match.group(1)
        params: dict[str, str] = {}
        param_matches = re.findall('<steptml:parameter name="([^"]+)">([^<]*)</steptml:parameter>', action_text)
        for name, value in param_matches:
            params[name] = value.strip()
        return (func_name, params)
    def _cast_arguments(self, func_name: str, params: dict[str, Any], request: ChatCompletionRequest) -> dict[str, Any]:
        ...
        while True:
            ...
            if self.current_tool_id != -1 and (not self.prev_tool_call_arr[self.current_tool_id].get('finished', False)):
                ...
                if not self.current_tool_name_sent:
                    ...
                    return DeltaMessage(tool_calls=[DeltaToolCall(index=self.current_tool_id, type='function', id=f'chatcmpl-tool-{random_uuid()}', function=DeltaFunctionCall(name=function_name))])
                self.prev_tool_call_arr[self.current_tool_id].update(tool_call_arr)
                if end_tool_pos != -1:
                    self.position += end_tool_pos + len(self.TOOL_CALL_END)
                    self.prev_tool_call_arr[self.current_tool_id]['finished'] = True
                    final_args = self._cast_arguments(function_name, tool_call_arr.get('parameters', {}), request)
                    if final_args:
                        final_args_json = json.dumps(final_args, ensure_ascii=False)
                        return DeltaMessage(tool_calls=[DeltaToolCall(index=self.current_tool_id, function=DeltaFunctionCall(arguments=final_args_json))])
                return None
            return None
