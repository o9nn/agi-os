...
class Llama4PythonicToolParser(ToolParser):
    ...
    def extract_tool_calls(self, model_output: str, request: ChatCompletionRequest) -> ExtractedToolCallInformation:
        ...
        try:
            is_tool_call_pattern = self.TOOL_CALL_REGEX.match(model_output, timeout=envs.APHRODITE_TOOL_PARSE_REGEX_TIMEOUT_SECONDS) is not None
        except TimeoutError:
            logger.warning('Regex timeout occurred when matching tool call pattern.')
            logger.debug('Regex timeout occurred when matching user input: {}', model_output)
