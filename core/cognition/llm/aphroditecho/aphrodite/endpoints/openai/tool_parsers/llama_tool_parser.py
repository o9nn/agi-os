...
class Llama3JsonToolParser(ToolParser):
    def __init__(self, tokenizer: PreTrainedTokenizerBase):
        ...
        self.tool_call_regex = re.compile('{[^{}]*(?:{[^{}]*}[^{}]*)*}(?:\\s*;\\s*{[^{}]*(?:{[^{}]*}[^{}]*)*})*', re.DOTALL)
    def extract_tool_calls(self, model_output: str, request: ChatCompletionRequest) -> ExtractedToolCallInformation:
        ...
        try:
            ...
            for json_obj in json_objects:
                ...
                tool_calls.append(ToolCall(type='function', function=FunctionCall(name=obj['name'], arguments=json.dumps(obj['arguments'] if 'arguments' in obj else obj['parameters'], ensure_ascii=False))))
