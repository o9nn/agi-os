...
def run_benchmarks():
    ...
    results = {'prefix': {'python': [], 'c': []}, 'suffix': {'python': [], 'c': []}, 'diff': {'python': [], 'c': []}, 'indices': {'python': [], 'c': []}}
    for size in sizes:
        s1 = generate_random_string(size)
        s2 = s1[:size // 2] + generate_random_string(size // 2)
        text = 'hello ' * (size // 5)
        ...
        for func_name in results:
            ...
            print(f'{func_name}:')
            print(f"  Python: {results[func_name]['python'][-1] * 1000000.0:.2f} µs")
            print(f"  C:      {results[func_name]['c'][-1] * 1000000.0:.2f} µs")
