...
class ABTestConfig:
    enabled: bool = False
    traffic_split_percent: float = 10.0
    model_a_name: str = ''
    model_b_name: str = ''
    ...
    def __post_init__(self):
        if self.success_criteria is None:
            self.success_criteria = {'max_error_rate_increase_percent': 50.0, 'max_latency_increase_percent': 20.0, 'min_improvement_threshold': 5.0}
        if self.failure_criteria is None:
            self.failure_criteria = {'max_error_rate_percent': 5.0, 'max_latency_ms': 5000.0, 'max_consecutive_errors': 10}
@dataclass
class ABTestMetrics:
    request_count: int = 0
    error_count: int = 0
...
class ABTestingManager:
    ...
    def _extract_user_identifier(self, request: Request) -> str:
        user_id = request.headers.get('x-session-id') or request.headers.get('authorization', '').split()[-1] if request.headers.get('authorization') else '' or request.client.host if request.client else ''
        return user_id or 'anonymous'
    async def start_ab_test(self, model_a: str, model_b: str) -> str:
        test_id = f'ab_test_{int(time.time())}'
        self.active_test = {'test_id': test_id, 'model_a': model_a, 'model_b': model_b, 'status': 'running', 'start_time': datetime.now(timezone.utc).isoformat()}
    ...
    def _evaluate_test_results(self, reason: str='completed') -> ABTestResult:
        ...
        if self.metrics_a.request_count > 50 and self.metrics_b.request_count > 50:
            if self.metrics_b.error_rate > self.config.failure_criteria['max_error_rate_percent'] or self.metrics_b.avg_latency_ms > self.config.failure_criteria['max_latency_ms']:
                decision = 'rollback'
        ...
        return ABTestResult(test_id=self.active_test['test_id'], status='completed', decision=decision, metrics_a=self.metrics_a, metrics_b=self.metrics_b, start_time=self.active_test['start_time'], end_time=datetime.now(timezone.utc).isoformat(), reason=evaluation_reason)
    def _meets_success_criteria(self) -> bool:
        error_rate_improvement = (self.metrics_a.error_rate - self.metrics_b.error_rate) / max(self.metrics_a.error_rate, 0.1) * 100
        latency_improvement = (self.metrics_a.avg_latency_ms - self.metrics_b.avg_latency_ms) / max(self.metrics_a.avg_latency_ms, 1.0) * 100
        return error_rate_improvement >= self.config.success_criteria['min_improvement_threshold'] or latency_improvement >= self.config.success_criteria['min_improvement_threshold']
    def _variant_a_significantly_better(self) -> bool:
        error_rate_degradation = (self.metrics_b.error_rate - self.metrics_a.error_rate) / max(self.metrics_a.error_rate, 0.1) * 100
        latency_degradation = (self.metrics_b.avg_latency_ms - self.metrics_a.avg_latency_ms) / max(self.metrics_a.avg_latency_ms, 1.0) * 100
        return error_rate_degradation > self.config.success_criteria['max_error_rate_increase_percent'] or latency_degradation > self.config.success_criteria['max_latency_increase_percent']
    def get_test_status(self) -> Optional[Dict[str, Any]]:
        ...
        return {'test_id': self.active_test['test_id'], 'status': self.active_test['status'], 'elapsed_minutes': elapsed_minutes, 'traffic_split_percent': self.config.traffic_split_percent, 'metrics': {'variant_a': asdict(self.metrics_a), 'variant_b': asdict(self.metrics_b)}, 'models': {'variant_a': self.config.model_a_name, 'variant_b': self.config.model_b_name}}
class ABTestingMiddleware(BaseHTTPMiddleware):
    ...
    def _is_inference_request(self, request: Request) -> bool:
        inference_paths = {'/v1/chat/completions', '/v1/completions', '/v1/embeddings', '/generate'}
