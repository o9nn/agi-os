...
from aphrodite.continuous_learning import ContinuousLearningSystem, ContinuousLearningConfig, InteractionData, InteractionFeedback, ServerSideConfig
...
class ServerSideDataCollector:
    def __init__(self, config: ServerSideConfig):
        ...
        self.user_sessions = {}
    def collect_interaction(self, request: Request, response: Response, response_time: float, metadata: Dict[str, Any]=None) -> Optional[InteractionData]:
        try:
            request_data = self._extract_request_data(request)
            response_data = self._extract_response_data(response)
            performance_feedback = self._calculate_performance_feedback(request, response, response_time, metadata or {})
            interaction = InteractionData(interaction_id=f'server_{uuid.uuid4().hex}', interaction_type=self._determine_interaction_type(request), input_data=request_data, output_data=response_data, performance_feedback=performance_feedback, timestamp=datetime.now(), context_metadata={'endpoint': str(request.url.path), 'method': request.method, 'response_time_ms': response_time, 'status_code': response.status_code, **(metadata or {})}, success=200 <= response.status_code < 300)
    ...
    def _extract_request_data(self, request: Request) -> Dict[str, Any]:
        data = {'method': request.method, 'path': request.url.path, 'headers': dict(request.headers), 'query_params': dict(request.query_params)}
        if hasattr(request.state, 'body_data'):
            body_data = request.state.body_data
            if isinstance(body_data, dict):
                data.update({'prompt': body_data.get('prompt') or (body_data.get('messages', [{}])[-1].get('content', '') if body_data.get('messages') else ''), 'model': body_data.get('model'), 'max_tokens': body_data.get('max_tokens'), 'temperature': body_data.get('temperature'), 'stream': body_data.get('stream', False)})
        return data
            ...
            try:
                ...
                if 'choices' in body_data:
                    ...
                    if choices and len(choices) > 0:
                        choice = choices[0]
                        data['response_text'] = choice.get('message', {}).get('content') or choice.get('text', '')
    ...
    def _determine_interaction_type(self, request: Request) -> str:
        ...
        if '/chat/completions' in path:
            ...
            return 'other'
    def _calculate_performance_feedback(self, request: Request, response: Response, response_time: float, metadata: Dict[str, Any]) -> float:
        feedback = 0.0
    ...
    def get_aggregated_interactions(self, min_quality: float=None) -> List[InteractionData]:
        ...
        quality_interactions = [interaction for interaction in self.interaction_buffer if interaction.performance_feedback >= min_quality]
        return quality_interactions
    def get_data_statistics(self) -> Dict[str, Any]:
        return {'total_interactions': len(self.interaction_buffer), 'total_feedback': len(self.feedback_buffer), 'avg_quality_score': sum(self.quality_scores) / max(1, len(self.quality_scores)), 'interaction_types': self._count_interaction_types(), 'quality_distribution': self._get_quality_distribution()}
...
class BackgroundLearningProcessor:
    def __init__(self, continuous_learning_system: ContinuousLearningSystem, data_collector: ServerSideDataCollector, config: ServerSideConfig):
    ...
    def _apply_production_constraints(self) -> Dict[str, Any]:
        original_config = {'learning_rate': self.learning_system.current_learning_rate, 'ewc_lambda': self.learning_system.config.ewc_lambda, 'replay_frequency': self.learning_system.config.replay_frequency}
        self.learning_system.current_learning_rate = min(self.learning_system.current_learning_rate, self.config.max_learning_rate_production)
    ...
    async def _check_rollback_conditions(self):
        now = datetime.now()
        recent_failures_count = sum((1 for failure_time in self.recent_failures if (now - failure_time).total_seconds() < 600))
    ...
    def get_learning_metrics(self) -> Dict[str, Any]:
        return {'server_metrics': {'total_requests': self.metrics.total_requests, 'learning_requests': self.metrics.learning_requests, 'background_updates': self.metrics.background_updates, 'failed_updates': self.metrics.failed_updates, 'avg_response_time': self.metrics.avg_response_time, 'learning_success_rate': self.metrics.learning_success_rate, 'last_update': self.metrics.last_update.isoformat() if self.metrics.last_update else None}, 'data_statistics': self.data_collector.get_data_statistics(), 'learning_statistics': self.learning_system.get_learning_stats(), 'recent_failures': len(self.recent_failures), 'system_status': {'background_processing': self.is_running, 'learning_lock_acquired': self.learning_lock.locked()}}
class ContinuousLearningMiddleware(BaseHTTPMiddleware):
    def __init__(self, app: ASGIApp, continuous_learning_system: ContinuousLearningSystem, config: Optional[ServerSideConfig]=None):
        super().__init__(app)
        ...
        self.background_processor = BackgroundLearningProcessor(continuous_learning_system=continuous_learning_system, data_collector=self.data_collector, config=self.config)
        self.learning_endpoints = {'/v1/chat/completions', '/v1/completions', '/v1/embeddings', '/score', '/rerank'}
        logger.info('ContinuousLearningMiddleware initialized')
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        should_monitor = any((endpoint in str(request.url.path) for endpoint in self.learning_endpoints)) and self.config.enable_request_monitoring
        ...
        if request.method == 'POST':
            try:
                ...
                if len(body) < 50000:
                    ...
                    request = StarletteRequest(scope=request.scope, receive=lambda: {'type': 'http.request', 'body': body})
        ...
        try:
            ...
            if self.config.enable_request_monitoring:
                interaction = self.data_collector.collect_interaction(request=request, response=response, response_time=response_time)
                self.background_processor.metrics.update_request_metrics(response_time=response_time, learning_occurred=interaction is not None)
            return response
        except Exception as e:
            response_time = (time.time() - start_time) * 1000
            ...
            if self.config.enable_request_monitoring:
                ...
                self.data_collector.collect_interaction(request=request, response=error_response, response_time=response_time, metadata={'error': True, 'exception': str(e)})
            raise
...
def setup_continuous_learning_middleware(app: FastAPI, continuous_learning_system: ContinuousLearningSystem, config: Optional[ServerSideConfig]=None) -> ContinuousLearningMiddleware:
    middleware = ContinuousLearningMiddleware(app=app, continuous_learning_system=continuous_learning_system, config=config)
    app.add_middleware(ContinuousLearningMiddleware, continuous_learning_system=continuous_learning_system, config=config)
