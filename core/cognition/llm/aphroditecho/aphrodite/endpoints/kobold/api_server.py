...
def _set_badwords(tokenizer, hf_config):
    ...
    if hf_config.bad_words_ids is not None:
        ...
        return
    badwordsids = [v for k, v in tokenizer.get_vocab().items() if any((c in str(k) for c in '[]'))]
...
def create_error_response(status_code: HTTPStatus, message: str) -> JSONResponse:
    return JSONResponse({'msg': message, 'type': 'invalid_request_error'}, status_code=status_code.value)
...
def prepare_engine_payload(kai_payload: KAIGenerationInputSchema) -> Tuple[SamplingParams, List[int]]:
    ...
    if kai_payload.max_context_length > max_model_len:
        raise ValueError(f'max_context_length ({kai_payload.max_context_length}) must be less than or equal to max_model_len ({max_model_len})')
...
async def generate(kai_payload: KAIGenerationInputSchema) -> JSONResponse:
    ...
    result_generator = engine.generate(None, sampling_params, kai_payload.genkey, input_tokens)
...
async def generate_stream(kai_payload: KAIGenerationInputSchema) -> StreamingResponse:
    sampling_params, input_tokens = prepare_engine_payload(kai_payload)
    results_generator = engine.generate(None, sampling_params, kai_payload.genkey, input_tokens)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Aphrodite KoboldAI-Compatible RESTful API server.')
    parser.add_argument('--host', type=str, default='localhost', help='host name')
    parser.add_argument('--port', type=int, default=2242, help='port number')
    parser.add_argument('--served-model-name', type=str, default=None, help='The model name used in the API. If not specified, the model name will be the same as the huggingface name.')
    parser.add_argument('--max-length', type=int, default=256, help='The maximum length of the generated text. For use with Kobold Horde.')
    parser = AsyncEngineArgs.add_cli_args(parser)
    args = parser.parse_args()
    logger.debug(f'args: {args}')
    logger.warning('The standalone Kobold API is deprecated and will not receive updates. Please use the OpenAI API with the --launch-kobold-api flag instead.')
    ...
    tokenizer = get_tokenizer(engine_args.tokenizer, tokenizer_mode=engine_args.tokenizer_mode, trust_remote_code=engine_args.trust_remote_code)
