        ...
        if message.get('role') == 'assistant':
            ...
            for tool_call in tool_calls:
                if len(tool_call['id']) > 9:
                    logger.warning('Truncating tool call ID: {} to {}', tool_call['id'], tool_call['id'][-9:])
            ...
            if 'tool_call_id' in message:
                ...
                if len(tool_call_id) > 9:
                    logger.warning('Truncating tool_call_id: {} to {}', tool_call_id, tool_call_id[-9:])
...
def _aggregate_content(content: list) -> list[dict[str, Any]]:
    ...
    for chunk in content:
        if chunk.get('type') == 'text' and aggregated_content and (aggregated_content[-1].get('type') == 'text'):
...
class MistralTokenizer(TokenizerBase):
            ...
            if any((isinstance(t, bytes) for t in tokens)):
                ...
                def _token_to_id(t: str):
                    ...
                    try:
                        ...
                        logger.warning('Failed to convert token {} to id, replacing with <unk>', t_bytes)
    ...
    def convert_ids_to_tokens(self, ids: list[int], skip_special_tokens: bool=True) -> list[str]:
        ...
        if self.is_tekken:
            non_skip_special_tokens = {self.tokenizer.get_control_token(SpecialTokens.tool_calls)}
            ...
            ids = [i for i in ids if i > self.tokenizer.num_special_tokens or i in non_skip_special_tokens]
