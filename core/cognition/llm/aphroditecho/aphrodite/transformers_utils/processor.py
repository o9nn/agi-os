...
def _merge_mm_kwargs(model_config: 'ModelConfig', processor_cls: Union[type, tuple[type, ...]], /, **kwargs):
    mm_config = model_config.get_multimodal_config()
    merged_kwargs = mm_config.merge_mm_processor_kwargs(kwargs)
    factory = _get_processor_factory_fn(processor_cls)
    allowed_kwargs = get_allowed_kwarg_only_overrides(factory, merged_kwargs, requires_kw_only=False, allow_var_kwargs=True)
...
def get_processor(processor_name: str, *args: Any, revision: Optional[str]=None, trust_remote_code: bool=False, processor_cls: Union[type[_P], tuple[type[_P], ...]]=ProcessorMixin, **kwargs: Any) -> _P:
    ...
    try:
        if isinstance(processor_cls, tuple) or processor_cls == ProcessorMixin:
            processor = AutoProcessor.from_pretrained(processor_name, *args, revision=revision, trust_remote_code=trust_remote_code, **kwargs)
        elif issubclass(processor_cls, ProcessorMixin):
            processor = processor_cls.from_pretrained(processor_name, *args, revision=revision, trust_remote_code=trust_remote_code, **kwargs)
        else:
...
def cached_processor_from_config(model_config: 'ModelConfig', processor_cls: Union[type[_P], tuple[type[_P], ...]]=ProcessorMixin, **kwargs: Any) -> _P:
    return cached_get_processor(model_config.model, revision=model_config.revision, trust_remote_code=model_config.trust_remote_code, processor_cls=processor_cls, **_merge_mm_kwargs(model_config, processor_cls, **kwargs))
...
def cached_feature_extractor_from_config(model_config: 'ModelConfig', **kwargs: Any):
    return cached_get_feature_extractor(model_config.model, revision=model_config.revision, trust_remote_code=model_config.trust_remote_code, **_merge_mm_kwargs(model_config, AutoFeatureExtractor, **kwargs))
...
def cached_image_processor_from_config(model_config: 'ModelConfig', **kwargs: Any):
    return cached_get_image_processor(model_config.model, revision=model_config.revision, trust_remote_code=model_config.trust_remote_code, **_merge_mm_kwargs(model_config, AutoImageProcessor, **kwargs))