...
class SpeculatorsConfig(PretrainedConfig):
    ...
    @classmethod
    def from_pretrained(cls, pretrained_model_name_or_path: Union[str, os.PathLike], **kwargs) -> 'SpeculatorsConfig':
        ...
        if speculators_model_type not in SUPPORTED_SPECULATORS_TYPES:
            raise ValueError(f"Expected one of: {SUPPORTED_SPECULATORS_TYPES}. Please ensure you're loading a speculators-format model.")
        cls.validate_speculators_config(config_dict=config_dict)
        aphrodite_config = cls.convert_speculators_to_aphrodite(config_dict=config_dict)
    ...
    @classmethod
    def convert_speculators_to_aphrodite(cls, config_dict: dict[str, Any]) -> dict[str, Any]:
        spec_config = config_dict['speculators_config']
        ...
        aphrodite_config = {'method': config_dict.get('speculators_model_type'), 'num_lookahead_tokens': num_lookahead_tokens, 'target_model': spec_config.get('verifier')['name_or_path']}
        aphrodite_config.update(config_dict['transformer_layer_config'])
