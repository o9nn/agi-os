...
class RTNLinearMethod(LinearMethodBase):
    ...
    def create_weights(self, layer: torch.nn.Module, input_size_per_partition: int, output_partition_sizes: list[int], input_size: int, output_size: int, params_dtype: torch.dtype, **extra_weight_attrs):
        ...
        factor = 1 if self.quant_config.weight_bits == 8 else 2
        weight = RTNParameter(data=torch.empty(output_size_per_partition // factor, input_size_per_partition, dtype=torch.uint8), scale=scale, quant_config=self.quant_config)
...
class RTNMoEMethod(FusedMoEMethodBase):
    ...
    def create_weights(self, layer: torch.nn.Module, num_experts: int, hidden_size: int, intermediate_size_per_partition: int, params_dtype: torch.dtype, **extra_weight_attrs):
        ...
        w13_scale = Parameter(torch.empty(num_experts, 2 * intermediate_size_per_partition, num_groups_per_col, dtype=params_dtype), requires_grad=False)
        layer.register_parameter('w13_scale', w13_scale)
        w13_weight = RTNParameter(data=torch.empty(num_experts, 2 * intermediate_size_per_partition // factor, hidden_size, dtype=torch.uint8), scale=w13_scale, quant_config=self.quant_config)
        layer.register_parameter('w13_weight', w13_weight)
        set_weight_attrs(w13_weight, extra_weight_attrs)
        num_groups_per_col = intermediate_size_per_partition // self.quant_config.group_size if self.quant_config.group_size != -1 else 1
        w2_scale = Parameter(torch.zeros(num_experts, hidden_size, num_groups_per_col, dtype=params_dtype), requires_grad=False)
        layer.register_parameter('w2_scale', w2_scale)
        w2_weight = RTNParameter(data=torch.empty(num_experts, hidden_size // factor, intermediate_size_per_partition, dtype=torch.uint8), scale=w2_scale, quant_config=self.quant_config)
