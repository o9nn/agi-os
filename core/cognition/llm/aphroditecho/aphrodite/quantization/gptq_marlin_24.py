...
class GPTQMarlin24LinearMethod(LinearMethodBase):
    ...
    def create_weights(self, layer: torch.nn.Module, input_size_per_partition: int, output_partition_sizes: list[int], input_size: int, output_size: int, params_dtype: torch.dtype, **extra_weight_attrs):
        ...
        meta = PackedAphroditeParameter(data=torch.empty(input_size_per_partition // 8 // 2 // 2, output_size_per_partition * 2, device='cuda', dtype=torch.int16), input_dim=0, output_dim=1, packed_dim=1, packed_factor=1, marlin_tile_size=2, weight_loader=weight_loader)
        ...
        workspace = BaseAphroditeParameter(data=torch.zeros(max_workspace_size, device='cuda', dtype=torch.int), weight_loader=weight_loader)
