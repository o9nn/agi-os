...
from aphrodite.modeling.layers.fused_moe import FusedMoE, FusedMoEMethodBase, FusedMoeWeightScaleSupported
...
class QuarkW4A4MXFp4MoEMethod(QuarkMoEMethod):
    def __init__(self, weight_config: dict[str, Any], input_config: dict[str, Any]):
        ...
        if not current_platform.supports_mx():
            self.emulate = True
            log_once('WARNING', 'The current platform does not support native MXFP4 computation. Simulated weight dequantization and activation QDQ (quantize and dequantize) will be used, with the linear layers computed in high precision.')
        else:
            self.emulate = True
            log_once('WARNING', 'The current platform supports native MXFP4 computation, but kernels are not yet integrated in vLLM. Simulated weight dequantization and activation QDQ (quantize and dequantize) will be used, with the linear layers computed in high precision.')
    def create_weights(self, layer: torch.nn.Module, num_experts: int, hidden_size: int, intermediate_size_per_partition: int, params_dtype: torch.dtype, **extra_weight_attrs):
