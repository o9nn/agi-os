...
class CompressedTensorsW4A16Sparse24(CompressedTensorsScheme):
    ...
    def create_weights(self, layer: torch.nn.Module, input_size: int, output_partition_sizes: list[int], input_size_per_partition: int, params_dtype: torch.dtype, weight_loader: Callable, **kwargs):
        ...
        qweight = PackedAphroditeParameter(data=torch.empty(input_size_per_partition // self.tile_size // 2, output_size_per_partition * self.tile_size // pack_factor, dtype=torch.int32), input_dim=0, output_dim=1, packed_dim=1, packed_factor=pack_factor, marlin_tile_size=self.tile_size, weight_loader=weight_loader)
        ...
        meta = PackedAphroditeParameter(data=torch.empty(input_size_per_partition // 8 // 2 // 2, output_size_per_partition * 2, dtype=torch.int16), input_dim=0, output_dim=1, packed_dim=1, packed_factor=1, marlin_tile_size=2, weight_loader=weight_loader)
