...
from aphrodite.modeling.parameter import BaseAphroditeParameter, ChannelQuantScaleParameter, GroupQuantScaleParameter, PackedAphroditeParameter
from aphrodite.modeling.utils import set_weight_attrs
from aphrodite.quantization import QuantizationMethods
from aphrodite.quantization.base_config import QuantizationConfig
from aphrodite.quantization.utils.bitblas_utils import BITBLAS_OPTIMIZE_FEATURES, BITBLAS_SUPPORTED_NUM_BITS, BITBLAS_SUPPORTED_SYM, MINIMUM_BITBLAS_VERSION
...
class BitBLASLinearMethod(LinearMethodBase):
    ...
    def create_weights_gptq(self, layer: torch.nn.Module, input_size_per_partition: int, output_partition_sizes: list[int], input_size: int, output_size: int, params_dtype: torch.dtype, **extra_weight_attrs):
        ...
        if self.quant_config.zeros_mode == 'quantized':
            zeros = PackedAphroditeParameter(data=torch.empty(input_groups, output_size_per_partition // self.quant_config.pack_factor, device='cuda', dtype=self.quant_config.storage_torch_dtype, requires_grad=False), input_dim=0, output_dim=1, packed_dim=1, packed_factor=self.quant_config.pack_factor, weight_loader=weight_loader)
        else:
            zeros = BaseAphroditeParameter(torch.empty(output_size_per_partition, input_groups, device='cuda', dtype=params_dtype), weight_loader=weight_loader)
