...
class TPUInt8LinearMethod(LinearMethodBase):
    ...
    def apply(self, layer: torch.nn.Module, x: torch.Tensor, bias: Optional[torch.Tensor]=None) -> torch.Tensor:
        try:
            ...
            raise ImportError('Please install torch_xla by following the instructions at https://docs.aphrodite.ai/en/latest/getting_started/tpu-installation.html to run vLLM on TPU.') from err
        weight = layer.weight
        scale = layer.scale
