...
class MetaData:
    def __init__(self):
        ...
        self.group_size = 0
        self.output_size = 0
def unpack_index_tensor(pack_tensor: torch.Tensor, index_bits: int, num_elements: int, res_bits: int=0, num_res_elements: int=0, index_dtype: torch.dtype=torch.uint16, as_dtype: torch.dtype=torch.int32) -> torch.Tensor:
...
def dequantize_weight(indices: torch.IntTensor, codebooks: torch.Tensor, res_codebooks: torch.Tensor, weight_scale: torch.Tensor, weight_bias: torch.Tensor, perm: torch.Tensor, metadata: MetaData) -> torch.Tensor:
    ...
    indices, res_indices = unpack_index_tensor(pack_tensor=indices, index_bits=index_bits, num_elements=group_size, res_bits=index_res_bits, num_res_elements=group_size, index_dtype=torch.uint16)
    ...
    if enable_residual:
        ...
        qweight = qweight + selected_res_centroids.reshape(num_codebooks, -1, group_size).permute(1, 0, 2).reshape(-1, num_codebooks * group_size)
    ...
    return qweight
def generic_dequantize_gemm(input: torch.Tensor, indices: torch.IntTensor, codebooks: torch.Tensor, res_codebooks: torch.Tensor, weight_scale: torch.Tensor, weight_bias: torch.Tensor, perm: torch.Tensor, bias: Optional[torch.Tensor], metadata: MetaData) -> torch.Tensor:
    dequantized_weight = dequantize_weight(indices, codebooks, res_codebooks, weight_scale, weight_bias, perm, metadata)
    return F.linear(input, dequantized_weight, bias)
def optimized_dequantize_gemm(input: torch.Tensor, indices: torch.IntTensor, codebooks: torch.Tensor, res_codebooks: torch.Tensor, weight_scale: torch.Tensor, weight_bias: torch.Tensor, perm: torch.Tensor, bias: Optional[torch.Tensor], metadata: MetaData) -> torch.Tensor:
    codebooks = codebooks.view(metadata.num_codebooks, metadata.num_centroids, metadata.vector_len)
    res_codebooks = res_codebooks.view(metadata.num_codebooks, metadata.num_res_centroids, metadata.vector_len) if metadata.num_res_centroids > 0 else None
    if input.numel() // input.shape[-1] < 3:
        return ops.vptq_gemm(input, indices, codebooks, weight_scale, weight_bias, [metadata.vector_len, weight_scale.shape[0], metadata.output_size], None, res_codebooks, None, None, perm, bias)
    if perm is None:
        invert_perm = None
    else:
        invert_perm = torch.argsort(perm.view(torch.uint16).to(torch.int64)).to(torch.uint16).view(torch.int16)
    dequantized_weight = ops.vptq_dequant(indices, codebooks, weight_scale, weight_bias, [metadata.vector_len, weight_scale.shape[0], metadata.output_size], None, res_codebooks, None, None, invert_perm)
    return F.linear(input, dequantized_weight, bias)
def merged_dequantize_gemm(input: torch.Tensor, indices: torch.IntTensor, codebooks: torch.Tensor, res_codebooks: torch.Tensor, weight_scale: torch.Tensor, weight_bias: torch.Tensor, perm: torch.Tensor, output_partition_sizes: List[int], bias: Optional[torch.Tensor], metadata: MetaData) -> torch.Tensor:
    ...
    for linear_idx, output_size, indice_size in zip(range(num_linears), output_partition_sizes, indice_sizes):
        ...
        shard_output = optimized_dequantize_gemm(input, indices.narrow(1, indice_offset, indice_size), codebooks.narrow(0, codebooks_offset, num_codebooks), res_codebooks.narrow(0, codebooks_offset, num_codebooks), weight_scale.narrow(0, input_offset, input_size), weight_bias.narrow(0, input_offset, input_size), perm.narrow(0, input_offset, input_size) if perm is not None else None, bias if bias is None else bias.narrow(0, output_offset, output_size), metadata)
...
class VPTQConfig(QuantizationConfig):
    def __init__(self, config_for_layers: Dict[str, Dict[str, Any]], shared_layer_config: Dict[str, Dict[str, Any]]) -> None:
        super().__init__()
    ...
    def from_config(cls, config: Dict[str, Any]) -> 'VPTQConfig':
        ...
        assert len(config_for_layers) > 0 or len(shared_layer_config) > 0, "VPTQConfig must have at least one of 'config_for_layers'             or 'shared_layer_config'"
    ...
    def get_quant_method(self, layer: torch.nn.Module, prefix: str) -> Optional['VPTQLinearMethod']:
        if isinstance(layer, LinearBase):
            ...
            if linear_name == 'qkv_proj':
                quant_config = {'q_proj': self.get_config_for_key(base_name, 'q_proj'), 'k_proj': self.get_config_for_key(base_name, 'k_proj'), 'v_proj': self.get_config_for_key(base_name, 'v_proj')}
            elif linear_name == 'gate_up_proj':
                quant_config = {'gate_proj': self.get_config_for_key(base_name, 'gate_proj'), 'up_proj': self.get_config_for_key(base_name, 'up_proj')}
            else:
...
class VPTQLinearMethod(LinearMethodBase):
    ...
    @staticmethod
    def quantized_weight_loader(indice_sizes, narrow_dim=1):
        def wrap_weight_loader(param: torch.nn.Parameter, loaded_weight: torch.Tensor, loaded_shard_id: Optional[Union[str, int]]=None):
            ...
            if loaded_shard_id is not None:
                param_data = param_data.narrow(narrow_dim, sum(shard_sizes[:_loaded_shard_id]), shard_sizes[_loaded_shard_id])
    ...
    def create_weights(self, layer: torch.nn.Module, input_size_per_partition: int, output_partition_sizes: List[int], input_size: int, output_size: int, params_dtype: torch.dtype, **extra_weight_attrs):
        ...
        assert not enable_perm, 'perm is not absorbed in this model, please process it by `pip install vptq && python -m vptq.tools.pre_process --input_path xx --output_path xx`'
        ...
        if enable_norm:
            wrapped_weight_loader = VPTQLinearMethod.quantized_weight_loader([[(input_size_per_partition * tp_ind, input_size_per_partition * (tp_ind + 1)) for num in output_partition_sizes] for tp_ind in range(row_parallel_tp_size)][get_tensor_model_parallel_rank() % row_parallel_tp_size], 0)
            extra_weight_attrs['weight_loader'] = wrapped_weight_loader
            extra_weight_attrs['output_dim'] = 0
            weight_scale = Parameter(torch.empty(input_size_per_partition * num_linears, dtype=params_dtype), requires_grad=False)
            weight_bias = Parameter(torch.empty(input_size_per_partition * num_linears, dtype=params_dtype), requires_grad=False)
        ...
        indice_sizes = [[(math.floor(num * tp_ind / vector_len), math.ceil(num * (tp_ind + 1) / vector_len)) for num in output_partition_sizes] for tp_ind in range(col_parallel_tp_size)]
        tp_output_offset = [[num * tp_ind % vector_len for num in output_partition_sizes] for tp_ind in range(col_parallel_tp_size)]
        ...
        indices = Parameter(torch.empty((num_codebooks, num_indices, packed_groupsize), dtype=torch.int32), requires_grad=False)
        if row_parallel_tp_size == 1:
            wrapped_weight_loader = VPTQLinearMethod.quantized_weight_loader(this_rank_indice_sizes)
            extra_weight_attrs['weight_loader'] = wrapped_weight_loader
        set_weight_attrs(indices, {'output_partition_sizes': output_partition_sizes, 'output_offset': tp_output_offset, 'shard_sizes': shard_sizes, 'input_dim': -1})
        ...
        set_weight_attrs(centroids.weight, {'codebook_sizes': [num_centroids * vector_len for _ in output_partition_sizes]})
        layer.centroids = centroids
        if enable_residual:
            res_centroids = torch.nn.Embedding(num_codebooks * num_linears, num_res_centroids * vector_len, dtype=params_dtype)
            set_weight_attrs(res_centroids.weight, extra_weight_attrs)
            layer.res_centroids = res_centroids
            set_weight_attrs(res_centroids.weight, {'codebook_sizes': [num_res_centroids * vector_len for _ in output_partition_sizes]})
    def apply(self, layer: torch.nn.Module, x: torch.Tensor, bias: Optional[torch.Tensor]=None) -> torch.Tensor:
        ...
        return merged_dequantize_gemm(x, indices, centroids, res_centroids, weight_scale, weight_bias, perm, output_partition_sizes, bias, layer.metadata)