...
class EETQConfig(QuantizationConfig):
    def __init__(self, weight_bits: int, zero_point: bool) -> None:
        super().__init__()
        self.weight_bits = weight_bits
        self.zero_point = zero_point
        if self.weight_bits != 8:
            raise ValueError(f'Currently, only 8-bit weight quantization is supported for EETQ, but got {self.weight_bits} bits.')
...
class EETQLinearMethod(LinearMethodBase):
    ...
    def create_weights(self, layer: torch.nn.Module, input_size_per_partition: int, output_partition_sizes: List[int], input_size: int, output_size: int, params_dtype: torch.dtype, **extra_weight_attrs):
        ...
        qweight = Parameter(torch.empty(input_size_per_partition, output_size_per_partition, dtype=torch.int8), requires_grad=False)
