...
class QuantLLMFPConfig(QuantizationConfig):
    def __init__(self, weight_bits: int=6, exp_bits: int=2) -> None:
        super().__init__()
        ...
        if self.weight_bits not in [2, 3, 4, 5, 6, 7]:
            raise ValueError(f'Currently, only 4-bit, 5-bit, 6-bit, and 7-bit quantization are supported for QuantLLM FP quantizaiton, but got {self.weight_bits} bits.')
        self._logged = False
            ...
            if model_parallel_is_initialized():
                try:
                    if get_tensor_model_parallel_rank() == 0:
                        logger.info(f'Loading model in FP{self.weight_bits}_E{self.exponent_bits}M{self.mantissa_bits} format.')
                        self._logged = True
...
class QuantLLMFPLinearMethod(LinearMethodBase):
    ...
    def create_weights(self, layer: torch.nn.Module, input_size_per_partition: int, output_partition_sizes: List[int], input_size: int, output_size: int, params_dtype: torch.dtype, weight_loader=None, **extra_weight_attrs):
        ...
        weight = QuantLLMFPParameter(torch.Size((output_size_per_partition, input_size_per_partition)), params_dtype=params_dtype, quant_config=self.quant_config)
    ...
    def apply(self, layer, x: torch.Tensor, bias: Optional[torch.Tensor]=None) -> torch.Tensor:
        ...
        if bias is None:
            return ops.fp_eXmY_linear_forward_cuda(self.quant_config.exponent_bits, self.quant_config.mantissa_bits, x, weights, scales, splitK)
        else:
            return ops.fp_eXmY_linear_forward_cuda(self.quant_config.exponent_bits, self.quant_config.mantissa_bits, x, weights, scales, splitK) + bias
class QuantLLMFPParameter(nn.Parameter):
    def __new__(cls, orig_shape: torch.Size, params_dtype: torch.dtype, quant_config: QuantLLMFPConfig):
        data = torch.empty(torch.Size((orig_shape[0], orig_shape[1] * quant_config.weight_bits // 8)), dtype=torch.uint8)
    ...
    def quant_llmdequantize(self, output_dtype=None):
        ...
        return from_scaled_tc_fpx(self.data, self.quant_config.exponent_bits, self.quant_config.mantissa_bits, self.scales).to(output_dtype)