...
class QuipLinearMethod(LinearMethodBase):
    ...
    def create_weights(self, layer: torch.nn.Module, input_size_per_partition: int, output_partition_sizes: List[int], input_size: int, output_size: int, params_dtype: torch.dtype, **extra_weight_attrs):
        ...
        if had_left is not None:
            layer.register_parameter('had_left', Parameter(had_left.to(dtype=params_dtype, device='cuda'), requires_grad=False))
            set_weight_attrs(layer.had_left, extra_weight_attrs)
        if had_right is not None:
            layer.register_parameter('had_right', Parameter(had_right.to(dtype=params_dtype, device='cuda'), requires_grad=False))
            set_weight_attrs(layer.had_right, extra_weight_attrs)
        layer.register_parameter('Qidxs', Parameter(torch.empty(q_out_features, q_in_features // self.pack, device='cuda', dtype=self.idx_dtype), requires_grad=False))
        set_weight_attrs(layer.Qidxs, extra_weight_attrs)
        layer.register_parameter('Wscale', Parameter(torch.ones((), dtype=torch.float, device='cuda'), requires_grad=False))
        set_weight_attrs(layer.Wscale, extra_weight_attrs)
        layer.register_parameter('SU', Parameter(torch.ones(input_size, device='cuda', dtype=params_dtype), requires_grad=False))
        set_weight_attrs(layer.SU, extra_weight_attrs)
        layer.register_parameter('SV', Parameter(torch.ones(output_size, device='cuda', dtype=params_dtype), requires_grad=False))
    ...
    def apply(self, layer: torch.nn.Module, x: torch.Tensor, bias: Optional[torch.Tensor]=None) -> torch.Tensor:
        ...
        reshaped_x = matmul_hadUt_cuda(reshaped_x, layer.get('had_left', None), layer.K_left, layer.q_in_features, layer.Wscale)
