...
from aphrodite.modeling.layers.linear import ColumnParallelLinear, LinearBase, LinearMethodBase, QKVParallelLinear, RowParallelLinear
from aphrodite.modeling.utils import set_weight_attrs
from aphrodite.quantization.base_config import QuantizationConfig
class AutoQuantConfig(QuantizationConfig):
    def __init__(self, weight_bits: int, group_size: int, zero_point: bool, from_float: bool, quant_mode: str) -> None:
        ...
        if quant_mode == 'weight_only' and self.weight_bits != 4:
            raise ValueError(f'Currently, only 4-bit weight quantization is supported for AutoQuant weight_only, but got {self.weight_bits} bits.')
        if quant_mode in ['llm_int8', 'smoothquant'] and self.weight_bits != 8:
            raise ValueError(f'Currently, only 8-bit weight quantization is supported for AutoQuant llm_int8 or smoothquant, but got {self.weight_bits} bits.')
        self.pack_factor = 32 // self.weight_bits
    def __repr__(self) -> str:
        return f'AutoQuantConfig(weight_bits={self.weight_bits}, group_size={self.group_size}, zero_point={self.zero_point}, from_float={self.from_float}, quant_mode={self.quant_mode})'
...
class AutoQuantLinearMethod(LinearMethodBase):
    ...
    def create_weights(self, layer: torch.nn.Module, input_size_per_partition: int, output_partition_sizes: List[int], input_size: int, output_size: int, params_dtype: torch.dtype, **extra_weight_attrs):
        if self.quant_config.quant_mode == 'weight_only' and input_size_per_partition % self.quant_config.group_size != 0:
        ...
        if self.quant_config.quant_mode == 'weight_only' and output_size_per_partition % self.quant_config.pack_factor != 0:
...
def cal_qparams_per_group_minmax(w: torch.Tensor, n_bits: int=4, group_size: int=128):
    ...
    q_min = 0
    scales = w_max - w_min
    scales = scales.clamp_(min=1e-05).div_(q_max)
...
def quant(weight: torch.Tensor, qparams: Optional[QParams]=None) -> torch.Tensor:
    if qparams is None:
...
def replace_quant_params(model, quant_config, modules_to_not_convert='lm_head'):
    ...
    for name, module in model.named_children():
        ...
        if isinstance(module, (ColumnParallelLinear, QKVParallelLinear, RowParallelLinear)) and name not in modules_to_not_convert:
            if quant_config.from_float:
                ...
                if quant_config.quant_mode in ('llm_int8', 'smoothquant'):
                    ...
                    set_weight_attrs(new_value, {'input_dim': 0, 'output_dim': 1, 'packed_dim': 1, 'pack_factor': quant_config.pack_factor})
                    del param
                    ...
                    set_weight_attrs(param_qweight, {'input_dim': 0, 'output_dim': 1, 'packed_dim': 1, 'pack_factor': quant_config.pack_factor})
                    set_weight_attrs(param_scales_zeros, {'input_dim': 0, 'output_dim': 1})
