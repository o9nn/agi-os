...
def maybe_warn_marlin_atomic_add(device, dtype):
    ...
    if device_capability[0] < 9 and dtype == torch.bfloat16:
        log_once('WARNING', 'You are running Marlin kernel with bf16 on GPUs before SM90. You can consider change to fp16 to achieve better performance if possible.')
def maybe_warn_marlin_atomic_add_env():
    if torch.compiler.is_dynamo_compiling():
        return
    if envs.APHRODITE_MARLIN_USE_ATOMIC_ADD:
        return
    log_once('INFO', 'Marlin kernel can achieve better performance for small size_n with experimental use_atomic_add feature. You can consider set environment variable APHRODITE_MARLIN_USE_ATOMIC_ADD to 1 if possible.')
