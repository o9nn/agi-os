...
class AutoRoundConfig(QuantizationConfig):
    ...
    def from_config(cls, config: dict[str, Any]) -> 'AutoRoundConfig':
        return cls(weight_bits=cls.get_from_keys(config, ['bits']), group_size=cls.get_from_keys(config, ['group_size']), sym=cls.get_from_keys(config, ['sym']), packing_format=cls.get_from_keys_or(config, ['packing_format'], 'auto_round:auto_gptq'), block_name_to_quantize=cls.get_from_keys_or(config, ['block_name_to_quantize', 'to_quant_block_names'], None), extra_config=cls.get_from_keys_or(config, ['extra_config'], None), data_type=cls.get_from_keys_or(config, ['data_type'], 'int'), backend=cls.get_from_keys_or(config, ['backend', 'aphrodite_backend'], 'auto'))
    def get_layer_config(self, layer, layer_name: str):
        def get_config(name: str, quantized: bool=True):
            cfg = self.extra_config.get(name, {}) if self.extra_config else {}
            return (cfg.get('bits', self.weight_bits if quantized else 16), cfg.get('group_size', self.group_size if quantized else -1), cfg.get('sym', self.sym if quantized else True))
    ...
    def apply_awq_quant_layer(self, layer, prefix: str, backend: str='auto'):
        ...
        if not self.check_quantized(weight_bits):
            if isinstance(layer, (LinearBase, ParallelLMHead)):
                ...
                return None
        logger.debug('[{}] Type: {}, Bits: {}, Group Size: {}, Sym: {}', prefix, layer.__class__.__name__, weight_bits, group_size, sym)
