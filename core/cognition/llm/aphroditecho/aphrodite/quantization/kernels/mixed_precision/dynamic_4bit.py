...
class Dynamic4bitLinearKernel(MPLinearKernel):
    ...
    def can_implement(cls, c: MPLinearLayerConfig) -> tuple[bool, Optional[str]]:
        ...
        if current_platform.get_cpu_architecture() == CpuArchEnum.ARM and c.act_type not in [torch.float32]:
        ...
        if current_platform.get_cpu_architecture() == CpuArchEnum.ARM:
            try:
                ...
                return (False, f'PyTorch {torch.__version__} does not support _dyn_quant_matmul_4bit. Install a newer version')
        return (True, None)
    def process_weights_after_loading(self, layer: torch.nn.Module):
        ...
        w = torch.ops.aten._dyn_quant_pack_4bit_weight(uint8_packed, scales, layer.bias, block_size, c.partition_weight_shape[0], c.partition_weight_shape[1])
    ...
    def apply_weights(self, layer: torch.nn.Module, x: torch.Tensor, bias: Optional[torch.Tensor]=None) -> torch.Tensor:
        ...
        output = torch.ops.aten._dyn_quant_matmul_4bit(x_2d, w_q, c.group_size, c.partition_weight_shape[0], c.partition_weight_shape[1])
