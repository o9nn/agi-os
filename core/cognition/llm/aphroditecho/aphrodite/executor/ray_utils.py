...
try:
    ...
    class RayWorkerWrapper(WorkerWrapperBase):
        ...
        def get_node_and_gpu_ids(self) -> Tuple[str, List[int]]:
            ...
            if not device_key:
                raise RuntimeError('current platform {} does not support ray.', aphrodite.platforms.current_platform.device_name)
...
def _wait_until_pg_ready(current_placement_group: 'PlacementGroup'):
    ...
    while time.time() - s < PG_WAIT_TIMEOUT:
        ...
        if len(ready) > 0:
            break
        wait_interval *= 2
        logger.info('Waiting for creating a placement group of specs for {} seconds. specs={}. Check `ray status` and `ray list nodes` to see if you have enough resources, and make sure the IP addresses used by ray cluster are the same as APHRODITE_HOST_IP environment variable specified in each node if you are running on a multi-node.', int(time.time() - s), placement_group_specs)
    try:
...
def _wait_until_pg_removed(current_placement_group: 'PlacementGroup'):
    ...
    while time.time() - s < PG_WAIT_TIMEOUT:
        ...
        if pg is None:
            break
        wait_interval *= 2
        logger.info('Waiting for removing a placement group of specs for {} seconds.', int(time.time() - s))
...
def initialize_ray_cluster(parallel_config: ParallelConfig, ray_address: Optional[str]=None):
    ...
    if ray.is_initialized():
        ...
        try:
            ...
            ray.init(address=ray_address, num_gpus=parallel_config.world_size, runtime_env=parallel_config.ray_runtime_env)
    else:
    ...
    if current_placement_group:
        ...
        if parallel_config.world_size > num_devices_in_cluster:
            logger.warning('The number of required {}s exceeds the total number of available {}s in the placement group.', device_str, device_str)
        ...
        if current_node_resource.get(device_str, 0) < 1:
            raise ValueError(f'Current node has no {device_str} available. current_node_resource={current_node_resource!r}. Aphrodite engine cannot start without {device_str}. Make sure you have at least 1 {device_str} available in a node current_node_id={current_node_id!r} current_ip={current_ip!r}.')
