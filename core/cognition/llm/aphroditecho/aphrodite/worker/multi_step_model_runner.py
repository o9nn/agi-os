...
import torch
from aphrodite.common.sequence import CompletionSequenceGroupOutput, IntermediateTensors, Logprob, SequenceGroupMetadata, SequenceOutput
from aphrodite.distributed import get_pp_group
from aphrodite.modeling.layers.sampler import PromptLogprobs, SampleLogprobs, SamplerOutput, SamplingMetadata, get_logprobs, get_pythonized_sample_results
from aphrodite.platforms import current_platform
from aphrodite.utils import PyObjectCache, async_tensor_h2d, current_stream
from aphrodite.worker.model_runner import GPUModelRunnerBase, ModelInputForGPUWithSamplingMetadata
from aphrodite.worker.model_runner_base import BroadcastableModelInput, _init_attn_metadata_from_tensor_dict, _init_frozen_model_input_from_tensor_dict, _init_sampling_metadata_from_tensor_dict
...
class MultiStepModelRunner(GPUModelRunnerBase[StatefulModelInput]):
    def __init__(self, base_model_runner: GPUModelRunnerBase, *args, **kwargs):
        ...
        if self.attn_backend.get_name() not in supported_attention_backends:
            ...
            raise ValueError(f'{ms_config_str} not supported for attention backend: {self.attn_backend.get_name()}. Set APHRODITE_ATTENTION_BACKEND to a value from {supported_attention_backends}.')
