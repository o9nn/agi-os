...
class PEFTHelper:
    ...
    def _validate_features(self) -> tuple[list[str], list[str]]:
        ...
        if self.modules_to_save:
            warning_msg.append('Aphrodite only supports modules_to_save being None.')
        if self.use_dora:
    ...
    def from_local_dir(cls, lora_path: str, max_position_embeddings: Optional[int], tensorizer_config_dict: Optional[dict]=None) -> 'PEFTHelper':
        ...
        if tensorizer_config_dict:
            ...
            with open_stream(lora_config_path, mode='rb', **tensorizer_args.stream_kwargs) as f:
                config = json.load(f)
            logger.info('Successfully deserialized LoRA config from {}', tensorizer_config.tensorizer_dir)
        else:
    ...
    def validate_legal(self, lora_config: LoRAConfig) -> None:
        ...
        if warning_msg:
            logger.warning('Aphrodite LoRA configuration has some unsupported features: {}', ' '.join(warning_msg))