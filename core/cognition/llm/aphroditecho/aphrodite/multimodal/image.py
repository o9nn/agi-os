...
def rescale_image_size(image: Image.Image, size_factor: float, transpose: int=-1) -> Image.Image:
    ...
    return image
def rgba_to_rgb(image: Image.Image, background_color: Union[tuple[int, int, int], list[int]]=(255, 255, 255)) -> Image.Image:
...
class ImageMediaIO(MediaIO[Image.Image]):
    def __init__(self, image_mode: str='RGB', **kwargs) -> None:
        ...
        if not (isinstance(rgba_bg, tuple) and len(rgba_bg) == 3 and all((isinstance(c, int) and 0 <= c <= 255 for c in rgba_bg))):
