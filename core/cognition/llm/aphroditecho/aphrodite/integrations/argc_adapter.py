...
class ArgcAdapter:
    ...
    def _find_argc_binary(self) -> Optional[str]:
        try:
            result = subprocess.run(['which', 'argc'], capture_output=True, text=True, timeout=5)
        ...
        if argc_src.exists():
            try:
                ...
                result = subprocess.run(['cargo', 'build', '--release'], cwd=argc_src, capture_output=True, text=True, timeout=300)
                if result.returncode == 0:
    ...
    def _initialize_argc(self):
        try:
            ...
            result = subprocess.run([self.argc_binary, '--version'], capture_output=True, text=True, timeout=10)
            if result.returncode == 0:
                self.available = True
    ...
    def _setup_compatibility_mode(self):
        ...
        if self._install_argc_binary():
            ...
            raise RuntimeError("Argc binary is required for production operation. Please install argc manually or ensure it's available in PATH. See installation instructions at: https://github.com/sigoden/argc")
    def _install_argc_binary(self) -> bool:
        try:
            ...
            if system == 'linux':
                result = subprocess.run(['cargo', 'install', 'argc'], capture_output=True, text=True, timeout=300)
                if result.returncode == 0:
                    import os
    ...
    def _parse_script_content(self, content: str, script_path: Path) -> Optional[ArgcSchema]:
        ...
        for line in lines:
            ...
            if line.startswith('# @describe'):
                ...
                current_command = ArgcCommand(name='', description=command_desc, script_path=str(script_path))
        ...
        if script_description or commands:
            return ArgcSchema(script_name=script_path.name, description=script_description, commands=commands, global_flags=global_flags, global_options=global_options, metadata={'source_path': str(script_path)})
        return None
    ...
    def _register_command_as_function(self, command_name: str, command: ArgcCommand):
        try:
            ...
            for arg in command.args:
                parameters[arg['name']] = ParameterSpec(type='string', description=arg.get('description', f"Argument: {arg['name']}"), required=arg.get('required', False))
            for flag in command.flags:
                param_name = flag.get('name', flag.get('short', 'flag'))
                parameters[param_name] = ParameterSpec(type='boolean', description=flag.get('description', f'Flag: {param_name}'), required=False, default=False)
            for option in command.options:
                param_name = option.get('name', option.get('short', 'option'))
                param_type = option.get('value_type', 'string')
                parameters[param_name] = ParameterSpec(type=param_type, description=option.get('description', f'Option: {param_name}'), required=False)
            func_spec = FunctionSpec(name=f'argc_{command_name}', description=command.description or f'Argc command: {command.name}', parameters=parameters, safety_class=SafetyClass.MEDIUM, cost_unit=0.5, implementation_ref=f'argc.command.{command_name}', tags=['argc', 'cli', 'command'], metadata={'argc_command': command.name, 'script_path': command.script_path, 'alias': command.alias})
    ...
    async def execute_command(self, command_name: str, arguments: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not self.available:
            raise RuntimeError(f'Argc binary not available. Command {command_name} cannot be executed. Please ensure argc is properly installed and initialized.')
        try:
            ...
            result = {'command': command.name, 'exit_code': process.returncode, 'stdout': stdout.decode().strip(), 'stderr': stderr.decode().strip(), 'arguments': arguments}
    ...
    def get_command_schema(self, command_name: str) -> Optional[Dict[str, Any]]:
        ...
        if not command:
            return None
        return {'name': command.name, 'description': command.description, 'alias': command.alias, 'flags': command.flags, 'options': command.options, 'args': command.args, 'script_path': command.script_path, 'metadata': command.metadata}
    def list_commands(self) -> List[Dict[str, Any]]:
        return [{'name': name, 'command': cmd.name, 'description': cmd.description, 'alias': cmd.alias, 'script': cmd.script_path, 'parameters': len(cmd.args) + len(cmd.flags) + len(cmd.options)} for name, cmd in self.commands.items()]
    def parse_command_string(self, command_string: str) -> Optional[Dict[str, Any]]:
        if not self.available:
            raise RuntimeError('Argc binary not available. Cannot parse command string. Please ensure argc is properly installed and initialized.')
        try:
            ...
            if self.argc_binary is None:
                raise RuntimeError('Argc binary path is None')
            result = subprocess.run([self.argc_binary, temp_script, 'parse_cmd', command_string], capture_output=True, text=True, timeout=10)
            Path(temp_script).unlink()
            if result.returncode == 0:
                return {'parsed': True, 'result': result.stdout.strip(), 'command_string': command_string}
            else:
                return {'parsed': False, 'error': result.stderr.strip(), 'command_string': command_string}
        except Exception as e:
            logger.error(f'Failed to parse command string: {e}')
            return {'parsed': False, 'error': str(e)}
    def health_check(self) -> Dict[str, Any]:
        return {'status': 'healthy' if self.available else 'compatibility_mode', 'argc_available': self.available, 'binary_path': self.argc_binary, 'schemas_loaded': len(self.schemas), 'commands_registered': len(self.commands), 'functions_registered': len([f for f in self.function_registry.functions.values() if f.implementation_ref.startswith('argc.command.')])}