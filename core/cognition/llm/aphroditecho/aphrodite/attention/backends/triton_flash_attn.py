...
from aphrodite.attention.backends.abstract import AttentionBackend, AttentionImpl, AttentionLayer, AttentionMetadata, AttentionType
from aphrodite.attention.backends.utils import CommonAttentionState, CommonMetadataBuilder
from aphrodite.attention.ops.paged_attn import PagedAttention, PagedAttentionMetadata
...
def is_rocm_aiter_paged_attn_enabled() -> bool:
    return envs.APHRODITE_ROCM_USE_AITER_PAGED_ATTN and envs.APHRODITE_ROCM_USE_AITER
@cache
def _get_paged_attn_module() -> PagedAttention:
    if is_rocm_aiter_paged_attn_enabled():
        from aphrodite.attention.ops.rocm_aiter_paged_attn import AITERPagedAttention
...
class ROCmFlashAttentionImpl(AttentionImpl):
    def __init__(self, num_heads: int, head_size: int, scale: float, num_kv_heads: int, alibi_slopes: Optional[List[float]], sliding_window: Optional[int], kv_cache_dtype: str, blocksparse_params: Optional[Dict[str, Any]]=None, logits_soft_cap: Optional[float]=None, attn_type: str=AttentionType.DECODER, use_irope: bool=False) -> None:
        if use_irope:
            log_once('WARNING', 'Using irope in ROCm Flash Attention is not supported yet, it will fail back to global attention for long context.')
        if blocksparse_params is not None:
            raise ValueError('ROCmFlashAttention does not support blocksparse attention.')
        if use_irope:
        ...
        if self.use_triton_flash_attn:
            if logits_soft_cap is not None:
                raise ValueError('ROCm Triton FlashAttention does not support attention logits soft capping. please try using the ROCm CK FA backend instead by setting the env var `APHRODITE_USE_TRITON_FLASH_ATTN=0`')
            ...
            if self.sliding_window != (-1, -1):
                logger.warning('ROCm Triton FA does not currently support sliding window attention. If using half precision, please try using the ROCm CK FA backend instead by setting the env var `APHRODITE_USE_TRITON_FLASH_ATTN=0`')
        else:
    ...
    def forward(self, layer: AttentionLayer, query: torch.Tensor, key: torch.Tensor, value: torch.Tensor, kv_cache: torch.Tensor, attn_metadata: ROCmFlashAttentionMetadata, output: Optional[torch.Tensor]=None) -> torch.Tensor:
        ...
        if (prefill_meta := attn_metadata.prefill_metadata):
            ...
            if kv_cache.numel() == 0 or prefill_meta.block_tables.numel() == 0:
                ...
                if self.use_triton_flash_attn:
                    ...
                    use_fp8_scales = layer._q_scale and layer._k_scale and layer._v_scale and layer._prob_scale and (self.kv_cache_dtype == 'fp8')
                    full_scales = (layer._q_scale, layer._k_scale, layer._v_scale, layer._prob_scale) if use_fp8_scales else None
                    self.triton_attn_func(query, key, value, output[:num_prefill_tokens], query_seq_start_loc, key_seq_start_loc, query_max_seq_len, key_max_seq_len, causal_mask, self.scale, attn_masks[0][None] if attn_masks is not None else None, full_scales)
        ...
        if (decode_meta := attn_metadata.decode_metadata) and self.attn_type != AttentionType.ENCODER_ONLY:
            ...
            use_custom = use_rocm_custom_paged_attention(decode_query.dtype, head_size, block_size, gqa_ratio, decode_meta.max_decode_seq_len, self.sliding_window)
            if use_custom:
