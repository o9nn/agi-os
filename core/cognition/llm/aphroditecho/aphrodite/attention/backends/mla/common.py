...
from aphrodite.attention.backends.abstract import AttentionBackend, AttentionLayer, AttentionMetadata, AttentionMetadataBuilder, AttentionState, MLAAttentionImpl
...
from aphrodite.utils import async_tensor_h2d, cdiv, make_tensor_with_pad, round_down
...
class MLACommonImpl(MLAAttentionImpl[T], Generic[T]):
    ...
    def _flash_attn_varlen_diff_headdims(self, q, k, v, softmax_scale, return_softmax_lse, **kwargs):
        ...
        if is_hip and envs.APHRODITE_USE_TRITON_FLASH_ATTN and (not return_softmax_lse):
    ...
    def _forward_prefill(self, q: torch.Tensor, kv_c_normed: torch.Tensor, k_pe: torch.Tensor, kv_c_and_k_pe_cache: torch.Tensor, attn_metadata: MLACommonMetadata) -> torch.Tensor:
        ...
        if has_context:
            ...
            merge_attn_states(output=output, prefix_output=context_output, prefix_lse=context_lse, suffix_output=suffix_output)
        if self._pad_v:
