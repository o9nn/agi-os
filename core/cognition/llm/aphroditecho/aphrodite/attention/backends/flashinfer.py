...
from aphrodite.attention.backends.abstract import AttentionBackend, AttentionImpl, AttentionLayer, AttentionMetadata, AttentionMetadataBuilder, AttentionState, AttentionType
from aphrodite.attention.backends.utils import PAD_SLOT_ID, compute_slot_mapping, compute_slot_mapping_start_idx, is_block_tables_empty
...
from aphrodite.utils import async_tensor_h2d, get_kv_cache_torch_dtype, make_tensor_with_pad
...
class PerLayerParameters:
    ...
    sm_scale: float
def get_per_layer_parameters(aphrodite_config: AphroditeConfig) -> Dict[str, PerLayerParameters]:
...
class FlashInferState(AttentionState):
    ...
    def get_kv_cache_layout():
        ...
        if _KV_CACHE_LAYOUT_OVERRIDE is not None:
            log_once('INFO', 'Using KV cache layout {}', _KV_CACHE_LAYOUT_OVERRIDE)
            return _KV_CACHE_LAYOUT_OVERRIDE
        cache_layout = envs.APHRODITE_KV_CACHE_LAYOUT
        if cache_layout is None:
            log_once('INFO', 'Using default KV cache layout NHD')
            return 'NHD'
        log_once('INFO', 'Using KV cache layout {}', cache_layout)
        return cache_layout
    ...
    def graph_capture_get_metadata_for_batch(self, batch_size: int, is_encoder_decoder_model: bool=False):
        ...
        global_params = infer_global_hyperparameters(get_per_layer_parameters(self.aphrodite_config))
...
class FlashInferMetadataBuilder(AttentionMetadataBuilder[FlashInferMetadata]):
    ...
    def prepare(self):
        ...
        if self.global_hyperparameters is None:
            inferred_params = infer_global_hyperparameters(get_per_layer_parameters(self.aphrodite_config))
...
class FlashInferImpl(AttentionImpl):
    def __init__(self, num_heads: int, head_size: int, scale: float, num_kv_heads: int, alibi_slopes: Optional[List[float]], sliding_window: Optional[int], kv_cache_dtype: str, logits_soft_cap: Optional[float]=None, attn_type: str=AttentionType.DECODER, kv_sharing_target_layer_name: Optional[str]=None, use_irope: bool=False) -> None:
        ...
        if use_irope:
            log_once('WARNING', 'Using irope in FlashInfer is not supported yet, it will fall back to global attention for long context.')
    ...
    def forward(self, layer: AttentionLayer, query: torch.Tensor, key: torch.Tensor, value: torch.Tensor, kv_cache: torch.Tensor, attn_metadata: FlashInferMetadata, output: Optional[torch.Tensor]=None, output_scale: Optional[torch.Tensor]=None) -> torch.Tensor:
        ...
        if num_decode_tokens > 0:
            decode_output = torch.empty(decode_query.shape, dtype=decode_query.dtype, device=decode_query.device)
        else:
            decode_output = None
        ...
        if (decode_meta := attn_metadata.decode_metadata):
            ...
            if not use_trtllm_attention(num_decode_tokens, attn_metadata.max_decode_seq_len, kv_cache_dtype, attn_metadata.num_qo_heads, attn_metadata.num_kv_heads, attn_metadata.head_dim):
                decode_meta.decode_wrapper.run(decode_query, kv_cache.permute(*stride_order), k_scale=layer._k_scale_float, v_scale=layer._v_scale_float, out=decode_output)
            else:
                workspace_buffer = decode_meta.decode_wrapper._float_workspace_buffer
                assert FlashInferState.get_kv_cache_layout() == 'HND'
                trtllm_batch_decode_with_kv_cache(query=decode_query, kv_cache=kv_cache.permute(*stride_order), workspace_buffer=workspace_buffer, block_tables=attn_metadata.block_tables, seq_lens=decode_meta.seq_lens_tensor, max_seq_len=attn_metadata.max_decode_seq_len, bmm1_scale=layer._k_scale_float * softmax_scale, bmm2_scale=layer._v_scale_float, out=decode_output)
