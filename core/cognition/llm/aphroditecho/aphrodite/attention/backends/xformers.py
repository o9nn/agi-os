...
from aphrodite.attention.backends.utils import CommonAttentionState, CommonMetadataBuilder, get_num_prefill_decode_query_kv_tokens, get_seq_len_block_table_args, is_all_cross_attn_metadata_set, is_all_encoder_attn_metadata_set
...
class XFormersImpl(AttentionImpl[XFormersMetadata]):
    def __init__(self, num_heads: int, head_size: int, scale: float, num_kv_heads: int, alibi_slopes: Optional[List[float]], sliding_window: Optional[int], kv_cache_dtype: str, logits_soft_cap: Optional[float]=None, attn_type: str=AttentionType.DECODER, kv_sharing_target_layer_name: Optional[str]=None, use_irope: bool=False) -> None:
        ...
        if logits_soft_cap is not None:
            log_once('WARNING', 'XFormers does not support logits soft cap. Outputs may be slightly off.')
        if use_irope:
            log_once('WARNING', 'Using irope in XFormers is not supported yet, it will fall back to global attention for long context.')
