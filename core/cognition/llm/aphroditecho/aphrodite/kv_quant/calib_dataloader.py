...
def get_c4(tokenizer, nsamples, seed, seqlen, path=None):
    from datasets import load_dataset
    traindata = load_dataset(path if path else 'allenai/c4', 'allenai--c4', data_files={'train': 'en/c4-train.00000-of-01024.json.gz'}, split='train', use_auth_token=False)
    valdata = load_dataset(path if path else 'allenai/c4', 'allenai--c4', data_files={'validation': 'en/c4-validation.00000-of-00008.json.gz'}, split='validation', use_auth_token=False)
    import random
    random.seed(seed)
    trainloader = []
    for _ in range(nsamples):
...
def get_pileval(tokenizer, nsamples, seed, path, seqlen=512):
    ...
    try:
        ...
        raise InterruptedError('There have been some issues when generating the dataset, you could try to download it locally first, and replace the `data_files`with local addresses or use other datasets (c4, wiki, ptb).') from err
