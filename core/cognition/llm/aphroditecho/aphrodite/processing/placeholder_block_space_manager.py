...
class ProductionBlockSpaceManager(BlockSpaceManager):
    def __init__(self, block_size: int=16, num_gpu_blocks: int=1000, num_cpu_blocks: int=1000, watermark: float=0.01, enable_caching: bool=False, **kwargs) -> None:
    ...
    def can_allocate(self, seq_group: SequenceGroup, num_lookahead_slots: int=0) -> AllocStatus:
        ...
        num_required_blocks += (num_lookahead_slots + self.block_size - 1) // self.block_size
    ...
    def _allocate_sequence(self, seq: Sequence) -> BlockTable:
        block_table = BlockTable(block_size=self.block_size, block_allocator=self.block_allocator)
        if seq.get_token_ids():
            block_table.allocate(token_ids=seq.get_token_ids(), extra_hash=seq.extra_hash())
        return block_table
    def can_append_slots(self, seq_group: SequenceGroup, num_lookahead_slots: int) -> bool:
        ...
        for seq in seq_group.get_seqs(status=SequenceStatus.RUNNING):
            if seq.seq_id in self.block_tables:
                ...
                num_touched_blocks += block_table.get_num_blocks_touched_by_append_slots(token_ids=unseen_token_ids, num_lookahead_slots=num_lookahead_slots)
        num_free_gpu_blocks = self.block_allocator.get_num_free_blocks(Device.GPU)
    ...
    def append_slots(self, seq: Sequence, num_lookahead_slots: int) -> List[Tuple[int, int]]:
        ...
        block_table = self.block_tables[seq.seq_id]
        unseen_token_ids = block_table.get_unseen_token_ids(seq.get_token_ids())
        block_table.append_token_ids(token_ids=unseen_token_ids, num_lookahead_slots=num_lookahead_slots, num_computed_slots=seq.data.get_num_computed_tokens(), extra_hash=seq.extra_hash())
    ...
    def can_swap_in(self, seq_group: SequenceGroup, num_lookahead_slots: int) -> AllocStatus:
        ...
        for seq in seq_group.get_seqs(status=SequenceStatus.SWAPPED):
            if seq.seq_id in self.block_tables:
                block_table = self.block_tables[seq.seq_id]
                unseen_token_ids = block_table.get_unseen_token_ids(seq.get_token_ids())
                num_blocks_touched += block_table.get_num_blocks_touched_by_append_slots(token_ids=unseen_token_ids, num_lookahead_slots=num_lookahead_slots)
                blocks.extend(block_table.blocks)
        num_blocks_touched += self.block_allocator.get_num_full_blocks_touched(blocks, device=Device.GPU)
        if self.block_allocator.get_num_total_blocks(Device.GPU) < num_blocks_touched:
            return AllocStatus.NEVER
        elif self.block_allocator.get_num_free_blocks(Device.GPU) - num_blocks_touched >= self.watermark_blocks:
            return AllocStatus.OK
        else:
            return AllocStatus.LATER
    ...
    def can_swap_out(self, seq_group: SequenceGroup) -> bool:
        ...
        for seq in seq_group.get_seqs(status=SequenceStatus.RUNNING):
            if seq.seq_id in self.block_tables:
                block_table = self.block_tables[seq.seq_id]
                unseen_token_ids = block_table.get_unseen_token_ids(seq.get_token_ids())
                num_blocks_touched += block_table.get_num_blocks_touched_by_append_slots(token_ids=unseen_token_ids, num_lookahead_slots=0)
                blocks.extend(block_table.blocks)
        num_blocks_touched += self.block_allocator.get_num_full_blocks_touched(blocks, device=Device.CPU)
        return self.block_allocator.get_num_free_blocks(Device.CPU) >= num_blocks_touched
    ...
    def free(self, seq: Sequence) -> None:
        ...
        if seq_id not in self.block_tables:
            return
        if self.enable_caching:
            self._last_access_blocks_tracker.update_seq_blocks_last_access(seq_id, self.block_tables[seq_id].physical_block_ids)
    ...
    def access_all_blocks_in_seq(self, seq: Sequence, access_time: float) -> None:
        if self.enable_caching and seq.seq_id in self.block_tables:
            self._last_access_blocks_tracker.update_last_access(seq.seq_id, access_time)
