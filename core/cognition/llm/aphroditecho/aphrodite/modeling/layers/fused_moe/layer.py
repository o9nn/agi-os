...
from aphrodite.distributed import get_dp_group, get_ep_group, get_tensor_model_parallel_world_size, tensor_model_parallel_all_reduce
...
from aphrodite.modeling.layers.fused_moe.config import FusedMoEConfig, FusedMoEParallelConfig
from aphrodite.modeling.layers.fused_moe.modular_kernel import FusedMoEActivationFormat, FusedMoEModularKernel, FusedMoEPermuteExpertsUnpermute, FusedMoEPrepareAndFinalize
...
from aphrodite.utils import direct_register_custom_op, has_deep_ep, has_pplx, round_up
...
if is_rocm_aiter_moe_enabled():
    from aphrodite.modeling.layers.fused_moe.rocm_aiter_fused_moe import rocm_aiter_grouped_topk as grouped_topk
...
class UnquantizedFusedMoEMethod(FusedMoEMethodBase, CustomOp):
    ...
    def _maybe_pad_weight(self, weight: torch.Tensor) -> torch.Tensor:
        if envs.APHRODITE_ROCM_MOE_PADDING and current_platform.is_rocm() and (weight.stride(-1) == 1) and (weight.stride(-2) * weight.element_size() % 512 == 0):
    ...
    def process_weights_after_loading(self, layer: torch.nn.Module) -> None:
        ...
        if current_platform.is_xpu():
            ...
            layer.ipex_fusion = ipex.llm.modules.GatedMLPMOE(layer.w13_weight, layer.w2_weight, use_prepack=True)
        elif current_platform.is_cpu():
            if current_platform.get_cpu_architecture() == CpuArchEnum.X86:
                from aphrodite.modeling.layers.fused_moe import cpu_fused_moe
                dtype = layer.w13_weight.dtype
                if envs.APHRODITE_CPU_SGL_KERNEL and torch._C._cpu._is_amx_tile_supported() and (dtype == torch.bfloat16):
    ...
    def forward_cpu(self, layer: torch.nn.Module, x: torch.Tensor, use_grouped_topk: bool, top_k: int, router_logits: torch.Tensor, renormalize: bool, topk_group: Optional[int]=None, num_expert_group: Optional[int]=None, global_num_experts: int=-1, expert_map: Optional[torch.Tensor]=None, custom_routing_function: Optional[Callable]=None, scoring_func: str='softmax', e_score_correction_bias: Optional[torch.Tensor]=None, apply_router_weight_on_input: bool=False, activation: str='silu', enable_eplb: bool=False, expert_load_view: Optional[torch.Tensor]=None, logical_to_physical_map: Optional[torch.Tensor]=None, logical_replica_count: Optional[torch.Tensor]=None):
        if enable_eplb is not False or expert_load_view is not None or logical_to_physical_map is not None or (logical_replica_count is not None):
    ...
    def forward_xpu(self, layer: torch.nn.Module, x: torch.Tensor, use_grouped_topk: bool, top_k: int, router_logits: torch.Tensor, renormalize: bool, topk_group: Optional[int]=None, num_expert_group: Optional[int]=None, global_num_experts: int=-1, expert_map: Optional[torch.Tensor]=None, custom_routing_function: Optional[Callable]=None, scoring_func: str='softmax', e_score_correction_bias: Optional[torch.Tensor]=None, apply_router_weight_on_input: bool=False, activation: str='silu', enable_eplb: bool=False, expert_load_view: Optional[torch.Tensor]=None, logical_to_physical_map: Optional[torch.Tensor]=None, logical_replica_count: Optional[torch.Tensor]=None):
        ...
        return layer.ipex_fusion(x, use_grouped_topk, top_k, router_logits, renormalize, topk_group, num_expert_group)
...
class FusedMoE(torch.nn.Module):
    def __init__(self, num_experts: int, top_k: int, hidden_size: int, intermediate_size: int, params_dtype: Optional[torch.dtype]=None, reduce_results: bool=False, renormalize: bool=True, use_grouped_topk: bool=False, num_expert_group: Optional[int]=None, topk_group: Optional[int]=None, quant_config: Optional[QuantizationConfig]=None, tp_size: Optional[int]=None, ep_size: Optional[int]=None, dp_size: Optional[int]=None, prefix: str='', custom_routing_function: Optional[Callable]=None, scoring_func: str='softmax', e_score_correction_bias: Optional[torch.Tensor]=None, apply_router_weight_on_input: bool=False, activation: str='silu', enable_eplb: bool=False, num_redundant_experts: int=0):
        ...
        self.moe_parallel_config: FusedMoEParallelConfig = FusedMoEParallelConfig.make(tp_size_=tp_size_, dp_size_=dp_size_, aphrodite_parallel_config=aphrodite_config.parallel_config)
        self.global_num_experts = num_experts + num_redundant_experts
        if quant_config and quant_config.get_name() == 'mxfp4' and (envs.APHRODITE_USE_FLASHINFER_MOE_MXFP4_MXFP8 or envs.APHRODITE_USE_FLASHINFER_MOE_MXFP4_BF16):
        ...
        moe = FusedMoEConfig.make(num_experts=self.global_num_experts, experts_per_token=top_k, hidden_dim=hidden_size, num_local_experts=self.local_num_experts, moe_parallel_config=self.moe_parallel_config, in_dtype=model_dtype, max_num_tokens=envs.APHRODITE_MOE_DP_CHUNK_SIZE, quant_config=quant_config)
        self.moe_config = moe
    ...
    def _load_combined_w13_weight_scale(self, shard_dim: int, loaded_weight: torch.Tensor, param: torch.Tensor, tp_rank: int):
    ...
    def weight_loader(self, param: torch.nn.Parameter, loaded_weight: torch.Tensor, weight_name: str, shard_id: str, expert_id: int, return_success: bool=False) -> Optional[bool]:
        ...
        if 'ModelOpt' in quant_method_name:
            ...
            is_per_tensor = ('weight_scale_2' in weight_name if uses_weight_scale_2 else 'weight_scale' in weight_name) or 'input_scale' in weight_name
            if is_per_tensor:
            ...
            if 'w13_weight_scale' in weight_name:
                ...
                if loaded_weight_hidden_out == param_hidden_out:
                    self._load_combined_w13_weight_scale(shard_dim=shard_dim, loaded_weight=loaded_weight, param=param, tp_rank=self.tp_rank)
                    return True if return_success else None
    ...
    def forward(self, hidden_states: torch.Tensor, router_logits: torch.Tensor):
        ...
        if self.hidden_size != og_hidden_states:
            hidden_states = F.pad(hidden_states, (0, self.hidden_size - og_hidden_states), mode='constant', value=0.0)
        if current_platform.is_tpu():
            return self.forward_impl(hidden_states, router_logits)
        else:
            return torch.ops.aphrodite.moe_forward(hidden_states, router_logits, self.layer_name)[..., :og_hidden_states]
