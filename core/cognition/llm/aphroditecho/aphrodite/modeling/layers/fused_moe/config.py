...
class FusedMoEParallelConfig:
    ...
    def use_pplx_kernels(self):
        return self.use_all2all_kernels and envs.APHRODITE_ALL2ALL_BACKEND == 'pplx'
    @property
    def use_deepep_ht_kernels(self):
        return self.use_all2all_kernels and envs.APHRODITE_ALL2ALL_BACKEND == 'deepep_high_throughput'
    @property
    def use_deepep_ll_kernels(self):
        return self.use_all2all_kernels and envs.APHRODITE_ALL2ALL_BACKEND == 'deepep_low_latency'
    @property
    def use_flashinfer_cutlass_kernels(self):
        return envs.APHRODITE_USE_FLASHINFER_MOE_FP4 and has_flashinfer_cutlass_fused_moe()
    @staticmethod
...
class FusedMoEConfig:
    ...
    def __post_init__(self):
        if self.dp_size > 1:
            log_once('DEBUG', 'Using FusedMoEConfig::max_num_tokens={}', self.max_num_tokens)
    ...
    @staticmethod
    def make(num_experts: int, experts_per_token: int, hidden_dim: int, num_local_experts: int, moe_parallel_config: FusedMoEParallelConfig, in_dtype: torch.dtype, max_num_tokens: int=envs.APHRODITE_MOE_DP_CHUNK_SIZE, quant_config: Optional[Union[FusedMoEQuantConfig, QuantizationConfig]]=None) -> 'FusedMoEConfig':
        ...
        if quant_config is not None and isinstance(quant_config, QuantizationConfig):
            ...
            if quant_dtype is not None:
                ...
                if moe_parallel_config.dp_size > 1:
                    log_once('WARNING', 'MoE DP setup unable to determine quantization scheme or unsupported quantization type. This model will not run with DP enabled.')
        else:
