...
from aphrodite.modeling.pooling_metadata import PoolingMetadata as V0PoolingMetadata
...
class ResolvedPoolingConfig:
    ...
    task: PoolingTask
    @classmethod
    def from_config_with_defaults(cls, task: PoolingTask, pooler_config: PoolerConfig, pooling_type: PoolingType) -> 'ResolvedPoolingConfig':
        return cls(task=task, pooling_type=PoolingType[pooler_config.pooling_type] if pooler_config.pooling_type is not None else pooling_type)
@dataclass(frozen=True)
...
class Pooler(nn.Module, ABC):
    @staticmethod
    def for_encode(pooler_config: PoolerConfig, *, default_pooling_type: PoolingType=PoolingType.ALL):
        resolved_config = ResolvedPoolingConfig.from_config_with_defaults(task='encode', pooler_config=pooler_config, pooling_type=default_pooling_type)
        if resolved_config.pooling_type == PoolingType.STEP:
            return StepPooler()
        return SimplePooler.from_config(resolved_config)
    @staticmethod
    def for_embed(pooler_config: PoolerConfig, *, default_pooling_type: PoolingType=PoolingType.LAST):
        resolved_config = ResolvedPoolingConfig.from_config_with_defaults(task='embed', pooler_config=pooler_config, pooling_type=default_pooling_type)
        return SimplePooler.from_config(resolved_config)
    @staticmethod
    def for_classify(pooler_config: PoolerConfig, classifier: Optional[ClassifierFn], *, default_pooling_type: PoolingType=PoolingType.LAST):
        resolved_config = ResolvedPoolingConfig.from_config_with_defaults(task='classify', pooler_config=pooler_config, pooling_type=default_pooling_type)
        pooling = PoolingMethod.from_pooling_type(resolved_config.pooling_type)
        return ClassifierPooler(pooling=pooling, classifier=classifier)
    @abstractmethod
...
def get_prompt_lens(hidden_states: Union[torch.Tensor, list[torch.Tensor]], pooling_metadata: PoolingMetadata) -> torch.Tensor:
    ...
    return PoolingTensors.from_pooling_metadata(pooling_metadata, hidden_states[0].device).prompt_lens
...
def get_cross_encoder_activation_function(config: PretrainedConfig):
    ...
    return PoolerScore()
def build_output(all_data: Union[torch.Tensor, list[torch.Tensor]]) -> PoolerOutput:
...
class ClassifierPooler(Pooler):
    ...
    def __init__(self, pooling: PoolingFn, classifier: Optional[ClassifierFn], act_fn: Optional[PoolerActivation]=None) -> None:
        super().__init__()
        self.pooling = pooling
...
class DispatchPooler(Pooler):
    def __init__(self, poolers_by_task: Mapping[PoolingTask, Pooler]) -> None:
        ...
        for task, pooler in poolers_by_task.items():
            if task not in pooler.get_supported_tasks():
                raise ValueError(f'pooler={pooler!r} does not support task={task!r}. Supported tasks: {pooler.get_supported_tasks()}')
    ...
    def forward(self, hidden_states: Union[torch.Tensor, list[torch.Tensor]], pooling_metadata: PoolingMetadata) -> PoolerOutput:
        ...
        for task, group in groupby(get_tasks(pooling_metadata)):
            ...
            group_output: PoolerOutput = pooler(hidden_states_lst[offset:offset + num_items], pooling_metadata[offset:offset + num_items])
