...
class SamplingTensors:
    ...
    @classmethod
    def from_sampling_metadata(cls, sampling_metadata: 'SamplingMetadata', vocab_size: int, device: torch.device, dtype: torch.dtype) -> tuple['SamplingTensors', bool, bool, bool, bool, bool, bool, bool, bool, bool, bool, bool, bool, bool, bool, bool, bool]:
        ...
        for seq_group in sampling_metadata.seq_groups:
            ...
            if temperature < _SAMPLING_EPS:
                temperature = 1.0
            do_temperatures |= temperature != 1.0 or params.dynatemp_min > _SAMPLING_EPS or params.dynatemp_max > _SAMPLING_EPS
            ...
            do_penalties |= abs(params.presence_penalty) >= _SAMPLING_EPS or abs(params.frequency_penalty) >= _SAMPLING_EPS or params.repetition_penalty > 1.0
        ...
        sampling_tensors = SamplingTensors.from_lists(temperatures, dynatemp_mins, dynatemp_maxs, dynatemp_exps, temperature_lasts, top_ps, top_ks, top_as, min_ps, presence_penalties, frequency_penalties, repetition_penalties, no_repeat_ngram_sizes, tfss, eta_cutoffs, epsilon_cutoffs, typical_ps, smoothing_factors, smoothing_curves, xtc_thresholds, xtc_probabilities, nsigmas, dry_multipliers, dry_bases, dry_allowed_lengths, dry_sequence_breaker_ids, dry_ranges, dry_max_ngram, dry_max_occurrences, dry_early_exit_match_len, skews, prompt_tokens, output_tokens, vocab_size, device, dtype)
        return (sampling_tensors, do_penalties, do_no_repeat_ngrams, do_temperatures, do_top_p_top_k, do_top_as, do_min_p, do_tfss, do_eta_cutoffs, do_epsilon_cutoffs, do_typical_ps, do_quadratic, do_xtc, do_nsigmas, do_dry, do_skews, do_temp_last)
    @classmethod
    def from_lists(cls, temperatures: list[float], dynatemp_mins: list[float], dynatemp_maxs: list[float], dynatemp_exps: list[float], temperature_lasts: list[bool], top_ps: list[float], top_ks: list[int], top_as: list[float], min_ps: list[float], presence_penalties: list[float], frequency_penalties: list[float], repetition_penalties: list[float], no_repeat_ngram_sizes: list[int], tfss: list[float], eta_cutoffs: list[float], epsilon_cutoffs: list[float], typical_ps: list[float], smoothing_factors: list[float], smoothing_curves: list[float], xtc_thresholds: list[float], xtc_probabilities: list[float], nsigmas: list[float], dry_multipliers: list[float], dry_bases: list[float], dry_allowed_lengths: list[int], dry_sequence_breaker_ids: list[list[int]], dry_ranges: list[int], dry_max_ngram: list[int], dry_max_occurrences: list[int], dry_early_exit_match_len: list[int], skews: list[float], prompt_tokens: list[array], output_tokens: list[array], vocab_size: int, device: torch.device, dtype: torch.dtype) -> 'SamplingTensors':
        ...
        dynatemp_mins_t = torch.tensor(dynatemp_mins, device='cpu', dtype=dtype, pin_memory=pin_memory)
        dynatemp_maxs_t = torch.tensor(dynatemp_maxs, device='cpu', dtype=dtype, pin_memory=pin_memory)
        dynatemp_exps_t = torch.tensor(dynatemp_exps, device='cpu', dtype=dtype, pin_memory=pin_memory)
        temp_lasts_t = torch.tensor(temperature_lasts, device='cpu', dtype=torch.bool, pin_memory=pin_memory)
        ...
        no_repeat_ngram_sizes_t = torch.tensor(no_repeat_ngram_sizes, device='cpu', dtype=torch.int, pin_memory=pin_memory)
        ...
        dry_multipliers_t = torch.tensor(dry_multipliers, device='cpu', dtype=dtype, pin_memory=pin_memory)
        dry_bases_t = torch.tensor(dry_bases, device='cpu', dtype=dtype, pin_memory=pin_memory)
        dry_allowed_lengths_t = torch.tensor(dry_allowed_lengths, device='cpu', dtype=torch.int, pin_memory=pin_memory)
        dry_sequence_breakers_t = torch.tensor([seq + [0] * (max((len(s) for s in dry_sequence_breaker_ids)) - len(seq)) for seq in dry_sequence_breaker_ids], device='cpu', dtype=torch.long, pin_memory=pin_memory)
        dry_ranges_t = torch.tensor(dry_ranges, device='cpu', dtype=torch.int, pin_memory=pin_memory)
        dry_max_ngram_t = torch.tensor(dry_max_ngram, device='cpu', dtype=torch.int, pin_memory=pin_memory)
        dry_max_occurrences_t = torch.tensor(dry_max_occurrences, device='cpu', dtype=torch.int, pin_memory=pin_memory)
        dry_early_exit_match_len_t = torch.tensor(dry_early_exit_match_len, device='cpu', dtype=torch.int, pin_memory=pin_memory)
        skews_t = torch.tensor(skews, device='cpu', dtype=dtype, pin_memory=pin_memory)
        return cls(temperatures=temperatures_t.to(device=device, non_blocking=True), dynatemp_mins=dynatemp_mins_t.to(device=device, non_blocking=True), dynatemp_maxs=dynatemp_maxs_t.to(device=device, non_blocking=True), dynatemp_exps=dynatemp_exps_t.to(device=device, non_blocking=True), temperature_lasts=temp_lasts_t.to(device=device, non_blocking=True), top_ps=top_ps_t.to(device=device, non_blocking=True), top_ks=top_ks_t.to(device=device, non_blocking=True), top_as=top_as_t.to(device=device, non_blocking=True), min_ps=min_ps_t.to(device=device, non_blocking=True), presence_penalties=presence_penalties_t.to(device=device, non_blocking=True), frequency_penalties=frequency_penalties_t.to(device=device, non_blocking=True), repetition_penalties=repetition_penalties_t.to(device=device, non_blocking=True), no_repeat_ngram_sizes=no_repeat_ngram_sizes_t.to(device=device, non_blocking=True), tfss=tfss_t.to(device=device, non_blocking=True), eta_cutoffs=eta_cutoffs_t.to(device=device, non_blocking=True), epsilon_cutoffs=epsilon_cutoffs_t.to(device=device, non_blocking=True), smoothing_factors=smoothing_factors_t.to(device=device, non_blocking=True), smoothing_curves=smoothing_curves_t.to(device=device, non_blocking=True), xtc_thresholds=xtc_thresholds_t.to(device=device, non_blocking=True), xtc_probabilities=xtc_probabilities_t.to(device=device, non_blocking=True), nsigmas=nsigmas_t.to(device=device, non_blocking=True), dry_multipliers=dry_multipliers_t.to(device=device, non_blocking=True), dry_bases=dry_bases_t.to(device=device, non_blocking=True), dry_allowed_lengths=dry_allowed_lengths_t.to(device=device, non_blocking=True), dry_sequence_breaker_ids=dry_sequence_breakers_t.to(device=device, non_blocking=True), dry_ranges=dry_ranges_t.to(device=device, non_blocking=True), dry_max_ngram=dry_max_ngram_t.to(device=device, non_blocking=True), dry_max_occurrences=dry_max_occurrences_t.to(device=device, non_blocking=True), dry_early_exit_match_len=dry_early_exit_match_len_t.to(device=device, non_blocking=True), skews=skews_t.to(device=device, non_blocking=True), typical_ps=typical_ps_t.to(device=device, non_blocking=True), prompt_tokens=prompt_t.to(device=device, non_blocking=True), output_tokens=output_t.to(device=device, non_blocking=True))