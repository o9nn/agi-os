...
def register_model_loader(load_format: str):
    def _wrapper(model_loader_cls):
        ...
        logger.info('Registered model loader `{}` with load format `{}`', model_loader_cls, load_format)
...
def get_model(*, aphrodite_config: AphroditeConfig, model_config: Optional[ModelConfig]=None) -> nn.Module:
...
__all__ = ['get_model', 'get_model_loader', 'get_architecture_class_name', 'get_model_architecture', 'get_model_cls', 'register_model_loader', 'BaseModelLoader', 'BitsAndBytesModelLoader', 'GGUFModelLoader', 'DefaultModelLoader', 'DummyModelLoader', 'RunaiModelStreamerLoader', 'ShardedStateLoader', 'TensorizerLoader']