...
from aphrodite.common.config import AphroditeConfig, ModelConfig, ParallelConfig, set_current_aphrodite_config
...
class TensorizerConfig(MutableMapping):
    ...
    def __post_init__(self):
        ...
        if self.tensorizer_dir and self.tensorizer_uri:
            log_once('WARNING', 'Provided both tensorizer_dir and tensorizer_uri. Inferring tensorizer_dir from tensorizer_uri as the latter takes precedence.')
    ...
    def verify_with_model_config(self, model_config: 'ModelConfig') -> None:
        if model_config.quantization is not None and self.tensorizer_uri is not None:
            logger.warning('Loading a model using Tensorizer with quantization on Aphrodite is unstable and may lead to errors.')
...
def deserialize_tensorizer_model(model: nn.Module, tensorizer_config: TensorizerConfig) -> None:
    ...
    deserializer.close()
    logger.info('Deserialized {} in %0.2fs, {}/s', total_bytes_str, end - start, per_second)
...
def tensorizer_weights_iterator(tensorizer_args: 'TensorizerArgs') -> Generator[tuple[str, torch.Tensor], None, None]:
    logger.warning('Deserializing HuggingFace models is not optimized for loading on Aphrodite, as tensorizer is forced to load to CPU. Consider deserializing a Aphrodite model instead for faster load times. See the examples/others/tensorize_aphrodite_model.py example script for serializing Aphrodite models.')
...
def is_aphrodite_tensorized(tensorizer_config: 'TensorizerConfig') -> bool:
    ...
    if tensorizer_config.aphrodite_tensorized:
        logger.warning('Please note that newly serialized Aphrodite models are automatically inferred as Aphrodite models, so setting aphrodite_tensorized=True is only necessary for models serialized prior to this change.')
        return True
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for artifact in os.scandir(tmpdir):
            ...
            with open(artifact.path, 'rb') as f, open_stream(f'{tensorizer_args.tensorizer_dir}/{artifact.name}', mode='wb+', **tensorizer_args.stream_kwargs) as stream:
                ...
                stream.write(f.read())
def serialize_aphrodite_model(model: nn.Module, tensorizer_config: TensorizerConfig, model_config: 'ModelConfig') -> nn.Module:
    model.register_parameter('aphrodite_tensorized_marker', nn.Parameter(torch.tensor((1,), device='meta'), requires_grad=False))
    tensorizer_args = tensorizer_config._construct_tensorizer_args()
    ...
    return model
def tensorize_aphrodite_model(engine_args: 'EngineArgs', tensorizer_config: TensorizerConfig, generate_keyfile: bool=True):
...
def tensorize_lora_adapter(lora_path: str, tensorizer_config: TensorizerConfig):
    ...
    with open_stream(lora_uri, mode='wb+', **tensorizer_args.stream_kwargs) as f:
        ...
        serializer.close()
    logger.info('Successfully serialized LoRA files to {}', str(tensorizer_config.tensorizer_dir))