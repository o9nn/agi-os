...
def _get_gemma_act_fn(hidden_act: Optional[str], hidden_activation: Optional[str]) -> nn.Module:
    if hidden_activation is None:
        if hidden_act is not None:
            logger.warning("Gemma's activation function was incorrectly set to exact GeLU in the config JSON file when it was initially released. Changing the activation function to approximate GeLU (`gelu_pytorch_tanh`). If you want to use the legacy `{}`, edit the config JSON to set `hidden_activation={}` instead of `hidden_act`. See https://github.com/huggingface/transformers/pull/29402 for more details.", hidden_act, hidden_act)
