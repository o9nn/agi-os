...
from aphrodite.multimodal.processing import BaseMultiModalProcessor, BaseProcessingInfo, MultiModalFieldConfig, PromptReplacement, PromptUpdate, PromptUpdateDetails
...
class AyaVisionForConditionalGeneration(nn.Module, SupportsMultiModal, SupportsPP):
    ...
    def __init__(self, *, aphrodite_config: AphroditeConfig, prefix: str=''):
        ...
        self.language_model = init_aphrodite_registered_model(aphrodite_config=aphrodite_config, hf_config=config.text_config, prefix=maybe_prefix(prefix, 'model'), architectures=['Cohere2ForCausalLM'])
    @property
    def dtype(self):
    ...
    def _parse_and_validate_image_input(self, **kwargs: object) -> Optional[AyaVisionImagePixelInputs]:
        ...
        if pixel_values is None:
            return None
        return AyaVisionImagePixelInputs(type='pixel_values', pixel_values=flatten_bn(pixel_values, concat=True), num_patches=flatten_bn(num_patches, concat=True), resolve_bindings={'h': self.config.vision_config.image_size, 'w': self.config.vision_config.image_size})
