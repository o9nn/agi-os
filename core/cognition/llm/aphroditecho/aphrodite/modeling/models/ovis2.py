from typing import Iterable, List, Literal, Mapping, Optional, Set, Tuple, TypedDict, Union
import torch
...
class Ovis2ProcessingInfo(BaseProcessingInfo):
    ...
    def get_supported_mm_limits(self) -> Mapping[str, Optional[int]]:
        return {'image': self.get_hf_config().multimodal_max_length // ((9 + 1) * NUMBER_OF_TOKEN_TO_RESERVE_FOR_SEGMENT)}
    def get_image_size_with_most_features(self) -> ImageSize:
        image_processor = self.get_image_processor()
        return ImageSize(width=image_processor.size['shortest_edge'] * 9 * 2, height=image_processor.size['shortest_edge'] * 9 * 2)
...
class Ovis2MultiModalProcessor(BaseMultiModalProcessor[Ovis2ProcessingInfo]):
    ...
    def _call_hf_processor(self, prompt: str, mm_data: Mapping[str, object], mm_kwargs: Mapping[str, object]) -> BatchFeature:
        ...
        processed_outputs = super()._call_hf_processor(prompt=prompt, mm_data=mm_data, mm_kwargs=mm_kwargs)
...
@MULTIMODAL_REGISTRY.register_processor(Ovis2MultiModalProcessor, info=Ovis2ProcessingInfo, dummy_inputs=Ovis2DummyInputsBuilder)
class Ovis2ForConditionalGeneration(nn.Module, SupportsMultiModal):
    def __init__(self, *, aphrodite_config: AphroditeConfig, prefix: str=''):
        ...
        self.llm = init_aphrodite_registered_model(aphrodite_config=aphrodite_config.with_hf_config(config.get_text_config()), prefix=maybe_prefix(prefix, 'llm'))
        self.visual_tokenizer = Aimv2VisualTokenizer(config=config.visual_tokenizer_config, quant_config=quant_config, prefix=f'{prefix}.visual_tokenizer', image_processor_name_or_path=config.visual_tokenizer_config.backbone_config.name_or_path)
        self.vte = VisualEmbedding(self.config.visual_tokenizer_config.vocab_size, self.config.hidden_size)
    def _parse_and_validate_image_input(self, **kwargs: object) -> Optional[Ovis2ImagePatchInputs]:
        ...
        if pixel_values is not None and indicator_tokens is not None:
            ...
            return Ovis2ImagePatchInputs(type='image_patches', flat_data=flatten_bn(flatten_bn(pixel_values), concat=True), patches_per_image=[x.shape[0] for x in flatten_bn(pixel_values)], indicator_tokens=flatten_bn(flatten_bn(indicator_tokens), concat=True))
    ...
    def get_input_embeddings(self, input_ids: torch.Tensor, multimodal_embeddings: Optional[MultiModalEmbeddings]=None) -> torch.Tensor:
        ...
        if multimodal_embeddings is not None:
            inputs_embeds = merge_multimodal_embeddings(input_ids, inputs_embeds, multimodal_embeddings, [IMAGE_PAD_TOKEN_ID])
