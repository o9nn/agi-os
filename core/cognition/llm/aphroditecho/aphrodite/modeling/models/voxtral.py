...
from aphrodite.common.config import AphroditeConfig, ModelConfig, SpeechToTextConfig
...
from aphrodite.multimodal.processing import BaseMultiModalProcessor, BaseProcessingInfo, MultiModalHashes, PromptReplacement, PromptUpdate
...
from .utils import flatten_bn, init_aphrodite_registered_model, maybe_prefix, merge_multimodal_embeddings
ISO639_1_SUPPORTED_LANGS = {'ar': 'Arabic', 'nl': 'Dutch', 'en': 'English', 'fr': 'French', 'de': 'German', 'hi': 'Hindi', 'it': 'Italian', 'pt': 'Portuguese', 'es': 'Spanish'}
class VoxtralProcessorAdapter:
    ...
    def __call__(self, text: Optional[Union[TextInput, list[TextInput]]]=None, audios: Optional[Union[np.ndarray, list[np.ndarray]]]=None, return_tensors: Optional[Union[str, TensorType]]=None, **kwargs) -> Mapping[str, NestedTensors]:
        ...
        if any((len(t) > 0 for t in text)):
            raise ValueError("You've passed text inputs instead of token inputs. Make sure to process your input via `mistral_common`'s tokenizer or pass a chat completion request. For more info, see: https://github.com/aphrodite-project/aphrodite/issues/8411.")
...
class VoxtralForConditionalGeneration(nn.Module, SupportsMultiModal, SupportsPP, SupportsTranscription):
    ...
    def __init__(self, *, aphrodite_config: AphroditeConfig, prefix: str=''):
        ...
        self.whisper_encoder = VoxtralEncoderModel(aphrodite_config.with_hf_config(config.audio_config), prefix=maybe_prefix(prefix, 'whisper_encoder'))
    ...
    @classmethod
    def get_generation_prompt(cls, audio: np.ndarray, model_config: ModelConfig, stt_config: SpeechToTextConfig, language: Optional[str], task_type: str, request_prompt: str) -> PromptType:
        tokenizer = cached_tokenizer_from_config(model_config)
...
class VoxtralEncoderModel(nn.Module):
    ...
    def __init__(self, aphrodite_config: AphroditeConfig, *, prefix: str='') -> None:
        ...
        self.whisper_encoder = WhisperEncoder(aphrodite_config=aphrodite_config, prefix=maybe_prefix(prefix, 'whisper_encoder'), is_standalone_encoder=True, init_in_fp32=True)
