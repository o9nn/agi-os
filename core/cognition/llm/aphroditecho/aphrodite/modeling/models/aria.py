...
from aphrodite.common.config import AphroditeConfig, CacheConfig, QuantizationConfig
...
class AriaImagePixelInputs(TensorSchema):
    ...
    pixel_mask: Annotated[Optional[torch.Tensor], TensorShape('bn', 'h', 'w')]
...
class AriaForConditionalGeneration(nn.Module, SupportsMultiModal):
    hf_to_aphrodite_mapper = WeightsMapper(orig_to_new_prefix={'model.language_model.': 'language_model.model.', 'model.vision_tower.': 'vision_tower.', 'model.multi_modal_projector.': 'multi_modal_projector.', 'language_model.model': 'language_model', 'language_model.lm_head': 'lm_head'}, orig_to_new_suffix={'router.weight': 'router_weight'})
    @classmethod
    ...
    def __init__(self, aphrodite_config: AphroditeConfig, prefix: str=''):
        ...
        self.language_model = AriaTextModel(aphrodite_config=aphrodite_config.with_hf_config(config.text_config), prefix=maybe_prefix(prefix, 'language_model.model'))
    ...
    def _parse_and_validate_image_input(self, **kwargs: object) -> Optional[AriaImagePixelInputs]:
        ...
        if pixel_values is None:
            return None
        return AriaImagePixelInputs(pixel_values=flatten_bn(pixel_values, concat=True), pixel_mask=flatten_bn(pixel_mask, concat=True))
