...
from transformers import BatchFeature, Phi4MultimodalAudioConfig, Phi4MultimodalConfig, Phi4MultimodalFeatureExtractor, Phi4MultimodalImageProcessorFast
from transformers import Phi4MultimodalProcessor as Phi4MMProcessor
from transformers.models.phi4_multimodal.modeling_phi4_multimodal import Phi4MultimodalAudioConvModule, Phi4MultimodalAudioNemoConvSubsampling, Phi4MultimodalAudioRelativeAttentionBias, adaptive_enc_mask, unfold_tensor
...
from aphrodite.modeling.layers.linear import ColumnParallelLinear, MergedColumnParallelLinear, QKVParallelLinear, RowParallelLinear
...
from aphrodite.multimodal.parse import AudioProcessorItems, ImageEmbeddingItems, ImageProcessorItems, ImageSize, MultiModalDataItems, MultiModalDataParser
...
class Phi4MMImageEmbedding(nn.Module):
    def __init__(self, config: Phi4MultimodalConfig):
        ...
        num_hidden_layers = config.vision_config.num_hidden_layers + self.layer_idx + 1 if self.layer_idx < 0 else self.layer_idx + 1
        self.img_processor = Idefics2VisionTransformer(config.vision_config, require_post_norm=False, num_hidden_layers_override=num_hidden_layers)
    ...
    def get_img_features(self, img_embeds: torch.FloatTensor, attention_mask: Optional[torch.Tensor]=None) -> torch.FloatTensor:
    ...
    def forward(self, image_pixel_values: torch.FloatTensor, image_sizes: Optional[torch.Tensor]=None, image_attention_mask: Optional[torch.Tensor]=None) -> torch.FloatTensor:
        ...
        img_features = self.get_img_features(image_pixel_values.flatten(0, 1), attention_mask=image_attention_mask.flatten(0, 1).to(dtype=bool, device=target_device))
        ...
        for idx in range(batch_size):
            ...
            sub_img = sub_img.reshape(height_ratio, width_ratio, base_feat_size, base_feat_size, self.image_dim_out).transpose(1, 2).reshape(1, height_ratio * base_feat_size, width_ratio * base_feat_size, self.image_dim_out).contiguous()
            if image_attention_mask is not None:
                reshaped_image_attention_mask = image_attention_mask[idx, 1:area_ratio + 1, 0::2, 0::2].reshape(height_ratio, width_ratio, base_feat_size, base_feat_size).transpose(1, 2).reshape(1, height_ratio * base_feat_size, width_ratio * base_feat_size)
...
class Phi4MultimodalAudioMLP(nn.Module):
    def __init__(self, config: Phi4MultimodalAudioConfig, quant_config: Optional[QuantizationConfig]=None, prefix: str=''):
        super().__init__()
        self.layer_norm = nn.LayerNorm(config.hidden_size)
        self.act_fn = MulAndSilu()
        self.gate_up_proj = MergedColumnParallelLinear(config.hidden_size, [config.intermediate_size] * 2, bias=True, quant_config=quant_config, prefix=f'{prefix}.gate_up_proj')
        self.down_proj = RowParallelLinear(config.intermediate_size, config.hidden_size, bias=True, quant_config=quant_config, prefix=f'{prefix}.down_proj')
...
class Phi4MultimodalAudioAttention(nn.Module):
    def __init__(self, config: Phi4MultimodalAudioConfig, quant_config: Optional[QuantizationConfig]=None, prefix: str=''):
        ...
        self.qkv_proj = QKVParallelLinear(hidden_size=self.embed_dim, head_size=self.head_dim, total_num_heads=self.total_num_heads, quant_config=quant_config, prefix=f'{prefix}.qkv_proj')
        self.o_proj = RowParallelLinear(input_size=self.embed_dim, output_size=self.embed_dim, quant_config=quant_config, prefix=f'{prefix}.out_proj')
    ...
    def forward(self, hidden_states: torch.Tensor, attention_mask: torch.Tensor) -> torch.Tensor:
        ...
        out = F.scaled_dot_product_attention(query, key, value, scale=self.scale, attn_mask=attention_mask)
...
class Phi4MultimodalAudioModel(nn.Module):
    def __init__(self, config: Phi4MultimodalAudioConfig):
        ...
        self.encoders = nn.ModuleList([Phi4MultimodalAudioConformerEncoderLayer(config) for _ in range(config.num_blocks)])
    def _streaming_mask(self, seq_len: int, batch_size: int, chunk_size: int, left_chunk: int):
        chunk_start_idx = np.arange(0, seq_len, chunk_size)
        enc_streaming_mask = adaptive_enc_mask(seq_len, chunk_start_idx, left_window=left_chunk).unsqueeze(0).expand([batch_size, -1, -1])
        return enc_streaming_mask
    def forward_embeddings(self, hidden_states: torch.Tensor, masks: torch.Tensor):
        seq_len = math.ceil(hidden_states.shape[1] / self.config.time_reduction)
        if seq_len <= 0:
            raise ValueError(f'Sequence length after time reduction is invalid: {seq_len}.Your input feature is too short.')
    ...
    def calculate_hs_mask(self, hidden_states: torch.Tensor, device: torch.device, mask: torch.Tensor):
        ...
        batch_size = hidden_states.shape[0]
        enc_streaming_mask = self._streaming_mask(max_audio_length, batch_size, self.config.chunk_size, self.config.left_chunk)
    ...
    def forward(self, hidden_states: torch.Tensor, mask: Optional[torch.Tensor]=None):
        ...
        if seq_len > max_seq_len:
            ...
            hs_mask = self.calculate_hs_mask(hidden_states, hidden_states.device, masks_unfold)
...
class Phi4MMAudioEmbedding(nn.Module):
    ...
    def get_projection(self, audio_projection_mode: Literal['speech', 'vision']) -> Phi4MMProjector:
    ...
    def forward(self, audio_input_features: torch.FloatTensor, audio_embed_sizes=None, audio_attention_mask=None, audio_projection_mode='speech') -> torch.FloatTensor:
...
class Phi4MMProcessingInfo(BaseProcessingInfo):
    ...
    def get_image_processor(self, processor: Optional[Phi4MMProcessor]=None) -> Phi4MultimodalImageProcessorFast:
        if processor is None:
            processor = self.get_hf_processor()
        return processor.image_processor
    def get_dynamic_hd(self, processor: Optional[Phi4MMProcessor]=None) -> int:
    ...
    def _find_target_aspect_ratio(self, orig_width: int, orig_height: int, image_size: int, max_num: int, min_num: int):
        ...
        if w_crop_num * h_crop_num > max_num:
            ...
            target_aspect_ratio = image_processor.find_closest_aspect_ratio(aspect_ratio, target_ratios, orig_width, orig_height)
    ...
    def get_num_image_tokens(self, *, image_width: int, image_height: int, processor: Optional[Phi4MMProcessor]=None) -> int:
        ...
        image_num_tokens = self._compute_num_image_tokens(image_width, image_height, dynamic_hd_size=dynamic_hd_size, vit_image_size=vit_image_size, vit_patch_size=vit_patch_size)
        return image_num_tokens
    def get_image_size_with_most_features(self, processor: Optional[Phi4MMProcessor]=None) -> ImageSize:
...
class Phi4MMMultiModalProcessor(BaseMultiModalProcessor[Phi4MMProcessingInfo]):
    ...
    def _call_hf_processor(self, prompt: str, mm_data: Mapping[str, object], mm_kwargs: Mapping[str, object], tok_kwargs: Mapping[str, object]) -> BatchFeature:
        ...
        if audio_data:
            ...
            processed_outputs['audio_input_features'] = [audio_features[idx, :size] for idx, size in enumerate(feature_sizes)]
    ...
    def _get_mm_fields_config(self, hf_inputs: BatchFeature, hf_processor_mm_kwargs: Mapping[str, object]) -> Mapping[str, MultiModalFieldConfig]:
        return dict(image_pixel_values=MultiModalFieldConfig.batched('image'), image_attention_mask=MultiModalFieldConfig.batched('image'), image_sizes=MultiModalFieldConfig.batched('image'), num_img_tokens=MultiModalFieldConfig.batched('image'), audio_input_features=MultiModalFieldConfig.batched('audio'))
    def _get_prompt_updates(self, mm_items: MultiModalDataItems, hf_processor_mm_kwargs: Mapping[str, Any], out_mm_kwargs: MultiModalKwargs) -> Sequence[PromptUpdate]:
        ...
        def get_audio_replacement_phi4mm(item_idx: int):
            ...
            return audio_tokens
        return [PromptReplacement(modality='audio', target=[audio_token_id], replacement=get_audio_replacement_phi4mm), PromptReplacement(modality='image', target=[image_token_id], replacement=get_image_replacement_phi4mm)]
...
class Phi4MultimodalForCausalLM(nn.Module, SupportsLoRA, SupportsMultiModal):
    ...
    hf_to_aphrodite_mapper = WeightsMapper(orig_to_new_prefix={'model.embed_tokens_extend.': '', 'model.': 'language_model.model.'}, orig_to_new_substr={'.img_projection_': '.img_projection.', '.up_proj_for_speech.': '.speech_projection.up.', '.up_proj_for_vision_speech.': '.vision_speech_projection.up.', '.down_proj_for_speech.': '.speech_projection.down.', '.down_proj_for_vision_speech.': '.vision_speech_projection.down.'})
    @classmethod
    ...
    def __init__(self, *, aphrodite_config: AphroditeConfig, prefix: str=''):
        ...
        self.language_model = init_aphrodite_registered_model(aphrodite_config=aphrodite_config, prefix=maybe_prefix(prefix, 'language_model'), architectures=['Phi3ForCausalLM'])
    ...
    def _process_audio_input(self, audio_input: Phi4MMAudioInputs, audio_projection_mode: str) -> NestedTensors:
        ...
        audio_embeds = [self.audio_embed(features.unsqueeze(0).to(dtype), audio_projection_mode=audio_projection_mode) for features in audio_features]
        return audio_embeds
    def _parse_and_validate_image_input(self, **kwargs: object) -> Optional[Phi4MMImagePixelInputs]:
        ...
        if is_list_of(image_pixel_values, torch.Tensor):
            assert all((p.dim() == 5 for p in image_pixel_values)), 'Incorrect image inputs'
        ...
        if isinstance(num_img_tokens, list):
            ...
            raise ValueError('Incorrect image_attention_mask inputs')
        return Phi4MMImagePixelInputs(type='pixel_values', data=image_pixel_values, image_sizes=image_sizes, image_attention_mask=image_attention_mask, num_img_tokens=num_img_tokens)
    ...
    def get_mm_mapping(self) -> MultiModelKeys:
        return MultiModelKeys.from_string_field(language_model='language_model.', connector=['img_projection', 'vision_speech_projection', 'speech_projection'], tower_model=['image_embed', 'audio_embed'])
