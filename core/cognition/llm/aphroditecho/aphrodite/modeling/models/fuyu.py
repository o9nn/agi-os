...
class FuyuImagePatchInputs(TensorSchema):
    ...
    flat_data: Annotated[torch.Tensor, TensorShape('bnp', 'fn')]
...
class FuyuForCausalLM(nn.Module, SupportsMultiModal, SupportsPP):
    hf_to_aphrodite_mapper = WeightsMapper(orig_to_new_prefix={'model.vision_embed_tokens.': 'vision_embed_tokens.', 'model.language_model.': 'language_model.model.', 'lm_head.': 'language_model.lm_head.'})
    @classmethod
    ...
    def __init__(self, *, aphrodite_config: AphroditeConfig, prefix: str=''):
        ...
        self.language_model = PersimmonForCausalLM(aphrodite_config=aphrodite_config.with_hf_config(config.text_config), prefix=maybe_prefix(prefix, 'language_model'))
    ...
    def _parse_and_validate_image_input(self, **kwargs: object) -> Optional[FuyuImagePatchInputs]:
        ...
        if image_patches is not None:
            ...
            return FuyuImagePatchInputs(type='image_patches', flat_data=flat_data, patches_per_image=[x.size(0) for x in image_patches_flat], resolve_bindings={'fn': self.image_feature_size})
        return None
