...
from aphrodite.multimodal.processing import BaseMultiModalProcessor, BaseProcessingInfo, MultiModalDataItems, PromptReplacement, PromptUpdate, PromptUpdateDetails
...
class Idefics3Model(nn.Module):
    def __init__(self, *, aphrodite_config: AphroditeConfig, prefix: str=''):
        ...
        self.text_model = LlamaModel(aphrodite_config=aphrodite_config.with_hf_config(config.text_config), prefix=maybe_prefix(prefix, 'text_model'))
...
class Idefics3ForConditionalGeneration(nn.Module, SupportsMultiModal, SupportsLoRA):
    ...
    def _parse_and_validate_image_input(self, **kwargs: object) -> Optional[ImageInputs]:
        ...
        if pixel_values is not None:
            ...
            return Idefics3ImagePixelInputs(type='pixel_values', pixel_values=flatten_bn(pixel_values, concat=True), pixel_attention_mask=flatten_bn(pixel_attention_mask, concat=True), num_patches=flatten_bn(num_patches, concat=True), resolve_bindings={'h': expected_h, 'w': expected_w})
