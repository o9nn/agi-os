...
class LlamaModel(nn.Module):
    def __init__(self, *, aphrodite_config: AphroditeConfig, prefix: str='', start_layer_id: int=0, quant_config: Optional[QuantizationConfig]=None) -> None:
        super().__init__()
    ...
    def forward(self, input_ids: Optional[torch.Tensor], positions: torch.Tensor, hidden_states: torch.Tensor, inputs_embeds: Optional[torch.Tensor]=None) -> tuple[torch.Tensor, torch.Tensor]:
...
class EagleLlama4ForCausalLM(Llama4ForCausalLM):
    def __init__(self, *, aphrodite_config: AphroditeConfig, prefix: str=''):
        ...
        quant_config = AphroditeConfig.get_quantization_config(aphrodite_config.speculative_config.draft_model_config, aphrodite_config.load_config)
        self.model = LlamaModel(aphrodite_config=aphrodite_config, prefix='model', start_layer_id=target_layer_num, quant_config=quant_config)
        logit_scale = getattr(self.config, 'logit_scale', 1.0)
        self.logits_processor = LogitsProcessor(self.config.vocab_size, scale=logit_scale)
    def forward(self, input_ids: torch.Tensor, positions: torch.Tensor, hidden_states: torch.Tensor, inputs_embeds: Optional[torch.Tensor]=None) -> tuple[torch.Tensor, torch.Tensor]:
