...
class Glm4MoeMultiTokenPredictor(nn.Module):
    def __init__(self, *, aphrodite_config: AphroditeConfig, prefix: str=''):
        ...
        self.layers = torch.nn.ModuleDict({str(idx): Glm4MoeMultiTokenPredictorLayer(config, f'{prefix}.layers.{idx}', cache_config=aphrodite_config.cache_config, quant_config=aphrodite_config.quant_config) for idx in range(self.mtp_start_layer_idx, self.mtp_start_layer_idx + self.num_mtp_layers)})
