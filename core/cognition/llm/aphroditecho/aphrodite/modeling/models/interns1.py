...
class InternS1MultiModalProjector(nn.Module):
    def __init__(self, config):
        ...
        self.linear_1 = nn.Linear(config.vision_config.hidden_size * int(1 / config.downsample_ratio) ** 2, config.text_config.hidden_size)
...
def resolve_interns1_min_max_num(min_dynamic_patch: int, max_dynamic_patch: int, dynamic_image_size: bool, use_thumbnail: bool) -> tuple[int, int]:
...
class InternS1ProcessingInfo(BaseProcessingInfo):
    ...
    def get_num_image_tokens(self, *, image_width: int, image_height: int, processor: Optional['GotOcr2ImageProcessorFast']=None) -> int:
        ...
        num_image_patches = processor.get_number_of_image_tokens(image_height, image_width, images_kwargs=dict())
    ...
    def resolve_target_ratios(self, use_thumbnail: Optional[bool]=None):
        ...
        min_num, max_num = resolve_interns1_min_max_num(min_dynamic_patch, max_dynamic_patch, dynamic_image_size, use_thumbnail=use_thumbnail)
    ...
    def get_image_size_with_most_features(self) -> ImageSize:
        ...
        for wr, hr in target_ratios:
            ...
            feat_size = self.get_num_image_tokens(image_width=width, image_height=height, processor=processor.image_processor)
            if feat_size > largest_feature_size:
                largest_feature_size = feat_size
                largest_feature_pinpoint = ImageSize(width=width, height=height)
        assert not (largest_feature_size == 0 or largest_feature_pinpoint is None), 'Cannot have a largest feature size of 0!'
        return largest_feature_pinpoint
    def get_max_image_tokens(self) -> int:
        processor = self.get_hf_processor()
        target_width, target_height = self.get_image_size_with_most_features()
        return self.get_num_image_tokens(image_width=target_width, image_height=target_height, processor=processor.image_processor)
...
class InternS1DummyInputsBuilder(BaseDummyInputsBuilder[InternS1ProcessingInfo]):
    ...
    def get_dummy_mm_data(self, seq_len: int, mm_counts: Mapping[str, int]) -> MultiModalDataDict:
        ...
        return {'image': self._get_dummy_images(width=target_width, height=target_height, num_images=num_images), 'video': self._get_dummy_videos(width=image_size_w, height=image_size_h, num_frames=target_num_frames, num_videos=num_videos)}
class InternS1MultiModalProcessor(BaseMultiModalProcessor[InternS1ProcessingInfo]):
    def _call_hf_processor(self, prompt: str, mm_data: Mapping[str, object], mm_kwargs: Mapping[str, object], tok_kwargs: Mapping[str, object]) -> Mapping[str, NestedTensors]:
        ...
        if images:
            image_pixel_values = []
            for image in images:
                processed_outputs = super()._call_hf_processor(prompt=hf_processor.image_token, mm_data={'images': image}, mm_kwargs=mm_kwargs, tok_kwargs=tok_kwargs)
            ...
            image_outputs: dict[str, NestedTensors] = {'pixel_values': torch.concat(image_pixel_values), 'image_num_patches': torch.tensor(num_patches), 'image_token_id': torch.tensor(hf_processor.image_token_id)}
        video_outputs = {}
        if videos:
            video_pixel_values = []
            for video in videos:
                processed_outputs = super()._call_hf_processor(prompt=hf_processor.video_token, mm_data={'videos': video}, mm_kwargs=mm_kwargs, tok_kwargs=tok_kwargs)
            ...
            video_outputs: dict[str, NestedTensors] = {'pixel_values_videos': torch.concat(video_pixel_values), 'video_num_patches': torch.tensor(num_frames), 'video_token_id': torch.tensor(video_token_id)}
    ...
    def _get_mm_fields_config(self, hf_inputs: Mapping[str, NestedTensors], hf_processor_mm_kwargs: Mapping[str, object]) -> Mapping[str, MultiModalFieldConfig]:
        ...
        return dict(pixel_values=MultiModalFieldConfig.flat_from_sizes('image', image_num_patches), image_num_patches=MultiModalFieldConfig.batched('image'), image_embeds=MultiModalFieldConfig.batched('image'), image_token_id=MultiModalFieldConfig.shared('image', num_images), pixel_values_videos=MultiModalFieldConfig.flat_from_sizes('video', video_num_patches), video_num_patches=MultiModalFieldConfig.batched('video'), video_token_id=MultiModalFieldConfig.shared('video', num_videos))
    def _get_prompt_updates(self, mm_items: MultiModalDataItems, hf_processor_mm_kwargs: Mapping[str, object], out_mm_kwargs: MultiModalKwargs) -> Sequence[PromptUpdate]:
        ...
        return [PromptReplacement(modality='image', target=img_context_token, replacement=get_replacement_interns1_image), PromptReplacement(modality='video', target=video_token, replacement=get_replacement_interns1_video)]
@MULTIMODAL_REGISTRY.register_processor(InternS1MultiModalProcessor, info=InternS1ProcessingInfo, dummy_inputs=InternS1DummyInputsBuilder)
class InternS1ForConditionalGeneration(nn.Module, SupportsMultiModal, SupportsPP, SupportsLoRA):
    hf_to_aphrodite_mapper = WeightsMapper(orig_to_new_prefix={'lm_head.': 'language_model.lm_head.', 'model.language_model.': 'language_model.model.', 'model.vision_tower.': 'vision_tower.', 'model.multi_modal_projector.': 'multi_modal_projector.'})
    @classmethod
    ...
    def __init__(self, *, aphrodite_config: AphroditeConfig, prefix: str='') -> None:
        ...
        self.vision_tower = self._init_vision_model(config, quant_config=quant_config, prefix=maybe_prefix(prefix, 'vision_tower'))
    ...
    def _init_vision_model(self, config: PretrainedConfig, quant_config: Optional[QuantizationConfig], *, prefix: str):
        ...
        return InternS1VisionModel(config.vision_config, quant_config=quant_config, num_hidden_layers_override=num_hidden_layers, prefix=prefix)
    ...
    def _validate_pixel_values(self, data: torch.Tensor) -> torch.Tensor:
        ...
        for d in data:
            _validate_shape(d)
        return data
    def _parse_and_validate_image_input(self, **kwargs: object) -> Optional[InternS1ImageInputs]:
        ...
        if image_embeds is not None:
            ...
            return InternS1ImageEmbeddingInputs(type='image_embeds', data=flatten_bn(image_embeds))
        ...
        if pixel_values is not None:
            ...
            return InternS1ImagePixelInputs(type='pixel_values', pixel_values=self._validate_pixel_values(pixel_values), num_patches=image_num_patches)
        raise AssertionError('This line should be unreachable.')
    def _parse_and_validate_video_input(self, **kwargs: object) -> Optional[InternS1VideoPixelInputs]:
        ...
        if video_embeds is not None:
            ...
            return InternS1ImageEmbeddingInputs(type='video_embeds', data=flatten_bn(video_embeds))
        ...
        if pixel_values_flat_video is not None:
            ...
            return InternS1VideoPixelInputs(type='pixel_values_videos', pixel_values=self._validate_pixel_values(pixel_values_flat_video), num_patches=video_num_patches)
        raise AssertionError('This line should be unreachable.')
    def _process_image_input(self, image_input: Union[InternS1ImageInputs, InternS1VideoPixelInputs]) -> tuple[torch.Tensor, ...]:
