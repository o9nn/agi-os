...
from aphrodite.multimodal.processing import BaseMultiModalProcessor, BaseProcessingInfo, PromptReplacement, PromptUpdate, PromptUpdateDetails
...
from .utils import AutoWeightsLoader, WeightsMapper, flatten_bn, init_aphrodite_registered_model, maybe_prefix, merge_multimodal_embeddings
...
class Step3VisionProcessor:
    def __init__(self, size, interpolation_mode='bicubic', patch_size=None):
        ...
        self.transform = transforms.Compose([transforms.ToTensor(), transforms.Normalize(mean, std), transforms.Resize((size, size), interpolation=InterpolationMode.BICUBIC if interpolation_mode == 'bicubic' else InterpolationMode.BILINEAR, antialias=True)])
        self.patch_transform = transforms.Compose([transforms.ToTensor(), transforms.Normalize(mean, std), transforms.Resize((patch_size, patch_size), interpolation=InterpolationMode.BICUBIC if interpolation_mode == 'bicubic' else InterpolationMode.BILINEAR, antialias=True)]) if patch_size is not None else None
...
class ImagePatcher:
    ...
    def slide_window(self, width: int, height: int, sizes: list[tuple[int, int]], steps: list[tuple[int, int]], img_rate_thr: float=0.6) -> tuple[list[tuple[int, int, int, int]], tuple[int, int]]:
    ...
    def get_num_patches(self, img_width: int, img_height: int) -> tuple[int, int]:
        ...
        if window_size == 0:
            ...
            center_list, (x_num, y_num) = self.slide_window(img_width, img_height, [(window_size, window_size)], [(window_size, window_size)])
    ...
    def __call__(self, img: Image.Image) -> tuple[Image.Image, list[Image.Image], list[bool] | None]:
        ...
        if window_size == 0:
            ...
            patches = []
            newlines = []
            center_list, (x_num, y_num) = self.slide_window(new_img_width, new_img_height, [(window_size, window_size)], [(window_size, window_size)])
...
class Step3VLProcessor:
    def __init__(self, config: PretrainedConfig, tokenizer: AnyTokenizer) -> None:
        super().__init__()
        self.config = config
    ...
    def _split_images(self, images: list[Image.Image]) -> list[ImageWithPatches]:
        ...
        return result
    def _convert_images_to_pixel_values(self, images: list[Image.Image], is_patch: bool=False) -> list[torch.Tensor]:
        return [self.image_preprocessor(img, is_patch=is_patch)['pixel_values'] for img in images]
    def _get_patch_repl(self, num_patches: int, patch_newline_mask: list[bool] | None) -> tuple[str, list[int]]:
        text = ''
        token_ids = []
        for i in range(num_patches):
            assert len(patch_newline_mask) == num_patches
            text += f'<patch_start>{self.patch_feature_placeholder}<patch_end>'
            token_ids.extend([self.tokenizer.convert_tokens_to_ids('<patch_start>')] + [self.image_token_id] * self.num_patch_feature_size + [self.tokenizer.convert_tokens_to_ids('<patch_end>')])
            if patch_newline_mask and patch_newline_mask[i]:
                text += '<patch_newline>'
                token_ids.append(self.tokenizer.convert_tokens_to_ids('<patch_newline>'))
        return (text, token_ids)
    def _get_image_repl(self, num_images: int) -> tuple[str, list[int]]:
        text = f'<im_start>{self.image_feature_placeholder}<im_end>'
        token_ids = [self.tokenizer.convert_tokens_to_ids('<im_start>')] + [self.image_token_id] * self.num_image_feature_size + [self.tokenizer.convert_tokens_to_ids('<im_end>')]
        return (text * num_images, token_ids * num_images)
    def _get_image_repl_features(self, num_images: int, num_patches: int, patch_new_line_idx: Optional[list[bool]]) -> tuple[str, list[int]]:
        if num_patches > 0:
    ...
    def replace_placeholder(self, text: str, placeholder: str, repls: list[str]) -> str:
        ...
        return ''.join(result)
    def __call__(self, text: Optional[Union[str, list[str]]]=None, images: Optional[Union[Image.Image, list[Image.Image]]]=None, return_tensors: Optional[Union[str, TensorType]]=None) -> BatchFeature:
        if text is None:
            text = []
        ...
        if len(images) == 0:
            ...
            image_inputs = {'pixel_values': torch.cat(pixel_values_lst), 'num_patches': num_patches}
...
class Step3VLProcessingInfo(BaseProcessingInfo):
    def get_hf_processor(self) -> Step3VLProcessor:
        return Step3VLProcessor(self.get_hf_config(), self.get_tokenizer())
    def get_supported_mm_limits(self) -> Mapping[str, Optional[int]]:
        return {'image': None}
    def get_max_image_tokens(self) -> int:
        hf_processor = self.get_hf_processor()
        return hf_processor.get_num_image_tokens(self.get_image_size_with_most_features().width, self.get_image_size_with_most_features().height)
...
class Step3VLMultiModalProcessor(BaseMultiModalProcessor[Step3VLProcessingInfo]):
    def _get_prompt_updates(self, mm_items: MultiModalDataItems, hf_processor_mm_kwargs: Mapping[str, Any], out_mm_kwargs: MultiModalKwargs) -> Sequence[PromptUpdate]:
        ...
        def get_replacement_step1o(item_idx: int):
            ...
            if num_patches > 0:
                ...
                image_repl_ids = hf_processor._get_image_repl_features(1, num_patches, patch_newline_mask)[1]
            else:
                image_repl_ids = hf_processor._get_image_repl_features(1, 0, None)[1]
            return PromptUpdateDetails.select_token_id(seq=image_repl_ids, embed_token_id=image_placeholder_token_id)
        return [PromptReplacement(modality='image', target=[image_placeholder_token_id], replacement=get_replacement_step1o)]
    def _get_mm_fields_config(self, hf_inputs: BatchFeature, hf_processor_mm_kwargs: Mapping[str, object]) -> Mapping[str, MultiModalFieldConfig]:
        ...
        return dict(pixel_values=MultiModalFieldConfig.batched('image'), patch_pixel_values=MultiModalFieldConfig.flat_from_sizes('image', num_patches), num_patches=MultiModalFieldConfig.batched('image'), patch_newline_mask=MultiModalFieldConfig.flat_from_sizes('image', num_patches))
def get_abs_pos(abs_pos, tgt_size):
    ...
    if src_size != tgt_size:
        ...
        new_pos_embed = F.interpolate(old_pos_embed, size=(tgt_size, tgt_size), mode='bicubic', antialias=True, align_corners=False).to(dtype)
...
class Step3VisionEmbeddings(nn.Module):
    def __init__(self, config: Step3VisionEncoderConfig):
        ...
        self.patch_embedding = nn.Conv2d(in_channels=config.num_channels, out_channels=self.embed_dim, kernel_size=self.patch_size, stride=self.patch_size, bias=True)
    ...
    def forward(self, pixel_values: torch.Tensor) -> torch.Tensor:
        ...
        embeddings = torch.cat([embeddings[:, 0, :].unsqueeze(1).repeat(1, self.pad_tp_size - 1, 1), embeddings], dim=1)
        return embeddings
class Step3VisionAttention(nn.Module):
    def __init__(self, config, quant_config: Optional[QuantizationConfig]=None, prefix: str=''):
        ...
        self.qkv_proj = QKVParallelLinear(self.embed_dim, self.head_dim, self.total_num_heads, bias=True, quant_config=quant_config, prefix=prefix)
        self.out_proj = RowParallelLinear(self.embed_dim, self.embed_dim, bias=True, quant_config=quant_config, prefix=prefix)
    ...
    def forward(self, hidden_states: torch.Tensor):
        ...
        k = k.transpose(1, 2)
        v = v.transpose(1, 2)
        attn_output = F.scaled_dot_product_attention(q, k, v, scale=self.scale, is_causal=False)
        attn_output = attn_output.transpose(1, 2).reshape(bsz, tgt_len, self.num_heads * self.head_dim)
...
class Step3VisionMLP(nn.Module):
    def __init__(self, config, quant_config: Optional[QuantizationConfig]=None, prefix: str=''):
        ...
        self.fc1 = ColumnParallelLinear(config.hidden_size, config.intermediate_size, bias=True, quant_config=quant_config, prefix=prefix)
        self.fc2 = RowParallelLinear(config.intermediate_size, config.hidden_size, bias=True, quant_config=quant_config, prefix=prefix)
...
class Step3VisionEncoderLayer(nn.Module):
    def __init__(self, config: Step3VisionEncoderConfig, quant_config: Optional[QuantizationConfig]=None, prefix: str=''):
        super().__init__()
        self.embed_dim = config.hidden_size
        self.self_attn = Step3VisionAttention(config, quant_config, prefix=f'{prefix}.self_attn')
...
class Step3VisionEncoder(nn.Module):
    def __init__(self, config: Step3VisionEncoderConfig, quant_config: Optional[QuantizationConfig]=None, prefix: str=''):
        super().__init__()
        self.config = config
        self.layers = nn.ModuleList([Step3VisionEncoderLayer(config, quant_config, prefix=f'{prefix}.layers.{i}') for i in range(config.num_hidden_layers)])
...
class Step3VisionTransformer(nn.Module):
    def __init__(self, config: Step3VisionEncoderConfig, quant_config: Optional[QuantizationConfig]=None, prefix: str=''):
        ...
        self.transformer = Step3VisionEncoder(config, quant_config, prefix=f'{prefix}.transformer')
    def forward(self, pixel_values: torch.Tensor):
        hidden_states = self.embeddings(pixel_values)
        hidden_states = self.transformer(inputs_embeds=hidden_states)
        return hidden_states
@MULTIMODAL_REGISTRY.register_processor(Step3VLMultiModalProcessor, info=Step3VLProcessingInfo, dummy_inputs=Step3VLDummyInputsBuilder)
class Step3VLForConditionalGeneration(nn.Module, SupportsMultiModal, SupportsPP):
    hf_to_aphrodite_mapper = WeightsMapper(orig_to_new_prefix={'model.': 'language_model.model.', 'lm_head.': 'language_model.lm_head.'})
    @classmethod
    ...
    def __init__(self, *, aphrodite_config: AphroditeConfig, prefix: str='') -> None:
        ...
        self.vision_model = Step3VisionTransformer(config.vision_config, None, prefix=maybe_prefix(prefix, 'vision_model'))
        self.vit_downsampler = nn.Conv2d(config.vision_config.hidden_size, config.vision_config.output_hidden_size, kernel_size=2, stride=config.understand_projector_stride)
        self.vit_downsampler2 = nn.Conv2d(config.vision_config.output_hidden_size, config.vision_config.output_hidden_size * 2, kernel_size=3, stride=2, padding=1)
        self.vit_large_projector = nn.Linear(config.vision_config.output_hidden_size * 2, config.hidden_size, bias=config.projector_bias)
        self.language_model = init_aphrodite_registered_model(aphrodite_config=aphrodite_config, hf_config=config.text_config, prefix=maybe_prefix(prefix, 'language_model'))
    ...
    def _parse_and_validate_image_input(self, **kwargs: object) -> Optional[Step3VLImageInputs]:
        ...
        if pixel_values is not None:
            ...
            return Step3VLImagePixelInputs(type='pixel_values', pixel_values=pixel_values.to(self.dtype).to(self.device), patch_pixel_values=patch_pixel_values.to(self.dtype).to(self.device) if patch_pixel_values is not None else None, num_patches=num_patches)
        if image_embeds is not None:
            ...
            return Step3VLImageEmbeddingInputs(type='image_embeds', image_embeds=image_embeds.to(self.dtype).to(self.device))
        return None
    ...
    def _process_image_input(self, image_input: Step3VLImageInputs) -> tuple[torch.Tensor, ...]:
        if image_input['type'] == 'image_embeds':
            ...
            patch_image_features = self._get_vision_model_output(image_input['patch_pixel_values']) if image_input['patch_pixel_values'] is not None else None
            num_patches = image_input['num_patches']
        image_features = self._process_image_features(image_features)
        patch_image_features = self._process_image_features(patch_image_features) if patch_image_features is not None else None
