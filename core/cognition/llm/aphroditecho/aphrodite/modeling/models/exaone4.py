...
class Exaone4Attention(nn.Module):
    def __init__(self, config: PretrainedConfig, hidden_size: int, num_heads: int, num_kv_heads: int, rope_theta: float=1000000, rope_scaling: Optional[dict[str, Any]]=None, max_position_embeddings: int=8192, quant_config: Optional[QuantizationConfig]=None, bias: bool=False, cache_config: Optional[CacheConfig]=None, prefix: str='') -> None:
        super().__init__()
...
class Exaone4ForCausalLM(nn.Module, SupportsLoRA, SupportsPP):
    ...
    def __init__(self, *, aphrodite_config: AphroditeConfig, prefix: str=''):
        ...
        self.quant_config = quant_config
        self.model = Exaone4Model(aphrodite_config=aphrodite_config, prefix=maybe_prefix(prefix, 'model'))
        if get_pp_group().is_last_rank:
