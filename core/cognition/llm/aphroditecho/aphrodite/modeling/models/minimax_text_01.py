...
from aphrodite.distributed.parallel_state import get_pp_group, get_tensor_model_parallel_rank, get_tensor_model_parallel_world_size
...
class MiniMaxText01ForCausalLM(nn.Module, HasInnerState, IsHybrid, SupportsV0Only):
    def __init__(self, *, aphrodite_config: AphroditeConfig, prefix: str='') -> None:
        ...
        self.model = MiniMaxText01Model(self.config, quant_config, cache_config=aphrodite_config.cache_config, scheduler_config=aphrodite_config.scheduler_config, prefix=maybe_prefix(prefix, 'model'))
        if get_pp_group().is_last_rank:
