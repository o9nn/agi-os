...
from aphrodite.modeling.layers.pooler import AllPool, PoolerHead, PoolerIdentity, SimplePooler
from aphrodite.modeling.model_loader.weight_utils import default_weight_loader
from aphrodite.modeling.models.interfaces import IsAttentionFree, MultiModalEmbeddings, SupportsMultiModalWithRawInput
from aphrodite.modeling.models.utils import AutoWeightsLoader
from aphrodite.multimodal import MULTIMODAL_REGISTRY
from aphrodite.multimodal.inputs import MultiModalDataDict, MultiModalFieldConfig, MultiModalFieldElem, MultiModalInputs, MultiModalKwargs, MultiModalKwargsItem, MultiModalSharedField, PlaceholderRange
...
class PrithviGeoSpatialMAEInputBuilder(BaseDummyInputsBuilder[PrithviGeoSpatialMAEProcessingInfo]):
    ...
    def get_dummy_mm_data(self, seq_len: int, mm_counts: Mapping[str, int]) -> MultiModalDataDict:
        return {'pixel_values': torch.full((6, 512, 512), 1.0, dtype=torch.float16), 'location_coords': torch.full((1, 2), 1.0, dtype=torch.float16)}
class PrithviGeoSpatialMAEMultiModalProcessor(BaseMultiModalProcessor):
    def _get_mm_fields_config(self, hf_inputs: BatchFeature, hf_processor_mm_kwargs: Mapping[str, object]) -> Mapping[str, MultiModalFieldConfig]:
        return dict(pixel_values=MultiModalFieldConfig.shared(batch_size=1, modality='image'), location_coords=MultiModalFieldConfig.shared(batch_size=1, modality='image'))
    ...
    def apply(self, prompt: Union[str, list[int]], mm_data: MultiModalDataDict, hf_processor_mm_kwargs: Mapping[str, object], tokenization_kwargs: Optional[Mapping[str, object]]=None, return_mm_hashes: bool=False) -> MultiModalInputs:
        ...
        multimodal_kwargs_items = [MultiModalKwargsItem.from_elems([MultiModalFieldElem(modality='image', key=key, data=data, field=MultiModalSharedField(1)) for key, data in mm_kwargs.items()])]
        return MultiModalInputs(type='multimodal', prompt=prompt, prompt_token_ids=[1], mm_kwargs=MultiModalKwargs.from_items(multimodal_kwargs_items), mm_hashes=None, mm_placeholders=mm_placeholders)
