...
def as_embedding_model(cls: _T) -> _T:
    ...
    class ModelForEmbedding(_create_pooling_model_cls(cls)):
        def _init_pooler(self, aphrodite_config: 'AphroditeConfig', prefix: str=''):
            ...
            self.pooler = DispatchPooler({'encode': Pooler.for_encode(pooler_config), 'embed': Pooler.for_embed(pooler_config)})
...
def as_seq_cls_model(cls: _T) -> _T:
    ...
    from aphrodite.modeling.layers.pooler import ClassifierPooler, DispatchPooler, Pooler, PoolingMethod, PoolingType
    ...
    class ModelForSequenceClassification(_create_pooling_model_cls(cls), SupportsCrossEncoding):
        def _init_pooler(self, aphrodite_config: 'AphroditeConfig', prefix: str=''):
            ...
            pooling_type = PoolingType.LAST if pooling_type_str is None else PoolingType[pooling_type_str]
            self.pooler = DispatchPooler({'encode': Pooler.for_encode(pooler_config), 'classify': ClassifierPooler(pooling=PoolingMethod.from_pooling_type(pooling_type), classifier=self._classifier, act_fn=ClassifierPooler.act_fn_for_seq_cls(aphrodite_config.model_config)), 'score': ClassifierPooler(pooling=PoolingMethod.from_pooling_type(pooling_type), classifier=self._classifier, act_fn=ClassifierPooler.act_fn_for_cross_encoder(aphrodite_config.model_config))})
