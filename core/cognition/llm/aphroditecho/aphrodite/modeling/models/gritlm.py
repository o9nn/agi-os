...
from aphrodite.modeling.layers.pooler import DispatchPooler, Pooler, PoolerHead, PoolerNormalize, PoolingParamsUpdate, build_output, get_prompt_lens, get_prompt_token_ids
...
class GritLM(LlamaForCausalLM, SupportsV0Only):
    ...
    def __init__(self, aphrodite_config: AphroditeConfig, prefix: str='', **kwargs) -> None:
        ...
        if pooler_config is not None:
            self.pooler = DispatchPooler({'encode': Pooler.for_encode(pooler_config), 'embed': GritLMPooler(aphrodite_config.model_config)})