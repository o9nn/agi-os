...
from aphrodite.multimodal.processing import BaseMultiModalProcessor, BaseProcessingInfo, ProcessingCache, PromptReplacement, PromptUpdate
...
class HCXVisionProcessingInfo(BaseProcessingInfo):
    ...
    def get_num_image_tokens(self, *, vision_query_length: Union[int, list[int]]) -> int:
        if isinstance(vision_query_length, int):
            return vision_query_length
        else:
            return sum(vision_query_length)
    def get_num_video_tokens(self, *, vision_query_length: Union[int, list[int]]) -> int:
        if isinstance(vision_query_length, int):
            return vision_query_length
        else:
            return sum(vision_query_length)
...
class HCXVisionDummyInputsBuilder(BaseDummyInputsBuilder[HCXVisionProcessingInfo]):
    ...
    def get_dummy_mm_data(self, seq_len: int, mm_counts: Mapping[str, int]) -> MultiModalDataDict:
        ...
        return {'image': self._get_dummy_images(width=target_width, height=target_height, num_images=num_images), 'video': self._get_dummy_videos(width=target_width - 1, height=target_height - 1, num_frames=target_num_frames, num_videos=num_videos)}
class HCXVisionMultiModalProcessor(BaseMultiModalProcessor[HCXVisionProcessingInfo]):
    def _call_hf_processor(self, prompt: str, mm_data: Mapping[str, object], mm_kwargs: Mapping[str, object], tok_kwargs: Mapping[str, object]) -> BatchFeature:
        def replace_multimodal_token(token_ids: torch.Tensor, target_token: int, repeats: list):
            output = list()
            _repeats_idx = 0
        ...
        processed_outputs = self.info.ctx.call_hf_processor(hf_processor=self.info.get_hf_processor(**mm_kwargs), data=dict(text=prompt, images=None, videos=None))
        if len(mm_data) > 0:
            ...
            if videos is not None:
                ...
                videos = [videos]
            _processed_outputs = self.info.ctx.call_hf_processor(hf_processor=self.info.get_hf_processor(**mm_kwargs), data=dict(text=None, images=images, videos=videos))
            ...
            if num_images > 0:
                ...
                processed_outputs['input_ids'] = torch.stack([replace_multimodal_token(token_ids=_input_ids, target_token=tokenizer.convert_tokens_to_ids(IMAGE_TOKEN), repeats=_processed_outputs['vision_query_lengths_images']) for _input_ids in processed_outputs['input_ids']], dim=0)
            if num_videos > 0:
                tokenizer = self.info.get_tokenizer()
                processed_outputs['input_ids'] = torch.stack([replace_multimodal_token(token_ids=_input_ids, target_token=tokenizer.convert_tokens_to_ids(VIDEO_TOKEN), repeats=_processed_outputs['vision_query_lengths_videos']) for _input_ids in processed_outputs['input_ids']], dim=0)
                _ratios = [len(_pixel_values) for _pixel_values in _processed_outputs['pixel_values_videos']]
                _num_per_videos = [int(_e / sum(_ratios) * len(_processed_outputs['vision_query_lengths_videos'])) for _e in _ratios]
                _processed_outputs['vision_query_lengths_videos'] = [_processed_outputs['vision_query_lengths_videos'][sum(_num_per_videos[:_i]):sum(_num_per_videos[:_i + 1])] for _i in range(0, num_videos)]
    ...
    def _get_prompt_updates(self, mm_items: MultiModalDataItems, hf_processor_mm_kwargs: Mapping[str, object], out_mm_kwargs: MultiModalKwargs) -> Sequence[PromptUpdate]:
        ...
        placeholder = {'image': hf_config.image_token_id, 'video': hf_config.video_token_id}
        def get_replacement_hyperclovax(item_idx: int, modality: str, out_mm_kwargs: MultiModalKwargs):
            num_tokens = None
            if modality == 'image':
                num_tokens = self.info.get_num_image_tokens(vision_query_length=out_mm_kwargs['vision_query_lengths_images'][item_idx])
            if modality == 'video':
                num_tokens = self.info.get_num_video_tokens(vision_query_length=out_mm_kwargs['vision_query_lengths_videos'][item_idx])
            assert isinstance(num_tokens, int)
            return [placeholder[modality]] * num_tokens
        return [PromptReplacement(modality=modality, target=[placeholder[modality]], replacement=partial(get_replacement_hyperclovax, modality=modality, out_mm_kwargs=out_mm_kwargs)) for modality in ('image', 'video')]
    def _get_mm_fields_config(self, hf_inputs: BatchFeature, hf_processor_mm_kwargs: Mapping[str, object]) -> Mapping[str, MultiModalFieldConfig]:
        return dict(pixel_values_images=MultiModalFieldConfig.batched('image'), image_sizes_images=MultiModalFieldConfig.batched('image'), vision_query_lengths_images=MultiModalFieldConfig.batched('image'), num_queries_vis_abstractors_images=MultiModalFieldConfig.batched('image'), num_queries_vis_abstractors_slow_images=MultiModalFieldConfig.batched('image'), first_last_frames_slows_images=MultiModalFieldConfig.batched('image'), pixel_values_videos=MultiModalFieldConfig.batched('video'), image_sizes_videos=MultiModalFieldConfig.batched('video'), vision_query_lengths_videos=MultiModalFieldConfig.batched('video'), num_queries_vis_abstractors_videos=MultiModalFieldConfig.batched('video'), num_queries_vis_abstractors_slow_videos=MultiModalFieldConfig.batched('video'), first_last_frames_slows_videos=MultiModalFieldConfig.batched('video'))
def _build_hcxvision_hf_info(ctx: InputProcessingContext) -> HCXVisionProcessingInfo:
    return HCXVisionProcessingInfo(ctx)
def _build_hcxvision_hf_processor(info: HCXVisionProcessingInfo, dummy_inputs: BaseDummyInputsBuilder[HCXVisionProcessingInfo], *, cache: Optional[ProcessingCache]=None) -> BaseMultiModalProcessor:
    if isinstance(info, HCXVisionProcessingInfo):
        return HCXVisionMultiModalProcessor(info, dummy_inputs, cache=cache)
    raise NotImplementedError(type(info))
def init_vision_tower_for_hcxvision(vision_config, quant_config: Optional[QuantizationConfig], *, use_nth_layer: Optional[int]=None, require_post_norm: Optional[bool]=None, prefix: str='') -> Union[CLIPVisionModel, SiglipVisionModel]:
...
class HCXVisionMlp(nn.Module):
    def __init__(self, mm_projector_type, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU):
        super().__init__()
...
class HCXVisionCAbstractor(nn.Module):
    def __init__(self, num_queries: int, num_input_tokens: int, encoder_hidden_size: int, hidden_size: int, output_hidden_size: int, pos_emb: bool=True, prenorm: bool=False):
        super().__init__()
    ...
    def forward(self, x: torch.Tensor, num_queries_vis_abstractors: Optional[list[list[int]]]=None, num_grids: Optional[list[int]]=None) -> torch.Tensor:
        ...
        return x
    def _forward(self, x: torch.Tensor, num_queries_vis_abstractors: Optional[list[list[int]]]=None, num_grids: Optional[list[int]]=None) -> torch.Tensor:
        B, L, dim = x.shape
        hw = int(L ** 0.5)
        ...
        x = self.readout(x)
        return x
    def _forward_adaptive_num_query(self, x: torch.Tensor, num_queries_vis_abstractors: Optional[list[list[int]]]=None, num_grids: Optional[list[int]]=None) -> list[torch.Tensor]:
        ...
        for i, num_queries in enumerate(num_queries_vis_abstractors):
            ...
            new_x.append(out)
        return new_x
    def build_net(self, n_queries: int, encoder_hidden_size: int, hidden_size: int, output_hidden_size: int, depth: int=3, mlp_depth: int=2):
...
@MULTIMODAL_REGISTRY.register_processor(_build_hcxvision_hf_processor, info=_build_hcxvision_hf_info, dummy_inputs=HCXVisionDummyInputsBuilder)
class HCXVisionForCausalLM(nn.Module, SupportsMultiModal, SupportsPP):
    ...
    def __init__(self, *, aphrodite_config: AphroditeConfig, prefix: str='', **kwargs: Optional[Any]) -> None:
        super().__init__()
        ...
        with no_init_weights():
            self.vision_model = init_vision_tower_for_hcxvision(vision_config, quant_config, use_nth_layer=getattr(config, 'use_nth_layer', -1), require_post_norm=False, prefix=maybe_prefix(prefix, 'vision_model'))
        ...
        self.language_model = init_aphrodite_registered_model(aphrodite_config=aphrodite_config, hf_config=text_config, prefix=maybe_prefix(prefix, 'language_model'))
        if config.anyres:
    ...
    def get_multimodal_embeddings(self, **kwargs: Unpack[HCXVisionMultimodalInputs]) -> Optional[MultiModalEmbeddings]:
        ...
        if kwargs.get('pixel_values_images') is not None:
            for _pixel_values_images, _image_sizes_images in zip(kwargs['pixel_values_images'], kwargs['image_sizes_images']):
                ...
                _multimodal_embeddings_images = self.forward_images(pixel_values_images=_pixel_values_images, image_sizes_images=_image_sizes_images, len_pixel_values_images=_len_pixel_values_images)
        ...
        if kwargs.get('pixel_values_videos') is not None:
            for _pixel_values_videos, _vision_query_lengths_videos in zip(kwargs['pixel_values_videos'], kwargs['vision_query_lengths_videos']):
                _len_pixel_values_videos = [len(_vision_query_lengths) for _vision_query_lengths in _vision_query_lengths_videos]
                _c, _w, _h = _pixel_values_videos.shape[-3:]
                _pixel_values_videos = _pixel_values_videos.reshape(sum(_len_pixel_values_videos), -1, _c, _w, _h).unsqueeze(dim=0)
                _multimodal_embeddings_videos = self.forward_videos(pixel_values_videos=_pixel_values_videos, len_pixel_values_videos=_len_pixel_values_videos)
    ...
    def get_input_embeddings(self, input_ids: torch.Tensor, multimodal_embeddings: Optional[MultiModalEmbeddings]=None, **kwargs) -> torch.Tensor:
        inputs_embeds = self.language_model.get_input_embeddings(input_ids)
        if kwargs.get('pixel_values_images') is not None or kwargs.get('pixel_values_videos') is not None:
    ...
    def forward(self, input_ids: torch.Tensor, positions: torch.Tensor, intermediate_tensors: Optional[IntermediateTensors]=None, inputs_embeds: Optional[torch.Tensor]=None, **kwargs: object) -> Union[torch.Tensor, IntermediateTensors]:
        if intermediate_tensors is not None:
            ...
            inputs_embeds = self.get_input_embeddings(input_ids=input_ids, **kwargs)
            input_ids = None
        hidden_states = self.language_model.model(input_ids, positions, intermediate_tensors, inputs_embeds=inputs_embeds)
        return hidden_states
    def forward_images(self, pixel_values_images: list[list[torch.FloatTensor]], image_sizes_images: list[list[tuple[int, int]]], len_pixel_values_images: list[int]) -> list[list[torch.Tensor]]:
    ...
    def forward_videos(self, pixel_values_videos: list[list[torch.FloatTensor]], len_pixel_values_videos: list[int]) -> list[torch.Tensor]:
        ...
        target_group_size = 0
        group_counter = 0
        video_groups = [len(frame) for frames in pixel_values_videos for frame in frames]
    ...
    def _prepare_multimodal_kwargs(self, **kwargs: object):
        ...
        for k, v in kwargs.items():
            ...
            for _sample_idx, _v in enumerate(v):
    ...
    def _init_possible_resolutions(self, config, vision_config):
        if not getattr(config, 'possible_resolutions', []):
            ...
            if config.anyres:
                ...
                possible_resolutions = [[ys * vision_config.image_size, xs * vision_config.image_size] for ys, xs in possible_resolutions]
    ...
    def _init_mm_projector(self, config, text_config, vision_config):
        ...
        if config.mm_projector_type == 'linear':
            ...
            mm_projector = HCXVisionCAbstractor(num_queries=config.num_queries_vis_abstractor_image, num_input_tokens=(vision_config.image_size // vision_config.patch_size) ** 2, encoder_hidden_size=input_hidden_size, hidden_size=input_hidden_size, output_hidden_size=text_config.hidden_size, pos_emb=config.proj_pos_emb, prenorm=config.proj_prenorm)
        else:
            mm_projector = HCXVisionMlp(config.mm_projector_type, input_hidden_size, hidden_features=input_hidden_size, out_features=self.text_config.hidden_size)
        return mm_projector
...
def select_best_resolution(original_size: tuple, possible_resolutions: list) -> tuple:
    ...
    return best_fit
def get_anyres_image_grid_shape(image_size: tuple[int, int], grid_pinpoints: Union[str, list[tuple[int, int]]], patch_size: int) -> tuple[int, int]:
...
def reshape_and_unpad_image_features(image_feature: torch.Tensor, height: int, width: int, image_size: tuple[int, int], possible_resolutions: list[tuple[int, int]], grid_size: int, unpad: bool, image_newline: torch.Tensor) -> torch.Tensor:
    base_image_feature = image_feature[0]
    image_feature = image_feature[1:]
    assert height * width == base_image_feature.shape[0], f'height: {height}, width: {width},         base_image_feature.shape[0]: {base_image_feature.shape[0]}'
...
def anyres_postprocessing(image_forward_outs: list[torch.FloatTensor], image_sizes: list[list[int]], possible_resolutions: list[tuple[int, int]], patch_size: int, grid_size: int, image_newline: torch.FloatTensor, num_queries_vis_abstractor: int=-1, unpad: bool=False) -> list[torch.FloatTensor]:
    ...
    for image_idx, image_feature in enumerate(image_forward_outs):
        if image_feature.shape[0] > 1:
            image_feature = reshape_and_unpad_image_features(image_feature=image_feature, height=height, width=width, image_size=image_sizes[image_idx], possible_resolutions=possible_resolutions, grid_size=grid_size, unpad=unpad, image_newline=image_newline)
        else:
    ...
    return image_features
def resize_image(image: Union[np.ndarray, PIL.Image.Image], max_side: int=378) -> np.ndarray:
    image_arr = image
