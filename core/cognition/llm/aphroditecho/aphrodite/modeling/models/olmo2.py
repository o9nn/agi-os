...
from aphrodite.modeling.models.utils import AutoWeightsLoader, is_pp_missing_parameter, make_empty_intermediate_tensors_factory, make_layers, maybe_prefix
...
class Olmo2Attention(nn.Module):
    def __init__(self, *, aphrodite_config: AphroditeConfig, prefix: str=''):
        ...
        self.qkv_proj = QKVParallelLinear(hidden_size, self.head_dim, self.total_num_heads, self.total_num_kv_heads, bias=False, quant_config=aphrodite_config.quant_config, prefix=f'{prefix}.qkv_proj')
        ...
        self.attn = Attention(self.num_heads, self.head_dim, self.scaling, num_kv_heads=self.num_kv_heads, cache_config=aphrodite_config.cache_config, quant_config=aphrodite_config.quant_config, prefix=prefix)
        self.o_proj = RowParallelLinear(self.total_num_heads * self.head_dim, hidden_size, bias=False, quant_config=aphrodite_config.quant_config, prefix=f'{prefix}.o_proj')
...
class Olmo2MLP(nn.Module):
    def __init__(self, *, aphrodite_config: AphroditeConfig, prefix: str=''):
        ...
        self.gate_up_proj = MergedColumnParallelLinear(hidden_size, [intermediate_size] * 2, bias=False, quant_config=aphrodite_config.quant_config, prefix=f'{prefix}.gate_up_proj')
        self.act_fn = SiluAndMul()
        self.down_proj = RowParallelLinear(intermediate_size, hidden_size, bias=False, quant_config=aphrodite_config.quant_config, prefix=f'{prefix}.down_proj')
...
class Olmo2Model(nn.Module):
    def __init__(self, *, aphrodite_config: AphroditeConfig, prefix: str=''):
        ...
        self.start_layer, self.end_layer, self.layers = make_layers(self.config.num_hidden_layers, lambda prefix: Olmo2DecoderLayer(aphrodite_config=aphrodite_config, prefix=prefix), prefix=f'{prefix}.layers')
...
class Olmo2ForCausalLM(nn.Module, SupportsPP):
    def __init__(self, *, aphrodite_config: AphroditeConfig, prefix: str=''):
        ...
        if config.tie_word_embeddings:
            ...
            self.lm_head = ParallelLMHead(config.vocab_size, config.hidden_size, org_num_embeddings=config.vocab_size, quant_config=aphrodite_config.quant_config, prefix=maybe_prefix(prefix, 'lm_head'))
