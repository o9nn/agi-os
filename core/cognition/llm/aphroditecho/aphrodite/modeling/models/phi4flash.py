...
from aphrodite.modeling.models.interfaces import HasInnerState, IsHybrid, SupportsV0Only
...
class SambaYAttention(nn.Module):
    def __init__(self, config, layer_idx: Optional[int]=None, yoco_cross: bool=False, cache_config: Optional[CacheConfig]=None, prefix: str=''):
        ...
        if layer_idx is None:
            log_once('WARNING', f'Instantiating {self.__class__.__name__} without passing a `layer_idx` is not recommended and will lead to errors during the forward call if caching is used. Please make sure to provide a `layer_idx` when creating this class.')
...
class Phi4Mamba(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor, attn_metadata: AttentionMetadata, mamba_cache_params: MambaCacheParams, yoco_key_values=None) -> torch.Tensor:
        ...
        if attn_metadata.query_start_loc is not None and attn_metadata.context_lens_tensor is not None:
            ...
            selective_state_update(mamba_cache_params.ssm_state, hidden_states.transpose(0, 1), discrete_time_step.transpose(0, 1), self.A, B, C, self.D, None if self.yoco_kv else gate.transpose(0, 1), time_proj_bias, dt_softplus=True, state_batch_indices=mamba_cache_params.state_indices_tensor, out=scan_outputs)
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]):
        ...
        for name, param in self.named_parameters():
            ...
            if weight is not None and weight.shape != param.shape:
                logger.warning('Shape mismatch: {} {} {}', name, weight.shape, param.shape)
