...
class CustomOp(nn.Module):
    def __new__(cls, *args, **kwargs):
        ...
        if op_name not in cls.op_registry_oot:
            ...
            logger.debug('Instantiating custom op: {} using {}', op_name, str(op_cls_to_instantiate))
    ...
    def enabled(cls) -> bool:
        ...
        if not hasattr(cls, 'name'):
            log_once('WARNING', "Custom op {} was not registered, which means it won't appear in the op registry. It will be enabled/disabled based on the global settings.", cls.__name__)
