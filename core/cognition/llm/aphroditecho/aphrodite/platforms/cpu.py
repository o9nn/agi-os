...
class CpuPlatform(Platform):
    ...
    def get_device_total_memory(cls, device_id: int=0) -> int:
        ...
        if kv_cache_space is None:
            kv_cache_space = 4 * GiB_bytes
            log_once('WARNING', 'Environment variable APHRODITE_CPU_KVCACHE_SPACE (GiB) for CPU backend is not set, using 4 by default.')
        else:
    ...
    def check_and_update_config(cls, aphrodite_config: AphroditeConfig) -> None:
        ...
        if parallel_config.world_size > 1 and parallel_config.distributed_executor_backend is not None and (parallel_config.distributed_executor_backend != 'mp'):
            logger.warning('{} is not supported on CPU, fallback to mp distributed executor backend.', parallel_config.distributed_executor_backend)
        ...
        if aphrodite_config.compilation_config.level == CompilationLevel.PIECEWISE:
            ...
            compilation_config.inductor_compile_config.update({'dce': True, 'size_asserts': False, 'nan_asserts': False, 'epilogue_fusion': True})
    ...
    def get_device_communicator_cls(cls) -> str:
        return 'aphrodite.distributed.device_communicators.cpu_communicator.CpuCommunicator'
    @classmethod
