...
def aphrodite_version_matches_substr(substr: str) -> bool:
    ...
    try:
        ...
        logger.warning('The Aphrodite package was not found, so its version could not be inspected. This may cause platform detection to fail.')
        raise e
...
def cuda_platform_plugin() -> Optional[str]:
    ...
    try:
        ...
        logger.debug('Exception happens when checking CUDA platform: {}', str(e))
...
def xpu_platform_plugin() -> Optional[str]:
    ...
    try:
        ...
        if hasattr(torch, 'xpu') and torch.xpu.is_available():
            ...
            logger.debug('Confirmed {} backend is available.', XPUPlatform.dist_backend)
...
def resolve_current_platform_cls_qualname() -> str:
    ...
    if len(activated_oot_plugins) >= 2:
        ...
        logger.info('Platform plugin {} is activated', activated_oot_plugins[0])
        ...
        logger.info('Automatically detected platform {}.', activated_builtin_plugins[0])
    else:
        platform_cls_qualname = 'aphrodite.platforms.interface.UnspecifiedPlatform'
        logger.info('No platform detected, Aphrodite is running on UnspecifiedPlatform')
