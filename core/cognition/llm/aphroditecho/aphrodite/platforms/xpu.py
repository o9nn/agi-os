...
class XPUPlatform(Platform):
    ...
    def check_and_update_config(cls, aphrodite_config: AphroditeConfig) -> None:
        ...
        if envs.APHRODITE_USE_V1 and model_config is not None and (not aphrodite_config.model_config.enforce_eager):
            from aphrodite.config import CompilationLevel
            aphrodite_config.compilation_config.level = CompilationLevel.NO_COMPILATION
        ...
        if parallel_config.distributed_executor_backend is None:
            ...
            logger.warning('{} is not supported on XPU, fallback to ray distributed executor backend.', parallel_config.distributed_executor_backend)
            parallel_config.distributed_executor_backend = 'ray'
        if model_config and model_config.use_mla:
            ...
            aphrodite_config.scheduler_config.max_num_batched_tokens = max(aphrodite_config.scheduler_config.max_model_len, DEFAULT_MAX_NUM_BATCHED_TOKENS)
    @classmethod
    ...
    def device_support_bf16(cls) -> bool:
        ...
        if cls.is_client_gpu_a770():
            ...
            logger.info('Device name {} supports bfloat16. Please file an issue if you encounter any accuracy problems with bfloat16.', device_name)
            return True
    @classmethod
    ...
    def get_device_communicator_cls(cls) -> str:
        return 'aphrodite.distributed.device_communicators.xpu_communicator.XpuCommunicator'
    @classmethod
