...
class NeuronPlatform(Platform):
    ...
    def check_and_update_config(cls, aphrodite_config: AphroditeConfig) -> None:
        ...
        if parallel_config.worker_cls == 'auto':
            parallel_config.worker_cls = 'aphrodite.worker.neuron_worker.NeuronWorker'
        if parallel_config.world_size > 1:
            parallel_config.distributed_executor_backend = 'uni'
        if aphrodite_config.cache_config and aphrodite_config.model_config:
            aphrodite_config.cache_config.block_size = aphrodite_config.model_config.max_model_len
    ...
    def get_device_communicator_cls(cls) -> str:
        if envs.APHRODITE_USE_V1:
            return 'aphrodite.distributed.device_communicators.neuron_communicator.NeuronCommunicator'
        else:
