...
class TpuPlatform(Platform):
    ...
    def get_attn_backend_cls(cls, selected_backend: _Backend, head_size: int, dtype: torch.dtype, kv_cache_dtype: Optional[str], block_size: int, use_v1: bool, use_mla: bool) -> str:
        if selected_backend != _Backend.PALLAS and selected_backend != _Backend.PALLAS_APHRODITE_V1:
    ...
    def check_and_update_config(cls, aphrodite_config: AphroditeConfig) -> None:
        ...
        if model_config is not None and model_config.dtype in (torch.float16, torch.float32):
            logger.warning('The TPU backend currently does not support {}. Using bfloat16 instead.', model_config.dtype)
            model_config.dtype = torch.bfloat16
        from aphrodite.v1.attention.backends.pallas import PallasAttentionBackend
        cache_config.block_size = PallasAttentionBackend.get_page_size(aphrodite_config)
    ...
    def get_device_communicator_cls(cls) -> str:
        return 'aphrodite.distributed.device_communicators.tpu_communicator.TpuCommunicator'
    @classmethod
