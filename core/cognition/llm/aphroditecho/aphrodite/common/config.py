...
from aphrodite.transformers_utils.config import ConfigFormat, get_config, get_hf_image_processor_config, get_hf_text_config, get_pooling_config, get_sentence_transformer_tokenizer_config, is_encoder_decoder, maybe_override_with_speculators_target_model, try_get_generation_config, try_get_safetensors_metadata, try_get_tokenizer_config, uses_mrope
...
if TYPE_CHECKING:
    ...
    me_quant = LazyLoader('quantization', globals(), 'aphrodite.quantization')
...
_ResolvedTask = Literal['generate', 'transcription', 'encode', 'embed', 'classify', 'reward', 'draft']
RunnerOption = Literal['auto', 'generate', 'pooling', 'draft']
RunnerType = Literal['generate', 'pooling', 'draft']
ConvertOption = Literal['auto', 'none', 'embed', 'classify', 'reward']
ConvertType = Literal['none', 'embed', 'classify', 'reward']
_RUNNER_TASKS: dict[RunnerType, list[TaskOption]] = {'generate': ['generate', 'transcription'], 'pooling': ['embedding', 'embed', 'classify', 'score', 'reward'], 'draft': ['draft']}
_RUNNER_CONVERTS: dict[RunnerType, list[ConvertType]] = {'generate': [], 'pooling': ['embed', 'classify', 'reward'], 'draft': []}
_SUFFIX_TO_DEFAULTS: list[tuple[str, tuple[RunnerType, ConvertType]]] = [('ForCausalLM', ('generate', 'none')), ('ForConditionalGeneration', ('generate', 'none')), ('ChatModel', ('generate', 'none')), ('LMHeadModel', ('generate', 'none')), ('ForTextEncoding', ('pooling', 'embed')), ('EmbeddingModel', ('pooling', 'embed')), ('ForSequenceClassification', ('pooling', 'classify')), ('ForAudioClassification', ('pooling', 'classify')), ('ForImageClassification', ('pooling', 'classify')), ('ForVideoClassification', ('pooling', 'classify')), ('ClassificationModel', ('pooling', 'classify')), ('ForRewardModeling', ('pooling', 'reward')), ('RewardModel', ('pooling', 'reward')), ('Model', ('pooling', 'embed'))]
def iter_architecture_defaults():
    yield from _SUFFIX_TO_DEFAULTS
def try_match_architecture_defaults(architecture: str, *, runner_type: Optional[RunnerType]=None, convert_type: Optional[ConvertType]=None) -> Optional[tuple[str, tuple[RunnerType, ConvertType]]]:
    for suffix, (default_runner_type, default_convert_type) in iter_architecture_defaults():
        if (runner_type is None or runner_type == default_runner_type) and (convert_type is None or convert_type == default_convert_type) and architecture.endswith(suffix):
...
LogprobsMode = Literal['raw_logprobs', 'raw_logits', 'processed_logprobs', 'processed_logits']
class ConfigMixin:
    def get_config_diff(self) -> dict[str, Any]:
        ...
        for param_name, param in sig.parameters.items():
            ...
            if param_name in ('served_model_name', 'tokenizer') and current_val == self.model:
                continue
            if param_name == 'max_model_len' and (not isinstance(self, ModelConfig)):
                continue
            if param_name == 'ignore_patterns' and current_val == ['original/**/*']:
                continue
            if param_name == 'cache_config' and isinstance(self, SchedulerConfig):
                continue
...
class ModelConfig:
    ...
    def __post_init__(self) -> None:
        if envs.APHRODITE_USE_V1 and self.seed is None:
            ...
            if not envs.APHRODITE_ENABLE_V1_MULTIPROCESSING:
                logger.warning('The global random seed is set to {}. Since APHRODITE_ENABLE_V1_MULTIPROCESSING is set to False, this may affect the random state of the Python process that launched Aphrodite.', self.seed)
        if self.runner != 'draft':
            self.model, self.tokenizer = maybe_override_with_speculators_target_model(model=self.model, tokenizer=self.tokenizer, revision=self.revision, trust_remote_code=self.trust_remote_code)
        ...
        if (backend := envs.APHRODITE_ATTENTION_BACKEND) and backend == 'FLASHINFER' and (find_spec('flashinfer') is None):
            raise ValueError('APHRODITE_ATTENTION_BACKEND is set to FLASHINFER, but flashinfer module was not found. See https://github.com/aphrodite-engine/aphrodite-engine/blob/main/docker/Dockerfile for instructions on how to install it.')
        from aphrodite.platforms import current_platform
        ...
        if self.task is not None:
            runner: RunnerOption = 'auto'
            convert: ConvertOption = 'auto'
            msg_prefix = "The 'task' option has been deprecated and will be removed in v0.13.0 or v1.0, whichever comes first."
            ...
            if is_generative_model and is_pooling_model:
                if is_generative_task:
                    runner = 'generate'
                    convert = 'auto'
                    msg_hint = 'Please replace this option with `--runner generate` to continue using this model as a generative model.'
                elif is_pooling_task:
                    runner = 'pooling'
                    convert = 'auto'
                    msg_hint = 'Please replace this option with `--runner pooling` to continue using this model as a pooling model.'
                else:
                    pass
                ...
                if is_generative_task:
                    ...
                    msg_hint = f'Please replace this option with `--convert {convert}` to continue using this model as a pooling model.'
                else:
                    pass
            else:
                raise AssertionError(f'The model should be a generative or pooling model when task is set to {self.task!r}.')
            self.runner = runner
        ...
        if self.runner_type == 'pooling' and (not is_pooling_model):
            ...
            if self.convert_type not in pooling_converts:
                ...
                raise ValueError(f'This model does not support `--runner pooling`. You can pass `--convert {convert_option} to adapt it into a pooling model.')
        self.supported_tasks = self._get_supported_tasks(architectures, self.runner_type, self.convert_type)
        ...
        if not self.disable_sliding_window and has_interleaved_attention:
            if not envs.APHRODITE_USE_V1 and (backend := envs.APHRODITE_ATTENTION_BACKEND) in ('XFORMERS', 'FLASHINFER'):
                sliding_window_len_min = get_min_sliding_window(self.hf_text_config.sliding_window)
                log_once('WARNING', '{} has interleaved attention, which is currently not supported by the {} backend. Disabling sliding window and capping the max length to the sliding window size ({}).', self.hf_text_config.model_type, backend, sliding_window_len_min)
    ...
    def _get_default_runner_type(self, architectures: list[str]) -> RunnerType:
        ...
        for arch in architectures:
            ...
            if match:
                ...
                return runner_type
        return 'generate'
    def _get_runner_type(self, architectures: list[str], runner: RunnerOption) -> RunnerType:
        if runner != 'auto':
            return runner
        runner_type = self._get_default_runner_type(architectures)
        if runner_type != 'generate':
            logger.info('Resolved `--runner auto` to `--runner {}`. Pass the value explicitly to silence this message.', runner_type)
        return runner_type
    def _get_default_convert_type(self, architectures: list[str], runner_type: RunnerType) -> ConvertType:
        registry = self.registry
        for arch in architectures:
            if arch in registry.get_supported_archs():
        ...
        if runner_type == 'pooling':
            return 'embed'
        return 'none'
    def _get_convert_type(self, architectures: list[str], runner_type: RunnerType, convert: ConvertOption) -> ConvertType:
        if convert != 'auto':
            return convert
        convert_type = self._get_default_convert_type(architectures, runner_type)
        if convert_type != 'none':
            logger.info('Resolved `--convert auto` to `--convert {}`. Pass the value explicitly to silence this message.', convert_type)
        return convert_type
    def _get_supported_generation_tasks(self, architectures: list[str], convert_type: ConvertType) -> list[_ResolvedTask]:
        registry = self.registry
    ...
    def _get_default_pooling_task(self, architectures: list[str]) -> Literal['embed', 'classify', 'reward']:
        ...
        for arch in architectures:
            ...
            if match:
                ...
                return convert_type
        return 'embed'
    def _get_supported_pooling_tasks(self, architectures: list[str], convert_type: ConvertType) -> list[_ResolvedTask]:
        registry = self.registry
        supported_tasks = list[_ResolvedTask]()
        ...
        return supported_tasks
    def _get_supported_tasks(self, architectures: list[str], runner_type: RunnerType, convert_type: ConvertType) -> list[_ResolvedTask]:
        if runner_type == 'generate':
    ...
    def _verify_quantization(self) -> None:
        ...
        if quant_cfg is not None:
            ...
            overrides = ['marlin', 'bitblas', 'gptq_marlin_24', 'gptq_marlin', 'gptq_bitblas', 'awq_marlin', 'ipex', 'moe_wna16', 'modelopt', 'modelopt_fp4']
        ...
        if self.quantization == 'deepspeedfp':
            ...
            import os
            self.hf_config.quantization_config = {'bits': self.deepspeed_fp_bits, 'group_size': int(os.environ.get('DEEPSPEED_GROUP_SIZE', gs)), 'quant_method': 'deepspeedfp'}
        VALID_QUANT_LLM_FP_BITS = [2, 3, 4, 5, 6, 7]
        VALID_QUANT_LLM_EXPONENTS = [1, 2, 3, 4, 5]
        DEFAULT_EXP_BITS = {2: 1, 3: 2, 4: 2, 5: 2, 6: 2, 7: 3}
        if self.quantization == 'quant_llm':
            if self.quant_llm_fp_bits is None:
                raise ValueError('quant_llm_fp_bits must be specified when using quant_llm quantization.')
            if self.quant_llm_fp_bits not in VALID_QUANT_LLM_FP_BITS:
                raise ValueError(f'Invalid quant_llm_fp_bits: {self.quant_llm_fp_bits}. Must be one of {VALID_QUANT_LLM_FP_BITS}.')
            if self.quant_llm_exp_bits is None:
                self.quant_llm_exp_bits = DEFAULT_EXP_BITS[self.quant_llm_fp_bits]
            elif self.quant_llm_exp_bits not in VALID_QUANT_LLM_EXPONENTS:
                raise ValueError(f'Invalid exponent bits: {self.quant_llm_exp_bits}. Must be one of {VALID_QUANT_LLM_EXPONENTS}.')
            self.hf_config.quantization_config = {'bits': self.quant_llm_fp_bits, 'exp_bits': self.quant_llm_exp_bits, 'quant_method': 'quant_llm'}
        ...
        if self.quantization is not None and self.quantization in online_quant_methods:
            ...
            if fp_bits not in VALID_QUANT_LLM_FP_BITS:
                raise ValueError(f'Invalid quant_llm_fp_bits: {fp_bits}. Must be one of {VALID_QUANT_LLM_FP_BITS}.')
            if fp_bits in [2, 3]:
                logger.warning('FP2 and FP3 quantization methods lead to significant accuracy loss. Use them with caution. Model may be incoherent.')
            exp_bits = DEFAULT_EXP_BITS[fp_bits]
            self.hf_config.quantization_config = {'bits': fp_bits, 'exp_bits': exp_bits, 'quant_method': self.quantization}
        ...
        if self.quantization is not None:
            ...
            if self.quantization == 'deepspeedfp' and self.deepspeed_fp_bits is None:
                raise ValueError('deepspeed_fp_bits must be specified when using deepspeedfp quantization.')
    def _verify_cuda_graph(self) -> None:
        ...
        if unsupported_rocm and (not self.enforce_eager) and current_platform.is_rocm():
            logger.warning('CUDA graph is not supported for {} on ROCm yet, fallback to eager mode.', self.hf_config.model_type)
            self.enforce_eager = True
    ...
    def verify_with_parallel_config(self, parallel_config: 'ParallelConfig') -> None:
        ...
        if pipeline_parallel_size > 1:
            if not self.registry.is_pp_supported_model(self.architectures, self):
    ...
    def get_diff_sampling_param(self) -> dict[str, Any]:
        ...
        if diff_sampling_param:
            log_once('WARNING', "Default sampling parameters have been overridden by the model's Hugging Face generation config recommended from the model creator. If this is not intended, please relaunch Aphrodite instance with `--generation-config aphrodite`.")
...
class CacheConfig:
    ...
    def _verify_args(self) -> Self:
        ...
        if self.kv_sharing_fast_prefill:
            log_once('WARNING', '--kv-sharing-fast-prefill is currently work in progress and not functional yet (i.e. no prefill savings)')
        return self
...
class LoadConfig:
    ...
    def __post_init__(self):
        ...
        if self.ignore_patterns is not None and len(self.ignore_patterns) > 0:
            logger.info('Ignoring the following patterns when downloading weights: {}', self.ignore_patterns)
        else:
    ...
    def __post_init__(self) -> None:
        ...
        if self.enable_eplb:
            ...
            if self.tensor_parallel_size * self.data_parallel_size <= 1:
                raise ValueError(f'EPLB requires tensor_parallel_size or data_parallel_size to be greater than 1, but got TP={self.tensor_parallel_size},DP={self.data_parallel_size}.')
        ...
        if self.distributed_executor_backend is None and self.world_size > 1:
            ...
            if current_platform.is_neuron():
                ...
                if not ray_found:
                    raise ValueError(f'Unable to load Ray: {ray_utils.ray_import_err}. Ray is required for multi-node inference, please install Ray with `pip install ray`.')
                backend = 'ray'
            ...
            logger.debug('Defaulting to use {} for distributed inference', backend)
        if self.distributed_executor_backend is None and self.world_size == 1:
...
class SchedulerConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if self.single_user_mode:
            ...
            if original_max_num_seqs != 1 or original_cuda_graph_sizes != [1]:
                logger.info('Single user mode enabled. Setting max_num_seqs from {} to 1, and cuda_graph_sizes from {} to [1].', original_max_num_seqs, original_cuda_graph_sizes)
        ...
        if self.enable_chunked_prefill:
            logger.info('Chunked prefill is enabled with max_num_batched_tokens={}.', self.max_num_batched_tokens)
        self.chunked_prefill_enabled = self.enable_chunked_prefill
        if self.max_num_partial_prefills > 1:
            if self.long_prefill_token_threshold == 0:
                self.long_prefill_token_threshold = int(self.max_model_len * 0.04)
            logger.info('Concurrent partial prefills enabled with max_num_partial_prefills={}, max_long_partial_prefills={}, long_prefill_token_threshold={}', self.max_num_partial_prefills, self.max_long_partial_prefills, self.long_prefill_token_threshold)
    ...
    def _verify_args(self) -> Self:
        ...
        if self.max_num_batched_tokens > self.max_num_seqs * self.max_model_len:
            logger.warning('max_num_batched_tokens ({}) exceeds max_num_seqs * max_model_len ({}). This may lead to unexpected behavior.', self.max_num_batched_tokens, self.max_num_seqs * self.max_model_len)
    ...
    def __post_init__(self):
        if self.device == 'auto':
            ...
            if not self.device_type:
                raise RuntimeError('Failed to infer device type, please set the environment variable `APHRODITE_LOGGING_LEVEL=DEBUG` to turn on verbose logging to help debug the issue.')
...
class SpeculativeConfig:
        ...
        if self.method in ('ngram', '[ngram]'):
            ...
            if self.model is not None:
                ...
                if self.method in ('eagle', 'eagle3'):
                    ...
                    if isinstance(self.draft_model_config.hf_config, (EAGLEConfig, SpeculatorsConfig)):
                        pass
                    else:
        ...
        if speculative_draft_tensor_parallel_size is None:
            if draft_hf_config.model_type == 'mlp_speculator':
                ...
                if target_parallel_config.tensor_parallel_size > 1:
                    logger.warning('{} cannot currently be run with tp>1; setting speculative_draft_tensor_parallel_size=1', draft_hf_config.model_type)
            else:
    ...
    def _verify_args(self) -> Self:
        ...
        if self.method == 'eagle3' and self.target_model_config and (not any((supported_model in self.target_model_config.hf_text_config.model_type for supported_model in eagle3_target_supported))):
            raise ValueError(f'Eagle3 is only supported for {eagle3_target_supported} models. Got self.target_model_config.hf_text_config.model_type={self.target_model_config.hf_text_config.model_type!r}')
        return self
    @property
...
class MultiModalConfig:
    ...
    def get_limit_per_prompt(self, modality: str) -> int:
        return self.limit_per_prompt.get(modality, 999 if envs.APHRODITE_USE_V1 else 1)
    def merge_mm_processor_kwargs(self, inference_kwargs: Mapping[str, object]) -> dict[str, object]:
...
def _resolve_auto_dtype(model_type: str, config_dtype: torch.dtype, *, is_pooling_model: bool):
    ...
    logger.warning("Your device {} doesn't support {}. Falling back to {} for compatibility.", device_str, config_dtype, preferred_dtype)
...
def _get_and_verify_max_len(hf_config: PretrainedConfig, tokenizer_config: Optional[dict], max_model_len: Optional[int], disable_sliding_window: bool, sliding_window_len: Optional[Union[int, list[Optional[int]]]], spec_target_max_model_len: Optional[int]=None, encoder_config: Optional[Any]=None) -> int:
    ...
    possible_keys = ['model_max_length', 'max_position_embeddings', 'n_positions', 'max_seq_len', 'seq_length', 'model_max_length', 'max_sequence_length', 'max_seq_length', 'seq_len']
    max_len_key = None
    ...
    if max_model_len is None:
        ...
        if current_platform.is_tpu():
            logger.warning("--max-model-len is not specified, it's currently using model's default length {}, which might be too large.Please input with --max-model-len based on your request input length and output length, to avoid unnecessary degradation.", max_model_len)
        ...
        if envs.APHRODITE_DYNAMIC_ROPE_SCALING:
            ...
            logger.info(f"Using dynamic RoPE scaling to extend the model's max context length from {derived_max_model_len} to {max_model_len}.")
            ...
            raise ValueError(f"User-specified max_model_len ({max_model_len}) is greater than the derived max_model_len ({max_len_key}={derived_max_model_len} or model_max_length={model_max_length} in model's config.json). To allow greater lengths, please set the env var APHRODITE_DYNAMIC_ROPE_SCALING=1")
    ...
    def __post_init__(self) -> None:
        if not self.enable_noop:
            if self.enable_fusion:
                log_once('WARNING', 'Fusion enabled but reshape elimination disabled. RMSNorm/SiluMul + quant (fp8) fusion might not work')
            if self.enable_attn_fusion:
                log_once('WARNING', 'Fusion enabled but reshape elimination disabled. Attention + quant (fp8) fusion might not work')
@config
@dataclass
class CompilationConfig:
    ...
    def compute_hash(self) -> str:
        ...
        return hashlib.sha256(str(factors).encode()).hexdigest()
    def __repr__(self) -> str:
        exclude = {'static_forward_context': True, 'enabled_custom_ops': True, 'disabled_custom_ops': True, 'compilation_time': True, 'bs_to_padded_graph_size': True, 'traced_files': True, 'inductor_compile_config': {'post_grad_custom_post_pass': True}}
        pass_config_exclude = {}
    ...
    def init_with_cudagraph_sizes(self, cudagraph_capture_sizes: list[int]) -> None:
        if self.cudagraph_capture_sizes is None:
            ...
            if len(dedup_sizes) < len(self.cudagraph_capture_sizes):
                logger.info('cudagraph sizes specified by model runner {} is overridden by config {}', cudagraph_capture_sizes, dedup_sizes)
    ...
    def set_splitting_ops_for_v1(self):
        ...
        if not self.splitting_ops:
            self.splitting_ops = [] if self.full_cuda_graph else ['aphrodite.unified_attention', 'aphrodite.unified_attention_with_output', 'aphrodite.mamba_mixer2']
@config
@dataclass(config=ConfigDict(arbitrary_types_allowed=True))
class AphroditeConfig:
    ...
    def with_hf_config(self, hf_config: PretrainedConfig, architectures: Optional[list[str]]=None) -> 'AphroditeConfig':
    ...
    def __post_init__(self):
        ...
        if self.model_config is not None and self.scheduler_config.chunked_prefill_enabled and (self.model_config.dtype == torch.float32) and (current_platform.get_device_capability() == (7, 5)):
            log_once('WARNING', "Turing devices tensor cores do not support float32 matmul. To workaround this limitation, Aphrodite will set 'ieee' input precision for chunked prefill triton kernels.")
        ...
        if envs.APHRODITE_USE_V1 and self.model_config is not None and (not self.model_config.enforce_eager):
            self.compilation_config.cudagraph_num_of_warmups = 1
            self.compilation_config.set_splitting_ops_for_v1()
        self._set_cudagraph_sizes()
        if self.cache_config.cpu_offload_gb > 0 and self.compilation_config.level != CompilationLevel.NO_COMPILATION and (not envs.APHRODITE_USE_V1):
            logger.warning('CPU offload is not supported with `torch.compile` in v0 yet. Disabling `torch.compile`.')
            self.compilation_config.level = CompilationLevel.NO_COMPILATION
        if not envs.APHRODITE_USE_V1 and self.lora_config is not None and (self.compilation_config.level != CompilationLevel.NO_COMPILATION):
        ...
        if envs.APHRODITE_USE_V1 and (not self.scheduler_config.disable_hybrid_kv_cache_manager):
            ...
            if self.model_config is not None and self.model_config.attention_chunk_size is not None:
                if self.speculative_config is not None and self.speculative_config.use_eagle():
                    ...
                    logger.warning('There is a latency regression when using chunked local attention with the hybrid KV cache manager. Disabling it, by default. To enable it, set the environment APHRODITE_ALLOW_CHUNKED_LOCAL_ATTN_WITH_HYBRID_KV_CACHE=1.')
                    self.scheduler_config.disable_hybrid_kv_cache_manager = True
    def update_sizes_for_sequence_parallelism(self, possible_sizes: list) -> list:
        removed_sizes = [size for size in possible_sizes if size % self.parallel_config.tensor_parallel_size != 0]
        if removed_sizes:
            logger.warning('Batch sizes {} are removed because they are not multiple of tp_size {} when sequence parallelism is enabled', removed_sizes, self.parallel_config.tensor_parallel_size)
...
@contextmanager
def set_current_aphrodite_config(aphrodite_config: AphroditeConfig, check_compile: bool=False, prefix: Optional[str]=None):
    ...
    try:
        ...
        logger.debug('enabled custom ops: {}', aphrodite_config.compilation_config.enabled_custom_ops)
        logger.debug('disabled custom ops: {}', aphrodite_config.compilation_config.disabled_custom_ops)
        if check_compile and aphrodite_config.compilation_config.level == CompilationLevel.PIECEWISE and (compilation_counter.num_models_seen == num_models_seen):
            logger.warning('`torch.compile` is turned on, but the model {} does not support it. Please open an issue on GitHub if you want it to be supported.', aphrodite_config.model_config.model)
    finally:
...
def assert_hashable(text):
    if not contains_object_print(text):
        return True
    raise AssertionError(f'Aphrodite tried to hash some configs that may have Python objects ids in them. This is a bug, please file an issue. Text being hashed: {text}')
T = TypeVar('T')
def get_layers_from_aphrodite_config(aphrodite_config: AphroditeConfig, layer_type: type[T], layer_names: Optional[list[str]]=None) -> dict[str, T]:
