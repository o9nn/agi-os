...
def get_loading_progress_bar():
    return Progress(TextColumn('[progress.description]{task.description}'), BarColumn(), TaskProgressColumn(), MofNCompleteColumn(), TimeRemainingColumn(), console=RICH_CONSOLE)
def _log_formatter(record: dict):
    color_map = {'TRACE': 'dim blue', 'DEBUG': 'cyan', 'INFO': 'green', 'SUCCESS': 'bold green', 'WARNING': 'yellow', 'ERROR': 'red', 'CRITICAL': 'bold white on red'}
...
UVICORN_LOG_CONFIG = {'version': 1, 'disable_existing_loggers': False, 'handlers': {'uvicorn': {'class': f'{UvicornLoggingHandler.__module__}.{UvicornLoggingHandler.__qualname__}'}}, 'root': {'handlers': ['uvicorn'], 'propagate': False, 'level': LOG_LEVEL}}
def setup_logger():
    logger.remove()
    ...
    if event in ['call', 'return']:
        ...
        try:
            ...
            with open(log_path, 'a') as f:
                if event == 'call':
                    f.write(f'{datetime.datetime.now()} Call to {func_name} in {filename}:{lineno} from {last_func_name} in {last_filename}:{last_lineno}\n')
                else:
                    f.write(f'{datetime.datetime.now()} Return from {func_name} in {filename}:{lineno} to {last_func_name} in {last_filename}:{last_lineno}\n')
        except NameError:
            pass
    return partial(_trace_calls, log_path, root_dir)
def enable_trace_function_call(log_file_path: str, root_dir: Optional[str]=None):
    logger.warning('APHRODITE_TRACE_FUNCTION is enabled. It will record every function executed by Python. This will slow down the code. It is suggested to be used for debugging hang or crashes only.')
