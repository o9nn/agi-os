...
class SequenceData(msgspec.Struct, omit_defaults=True):
    ...
    _output_token_ids: array = msgspec.field(default_factory=lambda: array(APHRODITE_TOKEN_ID_ARRAY_TYPE, []))
    ...
    def from_seqs(prompt_token_ids: GenericSequence[int], output_token_ids: Optional[GenericSequence[int]]=None, *, prompt_embeds: Optional[torch.Tensor]=None) -> 'SequenceData':
        prompt_token_ids_arr = array(APHRODITE_TOKEN_ID_ARRAY_TYPE, prompt_token_ids)
        if output_token_ids is None:
            return SequenceData(prompt_token_ids_arr, _prompt_embeds=prompt_embeds)
        output_token_ids_arr = array(APHRODITE_TOKEN_ID_ARRAY_TYPE, output_token_ids)
    ...
    def output_token_ids(self, new_output_token_ids: GenericSequence[int]) -> None:
        self._output_token_ids = array(APHRODITE_TOKEN_ID_ARRAY_TYPE, new_output_token_ids)
        self._update_cached_all_tokens()
    @property
...
class Sequence:
    def __init__(self, seq_id: int, inputs: SingletonInputs, block_size: int, eos_token_id: Optional[int]=None, lora_request: Optional[LoRARequest]=None) -> None:
        self.seq_id = seq_id
        self.inputs = inputs
