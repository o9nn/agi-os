...
class FastParserState(ParserState):
    ...
    def __copy__(self):
        ...
        new_instance = type(self)(self.parse_conf, self.lexer, copy(self.state_stack), new_value_stack)
        self.copy_memo[id(self)] = new_instance
        return new_instance
class FastInteractiveParser(InteractiveParser):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.parser_state = FastParserState(self.parser_state.parse_conf, self.parser_state.lexer, self.parser_state.state_stack, self.parser_state.value_stack)
        self.hash_val = None
    def __hash__(self):
...
def get_pattern_validator(pattern: Pattern):
    if isinstance(pattern, PatternRE):
        ...
        @functools.lru_cache(int(1000000.0))
...
def method_lru_cache(*lru_args, **lru_kwargs):
    def decorator(func):
        ...
        return wrapped_func
    return decorator
memoize_by_instance = method_lru_cache(int(10000000.0))
class TrieNode:
    def __init__(self):
        self.children = {}
...
class IncrementalParserState:
    ...
    def from_grammar(cls, grammar: str, start: str):
        lark_parser = Lark(grammar, regex=True, start=start, parser='lalr', cache=True)
        base_interactive_parser = lark_parser.parse_interactive()
        interactive_parser = FastInteractiveParser(base_interactive_parser.parser, base_interactive_parser.parser_state, base_interactive_parser.lexer_thread)
        ...
        parser = cls(interactive_parser=interactive_parser, terminal_candidates=None, _ignored_terms=set(lark_parser.lexer_conf.ignore), _seq_validator=_seq_validator, _memo={}, _full_seq_trie=Trie())
    ...
    def _next_with_new_terminal(self, terminal):
        ...
        return self.new(interactive_parser=new_interactive_parser, terminal_candidates=None)
...
class TokenVocab:
    def __init__(self, tokenizer: Union[PreTrainedTokenizer, PreTrainedTokenizerFast], legal_chars: Optional[Set[str]]=None):
...
class NextTokenValidator:
    def __init__(self, tokenizer, grammar: str, grammar_start: str='start', legal_chars: Optional[set[str]]=None):
        ...
        for token in self.vocab:
            if token is None:
                if partial_term == '' and parser.is_valid_next_seq(token):
                    yield None
            elif parser.is_valid_next_seq(partial_term + token):
                yield token
