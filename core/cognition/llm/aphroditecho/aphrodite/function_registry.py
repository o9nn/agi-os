...
class FunctionSpec:
    ...
    def to_contract_dict(self) -> Dict[str, Any]:
        return {'name': self.name, 'description': self.description, 'params': {name: {'type': param.type, 'description': param.description, 'required': param.required, 'default': param.default} for name, param in self.parameters.items()}, 'safety_class': self.safety_class.value, 'cost_unit': self.cost_unit, 'impl_ref': self.implementation_ref, 'tags': self.tags, 'allow_network': self.allow_network}
@dataclass
...
class FunctionRegistry:
    ...
    def _register_builtin_functions(self):
        builtin_functions = [FunctionSpec(name='echo', description='Echo back the provided text', parameters={'text': ParameterSpec(type='string', description='Text to echo back', required=True)}, safety_class=SafetyClass.LOW, cost_unit=0.1, implementation_ref='builtin.echo'), FunctionSpec(name='calculate', description='Perform mathematical calculations', parameters={'expression': ParameterSpec(type='string', description='Mathematical expression to evaluate', required=True)}, safety_class=SafetyClass.MEDIUM, cost_unit=0.5, implementation_ref='builtin.calculate'), FunctionSpec(name='web_search', description='Search the web for information', parameters={'query': ParameterSpec(type='string', description='Search query', required=True), 'max_results': ParameterSpec(type='integer', description='Maximum number of results', required=False, default=5)}, safety_class=SafetyClass.HIGH, cost_unit=2.0, implementation_ref='builtin.web_search', allow_network=True, tags=['search', 'web']), FunctionSpec(name='file_read', description='Read contents of a file', parameters={'path': ParameterSpec(type='string', description='Path to the file to read', required=True)}, safety_class=SafetyClass.HIGH, cost_unit=1.0, implementation_ref='builtin.file_read', tags=['filesystem'])]
    ...
    def _get_builtin_implementation(self, function_name: str) -> Callable:
        builtin_impls = {'echo': lambda text: {'echoed': text}, 'calculate': self._builtin_calculate, 'web_search': self._builtin_web_search, 'file_read': self._builtin_file_read}
    ...
    def _builtin_web_search(self, query: str, max_results: int=5) -> Dict[str, Any]:
        return {'query': query, 'results': [{'title': f'Result {i}', 'url': f'https://example.com/{i}'} for i in range(min(max_results, 3))], 'total': max_results}
    ...
    def register_function(self, spec: FunctionSpec, implementation: Callable) -> bool:
        try:
            ...
            self.invocation_stats[spec.name] = {'total_invocations': 0, 'total_cost': 0.0, 'total_execution_time': 0.0, 'success_count': 0, 'error_count': 0}
    ...
    def list_functions(self, tags: Optional[List[str]]=None, safety_class: Optional[SafetyClass]=None) -> List[FunctionSpec]:
    ...
    def get_all_schemas(self) -> Dict[str, Dict[str, Any]]:
        return {name: spec.to_contract_dict() for name, spec in self.functions.items()}
    async def invoke_function(self, invocation: FunctionInvocation) -> FunctionResult:
        ...
        try:
            ...
            if not func_spec:
                return FunctionResult(success=False, error=f'Function {invocation.function_name} not found')
            if func_spec.status != FunctionStatus.ACTIVE:
                return FunctionResult(success=False, error=f'Function {invocation.function_name} is not active')
            validation_result = self._validate_arguments(func_spec, invocation.arguments)
            if not validation_result['valid']:
                return FunctionResult(success=False, error=f"Argument validation failed: {validation_result['error']}")
            implementation = self.implementations.get(invocation.function_name)
            if not implementation:
                return FunctionResult(success=False, error=f'No implementation found for {invocation.function_name}')
            ...
            return FunctionResult(success=True, result=result, cost_incurred=cost, execution_time_ms=execution_time, metadata={'function_spec': func_spec.name})
        except Exception as e:
            execution_time = (asyncio.get_event_loop().time() - start_time) * 1000
            ...
            return FunctionResult(success=False, error=str(e), execution_time_ms=execution_time)
        ...
        try:
            ...
            for arg_name in arguments:
                if arg_name not in func_spec.parameters:
                    return {'valid': False, 'error': f"Unexpected parameter '{arg_name}'"}
            return {'valid': True}
        except Exception as e:
                ...
                if func_spec:
                    def make_argc_impl(cmd_name):
                        async def argc_impl_func(**kwargs):
                            return await self._execute_argc_command(schema_path, cmd_name, kwargs)
    ...
    def import_llm_functions(self, module_path: str) -> List[str]:
        ...
        try:
            ...
            for attr_name in dir(module):
                ...
                if callable(attr) and (not attr_name.startswith('_')) and (hasattr(attr, '__llm_function__') or hasattr(attr, '__doc__')):
            ...
            for line in comments.split('\n'):
                ...
                if line.startswith('# @describe'):
                    ...
                    if parts:
                        ...
                        parameters[arg_name] = ParameterSpec(type='string', description=arg_desc, required=required)
                    ...
                    if len(parts) >= 2:
                        ...
                        parameters[flag_name] = ParameterSpec(type='boolean', description=flag_desc, required=False, default=False)
            return FunctionSpec(name=f'argc_{command_name}', description=description or f'Argc command: {command_name}', parameters=parameters, safety_class=SafetyClass.MEDIUM, cost_unit=0.5, implementation_ref=f'argc.{command_name}', tags=['argc', 'cli'])
        except Exception as e:
    ...
    def _create_function_spec_from_callable(self, func: Callable, name: str) -> Optional[FunctionSpec]:
        try:
            ...
            for param_name, param in sig.parameters.items():
                ...
                parameters[param_name] = ParameterSpec(type=param_type, description=f'Parameter: {param_name}', required=required, default=default_val)
            description = func.__doc__ or f'LLM function: {name}'
            return FunctionSpec(name=f'llm_{name}', description=description.strip(), parameters=parameters, safety_class=SafetyClass.MEDIUM, cost_unit=1.0, implementation_ref=f'llm.{name}', tags=['llm', 'dynamic'])
        except Exception as e:
    ...
    async def _execute_argc_command(self, schema_path: Path, command_name: str, arguments: Dict[str, Any]) -> Dict[str, Any]:
        try:
            ...
            return {'success': process.returncode == 0, 'stdout': stdout.decode().strip(), 'stderr': stderr.decode().strip(), 'exit_code': process.returncode, 'command': command_name, 'arguments': arguments}
        except Exception as e:
            return {'success': False, 'error': str(e), 'command': command_name, 'arguments': arguments}
    def export_openai_tools_format(self) -> List[Dict[str, Any]]:
        tools = []
        for func_spec in self.functions.values():
            if func_spec.status == FunctionStatus.ACTIVE:
                tool = {'type': 'function', 'function': {'name': func_spec.name, 'description': func_spec.description, 'parameters': {'type': 'object', 'properties': {}, 'required': []}}}
                for param_name, param_spec in func_spec.parameters.items():
                    tool['function']['parameters']['properties'][param_name] = {'type': param_spec.type, 'description': param_spec.description}
    ...
    def health_check(self) -> Dict[str, Any]:
        ...
        return {'status': 'healthy', 'total_functions': total_functions, 'active_functions': active_functions, 'total_invocations': sum((stats['total_invocations'] for stats in self.invocation_stats.values())), 'implementation_coverage': len(self.implementations) / total_functions if total_functions > 0 else 0}