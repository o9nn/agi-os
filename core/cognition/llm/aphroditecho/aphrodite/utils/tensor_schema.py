...
class TensorShape:
    def __init__(self, *dims: Union[int, str], dynamic_dims: set[str, ...]=None) -> None:
        self.dims = dims
...
class TensorSchema:
    def __init__(self, *, validate: bool=True, resolve_bindings: dict[str, int]=None, **kwargs: Any) -> None:
    ...
    def _match_shape_with_dynamic(self, actual: tuple[int, ...], reference: tuple[int, ...], expected_shape: tuple[Union[int, str], ...], dynamic_dims: set[str, ...]) -> bool:
        ...
        for i, (a, r) in enumerate(zip(actual, reference)):
            ...
            if a != r:
                return False
        return True
    def _validate_nested_tensors(self, value: Union[list[torch.Tensor, ...], tuple[torch.Tensor, ...]], field_name: str, expected_shape: tuple[Union[int, str], ...], dynamic_dims: set[str, ...]) -> tuple[int, ...]:
        first = value[0]
        for i, v in enumerate(value):
            if not isinstance(v, torch.Tensor):
                raise ValueError(f'{field_name}[{i}] is not a torch.Tensor')
            if not self._match_shape_with_dynamic(v.shape, first.shape, expected_shape, dynamic_dims):
                raise ValueError(f'{field_name} contains inconsistent shapes: {first.shape} vs {v.shape} at index {i}')
        return (len(value),) + first.shape
    def _validate_tensor_shape_expected(self, actual_shape: tuple[int, ...], expected_shape: tuple[Union[int, str], ...], field_name: str, shape_env: dict[str, int], dynamic_dims: set[str, ...]) -> None:
        ...
        for i, dim in enumerate(expected_shape):
            if dim in dynamic_dims:
                ...
                if actual_shape[i] != dim:
                    raise ValueError(f'{field_name} dim[{i}] expected {dim}, got {actual_shape[i]}')
            elif isinstance(dim, str):
                if dim in shape_env:
                    if actual_shape[i] != shape_env[dim]:
                        raise ValueError(f"{field_name} dim[{i}] expected '{dim}'={shape_env[dim]}, got {actual_shape[i]}")
                else:
                    ...
                    if isinstance(arg, TensorShape):
                        ...
                        if isinstance(value, (list, tuple)):
                            if value and isinstance(value[0], torch.Tensor):
                                actual_shape = self._validate_nested_tensors(value, field_name, expected_shape, arg.dynamic_dims)
                            elif value:
                        ...
                        self._validate_tensor_shape_expected(actual_shape, expected_shape, field_name, shape_env, arg.dynamic_dims)
