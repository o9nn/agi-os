...
class SamplingOps:
    def apply_penalties(self, logits: torch.Tensor, sampling_metadata: SamplingMetadata) -> torch.Tensor:
        ...
        return logits
    def apply_dry(self, logits: torch.Tensor, sampling_metadata: SamplingMetadata) -> torch.Tensor:
        if sampling_metadata.dry_multiplier is not None and sampling_metadata.prompt_token_ids is not None:
            ...
            if sampling_metadata.dry_base is not None and sampling_metadata.dry_allowed_length is not None and (sampling_metadata.dry_sequence_breaker_ids is not None) and (sampling_metadata.dry_ranges is not None) and (sampling_metadata.dry_max_ngram is not None) and (sampling_metadata.dry_max_occurrences is not None) and (sampling_metadata.dry_early_exit_match_len is not None):
                logits = apply_all_dry(logits, sampling_metadata.prompt_token_ids, output_tokens_t, sampling_metadata.dry_multiplier, sampling_metadata.dry_base, sampling_metadata.dry_allowed_length, sampling_metadata.dry_sequence_breaker_ids, sampling_metadata.dry_ranges, sampling_metadata.dry_max_ngram, sampling_metadata.dry_max_occurrences, sampling_metadata.dry_early_exit_match_len)
        return logits
    def apply_no_repeat_ngram(self, logits: torch.Tensor, sampling_metadata: SamplingMetadata) -> torch.Tensor:
        return no_repeat_ngram(logits, sampling_metadata)
    def apply_min_p(self, logits: torch.Tensor, sampling_metadata: SamplingMetadata) -> torch.Tensor:
        return min_p(logits, sampling_metadata)
    def apply_top_a(self, logits: torch.Tensor, sampling_metadata: SamplingMetadata) -> torch.Tensor:
        return top_a(logits, sampling_metadata)
    def apply_tfs(self, logits: torch.Tensor, sampling_metadata: SamplingMetadata) -> torch.Tensor:
        return tfs(logits, sampling_metadata)
    def apply_eta_cutoff(self, logits: torch.Tensor, sampling_metadata: SamplingMetadata) -> torch.Tensor:
        return eta_cutoff(logits, sampling_metadata)
    def apply_epsilon_cutoff(self, logits: torch.Tensor, sampling_metadata: SamplingMetadata) -> torch.Tensor:
        return epsilon_cutoff(logits, sampling_metadata)
    def apply_typical_p(self, logits: torch.Tensor, sampling_metadata: SamplingMetadata) -> torch.Tensor:
        return typical_p(logits, sampling_metadata)
    def apply_quadratic(self, logits: torch.Tensor, sampling_metadata: SamplingMetadata) -> torch.Tensor:
        return quadratic(logits, sampling_metadata)
    def apply_xtc(self, logits: torch.Tensor, sampling_metadata: SamplingMetadata) -> torch.Tensor:
        return xtc(logits, sampling_metadata)
    def apply_top_nsigma(self, logits: torch.Tensor, sampling_metadata: SamplingMetadata) -> torch.Tensor:
        return top_nsigma(logits, sampling_metadata)
    def apply_skew(self, logits: torch.Tensor, sampling_metadata: SamplingMetadata) -> torch.Tensor:
        return skew(logits, sampling_metadata)
    def apply_logits_bias(self, logits: torch.Tensor, sampling_metadata: SamplingMetadata) -> torch.Tensor:
        vocab_size = logits.shape[-1]
        for i, logit_bias in enumerate(sampling_metadata.logit_bias):
            if logit_bias:
                for token_id, bias in logit_bias.items():
                    if token_id < 0 or token_id >= vocab_size:
                        raise ValueError(f'token_id {token_id} in logit_bias contains out-of-vocab token id. Vocabulary size: {vocab_size}')
