...
class CPUWorker(Worker):
    def __init__(self, aphrodite_config: AphroditeConfig, local_rank: int, rank: int, distributed_init_method: str, is_driver_worker: bool=False):
        super().__init__(aphrodite_config, local_rank, rank, distributed_init_method, is_driver_worker=is_driver_worker)
    ...
    def _get_autobind_cpu_ids(self, cpu_selector: Callable[[list[LogicalCPUInfo]], list[LogicalCPUInfo]]) -> str:
        ...
        assert len(logical_cpu_list) > reserve_cpu_num, f'APHRODITE_CPU_NUM_OF_RESERVED_CPU ({reserve_cpu_num}) should less than {len(logical_cpu_list)}.'
        if reserve_cpu_num != 0:
            logical_cpu_list = logical_cpu_list[:-reserve_cpu_num]
        logger.info('auto thread-binding list (id, physical core): {}', [(x.id, x.physical_core) for x in logical_cpu_list])
