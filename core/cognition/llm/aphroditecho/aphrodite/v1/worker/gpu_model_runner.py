...
from aphrodite.common.config import AphroditeConfig, CompilationLevel, get_layers_from_aphrodite_config, update_config
...
from aphrodite.distributed.parallel_state import get_pp_group, get_tp_group, graph_capture, is_global_first_rank, prepare_communication_buffer_for_model
...
from aphrodite.modeling.models.interfaces import is_mixture_of_experts, supports_transcription
from aphrodite.modeling.models.interfaces_base import AphroditeModelForPooling, is_pooling_model, is_text_generation_model
from aphrodite.multimodal import MULTIMODAL_REGISTRY
from aphrodite.multimodal.inputs import BatchedTensorInputs, MultiModalKwargs, PlaceholderRange
...
from aphrodite.utils import STR_DTYPE_TO_TORCH_DTYPE, DeviceMemoryProfiler, GiB_bytes, LazyLoader, check_use_alibi, get_dtype_size, is_pin_memory_available, round_up, supports_dynamo
from aphrodite.v1.attention.backends.mamba_selectors import get_mamba_attn_backend
from aphrodite.v1.attention.backends.utils import AttentionCGSupport, AttentionMetadataBuilder, CommonAttentionMetadata, make_kv_sharing_fast_prefill_attention_metadata, make_local_attention_virtual_batches, reorder_batch_to_split_decodes_and_prefills
from aphrodite.v1.kv_cache_interface import AttentionSpec, ChunkedLocalAttentionSpec, FullAttentionSpec, KVCacheConfig, KVCacheSpec, MambaSpec, SlidingWindowSpec
from aphrodite.v1.outputs import EMPTY_MODEL_RUNNER_OUTPUT, LogprobsTensors, ModelRunnerOutput
...
from .utils import MultiModalBudget, bind_kv_cache, gather_mm_placeholders, initialize_kv_cache_for_kv_sharing, sanity_check_mm_encoder_outputs, scatter_mm_placeholders
if TYPE_CHECKING:
...
class GPUModelRunner(LoRAModelRunnerMixin, KVConnectorModelRunnerMixin):
    def __init__(self, aphrodite_config: AphroditeConfig, device: torch.device):
        ...
        self.is_multimodal_raw_input_supported = model_config.is_multimodal_raw_input_supported
        ...
        self.input_batch = InputBatch(max_num_reqs=self.max_num_reqs, max_model_len=self.max_model_len, max_num_batched_tokens=self.max_num_tokens, device=self.device, pin_memory=self.pin_memory, vocab_size=self.model_config.get_vocab_size(), block_sizes=[self.cache_config.block_size], is_spec_decode=bool(self.aphrodite_config.speculative_config))
        self.use_cuda_graph = self.aphrodite_config.compilation_config.level == CompilationLevel.PIECEWISE and self.aphrodite_config.compilation_config.use_cudagraph and (not self.model_config.enforce_eager)
        ...
        if self.cache_config.kv_sharing_fast_prefill:
            self.kv_sharing_fast_prefill_logits_indices = torch.zeros(self.max_num_tokens, dtype=torch.int32, device=self.device)
        self.mm_budget = MultiModalBudget(self.model_config, self.scheduler_config, self.mm_registry, max_model_len=self.max_model_len, max_num_reqs=self.max_num_reqs) if self.is_multimodal_model else None
        self.reorder_batch_threshold: Optional[int] = None
    def _may_reorder_batch(self, scheduler_output: 'SchedulerOutput') -> None:
        if len(self.kv_cache_config.kv_cache_groups) == 0:
            return
        if self.reorder_batch_threshold is not None:
            reorder_batch_to_split_decodes_and_prefills(self.input_batch, scheduler_output, decode_threshold=self.reorder_batch_threshold)
    ...
    def _extract_mm_kwargs(self, scheduler_output: 'SchedulerOutput') -> BatchedTensorInputs:
    ...
    def _prepare_inputs(self, scheduler_output: 'SchedulerOutput') -> tuple[dict[str, Any], bool, torch.Tensor, Optional[SpecDecodeMetadata], np.ndarray, Optional[CommonAttentionMetadata]]:
        ...
        if self.cache_config.kv_sharing_fast_prefill:
            ...
            if self.use_cuda_graph and num_logits <= self.cudagraph_batch_sizes[-1]:
        ...
        if self.is_encoder_only_model:
            common_attn_metadata, encoder_attn_metadata = self._build_encoder_only_attn_metadata(scheduler_output)
            attention_layers = get_layers_from_aphrodite_config(self.aphrodite_config, Attention)
        ...
        for kv_cache_group_id, kv_cache_group_spec in enumerate(self.kv_cache_config.kv_cache_groups):
            ...
            if self.cache_config.kv_sharing_fast_prefill and self.kv_sharing_fast_prefill_eligible_layers:
                fast_prefill_metadata_type = make_kv_sharing_fast_prefill_attention_metadata(metadata_cls=type(attn_metadata_i))
                fast_prefill_metadata = fast_prefill_metadata_type(**dataclasses.asdict(attn_metadata_i), logits_indices_padded=logits_indices_padded, num_logits_indices=logits_indices.size(0))
            for layer_name in kv_cache_group_spec.layer_names:
                if self.cache_config.kv_sharing_fast_prefill and layer_name in self.kv_sharing_fast_prefill_eligible_layers:
                    attn_metadata[layer_name] = fast_prefill_metadata
                    continue
                attn_metadata[layer_name] = attn_metadata_i
            if self.attention_chunk_size is not None and self.scheduler_config.disable_hybrid_kv_cache_manager:
                ...
                local_attn_metadata_i = builder.build(common_prefix_len=0, common_attn_metadata=make_local_attention_virtual_batches(self.attention_chunk_size, common_attn_metadata, self.cache_config.block_size))
    ...
    def _gather_mm_embeddings(self, scheduler_output: 'SchedulerOutput', shift_computed_tokens: int=0) -> list[torch.Tensor]:
    ...
    def apply_grammar_bitmask(self, scheduler_output: 'SchedulerOutput', logits: torch.Tensor):
        ...
        xgr_torch_compile.apply_token_bitmask_inplace_torch_compile(logits, grammar_bitmask.to(self.device, non_blocking=True), indices=out_indices if not skip_out_indices else None)
    ...
    def _pool(self, hidden_states: torch.Tensor, num_scheduled_tokens: int, num_scheduled_tokens_np: np.ndarray, kv_connector_output: Optional[KVConnectorOutput]) -> ModelRunnerOutput:
        ...
        return ModelRunnerOutput(req_ids=self.input_batch.req_ids, req_id_to_index=self.input_batch.req_id_to_index, sampled_token_ids=[], spec_token_ids=None, logprobs=None, prompt_logprobs_dict={}, pooler_output=pooler_output, kv_connector_output=kv_connector_output)
    ...
    def execute_model(self, scheduler_output: 'SchedulerOutput', intermediate_tensors: Optional[IntermediateTensors]=None) -> Union[ModelRunnerOutput, IntermediateTensors]:
        ...
        if self.use_cuda_graph and num_scheduled_tokens <= self.cudagraph_batch_sizes[-1]:
            num_input_tokens = self.aphrodite_config.pad_for_cudagraph(num_scheduled_tokens)
        else:
        ...
        if self.is_multimodal_model and get_pp_group().is_first_rank:
            inputs_embeds_scheduled = self.model.get_input_embeddings(input_ids=self.input_ids[:num_scheduled_tokens], multimodal_embeddings=mm_embeds or None)
        ...
        with set_forward_context(attn_metadata, self.aphrodite_config, num_tokens=num_input_tokens, num_tokens_across_dp=num_tokens_across_dp, skip_cuda_graphs=skip_cuda_graphs), self.maybe_get_kv_connector_output(scheduler_output) as kv_connector_output:
            model_output = self.model(input_ids=input_ids, positions=positions, intermediate_tensors=intermediate_tensors, inputs_embeds=inputs_embeds, **MultiModalKwargs.as_kwargs(model_mm_kwargs, device=self.device))
        ...
        self.eplb_step()
        return ModelRunnerOutput(req_ids=self.input_batch.req_ids, req_id_to_index=self.input_batch.req_id_to_index, sampled_token_ids=valid_sampled_token_ids, spec_token_ids=spec_token_ids, logprobs=logprobs_lists, prompt_logprobs_dict=prompt_logprobs_dict, pooler_output=[], kv_connector_output=kv_connector_output, num_nans_in_logits=num_nans_in_logits)
    def propose_draft_token_ids(self, scheduler_output: 'SchedulerOutput', sampled_token_ids: list[list[int]], sampling_metadata: SamplingMetadata, hidden_states: torch.Tensor, sample_hidden_states: torch.Tensor, aux_hidden_states: Optional[torch.Tensor], spec_decode_metadata: Optional[SpecDecodeMetadata], common_attn_metadata: CommonAttentionMetadata) -> list[list[int]]:
        ...
        if self.speculative_config.method == 'ngram':
            ...
            mm_embeds = None
            if self.is_multimodal_model:
                mm_embeds = self._gather_mm_embeddings(scheduler_output, shift_computed_tokens=1)
            draft_token_ids = self.drafter.propose(target_token_ids=target_token_ids, target_positions=target_positions, target_hidden_states=target_hidden_states, next_token_ids=next_token_ids, sampling_metadata=sampling_metadata, common_attn_metadata=common_attn_metadata, mm_embeds=mm_embeds)
    ...
    def load_model(self, eep_scale_up: bool=False) -> None:
        ...
        logger.info('Model loading took {:.2f} GiB and {:.2f} seconds', self.model_memory_usage / GiB_bytes, time_after_load - time_before_load)
        prepare_communication_buffer_for_model(self.model)
        if is_mixture_of_experts(self.model) and self.parallel_config.enable_eplb:
            logger.info('EPLB is enabled for model {}.', self.model_config.model)
        ...
        if self.aphrodite_config.compilation_config.level == CompilationLevel.DYNAMO_AS_IS and supports_dynamo():
            backend = self.aphrodite_config.compilation_config.init_backend(self.aphrodite_config)
            compilation_counter.dynamo_as_is_count += 1
            self.model.compile(fullgraph=envs.APHRODITE_TEST_DYNAMO_FULLGRAPH_CAPTURE, backend=backend)
    ...
    def maybe_randomize_inputs(self, input_ids: torch.Tensor):
        ...
        if not randomize_inputs:
            ...
            yield
            input_ids.fill_(0)
    def _get_mm_dummy_batch(self, modality: str, max_items_per_batch: int) -> BatchedTensorInputs:
        dummy_decoder_data = self.mm_registry.get_decoder_dummy_data(model_config=self.model_config, seq_len=self.max_num_tokens, mm_counts={modality: 1})
        dummy_mm_data = dummy_decoder_data.multi_modal_data
        dummy_mm_item = dummy_mm_data.get_item(modality=modality, item_index=0)
        dummy_mm_kwargs = MultiModalKwargs.from_items([dummy_mm_item])
        batched_dummy_mm_inputs = MultiModalKwargs.batch([dummy_mm_kwargs] * max_items_per_batch)
    ...
    def _dummy_run(self, num_tokens: int, capture_attn_cudagraph: bool=False, skip_eplb: bool=False, is_profile: bool=False) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        if capture_attn_cudagraph:
            ...
            self.seq_lens[:num_reqs].copy_(self.seq_lens_cpu[:num_reqs], non_blocking=True)
            for kv_cache_group_id, kv_cache_group_spec in enumerate(self.kv_cache_config.kv_cache_groups):
                common_attn_metadata = CommonAttentionMetadata(query_start_loc=self.query_start_loc[:num_reqs + 1], query_start_loc_cpu=self.query_start_loc_cpu[:num_reqs + 1], seq_lens=self.seq_lens[:num_reqs], seq_lens_cpu=self.seq_lens_cpu[:num_reqs], num_computed_tokens_cpu=self.input_batch.num_computed_tokens_cpu_tensor[:num_reqs], num_reqs=num_reqs, num_actual_tokens=num_tokens, max_query_len=num_tokens, block_table_tensor=self.input_batch.block_table[kv_cache_group_id].get_device_tensor()[:num_reqs], slot_mapping=self.input_batch.block_table[kv_cache_group_id].slot_mapping[:num_tokens], causal=True)
        ...
        with self.maybe_dummy_run_with_lora(self.lora_config, num_scheduled_tokens):
            ...
            with self.maybe_randomize_inputs(input_ids), set_forward_context(attn_metadata, self.aphrodite_config, num_tokens=num_tokens, num_tokens_across_dp=num_tokens_across_dp):
                outputs = self.model(input_ids=input_ids, positions=positions, intermediate_tensors=intermediate_tensors, inputs_embeds=inputs_embeds, **MultiModalKwargs.as_kwargs(model_mm_kwargs, device=self.device))
            if self.use_aux_hidden_state_outputs:
    ...
    def _dummy_sampler_run(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        dummy_metadata = SamplingMetadata(temperature=dummy_tensors(0.5), dynatemp_min=None, dynatemp_max=None, dynatemp_exp=None, all_greedy=False, all_random=False, top_p=dummy_tensors(0.9), top_k=dummy_tensors(logits.size(1) - 1), top_a=None, dry_multiplier=None, dry_base=None, dry_allowed_length=None, dry_sequence_breaker_ids=None, dry_ranges=None, dry_max_ngram=None, dry_max_occurrences=None, dry_early_exit_match_len=None, no_repeat_ngram_size=None, tfs=None, eta_cutoff=None, epsilon_cutoff=None, typical_p=None, quadratic_smoothing_factor=None, quadratic_smoothing_curve=None, xtc_threshold=None, xtc_probability=None, top_nsigma=None, skew=None, generators={}, max_num_logprobs=None, no_penalties=True, prompt_token_ids=None, frequency_penalties=dummy_tensors(0.1), presence_penalties=dummy_tensors(0.1), repetition_penalties=dummy_tensors(0.1), output_token_ids=[[] for _ in range(num_reqs)], allowed_token_ids_mask=None, bad_words_token_ids={}, logitsprocs=LogitsProcessorManager())
        try:
    ...
    def _dummy_pooler_run_task(self, hidden_states: torch.Tensor, task: PoolingTask) -> PoolerOutput:
        ...
        dummy_prompt_lens = torch.tensor([h.shape[0] for h in hidden_states_list], device=self.device)
        dummy_token_ids = torch.zeros((num_reqs, req_num_tokens), dtype=torch.int32, device=self.device)
        model = cast(AphroditeModelForPooling, self.model)
        dummy_pooling_params = PoolingParams(task=task)
        to_update = model.pooler.get_pooling_updates(task)
        to_update.apply(dummy_pooling_params)
        dummy_metadata = PoolingMetadata(prompt_lens=dummy_prompt_lens, prompt_token_ids=dummy_token_ids, pooling_params=[dummy_pooling_params] * num_reqs)
        try:
            return model.pooler(hidden_states=hidden_states_list, pooling_metadata=dummy_metadata)
        except RuntimeError as e:
            if 'out of memory' in str(e):
                raise RuntimeError(f'CUDA out of memory occurred when warming up pooler (task={task!r}) with {num_reqs} dummy requests. Please try lowering `max_num_seqs` or `gpu_memory_utilization` when initializing the engine.') from e
            else:
                raise e
    @torch.inference_mode()
    ...
    def profile_run(self) -> None:
        if self.is_multimodal_model:
            ...
            if (encoder_budget := mm_budget.get_encoder_budget()) > 0:
                dummy_modality, max_tokens = mm_budget.get_modality_with_max_tokens()
                max_mm_items_per_prompt, max_mm_items_per_batch = mm_budget.get_max_items(dummy_modality, max_tokens)
                logger.info('Encoder cache will be initialized with a budget of {} tokens, and profiled with {} {} items of the maximum feature size.', encoder_budget, max_mm_items_per_batch, dummy_modality)
                batched_dummy_mm_inputs = self._get_mm_dummy_batch(dummy_modality, max_mm_items_per_batch)
                dummy_encoder_outputs = self.model.get_multimodal_embeddings(**batched_dummy_mm_inputs)
                sanity_check_mm_encoder_outputs(dummy_encoder_outputs, expected_num_items=max_mm_items_per_batch)
    ...
    def capture_model(self) -> None:
        if not self.use_cuda_graph:
            logger.warning('Skipping CUDA graph capture. To turn on CUDA graph capture, set -O {} and ensure `use_cudagraph` was not manually set to False', CompilationLevel.PIECEWISE)
            return
        ...
        with freeze_gc(), graph_capture(device=self.device):
            ...
            if is_global_first_rank() and self.load_config.use_tqdm_on_load:
                with Progress(SpinnerColumn(), TextColumn('[bold blue]{task.description}'), BarColumn(), TextColumn('[progress.percentage]{task.percentage:>3.0f}%'), TimeElapsedColumn()) as progress:
                    task = progress.add_task('Capturing CUDA graph shapes', total=len(compilation_cases))
    ...
    def _initialize_single_attn_backend(self, kv_cache_spec: KVCacheSpec, layer_names: list[str]) -> tuple[AttentionBackend, AttentionMetadataBuilder]:
        ...
        attn_metadata_builder_i = attn_backend_i.get_builder_cls()(kv_cache_spec, layer_names, self.aphrodite_config, self.device)
        if self.full_cuda_graph:
            if attn_metadata_builder_i.attn_cudagraph_support == AttentionCGSupport.NEVER:
            ...
            if attn_metadata_builder_i.attn_cudagraph_support == AttentionCGSupport.PURE_DECODE_ONLY:
                self.cudagraph_batch_sizes = [size for size in self.cudagraph_batch_sizes if size <= self.scheduler_config.max_num_seqs]
    ...
    def initialize_attn_backend(self, kv_cache_config: KVCacheConfig) -> None:
        ...
        for i, kv_cache_group_spec in enumerate(kv_cache_config.kv_cache_groups):
            ...
            attn_backend_i, attn_metadata_builder_i = self._initialize_single_attn_backend(kv_cache_spec, kv_cache_group_spec.layer_names)
        ...
        for attn_module in attn_layers.values():
            if attn_module.attn_type == AttentionType.ENCODER_ONLY:
                ...
                attn_specs.append(FullAttentionSpec(block_size=block_size, num_kv_heads=attn_module.num_kv_heads, head_size=attn_module.head_size, dtype=self.kv_cache_dtype, use_mla=use_mla))
            else:
                raise ValueError('Expected only encoder-only layers')
        if len(attn_specs) > 0:
            assert len(attn_specs) == len(attn_layers), 'All or none of the layers are expected to be encoder-only'
            attn_backend, attn_metadata_builder = self._initialize_single_attn_backend(attn_specs[0], attn_layers.keys())
            ...
            if reorder_batch_threshold_i is not None:
                if self.reorder_batch_threshold is not None:
                    if reorder_batch_threshold_i != self.reorder_batch_threshold:
                        raise ValueError(f'Attention backend reorders decodes with threshold {reorder_batch_threshold_i} but other backend uses threshold {self.reorder_batch_threshold}')
                else:
    ...
    def may_reinitialize_input_batch(self, kv_cache_config: KVCacheConfig) -> None:
        ...
        if block_sizes != [self.cache_config.block_size]:
            assert self.cache_config.cpu_offload_gb == 0, 'Cannot re-initialize the input batch when CPU weight offloading is enabled. See https://github.com/aphrodite-project/aphrodite/pull/18298 for more details.'
            self.input_batch = InputBatch(max_num_reqs=self.max_num_reqs, max_model_len=self.max_model_len, max_num_batched_tokens=self.max_num_tokens, device=self.device, pin_memory=self.pin_memory, vocab_size=self.model_config.get_vocab_size(), block_sizes=block_sizes, is_spec_decode=bool(self.aphrodite_config.speculative_config))
            ...
            for layer_name in kv_cache_group_spec.layer_names:
                ...
                if isinstance(kv_cache_spec, AttentionSpec):
                    ...
                    if kv_cache_shape[0] != num_blocks or kv_cache_shape[1] != 2:
                        raise ValueError('Hybrid models in V1 require an attention backend with kv_cache_shape=(num_blocks, 2, ...). Please try setting APHRODITE_ATTENTION_BACKEND=FLASHINFER')
    ...
    def get_kv_cache_spec(self) -> dict[str, KVCacheSpec]:
        ...
        for layer_name, attn_module in attn_layers.items():
            ...
            if attn_module.attn_type == AttentionType.DECODER:
                use_local_attention = self.attention_chunk_size is not None and attn_module.use_irope
        ...
        if len(mamba_layers) > 0:
            ...
            page_size_padded = self.aphrodite_config.cache_config.mamba_page_size_padded
            for layer_name, mamba_module in mamba_layers.items():
                kv_cache_spec[layer_name] = MambaSpec(shapes=mamba_module.get_state_shape(), dtype=self.kv_cache_dtype, block_size=max_model_len, page_size_padded=page_size_padded, mamba_type=mamba_module.mamba_type)
        return kv_cache_spec
    def _build_encoder_only_attn_metadata(self, scheduler_output: 'SchedulerOutput') -> tuple[CommonAttentionMetadata, Any]:
        num_reqs = self.input_batch.num_reqs
        total_num_scheduled_tokens = scheduler_output.total_num_scheduled_tokens
        req_ids = self.input_batch.req_ids
        tokens = [scheduler_output.num_scheduled_tokens[i] for i in req_ids]
        ...
        dummy_block_table = torch.zeros((num_reqs, 1), dtype=torch.int32, device=self.device)
        dummy_slot_mapping = torch.zeros((total_num_scheduled_tokens,), dtype=torch.int32, device=self.device)
        common_metadata = CommonAttentionMetadata(query_start_loc=self.query_start_loc[:num_reqs + 1], query_start_loc_cpu=self.query_start_loc_cpu[:num_reqs + 1], seq_lens=self.seq_lens[:num_reqs], seq_lens_cpu=self.seq_lens_cpu[:num_reqs], num_computed_tokens_cpu=self.input_batch.num_computed_tokens_cpu_tensor[:num_reqs], num_reqs=num_reqs, num_actual_tokens=total_num_scheduled_tokens, max_query_len=max_num_scheduled_tokens, block_table_tensor=dummy_block_table, slot_mapping=dummy_slot_mapping, causal=False)
        return (common_metadata, builder.build(common_prefix_len=0, common_attn_metadata=common_metadata))