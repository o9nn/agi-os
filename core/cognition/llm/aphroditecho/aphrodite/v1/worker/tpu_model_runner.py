...
from aphrodite.common.config import AphroditeConfig, ParallelConfig, get_layers_from_aphrodite_config, update_config
...
from aphrodite.utils import LayerBlockType, cdiv, is_pin_memory_available, prev_power_of_2
from aphrodite.v1.attention.backends.pallas import TPU_STR_DTYPE_TO_TORCH_DTYPE, PallasAttentionBackend, PallasMetadata, get_page_size_bytes
from aphrodite.v1.kv_cache_interface import AttentionSpec, FullAttentionSpec, KVCacheConfig, KVCacheSpec, SlidingWindowSpec
...
from .utils import MultiModalBudget, bind_kv_cache, initialize_kv_cache_for_kv_sharing, sanity_check_mm_encoder_outputs
if TYPE_CHECKING:
    from aphrodite.v1.core.sched.output import SchedulerOutput
INVALID_TOKEN_ID = -1
MIN_NUM_SEQS = 8
class TPUModelRunner(LoRAModelRunnerMixin, KVConnectorModelRunnerMixin):
    def __init__(self, aphrodite_config: AphroditeConfig, device: torch.device, original_parallel_config: Optional[ParallelConfig]=None):
        ...
        self.num_tokens_paddings = _get_token_paddings(min_token_size=16, max_token_size=scheduler_config.max_num_batched_tokens, padding_gap=envs.APHRODITE_TPU_BUCKET_PADDING_GAP)
    ...
    def _update_num_xla_graphs(self, case_str):
        ...
        if new_compiled_graphs == 0:
            return
        logger.info('Add new {} compiled XLA graphs due to {}', new_compiled_graphs, case_str)
        ...
        for layer_name, attn_module in layers.items():
            ...
            if attn_module.attn_type == AttentionType.DECODER:
                if attn_module.use_irope:
                    log_once('WARNING', 'Using irope in Pallas is not supported yet, it will fall back to global attention for long context.')
    ...
    def execute_model(self, scheduler_output: 'SchedulerOutput', intermediate_tensors: Optional[IntermediateTensors]=None) -> ModelRunnerOutput:
        ...
        while start_index < self.input_batch.num_reqs:
            ...
            xm.mark_step()
            with set_forward_context(attn_metadata, self.aphrodite_config, num_tokens=scheduler_output.total_num_scheduled_tokens):
        ...
        model_runner_output = ModelRunnerOutput(req_ids=req_ids, req_id_to_index=self.input_batch.req_id_to_index, sampled_token_ids=valid_sampled_token_ids, spec_token_ids=None, logprobs=logprobs_lists, prompt_logprobs_dict=prompt_logprobs_dict, pooler_output=[], kv_connector_output=KVConnectorOutput(finished_sending=finished_sending, finished_recving=finished_recving))
    ...
    def load_model(self) -> None:
        ...
        with patch('aphrodite.modeling.layers.vocab_parallel_embedding.get_tensor_model_parallel_rank', return_value=xm_tp_rank):
            try:
                if self.use_spmd:
                    tpu_loader = TPUModelLoader(load_config=self.aphrodite_config.load_config)
                    model = tpu_loader.load_model(aphrodite_config=self.aphrodite_config, model_config=self.aphrodite_config.model_config, mesh=self.mesh)
                else:
                    model_loader = get_model_loader(self.load_config)
                    logger.info('Loading model from scratch...')
                    model = model_loader.load_model(aphrodite_config=self.aphrodite_config, model_config=self.model_config)
    ...
    def _dummy_run(self, num_tokens: int, num_reqs: int, num_blocks: int) -> None:
        ...
        layer_names = get_layers_from_aphrodite_config(self.aphrodite_config, Attention).keys()
        per_layer_attn_metadata = {layer_name: attn_metadata for layer_name in layer_names}
        with self.maybe_select_dummy_loras(self.lora_config, np.array([num_tokens], dtype=np.int32)), set_forward_context(per_layer_attn_metadata, self.aphrodite_config, 0):
    ...
    def _precompile_mm_encoder(self) -> None:
        ...
        max_items_per_seq_by_modality = mm_budget.max_items_per_batch_by_modality
        for mode, max_items_per_seq in max_items_per_seq_by_modality.items():
            logger.info('Compiling Multimodal {} Encoder with different input shapes.', mode)
            ...
            logger.info('Multimodal {} Encoder compilation finished in in {:.2f} [secs].', mode, end - start)
    ...
    def _precompile_select_hidden_states(self) -> None:
        ...
        for num_tokens in self.num_tokens_paddings:
            ...
            for num_reqs in self.num_reqs_paddings:
                ...
                logger.info('  -- num_tokens: {}, num_seqs: {}', num_tokens, num_reqs)
    ...
    def initialize_kv_cache(self, kv_cache_config: KVCacheConfig) -> None:
        ...
        bind_kv_cache(kv_caches, self.aphrodite_config.compilation_config.static_forward_context, self.kv_caches)
        if self.use_spmd:
...
def _make_src_and_dst_indices(src_block_ids: list[int], dst_block_ids: list[int], src_device: Union[torch.device, str], dst_device: Union[torch.device, str]) -> tuple[torch.Tensor, torch.Tensor]:
...
def _insert_blocks_to_tpu(cpu_cache: torch.Tensor, tpu_cache: torch.Tensor, cpu_block_indices: torch.Tensor, tpu_block_indices: torch.Tensor) -> None:
...
@torch.compile(backend='openxla')
def _swap_out_tpu_blocks(tpu_cache: torch.Tensor, cpu_cache: torch.Tensor, tpu_block_indices: torch.Tensor, cpu_block_indices: torch.Tensor) -> None:
    torch.ops.xla.dynamo_set_buffer_donor_(tpu_cache, True)
    cpu_cache[cpu_block_indices] = tpu_cache[tpu_block_indices].cpu()
def copy_kv_blocks(src_kv_caches: dict[str, torch.Tensor], dst_kv_caches: dict[str, torch.Tensor], src_block_ids: list[int], dst_block_ids: list[int], direction: Literal['h2d', 'd2h']) -> None:
    if not src_kv_caches or not dst_kv_caches or (not src_block_ids) or (not dst_block_ids) or (len(src_block_ids) != len(dst_block_ids)):
        return
    src_device = next(iter(src_kv_caches.values())).device
    dst_device = next(iter(dst_kv_caches.values())).device
    src_indices, dst_indices = _make_src_and_dst_indices(src_block_ids=src_block_ids, dst_block_ids=dst_block_ids, src_device=src_device, dst_device=dst_device)
