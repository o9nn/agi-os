...
from aphrodite.v1.sample.logits_processor import BatchUpdateBuilder, MoveDirectionality, init_builtin_logitsprocs
...
class InputBatch:
    def __init__(self, max_num_reqs: int, max_model_len: int, max_num_batched_tokens: int, device: torch.device, pin_memory: bool, vocab_size: int, block_sizes: list[int], is_spec_decode: bool=False):
        ...
        self.dynatemp_min_cpu_tensor = torch.empty((max_num_reqs,), dtype=torch.float32, device='cpu', pin_memory=pin_memory)
        ...
        self.dynatemp_max_cpu_tensor = torch.empty((max_num_reqs,), dtype=torch.float32, device='cpu', pin_memory=pin_memory)
        ...
        self.dynatemp_exp_cpu_tensor = torch.empty((max_num_reqs,), dtype=torch.float32, device='cpu', pin_memory=pin_memory)
        ...
        self.eta_cutoff_cpu_tensor = torch.empty((max_num_reqs,), dtype=torch.float32, device='cpu', pin_memory=pin_memory)
        ...
        self.epsilon_cutoff_cpu_tensor = torch.empty((max_num_reqs,), dtype=torch.float32, device='cpu', pin_memory=pin_memory)
        ...
        self.typical_p_cpu_tensor = torch.empty((max_num_reqs,), dtype=torch.float32, device='cpu', pin_memory=pin_memory)
        ...
        self.quadratic_smoothing_factor_cpu_tensor = torch.empty((max_num_reqs,), dtype=torch.float32, device='cpu', pin_memory=pin_memory)
        ...
        self.quadratic_smoothing_curve_cpu_tensor = torch.empty((max_num_reqs,), dtype=torch.float32, device='cpu', pin_memory=pin_memory)
        ...
        self.xtc_threshold_cpu_tensor = torch.empty((max_num_reqs,), dtype=torch.float32, device='cpu', pin_memory=pin_memory)
        ...
        self.xtc_probability_cpu_tensor = torch.empty((max_num_reqs,), dtype=torch.float32, device='cpu', pin_memory=pin_memory)
        ...
        self.top_nsigma_cpu_tensor = torch.empty((max_num_reqs,), dtype=torch.float32, device='cpu', pin_memory=pin_memory)
        ...
        self.dry_ranges_cpu_tensor = torch.empty((max_num_reqs,), dtype=torch.int32, device='cpu', pin_memory=pin_memory)
        ...
        self.dry_max_ngram_cpu_tensor = torch.empty((max_num_reqs,), dtype=torch.int32, device='cpu', pin_memory=pin_memory)
        ...
        self.dry_max_occurrences_cpu_tensor = torch.empty((max_num_reqs,), dtype=torch.int32, device='cpu', pin_memory=pin_memory)
        ...
        self.dry_early_exit_match_len_cpu_tensor = torch.empty((max_num_reqs,), dtype=torch.int32, device='cpu', pin_memory=pin_memory)
        ...
        self.no_repeat_ngram_size_cpu_tensor = torch.empty((max_num_reqs,), dtype=torch.int32, device='cpu', pin_memory=pin_memory)
    ...
    def swap_states(self, i1: int, i2: int) -> None:
        ...
        self.epsilon_cutoff_cpu[i1], self.epsilon_cutoff_cpu[i2] = (self.epsilon_cutoff_cpu[i2], self.epsilon_cutoff_cpu[i1])
        self.typical_p_cpu[i1], self.typical_p_cpu[i2] = (self.typical_p_cpu[i2], self.typical_p_cpu[i1])
        self.quadratic_smoothing_factor_cpu[i1], self.quadratic_smoothing_factor_cpu[i2] = (self.quadratic_smoothing_factor_cpu[i2], self.quadratic_smoothing_factor_cpu[i1])
        self.quadratic_smoothing_curve_cpu[i1], self.quadratic_smoothing_curve_cpu[i2] = (self.quadratic_smoothing_curve_cpu[i2], self.quadratic_smoothing_curve_cpu[i1])
        self.xtc_threshold_cpu[i1], self.xtc_threshold_cpu[i2] = (self.xtc_threshold_cpu[i2], self.xtc_threshold_cpu[i1])
        self.xtc_probability_cpu[i1], self.xtc_probability_cpu[i2] = (self.xtc_probability_cpu[i2], self.xtc_probability_cpu[i1])
        ...
        self.dry_max_occurrences_cpu[i1], self.dry_max_occurrences_cpu[i2] = (self.dry_max_occurrences_cpu[i2], self.dry_max_occurrences_cpu[i1])
        self.dry_early_exit_match_len_cpu[i1], self.dry_early_exit_match_len_cpu[i2] = (self.dry_early_exit_match_len_cpu[i2], self.dry_early_exit_match_len_cpu[i1])
        self.no_repeat_ngram_size_cpu[i1], self.no_repeat_ngram_size_cpu[i2] = (self.no_repeat_ngram_size_cpu[i2], self.no_repeat_ngram_size_cpu[i1])
        ...
        if any(self.sampler_priority[:num_reqs]):
            for priority_list in self.sampler_priority[:num_reqs]:
                if priority_list is not None:
                    ...
                    break
        return SamplingMetadata(temperature=temperature, dynatemp_min=None if self.no_dynatemp else self.dynatemp_min[:num_reqs], dynatemp_max=None if self.no_dynatemp else self.dynatemp_max[:num_reqs], dynatemp_exp=None if self.no_dynatemp else self.dynatemp_exp[:num_reqs], all_greedy=self.all_greedy, all_random=self.all_random, top_p=None if self.no_top_p else self.top_p[:num_reqs], top_k=None if self.no_top_k else self.top_k[:num_reqs], top_a=None if self.no_top_a else self.top_a[:num_reqs], dry_multiplier=None if self.no_dry else self.dry_multiplier[:num_reqs], dry_base=None if self.no_dry else self.dry_base[:num_reqs], dry_allowed_length=None if self.no_dry else self.dry_allowed_length[:num_reqs], dry_sequence_breaker_ids=None if self.no_dry else self._make_dry_sequence_breaker_ids_tensor(num_reqs), dry_ranges=None if self.no_dry else self.dry_ranges[:num_reqs], dry_max_ngram=None if self.no_dry else self.dry_max_ngram[:num_reqs], dry_max_occurrences=None if self.no_dry else self.dry_max_occurrences[:num_reqs], dry_early_exit_match_len=None if self.no_dry else self.dry_early_exit_match_len[:num_reqs], no_repeat_ngram_size=None if self.no_no_repeat_ngram else self.no_repeat_ngram_size[:num_reqs], tfs=None if self.no_tfs else self.tfs[:num_reqs], eta_cutoff=None if self.no_eta_cutoff else self.eta_cutoff[:num_reqs], epsilon_cutoff=None if self.no_epsilon_cutoff else self.epsilon_cutoff[:num_reqs], typical_p=None if self.no_typical_p else self.typical_p[:num_reqs], quadratic_smoothing_factor=None if self.no_quadratic else self.quadratic_smoothing_factor[:num_reqs], quadratic_smoothing_curve=None if self.no_quadratic else self.quadratic_smoothing_curve[:num_reqs], xtc_threshold=None if self.no_xtc else self.xtc_threshold[:num_reqs], xtc_probability=None if self.no_xtc else self.xtc_probability[:num_reqs], top_nsigma=None if self.no_top_nsigma else self.top_nsigma[:num_reqs], skew=None if self.no_skew else self.skew[:num_reqs], generators=self.generators, max_num_logprobs=self.max_num_logprobs, prompt_token_ids=prompt_token_ids, frequency_penalties=self.frequency_penalties[:num_reqs], presence_penalties=self.presence_penalties[:num_reqs], repetition_penalties=self.repetition_penalties[:num_reqs], output_token_ids=cast(list[list[int]], self.req_output_token_ids), no_penalties=self.no_penalties, allowed_token_ids_mask=allowed_token_ids_mask, bad_words_token_ids=self.bad_words_token_ids, sampler_priority=sampler_priority_processed, temperature_last=any(self.temperature_last[:num_reqs]), logitsprocs=self.logitsprocs)
    @property
