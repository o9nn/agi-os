...
class TPUWorker:
    def __init__(self, aphrodite_config: AphroditeConfig, local_rank: int, rank: int, distributed_init_method: str, is_driver_worker: bool=False):
        ...
        if envs.APHRODITE_TORCH_PROFILER_DIR and self.rank < 1:
            ...
            logger.info('Profiling enabled. Traces will be saved to: {}', self.profile_dir)
    ...
    def init_device(self):
        ...
        self._init_tpu_worker_distributed_environment(self.aphrodite_config, self.rank, self.distributed_init_method, self.local_rank)
    ...
    def determine_available_memory(self) -> int:
        ...
        bind_kv_cache(kv_caches, self.aphrodite_config.compilation_config.static_forward_context, runner_kv_caches)
        ...
        if head_size > 0:
            ...
            if padded_head_size != head_size:
                log_once('WARNING', 'head size is padded to {}', padded_head_size)
    ...
    def check_health(self) -> None:
        return
    def _init_tpu_worker_distributed_environment(self, aphrodite_config: AphroditeConfig, rank: int, distributed_init_method: Optional[str]=None, local_rank: int=-1) -> None:
        if self.use_spmd:
            xr.use_spmd()
