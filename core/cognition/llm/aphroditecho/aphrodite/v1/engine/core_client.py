...
from aphrodite.utils import get_open_port, get_open_zmq_inproc_path, make_zmq_socket
from aphrodite.v1.engine import EngineCoreOutputs, EngineCoreRequest, EngineCoreRequestType, ReconfigureDistributedRequest, ReconfigureRankType, UtilityOutput
...
class EngineCoreClient(ABC):
    @staticmethod
    def make_client(multiprocess_mode: bool, asyncio_mode: bool, aphrodite_config: AphroditeConfig, executor_class: type[Executor], log_stats: bool) -> 'EngineCoreClient':
    ...
    @staticmethod
    def make_async_mp_client(aphrodite_config: AphroditeConfig, executor_class: type[Executor], log_stats: bool, client_addresses: Optional[dict[str, str]]=None, client_count: int=1, client_index: int=0) -> 'MPClient':
        parallel_config = aphrodite_config.parallel_config
        client_args = (aphrodite_config, executor_class, log_stats, client_addresses, client_count, client_index)
...
class MPClient(EngineCoreClient):
    def __init__(self, asyncio_mode: bool, aphrodite_config: AphroditeConfig, executor_class: type[Executor], log_stats: bool, client_addresses: Optional[dict[str, str]]=None):
        ...
        try:
            ...
            if client_addresses is not None:
                ...
                with launch_core_engines(aphrodite_config, executor_class, log_stats) as (engine_manager, coordinator, addresses):
    ...
    def start_engine_core_monitor(self):
        ...
        def monitor_engine_cores():
            ...
            logger.error('Engine core proc {} died unexpectedly, shutting down client.', proc_name)
            _self.shutdown()
...
class SyncMPClient(MPClient):
    def __init__(self, aphrodite_config: AphroditeConfig, executor_class: type[Executor], log_stats: bool):
        super().__init__(asyncio_mode=False, aphrodite_config=aphrodite_config, executor_class=executor_class, log_stats=log_stats)
...
class AsyncMPClient(MPClient):
    def __init__(self, aphrodite_config: AphroditeConfig, executor_class: type[Executor], log_stats: bool, client_addresses: Optional[dict[str, str]]=None, client_count: int=1, client_index: int=0):
        super().__init__(asyncio_mode=True, aphrodite_config=aphrodite_config, executor_class=executor_class, log_stats=log_stats, client_addresses=client_addresses)
...
class DPAsyncMPClient(AsyncMPClient):
    def __init__(self, aphrodite_config: AphroditeConfig, executor_class: type[Executor], log_stats: bool, client_addresses: Optional[dict[str, str]]=None, client_count: int=1, client_index: int=0):
        self.current_wave = 0
        super().__init__(aphrodite_config, executor_class, log_stats, client_addresses, client_count, client_index)
            ...
            with make_zmq_socket(self.ctx, self.stats_update_address, zmq.XSUB) as socket, make_zmq_socket(self.ctx, self.first_req_sock_addr, zmq.PAIR, bind=False) as first_req_rcv_socket:
                ...
                while True:
                    ...
                    if counts is not None:
                        ...
                        logger.debug('Received counts: {} ({})', sliced_counts, count_slice)
...
class DPLBAsyncMPClient(DPAsyncMPClient):
    ...
    async def scale_elastic_ep(self, new_data_parallel_size: int) -> None:
        ...
        assert self.aphrodite_config.parallel_config.data_parallel_backend == 'ray', 'Only ray DP backend supports scaling elastic EP'
    ...
    async def _scale_up_elastic_ep(self, cur_data_parallel_size: int, new_data_parallel_size: int) -> None:
        ...
        for engine in self.core_engines:
            reconfig_request = ReconfigureDistributedRequest(new_data_parallel_size=new_data_parallel_size, new_data_parallel_rank=ReconfigureRankType.KEEP_CURRENT_RANK, new_data_parallel_rank_local=ReconfigureRankType.KEEP_CURRENT_RANK, new_data_parallel_master_ip=self.aphrodite_config.parallel_config.data_parallel_master_ip, new_data_parallel_master_port=self.aphrodite_config.parallel_config.data_parallel_master_port)
        ...
        logger.info('[Elastic EP] Scale up completed, new data parallel size: {}', new_data_parallel_size)
    async def _scale_down_elastic_ep(self, cur_data_parallel_size: int, new_data_parallel_size: int) -> None:
        ...
        self.aphrodite_config.parallel_config.data_parallel_size = new_data_parallel_size
        logger.info('[Elastic EP] Scale down completed, new data parallel size: {}', new_data_parallel_size)