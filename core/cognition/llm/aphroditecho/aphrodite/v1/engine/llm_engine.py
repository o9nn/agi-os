...
from aphrodite.v1.metrics.loggers import PrometheusStatLogger, StatLoggerBase, StatLoggerFactory
...
class LLMEngine:
    def __init__(self, aphrodite_config: AphroditeConfig, executor_class: type[Executor], log_stats: bool, usage_context: UsageContext=UsageContext.ENGINE_CONTEXT, stat_loggers: Optional[list[StatLoggerFactory]]=None, mm_registry: MultiModalRegistry=MULTIMODAL_REGISTRY, use_cached_outputs: bool=False, multiprocess_mode: bool=False) -> None:
        if not envs.APHRODITE_USE_V1:
            raise ValueError('Using V1 LLMEngine, but envs.APHRODITE_USE_V1=False. This should not happen. As a workaround, try using LLMEngine.from_aphrodite_config(...) or explicitly set APHRODITE_USE_V1=0 or 1 and report this issue on Github.')
        if stat_loggers is not None:
            raise NotImplementedError('Passing StatLoggers to LLMEngine in V1 is not yet supported. Set APHRODITE_USE_V1=0 and file and issue on Github.')
        ...
        self.engine_core = EngineCoreClient.make_client(multiprocess_mode=multiprocess_mode, asyncio_mode=False, aphrodite_config=aphrodite_config, executor_class=executor_class, log_stats=self.log_stats)
        if not multiprocess_mode:
            self.model_executor = self.engine_core.engine_core.model_executor
        self.reset_mm_cache()
    @classmethod
    def from_aphrodite_config(cls, aphrodite_config: AphroditeConfig, usage_context: UsageContext=UsageContext.ENGINE_CONTEXT, stat_loggers: Optional[list[StatLoggerFactory]]=None, disable_log_stats: bool=False) -> 'LLMEngine':
        return cls(aphrodite_config=aphrodite_config, executor_class=Executor.get_class(aphrodite_config), log_stats=not disable_log_stats, usage_context=usage_context, stat_loggers=stat_loggers, multiprocess_mode=envs.APHRODITE_ENABLE_V1_MULTIPROCESSING)
    @classmethod
    def from_engine_args(cls, engine_args: EngineArgs, usage_context: UsageContext=UsageContext.ENGINE_CONTEXT, stat_loggers: Optional[list[StatLoggerFactory]]=None, enable_multiprocessing: bool=False) -> 'LLMEngine':
        ...
        return cls(aphrodite_config=aphrodite_config, executor_class=executor_class, log_stats=not engine_args.disable_log_stats, usage_context=usage_context, stat_loggers=stat_loggers, multiprocess_mode=enable_multiprocessing)
    ...
    def add_request(self, request_id: str, prompt: PromptType, params: Union[SamplingParams, PoolingParams], arrival_time: Optional[float]=None, lora_request: Optional[LoRARequest]=None, tokenization_kwargs: Optional[dict[str, Any]]=None, trace_headers: Optional[Mapping[str, str]]=None, priority: int=0) -> None:
        ...
        prompt_str, request = self.processor.process_inputs(request_id, prompt, params, arrival_time, lora_request, tokenization_kwargs, trace_headers, priority)
