...
class Processor:
    def __init__(self, aphrodite_config: AphroditeConfig, tokenizer: TokenizerGroup, mm_registry: MultiModalRegistry=MULTIMODAL_REGISTRY):
    ...
    def _validate_logprobs(self, params: SamplingParams) -> None:
        ...
        if max_logprobs == -1:
            return
        if params.logprobs and (params.logprobs == -1 or params.logprobs > max_logprobs):
    ...
    def _validate_structured_output(self, params: SamplingParams) -> None:
        ...
        if params.guided_decoding.backend:
            if params.guided_decoding.backend != engine_level_backend and (not (engine_level_backend == 'auto' and params.guided_decoding.backend_was_auto)):
                raise ValueError(f"Request-level structured output backend selection is no longer supported. The request specified '{params.guided_decoding.backend}', but Aphrodite was initialised with '{engine_level_backend}'. This error can be resolved by removing backend selection from the request.")
        else:
    ...
    def process_inputs(self, request_id: str, prompt: PromptType, params: Union[SamplingParams, PoolingParams], arrival_time: Optional[float]=None, lora_request: Optional[LoRARequest]=None, tokenization_kwargs: Optional[dict[str, Any]]=None, trace_headers: Optional[Mapping[str, str]]=None, priority: int=0, data_parallel_rank: Optional[int]=None) -> tuple[Optional[str], EngineCoreRequest]:
        ...
        processed_inputs: ProcessorInputs = self.input_preprocessor.preprocess(prompt, tokenization_kwargs=tokenization_kwargs, lora_request=lora_request, return_mm_hashes=self.use_hash)
