...
from aphrodite.multimodal.inputs import BaseMultiModalField, MultiModalBatchedField, MultiModalFieldConfig, MultiModalFieldElem, MultiModalFlatField, MultiModalKwargs, MultiModalKwargsItem, MultiModalSharedField, NestedTensors
...
def _log_insecure_serialization_warning():
    log_once('WARNING', 'Allowing insecure serialization using pickle due to APHRODITE_ALLOW_INSECURE_SERIALIZATION=1')
    ...
    def enc_hook(self, obj: Any) -> Any:
        ...
        if isinstance(obj, UtilityResult):
            ...
            if not envs.APHRODITE_ALLOW_INSECURE_SERIALIZATION:
                return (None, result)
            return (_typestr(result) if type(result) is not list else [_typestr(v) for v in result], result)
        if not envs.APHRODITE_ALLOW_INSECURE_SERIALIZATION:
            raise TypeError(f'Object of type {type(obj)} is not serializableSet APHRODITE_ALLOW_INSECURE_SERIALIZATION=1 to allow fallback to pickle-based serialization.')
    ...
    def _decode_utility_result(self, obj: Any) -> UtilityResult:
        ...
        if result_type is not None:
            ...
            if len(result_type) == 2 and isinstance(result_type[0], str):
                ...
                result = [self._convert_result(rt, r) for rt, r in zip(result_type, result)]
