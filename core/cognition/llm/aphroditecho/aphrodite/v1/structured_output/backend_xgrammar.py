...
from aphrodite.v1.structured_output.backend_types import StructuredOutputBackend, StructuredOutputGrammar, StructuredOutputOptions
...
class XgrammarBackend(StructuredOutputBackend):
    def __post_init__(self):
        self.disable_any_whitespace = self.aphrodite_config.decoding_config.disable_any_whitespace
        ...
        self.compiler = xgr.GrammarCompiler(tokenizer_info, max_threads=8, cache_enabled=True, cache_limit_bytes=aphrodite.common.envs.APHRODITE_XGRAMMAR_CACHE_MB * 1024 * 1024)
    ...
    def accept_tokens(self, request_id: str, tokens: list[int]) -> bool:
        ...
        for token in tokens:
            if not self.matcher.accept_token(token):
                logger.error('Failed to advance FSM for request {} for tokens {}. Please file an issue.', request_id, token)
                return False
