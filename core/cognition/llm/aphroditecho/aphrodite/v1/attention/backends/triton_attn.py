...
class TritonAttentionMetadataBuilder(AttentionMetadataBuilder[TritonAttentionMetadata]):
    attn_cudagraph_support: ClassVar[AttentionCGSupport] = AttentionCGSupport.ALWAYS
...
def use_aiter_unified_attention() -> bool:
    return envs.APHRODITE_ROCM_USE_AITER and envs.APHRODITE_USE_AITER_UNIFIED_ATTENTION
class TritonAttentionImpl(AttentionImpl):
    def __init__(self, num_heads: int, head_size: int, scale: float, num_kv_heads: int, alibi_slopes: Optional[list[float]], sliding_window: Optional[int], kv_cache_dtype: str, logits_soft_cap: Optional[float]=None, attn_type: AttentionType=AttentionType.DECODER, kv_sharing_target_layer_name: Optional[int]=None, sinks: Optional[torch.Tensor]=None) -> None:
        ...
        self.force_prefill_decode_attn = envs.APHRODITE_V1_USE_PREFILL_DECODE_ATTENTION
        if not self.force_prefill_decode_attn:
            if use_aiter_unified_attention():
                log_once('INFO', 'Using aiter unified attention for TritonAttentionImpl')
                from aiter.ops.triton.unified_attention import unified_attention
                self.unified_attention = unified_attention
            else:
                log_once('INFO', 'Using aphrodite unified attention for TritonAttentionImpl')
                from aphrodite.attention.ops.triton_unified_attention import unified_attention
                self.unified_attention = unified_attention
        self.sinks = sinks
        if sinks is not None:
            assert sinks.shape[0] == num_heads, f'Sinks must have the same number of heads as the number of heads in the layer. Sinks shape: {sinks.shape}, num_heads: {num_heads}.'
