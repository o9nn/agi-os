...
class Scheduler(SchedulerInterface):
    def __init__(self, aphrodite_config: AphroditeConfig, kv_cache_config: KVCacheConfig, structured_output_manager: StructuredOutputManager, mm_registry: MultiModalRegistry=MULTIMODAL_REGISTRY, include_finished_set: bool=False, log_stats: bool=False) -> None:
        ...
        if self.aphrodite_config.kv_transfer_config is not None:
            ...
            self.connector = KVConnectorFactory.create_connector(config=self.aphrodite_config, role=KVConnectorRole.SCHEDULER)
        ...
        encoder_compute_budget, encoder_cache_size = compute_encoder_budget(model_config=aphrodite_config.model_config, scheduler_config=aphrodite_config.scheduler_config, mm_registry=mm_registry)
            ...
            while self.waiting and token_budget > 0:
                ...
                if request.status == RequestStatus.WAITING_FOR_REMOTE_KVS:
                    ...
                    if is_ready:
                        ...
                        logger.debug('{} is still in WAITING_FOR_REMOTE_KVS state.', request.request_id)
