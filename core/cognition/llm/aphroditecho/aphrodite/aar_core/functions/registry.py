...
class FunctionRegistry:
    ...
    def register(self, fn: Callable[..., Any], *, name: str, description: str, params_schema: Dict[str, Any], safety_class: str='low', cost_unit: float=0.0, tags: List[str] | None=None) -> None:
        ...
        self._functions[name] = RegisteredFunction(name=name, description=description, params_schema=params_schema, safety_class=safety_class, cost_unit=cost_unit, callable=fn, tags=tags or [])
    def has(self, name: str) -> bool:
        return name in self._functions
    def list_functions(self) -> List[Dict[str, Any]]:
        return [{'name': f.name, 'description': f.description, 'safety_class': f.safety_class, 'cost_unit': f.cost_unit, 'tags': f.tags, 'params_schema': f.params_schema} for f in self._functions.values()]
