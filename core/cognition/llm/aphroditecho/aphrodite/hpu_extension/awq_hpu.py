...
class AWQHPULinearMethod(LinearMethodBase):
    ...
    def create_weights(self, layer: torch.nn.Module, input_size_per_partition: int, output_partition_sizes: List[int], input_size: int, output_size: int, params_dtype: torch.dtype, **extra_weight_attrs):
        ...
        qweight = PackedAphroditeParameter(data=torch.empty(input_size_per_partition, output_size_per_partition // self.quant_config.pack_factor, dtype=torch.int32), input_dim=0, output_dim=1, packed_dim=1, packed_factor=self.quant_config.pack_factor, weight_loader=weight_loader)
        qzeros = PackedAphroditeParameter(data=torch.empty(input_size_per_partition // self.quant_config.group_size, output_size_per_partition // self.quant_config.pack_factor, dtype=torch.int32), input_dim=0, output_dim=1, packed_dim=1, packed_factor=self.quant_config.pack_factor, weight_loader=weight_loader)
        scales = GroupQuantScaleParameter(data=torch.empty(input_size_per_partition // self.quant_config.group_size, output_size_per_partition, dtype=params_dtype), input_dim=0, output_dim=1, weight_loader=weight_loader)
    ...
    def pack_tensor(self, x):
        ...
        xp = torch.sum(torch.bitwise_left_shift(x.reshape(x.shape[0], -1, 32 // self.quant_config.weight_bits), wf.unsqueeze(0)), dim=-1).to(torch.int32)
        return xp
    def unpack_tensor(self, xp):
        wf = torch.tensor(list(range(0, 32, self.quant_config.weight_bits)), dtype=torch.int32).unsqueeze(0)
        x = torch.bitwise_right_shift(torch.unsqueeze(xp, -1).expand(xp.shape[0], -1, 32 // self.quant_config.weight_bits), wf.unsqueeze(0)).to(torch.int8)
        x = torch.bitwise_and(x, 2 ** self.quant_config.weight_bits - 1)
        x = x.reshape((x.shape[0], -1))
        return x
    def awq_order(self, x):
        order = [0, 4, 1, 5, 2, 6, 3, 7]
        idx = torch.arange(x.shape[-1], dtype=torch.int32, device=x.device)
