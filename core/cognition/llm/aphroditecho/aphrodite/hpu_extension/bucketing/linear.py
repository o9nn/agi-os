...
class HPUBucketingContext(metaclass=WeakSingleton):
    ...
    def _setup_buckets(self) -> None:
        ...
        self.global_state.prompt_bs_bucket_cfg = read_bucket_settings('prompt', 'bs', min=1, step=32, max=self.max_num_prefill_seqs)
        self.global_state.decode_bs_bucket_cfg = read_bucket_settings('decode', 'bs', min=1, step=32, max=self.max_num_seqs)
        self.global_state.prompt_seq_bucket_cfg = read_bucket_settings('prompt', 'seq', min=self.block_size, step=self.block_size, max=max_prompt_seq)
        self.global_state.decode_block_bucket_cfg = read_bucket_settings('decode', 'block', min=self.block_size, step=self.block_size, max=max_blocks)
        if self.use_merged_prefill:
            ...
            print(f'Merged prefill is enabled!\nOverriding prompt bucketing settings!\nprompt bs cfg: {prev_prompt_bs_bucket_cfg} -> {new_prompt_bs_bucket_cfg}\nprompt seq cfg: {prev_prompt_seq_bucket_cfg} -> {new_prompt_seq_bucket_cfg}\n')
        msg = f'Prompt bucket config (min, step, max_warmup) bs:{self.global_state.prompt_bs_bucket_cfg}, seq:{self.global_state.prompt_seq_bucket_cfg}'
        logger.info(msg)
        msg = f'Decode bucket config (min, step, max_warmup) bs:{self.global_state.decode_bs_bucket_cfg}, block:{self.global_state.decode_block_bucket_cfg}'
        logger.info(msg)
    def generate_prompt_buckets(self):
        self.global_state.prompt_buckets, prompt_omitted_buckets = generate_prompt_buckets(self.global_state.prompt_bs_bucket_cfg, self.global_state.prompt_seq_bucket_cfg, self.max_num_batched_tokens)
    ...
    def generate_decode_buckets(self, max_blocks):
        self.global_state.decode_buckets = generate_decode_buckets(self.global_state.decode_bs_bucket_cfg, self.global_state.decode_block_bucket_cfg, max_blocks)
    ...
    def get_max_prompt_shape(self):
        return (self.global_state.prompt_bs_bucket_cfg[-1], self.global_state.prompt_seq_bucket_cfg[-1])
    ...
    def get_padded_decode_num_blocks(self, num_blocks):
        ...
        bucket_size = find_bucket(num_blocks, self.global_state.decode_block_bucket_cfg)
...
def warmup_range(config: Tuple[int, int, int]):
    ...
    assert bmin <= bmax, 'Min. batch size cannot be greater than max. batch size. If you want to skip warmup, set APHRODITE_SKIP_WARMUP=true'
    base = itertools.repeat(2)
    ramp_up_acc = itertools.accumulate(base, func=operator.mul, initial=bmin)
    ramp_up_tw = itertools.takewhile(lambda x: x < bstep and x <= bmax, ramp_up_acc)
    stable = range(bstep, bmax + 1, bstep)
    buckets = list(ramp_up_tw) + list(stable)
    return list(filter(lambda bucket: bucket >= bmin, buckets))
def generate_prompt_buckets(bs_bucket_config, seq_bucket_config, max_num_batched_tokens=None):
    buckets = list(itertools.product(warmup_range(bs_bucket_config), warmup_range(seq_bucket_config)))
    if len(buckets) == 0:
    ...
    if max_num_batched_tokens is not None:
        filtered_buckets = list(filter(lambda bucket: bucket[0] * bucket[1] <= max_num_batched_tokens, buckets))
