...
class GPTQHPUConfig(QuantizationConfig):
    def __init__(self, weight_bits: int, group_size: int, desc_act: bool, lm_head_quantized: bool) -> None:
    ...
    def __repr__(self) -> str:
        return f'GPTQHPUConfig(weight_bits={self.weight_bits}, group_size={self.group_size}, desc_act={self.desc_act}),lm_head_quantized={self.lm_head_quantized}'
    @classmethod
...
class GPTQHPULinearMethod(LinearMethodBase):
    ...
    def create_weights(self, layer: torch.nn.Module, input_size_per_partition: int, output_partition_sizes: List[int], input_size: int, output_size: int, params_dtype: torch.dtype, **extra_weight_attrs):
        ...
        g_idx = RowAphroditeParameter(data=torch.tensor([i // self.quant_config.group_size for i in range(input_size_per_partition)], dtype=torch.int32), input_dim=0, weight_loader=weight_loader)
    ...
    def apply(self, layer: torch.nn.Module, x: torch.Tensor, bias: Optional[torch.Tensor]=None) -> torch.Tensor:
        ...
        weight = torch.ops.hpu.convert_from_uint4(layer.qweight, layer.scales, layer.qzeros, x.dtype)
    ...
    def unpack_zeros_from_cuda_old_format(self, layer):
        ...
        zeros = torch.bitwise_right_shift(torch.unsqueeze(layer.qzeros.to('cpu'), 2).expand(-1, -1, 32 // bits), self.wf.unsqueeze(0)).to(torch.int16 if bits == 8 else torch.int8)
        zeros = zeros + 1
    ...
    def unpack_weight_from_cuda_old_format(self, layer):
        ...
        bits = self.quant_config.weight_bits
        weight = torch.bitwise_right_shift(torch.unsqueeze(qweight, 1).expand(-1, 32 // bits, -1), self.wf.unsqueeze(-1)).to(torch.int16 if bits == 8 else torch.int8)
