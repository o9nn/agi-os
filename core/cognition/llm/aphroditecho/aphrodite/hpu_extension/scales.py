...
import contextlib
def is_hpu_gaudi2():
    return current_platform.is_hpu() and htexp._get_device_type() == htexp.synDeviceType.synDeviceGaudi2
...
EXP_BIAS_SETS = {('GAUDI2', torch.float8_e4m3fn): [3, 7, 11, 15], ('GAUDI2', torch.float8_e5m2): [15], ('GAUDI3', torch.float8_e4m3fn): range(0, 63), ('GAUDI3', torch.float8_e5m2): range(0, 63)}
MAX_RANGE = {torch.float32: torch.finfo(torch.float32).max, torch.bfloat16: torch.finfo(torch.bfloat16).max, torch.float8_e4m3fn: torch.finfo(torch.float8_e4m3fn).max, torch.float8_e5m2: torch.finfo(torch.float8_e5m2).max}
try:
...
def get_fp8_hw_alligned_scales(dtype, device):
    ...
    return None if exp_bias_set is None else [x / get_fullscale(dtype, device) for x in get_fullscales_by_expbias_set(dtype, device, exp_bias_set)]
DEVICES_SCALE_FACTORS = {'GAUDI2': 4, 'GAUDI3': 1}
FP8_143_SCALES = {device: get_fp8_hw_alligned_scales(torch.float8_e4m3fn, device) for device in DEVICES_SCALE_FACTORS}
FP8_143_SCALES_TRAITS = {device: (min(FP8_143_SCALES[device]), max(FP8_143_SCALES[device]), DEVICES_SCALE_FACTORS[device]) for device in DEVICES_SCALE_FACTORS}
class ScaleToPow2:
    def calc(self, scale):
...
class ConvertScaleToHwAligned:
    ...
    def calc(self, scale):
        ...
        scale_pow2_hw = torch.minimum(torch.maximum(2.0 ** (torch.ceil(torch.log2(scale_pow2) / scale_factor) * scale_factor), torch.tensor(min_scale, dtype=scale.dtype, device=scale.device)), torch.tensor(max_scale, dtype=scale.dtype, device=scale.device))
