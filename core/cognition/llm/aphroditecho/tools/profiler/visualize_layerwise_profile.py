...
def group_trace_by_operations(trace_df: pd.DataFrame) -> pd.DataFrame:
    ...
    def is_gemm_op(op_name: str):
        if is_quant(op_name):
            return False
        if 'xmma_gemm' in op_name or 'gemv2T_kernel' in op_name or 'splitKreduce' in op_name or ('void cutlass::Kernel' in op_name) or ('void cutlass::device_kernel' in op_name) or ('s16816gemm' in op_name):
            return True
    ...
    trace_df.drop(attention_ops + quant_ops + gemm_ops + rms_norm_ops + vocab_embed_ops + mem_ops + elementwise_ops + nccl_all_reduce_ops + nccl_gather_ops + nccl_broadcast_ops + nccl_other_ops + cross_device_reduce_1stage_ops + cross_device_reduce_2stage_ops + custom_ar_all_reduce_unreg_ops + reduce_kernel_ops, axis=1, inplace=True)
    return trace_df
...
def main(json_trace: Path, output_directory: Path, depth: int, plot_metric: str, make_names_unique: bool, top_k: int, json_nodes_to_fold: List[str]):
    ...
    def make_plot_title_suffix(profile_json: dict) -> str:
        ...
        return f"{context['model']}\nBatch={context['batch_size']}, PromptLen={context['prompt_len']}, OutputLen={context['output_len']},NumGpus={context['tensor_parallel_size']}{(', Sparsity ' + sparsity if sparsity else '')}"
    profile_json = None
...
if __name__ == '__main__':
    ...
    parser.add_argument('--json-trace', type=str, required=True, help='json trace file output by examples/offline_profile.py')
