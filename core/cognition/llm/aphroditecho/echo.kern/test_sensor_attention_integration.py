...
from sensor_attention_integration import AttentionGuidedSensorSystem, create_attention_guided_system, integrate_with_existing_sensory_motor
...
class TestAttentionGuidedSensorSystem(unittest.TestCase):
    def setUp(self):
        ...
        self.env_data = {'screen_data': {'frame': 'test_frame'}, 'motion_detected': True, 'velocity': [1.5, 0.8], 'audio_level': 0.6, 'mouse_moved': True, 'cursor_pos': (100, 200), 'detected_objects': ['object1', 'object2']}
        self.sensory_motor_data = {'status': 'processed', 'motion': {'motion_detected': True, 'velocity': [1.0, 2.0]}, 'objects': ['detected_object'], 'mouse_moved': True}
    ...
    def test_dynamic_sensor_prioritization(self):
        ...
        context_hints = {'task': 'navigation', 'urgency': 0.9}
        urgent_priorities = self.system.prioritize_sensors_dynamically(context_hints)
        self.assertIsInstance(urgent_priorities, dict)
        self.assertGreater(urgent_priorities[SensorModalityType.MOTION], priorities[SensorModalityType.MOTION])
    ...
    def test_performance_tracking(self):
        ...
        self.assertGreater(final_summary['system_state']['total_inputs_processed'], initial_summary['system_state']['total_inputs_processed'])
...
class TestFactoryFunctions(unittest.TestCase):
    def test_create_attention_guided_system(self):
        ...
        config = {'max_foci': 5, 'attention_threshold': 0.8, 'enable_logging': False}
        system2 = create_attention_guided_system(config)
        self.assertIsInstance(system2, AttentionGuidedSensorSystem)
        self.assertEqual(system2.sensor_attention.config.max_concurrent_foci, 5)
        self.assertEqual(system2.sensor_attention.config.attention_switch_threshold, 0.8)
...
class TestRealTimeConstraints(unittest.TestCase):
    def setUp(self):
        self.system = AttentionGuidedSensorSystem(enable_logging=False)
        self.complex_env_data = {'screen_data': {'frame': 'large_frame_data' * 100}, 'motion_detected': True, 'velocity': [1.5, 0.8, 2.3], 'audio_level': 0.8, 'audio_change': True, 'mouse_moved': True, 'cursor_pos': (150, 300), 'key_pressed': True, 'detected_objects': [f'object_{i}' for i in range(20)], 'environment_state': {'complexity': 'high'}, 'env_type': 'simulation'}
    ...
    def test_complex_environment_processing(self):
        ...
        focused_inputs = self.system.focus_on_salient_features(sensor_inputs)
        priorities = self.system.prioritize_sensors_dynamically({'urgency': 0.8, 'task': 'navigation'})
        end_time = time.time()
        processing_time = end_time - start_time
        self.assertLess(processing_time, 0.1, f'Complex processing took {processing_time * 1000:.2f}ms')
        self.assertGreater(len(sensor_inputs), 0)
        self.assertIsInstance(focused_inputs, list)
        self.assertIsInstance(priorities, dict)
    def test_memory_usage_stability(self):
        initial_summary = self.system.get_attention_summary()
        for i in range(100):
            test_data = {'iteration': i, 'motion_detected': i % 2 == 0, 'audio_level': i / 100.0}
            self.system.process_sensory_motor_data(test_data)
        final_summary = self.system.get_attention_summary()
        self.assertEqual(final_summary['system_state']['total_inputs_processed'], initial_summary['system_state']['total_inputs_processed'] + 100)
