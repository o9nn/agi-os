...
float frequency_factor = log1pf((float)node->access_count) / log1pf(100.0f);
uint64_t current_time = get_time_ns();
uint64_t age_ns = current_time - node->timestamp_ns;
float age_hours = age_ns / (3600.0f * 1000000000.0f);
float recency_factor = expf(-age_hours / 24.0f);
...
uint32_t consolidated_count = 0;
uint64_t current_time = get_time_ns();
const uint64_t delay_threshold = 3600ULL * 1000000000ULL;
while (current) {
