...
uint32_t membrane_ids[8];
for (int i = 0; i < 8; i++) {
result = dtesn_membrane_create(handle, 1, &membrane_ids[i]);
if (result != 0) {
...
bool performance_ok = (stats.avg_call_overhead_ns <= 1000);
