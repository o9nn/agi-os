...
base_latency_ns = 50 * 1000;
break;
case DTESN_COMPONENT_PSYSTEM:
base_latency_ns = DTESN_PERF_MEMBRANE_EVOLUTION_US * 800;
break;
case DTESN_COMPONENT_BSERIES:
base_latency_ns = DTESN_PERF_BSERIES_COMPUTATION_US * 800;
break;
case DTESN_COMPONENT_ESN:
base_latency_ns = DTESN_PERF_ESN_STATE_UPDATE_US * 800;
break;
case DTESN_COMPONENT_SCHEDULER:
base_latency_ns = DTESN_PERF_CONTEXT_SWITCH_US * 800;
break;
default:
base_latency_ns = 100 * 1000;
break;
}
base_latency_ns += (rand() % 1000) * 100;
usleep(duration_ms * 1000 / 100);
...
{
memset(&g_integration_state, 0, sizeof(g_integration_state));
g_integration_state.code_coverage_pct = 95.0;
}
static dtesn_test_result_t run_component_pair_test(dtesn_component_id_t comp1, dtesn_component_id_t comp2)
{
