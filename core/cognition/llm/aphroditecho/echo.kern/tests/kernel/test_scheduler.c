...
if (task) {
task->deadline_ns = current_time + 10000000;
...
task->deadline_ns = current_time - 1000000;
...
task->budget_ns = 5000000;
task->runtime_ns = 6000000;
deadline_met = dtesn_deadline_check(task, current_time);
...
DTESN_SCHED_POLICY_EDF, 20);
if (edf_task) {
edf_task->deadline_ns = current_time + 5000000;
...
if (rm_task) {
rm_task->period_ns = 10000000;
rm_task->budget_ns = 5000000;
...
for (int i = 0; i < 50; i++) {
dtesn_workload_type_t workload = i % 6;
dtesn_sched_policy_t policy = i % 4;
...
esn_task->deadline_ns = get_time_ns() + 10000000;
bseries_task->period_ns = 20000000;
bseries_task->budget_ns = 10000000;
