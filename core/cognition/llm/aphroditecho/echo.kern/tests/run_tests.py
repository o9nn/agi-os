...
class RealTimeTestRunner:
    ...
    def run_comprehensive_test_suite(self, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        print()
        results = {'framework_version': '1.0.0', 'start_time': self.start_time, 'config': config, 'tests': {}}
        if config.get('run_performance_tests', True):
            print('1. RUNNING PERFORMANCE TESTS')
            print('-' * 30)
            try:
                perf_success = run_dtesn_performance_suite(runs_per_test=config.get('performance_runs', 10), output_file=config.get('performance_output'))
                results['tests']['performance'] = {'success': perf_success, 'type': 'dtesn_performance_suite'}
        ...
        if config.get('run_interactive_tests', True):
            ...
            print('-' * 30)
            try:
                interactive_success = run_interactive_tests(base_url=config.get('base_url', 'http://localhost:8000'), output_file=config.get('interactive_output'))
                results['tests']['interactive'] = {'success': interactive_success, 'type': 'web_application_tests'}
                print(f"Interactive tests: {('✅ PASSED' if interactive_success else '❌ FAILED')}")
            except Exception as e:
                print(f'❌ Interactive tests failed: {e}')
                results['tests']['interactive'] = {'success': False, 'error': str(e)}
            print()
        if config.get('run_integration_tests', True):
            ...
            try:
                ...
                result = subprocess.run(['python3', 'test_oeis_a000081.py'], cwd=project_dir, capture_output=True, timeout=30)
                integration_success = result.returncode == 0
                results['tests']['integration'] = {'success': integration_success, 'type': 'oeis_validation_tests', 'stdout': result.stdout.decode() if result.stdout else '', 'stderr': result.stderr.decode() if result.stderr else ''}
                print(f"Integration tests: {('✅ PASSED' if integration_success else '❌ FAILED')}")
                if not integration_success and result.stderr:
                    print(f'Error output: {result.stderr.decode()}')
            except Exception as e:
                print(f'❌ Integration tests failed: {e}')
                results['tests']['integration'] = {'success': False, 'error': str(e)}
            print()
        if config.get('run_monitoring_test', True):
            ...
            try:
                ...
                monitoring_success = run_continuous_monitoring(interval_ms=config.get('monitoring_interval', 100), duration_seconds=monitoring_duration, output_file=config.get('monitoring_output'))
                results['tests']['monitoring'] = {'success': monitoring_success, 'type': 'continuous_monitoring', 'duration_seconds': monitoring_duration}
                print(f"Monitoring test: {('✅ PASSED' if monitoring_success else '❌ FAILED')}")
            except Exception as e:
                print(f'❌ Monitoring test failed: {e}')
                results['tests']['monitoring'] = {'success': False, 'error': str(e)}
            print()
        ...
        results['summary'] = {'overall_success': overall_success, 'total_test_categories': len(test_results), 'passed_categories': sum(test_results), 'pass_rate': pass_rate, 'end_time': time.time(), 'total_duration': time.time() - self.start_time}
...
def create_default_config() -> Dict[str, Any]:
    return {'run_performance_tests': True, 'run_interactive_tests': True, 'run_integration_tests': True, 'run_monitoring_test': True, 'performance_runs': 10, 'monitoring_duration': 10, 'monitoring_interval': 100, 'base_url': 'http://localhost:8000', 'performance_output': None, 'interactive_output': None, 'monitoring_output': None}
def main():
    parser = argparse.ArgumentParser(description='Echo.Kern Real-Time Testing Framework', formatter_class=argparse.RawDescriptionHelpFormatter, epilog='\nExamples:\n  %(prog)s --quick                     # Quick validation\n  %(prog)s --comprehensive             # Full test suite\n  %(prog)s --performance-only          # Performance tests only\n  %(prog)s --config config.json       # Use custom configuration\n  %(prog)s --monitoring-duration 30    # 30-second monitoring test\n        ')
    ...
    parser.add_argument('--interactive-only', action='store_true', help='Run interactive tests only')
    parser.add_argument('--monitoring-only', action='store_true', help='Run monitoring tests only')
    parser.add_argument('--config', type=str, help='Configuration file (JSON format)')
    parser.add_argument('--performance-runs', type=int, default=10, help='Number of runs per performance test')
    parser.add_argument('--monitoring-duration', type=int, default=10, help='Monitoring test duration in seconds')
    parser.add_argument('--base-url', default='http://localhost:8000', help='Base URL for web application tests')
    ...
    if args.performance_only:
        config.update({'run_performance_tests': True, 'run_interactive_tests': False, 'run_integration_tests': False, 'run_monitoring_test': False})
    elif args.interactive_only:
        config.update({'run_performance_tests': False, 'run_interactive_tests': True, 'run_integration_tests': False, 'run_monitoring_test': False})
    elif args.monitoring_only:
        config.update({'run_performance_tests': False, 'run_interactive_tests': False, 'run_integration_tests': False, 'run_monitoring_test': True})
