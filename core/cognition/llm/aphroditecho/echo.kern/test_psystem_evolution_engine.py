...
try:
    from psystem_evolution_engine import PSystemEvolutionEngine, EvolutionConfig, EvolutionStrategy, EvolutionMetrics, EvolutionAnalytics, SynchronousEvolutionStrategy, ParallelEvolutionStrategy, AdaptiveEvolutionStrategy, create_evolution_engine_demo
    from psystem_membranes import PSystemMembraneHierarchy, MembraneStructure, EvolutionRule, Multiset, MembraneType, RuleType, create_dtesn_psystem_example
...
class TestEvolutionConfig(unittest.TestCase):
    ...
    def test_custom_config(self):
        config = EvolutionConfig(strategy=EvolutionStrategy.ADAPTIVE, max_parallel_workers=8, target_evolution_time_us=25.0, max_evolution_time_us=500.0, enable_analytics=False)
...
class TestEvolutionMetrics(unittest.TestCase):
    ...
    def test_average_membrane_time(self):
        metrics = EvolutionMetrics()
        metrics.membrane_evolution_times = {'mem1': 10.0, 'mem2': 20.0, 'mem3': 30.0}
...
class TestEvolutionAnalytics(unittest.TestCase):
    ...
    def test_metrics_recording(self):
        metrics = EvolutionMetrics(total_evolution_time_us=50.0, rules_applied=5, performance_score=0.8)
    ...
    def test_performance_trends(self):
        for i in range(10):
            metrics = EvolutionMetrics(total_evolution_time_us=100.0 - i * 5, rules_applied=i + 1)
            self.analytics.record_metrics(metrics)
        time_trend = self.analytics.get_performance_trend('evolution_time', 10)
        self.assertLess(time_trend, 0)
        rules_trend = self.analytics.get_performance_trend('rules_applied', 10)
        self.assertGreater(rules_trend, 0)
    def test_current_performance(self):
        for i in range(5):
            metrics = EvolutionMetrics(total_evolution_time_us=50.0 + i, rules_applied=3 + i, performance_score=0.8 + i * 0.02)
            self.analytics.record_metrics(metrics)
...
class TestSynchronousEvolutionStrategy(unittest.TestCase):
    ...
    def test_rule_selection(self):
        ...
        self.assertEqual(selected_rules[0].priority, 3)
...
class TestAdaptiveEvolutionStrategy(unittest.TestCase):
    ...
    def test_strategy_switching(self):
        ...
        with patch.object(self.analytics, 'get_current_performance') as mock_perf:
            mock_perf.return_value = {'avg_performance_score': 0.3}
            for _ in range(15):
...
class TestPSystemEvolutionEngine(unittest.TestCase):
    def setUp(self):
        self.config = EvolutionConfig(strategy=EvolutionStrategy.SYNCHRONOUS, target_evolution_time_us=50.0, max_evolution_time_us=200.0)
    ...
    def test_timing_constraint_violations(self):
        strict_config = EvolutionConfig(max_evolution_time_us=0.1)
    ...
    def test_configuration_optimization(self):
        for i in range(20):
            metrics = EvolutionMetrics(total_evolution_time_us=100.0 + i * 10, rules_applied=5, performance_score=0.8 - i * 0.01)
...
class TestPerformanceConstraints(unittest.TestCase):
    def test_target_timing_achievement(self):
        config = EvolutionConfig(strategy=EvolutionStrategy.SYNCHRONOUS, target_evolution_time_us=100.0, max_evolution_time_us=500.0)
        ...
        if times:
            ...
            self.assertLess(avg_time, config.max_evolution_time_us * 2)
    def test_parallel_performance_improvement(self):
        ...
        parallel_config = EvolutionConfig(strategy=EvolutionStrategy.ASYNCHRONOUS, max_parallel_workers=4)
...
def run_all_tests():
    test_suites = [TestEvolutionConfig, TestEvolutionMetrics, TestEvolutionAnalytics, TestSynchronousEvolutionStrategy, TestParallelEvolutionStrategy, TestAdaptiveEvolutionStrategy, TestPSystemEvolutionEngine, TestEvolutionEngineIntegration, TestPerformanceConstraints]
    overall_result = True
    total_tests = 0
    total_failures = 0
