...
def simulate_environment_scenario(scenario_name: str) -> dict:
    scenarios = {'calm_environment': {'screen_data': {'brightness': 0.3, 'activity': 'low'}, 'motion_detected': False, 'velocity': [0.0, 0.0], 'audio_level': 0.2, 'mouse_moved': False, 'detected_objects': [], 'scenario': scenario_name}, 'high_activity': {'screen_data': {'brightness': 0.8, 'activity': 'high', 'flashing': True}, 'motion_detected': True, 'velocity': [2.5, 1.8, 0.9], 'audio_level': 0.9, 'audio_change': True, 'mouse_moved': True, 'cursor_pos': (300, 150), 'key_pressed': True, 'detected_objects': ['moving_object', 'alert_icon', 'notification'], 'scenario': scenario_name}, 'navigation_task': {'screen_data': {'map_visible': True, 'route_active': True}, 'motion_detected': True, 'velocity': [1.2, 0.5], 'audio_level': 0.4, 'detected_objects': ['waypoint', 'obstacle'], 'environment_state': {'terrain': 'complex'}, 'scenario': scenario_name}, 'interaction_scenario': {'screen_data': {'ui_elements': ['button', 'dialog', 'menu']}, 'motion_detected': False, 'audio_level': 0.7, 'audio_change': True, 'mouse_moved': True, 'cursor_pos': (250, 200), 'key_pressed': True, 'detected_objects': ['clickable_element'], 'scenario': scenario_name}}
...
def demonstrate_attention_mechanisms():
    ...
    print('=' * 50)
    config = {'max_foci': 3, 'attention_threshold': 0.6, 'cooperative_weight': 0.8, 'enable_logging': True}
    ...
    for scenario_name in scenarios:
        ...
        priorities = system.prioritize_sensors_dynamically({'urgency': 0.8 if 'high_activity' in scenario_name else 0.4})
        ...
        results[scenario_name] = {'input_count': len(sensor_inputs), 'focused_count': len(focused_inputs), 'processing_time_ms': processing_time * 1000, 'priorities': {k.value: v for k, v in priorities.items()}, 'attention_metadata': sensory_motor_result.get('attention_metadata', {})}
    print('\n📈 Performance Summary')
    print('-' * 30)
    ...
    for scenario, data in results.items():
        ...
        print(f"  Attention efficiency: {data['focused_count']}/{data['input_count']} ({data['focused_count'] / max(1, data['input_count']) * 100:.1f}%)")
...
def demonstrate_integration_with_mock_sensory_motor():
    ...
    class MockSensoryMotorSystem:
        ...
        def process_input(self):
            ...
            return {'status': 'processed', 'motion': {'motion_detected': self.processed_count % 2 == 0, 'velocity': [1.0, 0.5]}, 'objects': [f'object_{self.processed_count}'], 'timestamp': time.time()}
...
def save_demonstration_results(results: dict, summary: dict):
    ...
    with open(results_file, 'w') as f:
        json.dump({'scenario_results': results, 'system_summary': summary, 'timestamp': time.time(), 'demo_info': {'version': '1.0', 'phase': 'Phase 3.1.3', 'task': 'Create Attention Mechanisms for Sensors'}}, f, indent=2)
