...
uint32_t core_map[LOIHI_MAX_CORES];
uint32_t neuron_map[LOIHI_MAX_NEURONS_PER_CORE * LOIHI_MAX_CORES];
uint64_t spikes_sent;
uint64_t spikes_received;
...
device->caps.max_spike_rate_hz = 1000000;
...
device->caps.memory_size_bytes = 16 * 1024 * 1024;
...
ctx->hardware_available = true;
ctx->firmware_version = 0x01000000;
ctx->temperature_celsius = 35.0f;
...
ctx->power_cycles++;
device->stats.power_consumption_mw = 1.0f;
