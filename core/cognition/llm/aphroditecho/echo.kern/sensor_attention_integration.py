...
import threading
try:
    from kernel.dtesn.sensor_attention_mechanism import SensorModalityType, SensorInput, SensorAttentionMechanism, create_sensor_attention_for_dtesn, integrate_with_sensory_motor
except ImportError:
    import sys
    import os
    sys.path.insert(0, os.path.dirname(__file__))
    from kernel.dtesn.sensor_attention_mechanism import SensorModalityType, SensorInput, create_sensor_attention_for_dtesn, integrate_with_sensory_motor
class AttentionGuidedSensorSystem:
    def __init__(self, dtesn_config: Optional[Dict]=None, enable_logging: bool=True):
    ...
    def process_sensory_motor_data(self, sensory_motor_data: Dict) -> Dict:
        ...
        try:
            ...
            filtered_data = integrate_with_sensory_motor(self.sensor_attention, sensory_motor_data)
            attention_state = self.sensor_attention.get_attention_state()
            filtered_data['attention_metadata'] = {'context': self.context_state, 'active_foci': attention_state['current_foci'], 'attention_switches': attention_state['attention_switches'], 'processing_time_ms': (time.time() - start_time) * 1000}
    ...
    def create_sensor_inputs_from_environment(self, env_data: Dict) -> List[SensorInput]:
        ...
        if 'screen_data' in env_data or 'visual_data' in env_data:
            ...
            sensor_inputs.append(SensorInput(modality=SensorModalityType.VISUAL, data=visual_data, timestamp=current_time, confidence=confidence, priority=0.7, metadata={'source': 'screen_capture', 'has_motion': env_data.get('motion_detected', False), 'object_count': len(env_data.get('detected_objects', []))}))
        if 'motion_data' in env_data or env_data.get('motion_detected', False):
            motion_data = env_data.get('motion_data', {'detected': True})
            sensor_inputs.append(SensorInput(modality=SensorModalityType.MOTION, data=motion_data, timestamp=current_time, confidence=0.9 if env_data.get('motion_detected', False) else 0.3, priority=0.8, metadata={'motion_detected': env_data.get('motion_detected', False), 'velocity': env_data.get('velocity', [0.0, 0.0]), 'source': 'motion_detector'}))
        if 'audio_data' in env_data or 'audio_level' in env_data:
            ...
            sensor_inputs.append(SensorInput(modality=SensorModalityType.AUDITORY, data=audio_data, timestamp=current_time, confidence=0.7, priority=0.5, metadata={'audio_level': env_data.get('audio_level', 0.0), 'sudden_change': env_data.get('audio_change', False), 'source': 'audio_capture'}))
        if any((key in env_data for key in ['mouse_moved', 'key_pressed', 'cursor_pos'])):
            proprioceptive_data = {'mouse_moved': env_data.get('mouse_moved', False), 'key_pressed': env_data.get('key_pressed', False), 'cursor_pos': env_data.get('cursor_pos', (0, 0))}
            sensor_inputs.append(SensorInput(modality=SensorModalityType.PROPRIOCEPTIVE, data=proprioceptive_data, timestamp=current_time, confidence=0.6, priority=0.4, metadata={'interaction_type': 'user_input', 'source': 'input_devices'}))
        if 'environment_state' in env_data:
            env_state = env_data['environment_state']
            sensor_inputs.append(SensorInput(modality=SensorModalityType.ENVIRONMENTAL, data=env_state, timestamp=current_time, confidence=0.5, priority=0.3, metadata={'environment_type': env_data.get('env_type', 'unknown'), 'source': 'environment_monitor'}))
    ...
    def get_attention_summary(self) -> Dict:
        ...
        return {'system_state': {'active': self.active, 'context': self.context_state, 'total_inputs_processed': self.total_inputs_processed, 'avg_processing_time_ms': self.total_processing_time / max(1, self.total_inputs_processed) * 1000}, 'attention_state': base_state, 'performance_metrics': {'attention_switches_per_input': self.attention_switches / max(1, self.total_inputs_processed), 'total_processing_time_s': self.total_processing_time, 'meets_realtime_constraints': base_state.get('avg_switch_time_ms', 0) <= 10.0}}
...
def create_attention_guided_system(config: Optional[Dict]=None) -> AttentionGuidedSensorSystem:
    dtesn_config = None
    if config:
        dtesn_config = {'max_attention_channels': config.get('max_foci', 3), 'attention_threshold': config.get('attention_threshold', 0.7), 'cooperative_weight': config.get('cooperative_weight', 0.8)}
    return AttentionGuidedSensorSystem(dtesn_config=dtesn_config, enable_logging=config.get('enable_logging', True) if config else True)
...
if __name__ == '__main__':
    ...
    print('=' * 50)
    attention_system = create_attention_guided_system({'max_foci': 3, 'attention_threshold': 0.6, 'enable_logging': True})
    attention_system.set_attention_context('exploration')
    env_data = {'screen_data': {'frame': 'test_frame'}, 'motion_detected': True, 'velocity': [1.5, 0.8], 'audio_level': 0.6, 'mouse_moved': True, 'cursor_pos': (100, 200), 'detected_objects': ['object1', 'object2']}
    ...
    sensory_motor_data = {'status': 'processed', 'motion': {'motion_detected': True, 'velocity': [1.0, 2.0]}, 'objects': ['detected_object'], 'mouse_moved': True}
    ...
    if 'attention_metadata' in result:
        ...
        print(f"Attention metadata: {metadata['active_foci']} active foci, {metadata['processing_time_ms']:.2f}ms processing time")
