...
class PromptAsset:
    def __init__(self, id: str, version: str, role: str, template: str, tags: Optional[List[str]]=None, parent_version: Optional[str]=None, embedding_fingerprint: Optional[str]=None):
        self.id = id
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {'id': self.id, 'version': self.version, 'role': self.role, 'tags': self.tags, 'sha256': self.sha256, 'parent_version': self.parent_version, 'template': self.template, 'embedding_fingerprint': self.embedding_fingerprint}
    @classmethod
    def from_dict(cls, data: Dict[str, Any]) -> PromptAsset:
        return cls(id=data['id'], version=data['version'], role=data['role'], template=data['template'], tags=data.get('tags', []), parent_version=data.get('parent_version'), embedding_fingerprint=data.get('embedding_fingerprint'))
class PromptStore:
    ...
    def _load_prompts(self) -> None:
        for path in self.base_dir.rglob('*.md'):
            try:
                ...
                prompt = PromptAsset(id=path.stem, version=metadata.get('version', 'v1.0.0'), role=metadata.get('role', 'system'), template=content, tags=metadata.get('tags', []), parent_version=metadata.get('parent_version'))
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {'total_prompts': total_prompts, 'unique_prompts': unique_prompts, 'roles': roles, 'tags': tags, 'role_count': len(roles), 'tag_count': len(tags)}