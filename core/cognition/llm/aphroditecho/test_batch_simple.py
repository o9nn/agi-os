...
from aphrodite.endpoints.deep_tree_echo.batch_manager import BatchConfiguration, DynamicBatchManager, BatchingMetrics
def test_batch_configuration():
    config = BatchConfiguration(min_batch_size=2, max_batch_size=16, target_batch_size=8, max_batch_wait_ms=30.0)
...
def test_batching_metrics():
    metrics = BatchingMetrics(requests_processed=100, avg_batch_size=8.5, throughput_improvement=25.0)
...
async def test_dynamic_batch_manager():
    ...
    def mock_load_tracker():
        return 0.5
    manager = DynamicBatchManager(config=config, load_tracker=mock_load_tracker)
...
async def test_batch_size_adaptation():
    ...
    for load in load_scenarios:
        manager = DynamicBatchManager(config=config, load_tracker=lambda: load)
        batch_size = manager._calculate_dynamic_batch_size()
        assert config.min_batch_size <= batch_size <= config.max_batch_size
