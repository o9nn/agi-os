...
class RepositoryIntrospector:
    ...
    def assemble_hypergraph_input(self, root: Path, attention_threshold: float) -> List[HypergraphNode]:
        ...
        self.logger.info(f'Assembled {len(nodes)} hypergraph nodes with threshold {attention_threshold:.3f}')
        return nodes
    ...
    def inject_repo_input_into_prompt(self, current_load: float=0.5, recent_activity: float=0.3) -> str:
        ...
        self.logger.info(f'Generated introspection prompt with {len(nodes)} files, attention threshold: {attention_threshold:.3f}')
        return prompt
