...
def debug_target_propagation():
    ...
    goal = MotorGoal(goal_id='debug_reach', goal_type=MotorGoalType.REACH_POSITION, target_data={'target_position': (0.3, 0.1, 1.0), 'end_effector': 'right_elbow', 'joint_targets': {'right_shoulder': 0.3, 'right_elbow': -0.5}, 'duration': 0.5}, priority=1.0)
    ...
    if objectives:
        ...
        if trajectory:
            ...
            for step in range(5):
                ...
                before_states = {'right_shoulder': agent.get_joint_state('right_shoulder')['angle'], 'right_elbow': agent.get_joint_state('right_elbow')['angle']}
                status = controller.update(dt)
                after_commands = agent.motor_commands.copy()
                {'right_shoulder': agent.get_joint_state('right_shoulder')['angle'], 'right_elbow': agent.get_joint_state('right_elbow')['angle']}
                agent.update(dt, physics, environment)
                final_states = {'right_shoulder': agent.get_joint_state('right_shoulder')['angle'], 'right_elbow': agent.get_joint_state('right_elbow')['angle']}
                print(f'\n  Step {step}:')
