...
async def demonstrate_echo_self_aar_integration():
    ...
    echo_config = EvolutionConfig(population_size=6, max_generations=10, mutation_rate=0.2, crossover_rate=0.8, selection_pressure=0.75, elitism_ratio=0.25)
    ...
    aar_config = AARConfig(max_concurrent_agents=25, arena_simulation_enabled=True, relation_graph_depth=3)
    ...
    for generation in range(3):
        ...
        if stats.get('evaluation_results'):
            ...
            print(f"     Sample agent: {sample.get('agent_id', 'N/A')[:12]}... (fitness: {sample.get('fitness_score', 0):.3f})")
    ...
    validations = {'Agent evolution system operational': len(evolution_results) > 0, 'Multiple generations completed': len(evolution_results) >= 3, 'Performance tracking functional': all(('best_fitness' in r for r in evolution_results)), 'Agent diversity maintained': any((r['elite_count'] > 0 and r['offspring_count'] > 0 for r in evolution_results)), 'Arena integration working': any((len(r.get('evaluation_results', [])) > 0 for r in evolution_results)), 'System health maintained': final_stats['system_health']['overall_score'] > 0.8}
