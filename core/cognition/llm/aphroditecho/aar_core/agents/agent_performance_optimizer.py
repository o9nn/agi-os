...
class PerformanceMetrics:
    ...
    def add_measurement(self, response_time: float, success: bool, resource_usage: float=0.0):
    ...
    def get_performance_score(self) -> float:
        ...
        if self.resource_utilization:
            ...
            resource_score = max(0.0, 1.0 - avg_resource)
...
class AgentPerformanceOptimizer:
    ...
    def record_performance(self, agent_id: str, response_time: float, success: bool, resource_usage: float=0.0) -> None:
        if agent_id in self.agent_metrics:
            self.agent_metrics[agent_id].add_measurement(response_time, success, resource_usage)
    def get_optimal_agents(self, available_agents: List[str], count: int=1, capability_requirements: Optional[Dict[str, Any]]=None) -> List[str]:
    ...
    def _round_robin_selection(self, agents: List[str], count: int) -> List[str]:
        ...
        for _ in range(min(count, len(agents))):
            ...
            candidates = [agent_id for agent_id in agents if self.load_balancer_state.get(agent_id, 0) == min_count]
    ...
    def _capability_weighted_selection(self, agents: List[str], count: int, requirements: Optional[Dict[str, Any]]=None) -> List[str]:
        if not requirements:
    ...
    def get_performance_report(self) -> Dict[str, Any]:
        report = {'optimization_strategy': self.strategy.value, 'total_agents_monitored': len(self.agent_metrics), 'last_optimization': self.last_optimization, 'agent_performance': {}}
        for agent_id, metrics in self.agent_metrics.items():
            agent_report = {'performance_score': metrics.get_performance_score(), 'avg_response_time': statistics.mean(metrics.response_times) if metrics.response_times else 0.0, 'measurements_count': len(metrics.response_times), 'last_updated': metrics.last_updated}
        ...
        if all_scores:
            report['system_performance'] = {'average_score': statistics.mean(all_scores), 'min_score': min(all_scores), 'max_score': max(all_scores), 'score_deviation': statistics.stdev(all_scores) if len(all_scores) > 1 else 0.0}
        return report
    ...
    def suggest_optimizations(self) -> List[Dict[str, Any]]:
        ...
        if underperforming:
            suggestions.append({'type': 'agent_optimization', 'action': 'evolve_or_replace', 'agents': underperforming, 'reason': 'Performance below threshold'})
        if len(self.agent_metrics) > 1:
            ...
            if len(scores) > 1 and statistics.stdev(scores) > 0.3:
                suggestions.append({'type': 'load_balancing', 'action': 'redistribute_load', 'reason': 'High performance variance detected'})
        return suggestions
    async def optimize_system(self, agent_manager) -> Dict[str, Any]:
        ...
        return {'suggestions': suggestions, 'applied_optimizations': applied_optimizations, 'optimization_time': optimization_time, 'timestamp': time.time()}