...
def benchmark_memory_operations() -> Dict[str, Any]:
    ...
    for i in range(1000):
        memory_manager.add_memory(content=f'Test memory content {i}', memory_type=MemoryType.WORKING, tags=[f'tag_{i % 10}'])
    add_time = time.perf_counter() - start_time
    results['add_1000_records_ms'] = add_time * 1000
    start_time = time.perf_counter()
    for i in range(100):
        memory_manager.query_memory(query=f'test {i}', limit=10)
