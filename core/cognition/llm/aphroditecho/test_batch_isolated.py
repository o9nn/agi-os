...
def test_batch_configuration():
    config = BatchConfiguration(min_batch_size=2, max_batch_size=16, target_batch_size=8)
...
def test_dynamic_batch_sizing():
    config = BatchConfiguration(min_batch_size=1, max_batch_size=32, target_batch_size=8, enable_adaptive_sizing=True)
    test_cases = [(0.2, 'low load - should increase batch size'), (0.5, 'normal load - should maintain batch size'), (0.8, 'high load - should decrease batch size')]
...
def test_batch_wait_time_calculation():
    config = BatchConfiguration(max_batch_wait_ms=100.0, min_batch_wait_ms=10.0)
    manager = SimpleDynamicBatchManager(config, lambda: 0.5)
    test_cases = [(8, 8, 'full batch - no wait'), (4, 8, 'half full - moderate wait'), (1, 8, 'almost empty - max wait')]
...
def test_load_aware_adaptation():
    config = BatchConfiguration(target_batch_size=10, load_adjustment_factor=0.3, low_load_threshold=0.3, high_load_threshold=0.7)
...
def test_performance_factor_calculation():
    ...
    manager = SimpleDynamicBatchManager(config, lambda: 0.5)
    improving_history = [{'timestamp': time.time() - i, 'throughput': 10.0 + i * 0.5} for i in range(20, 0, -1)]
    ...
    declining_history = [{'timestamp': time.time() - i, 'throughput': 15.0 - i * 0.3} for i in range(20, 0, -1)]
...
def test_throughput_calculation():
    ...
    current_throughput = 70.0
    if manager._baseline_throughput:
        improvement = (current_throughput - manager._baseline_throughput) / manager._baseline_throughput * 100
