...
from aar_core.embodied.hardware_abstraction import VisionSensor, AuditorySensor, TactileSensor, MultiModalSensorManager, SensorType
def test_dtesn_integration():
    ...
    environment_data = {'objects': [{'type': 'target_object', 'position': [1, 0, 1]}], 'ambient_light': 0.7, 'motion_detected': True, 'dominant_color': [0.5, 0.5, 0.8], 'sound_sources': [{'position': [1, 0, 1], 'volume': 0.6, 'frequency': 1000}], 'ambient_noise': 0.1, 'contact_info': {'in_contact': True, 'pressure': 3.0}}
    readings = manager.get_synchronized_readings(environment_data)
    manager.fuse_sensor_data(readings)
    dtesn_modality_data = []
    for sensor_id, reading in readings.items():
        modality_data = {'modality_type': reading.sensor_type.value, 'data': reading.value if isinstance(reading.value, (int, float)) else reading.value.tolist(), 'confidence': reading.confidence, 'timestamp': reading.timestamp, 'data_size': len(reading.value) if hasattr(reading.value, '__len__') else 1, 'valid': True}
    ...
    fusion_weights = [1.0 / len(dtesn_modality_data)] * len(dtesn_modality_data)
    for i, data in enumerate(dtesn_modality_data):
