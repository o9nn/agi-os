...
def run_profile(context: ProfileContext, csv_output: Optional[str], json_output: Optional[str]):
    ...
    if prompt_len + output_len > llm.llm_engine.model_config.max_model_len:
        print(f"ERROR: chosen prompt_len + output_len ({prompt_len} + {output_len} = {prompt_len + output_len}) is larger than the model's max_model_len ({max_model_len}), please choose a smaller prompt_len or output_len, or increase --max-model-len")
        sys.exit(-1)
    def add_requests():
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output-len', type=int, default=OUTPUT_LEN_DEFAULT, help='Number of llm steps to run (includes prefill and decode) - default={OUTPUT_LEN_DEFAULT}')
