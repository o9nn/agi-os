...
def unpack_4bit_to_32bit_signed(qweight, qzeros):
    ...
    for row in range(unpacked_weights.shape[0]):
        i = row % 8
        unpacked_weights[row, :] = qweight[row // 8, :] >> 4 * i & 15
...
def convert_model(model, verbose=True):
    for name, module in model.named_modules():
        ...
        linear_module = torch.nn.Linear(in_features=dequantized_weight.shape[1], out_features=dequantized_weight.shape[0], bias=False, dtype=torch.float16, device='cuda')
        linear_module.weight.data.copy_(dequantized_weight)
        new_module = MarlinLayer(infeatures=linear_module.in_features, outfeatures=linear_module.out_features, groupsize=model.config.quantization_config.group_size)
...
def dequantize_model(model, verbose=True):
    for name, module in model.named_modules():
        ...
        new_module = torch.nn.Linear(in_features=dequantized_weight_cpu.shape[1], out_features=dequantized_weight_cpu.shape[0], bias=False, dtype=torch.float16)
...
if __name__ == '__main__':
    ...
    model.config.quantization_config = {'group_size': model.config.quantization_config.group_size, 'quant_method': 'marlin'}
