...
RAND_SEED = 1234
MAX_SEQ_LEN = 2048
EMPTY_CFG = {'quant_cfg': {'*weight_quantizer': {'enable': False}, '*input_quantizer': {'enable': False}, '*lm_head*': {'enable': False}, '*output_layer*': {'enable': False}, 'default': {'enable': False}}, 'algorithm': 'max'}
KV_CACHE_CFG = {'*.query_key_value.output_quantizer': {'num_bits': 8, 'axis': None, 'enable': True}, '*.Wqkv.output_quantizer': {'num_bits': 8, 'axis': None, 'enable': True}, '*.W_pack.output_quantizer': {'num_bits': 8, 'axis': None, 'enable': True}, '*.c_attn.output_quantizer': {'num_bits': 8, 'axis': None, 'enable': True}, '*.k_proj.output_quantizer': {'num_bits': 8, 'axis': None, 'enable': True}, '*.v_proj.output_quantizer': {'num_bits': 8, 'axis': None, 'enable': True}}
QUANT_CFG_CHOICES = {'int8_sq': atq.INT8_SMOOTHQUANT_CFG, 'fp8': atq.FP8_DEFAULT_CFG, 'int4_awq': atq.INT4_AWQ_CFG, 'w4a8_awq': atq.W4A8_AWQ_BETA_CFG, 'int8_wo': EMPTY_CFG, 'int4_wo': EMPTY_CFG, 'full_prec': EMPTY_CFG}
MODEL_NAME_PATTERN_MAP = {'GPT2': 'gpt2', 'Xverse': 'llama', 'Llama': 'llama', 'Mistral': 'llama', 'GPTJ': 'gptj', 'FalconForCausalLM': 'falcon', 'RWForCausalLM': 'falcon', 'baichuan': 'baichuan', 'MPT': 'mpt', 'Bloom': 'bloom', 'ChatGLM': 'chatglm', 'QWen': 'qwen'}
def get_tokenizer(ckpt_path, max_seq_len=MAX_SEQ_LEN, model_type=None):
    print(f'Initializing tokenizer from {ckpt_path}')
    tokenizer = AutoTokenizer.from_pretrained(ckpt_path, model_max_length=max_seq_len, padding_side='left', trust_remote_code=True)
...
def get_model(ckpt_path, dtype='fp16', device='cuda'):
    ...
    if dtype != model_dtype:
        print(f'[TensorRT-LLM][WARNING] The manually set model data type is {dtype}, but the data type of the HuggingFace model is {model_dtype}.')
    return model
...
def get_calib_dataloader(data='cnn_dailymail', tokenizer=None, batch_size=1, calib_size=512, block_size=512, device=None):
    ...
    batch_encoded = tokenizer.batch_encode_plus(dataset, return_tensors='pt', padding='max_length', truncation=True, max_length=block_size)
    if device:
...
def main(args):
    ...
    if args.qformat in ['full_prec', 'int8_wo', 'int4_wo'] and args.kv_cache_dtype is None:
        ...
        if 'awq' in args.qformat:
            if args.calib_size > 32:
                print(f'AWQ calibration could take longer with calib_size = {args.calib_size}, Using calib_size=32 instead')
                args.calib_size = 32
        ...
        calib_dataloader = get_calib_dataloader(tokenizer=tokenizer, batch_size=args.batch_size, calib_size=args.calib_size, device=args.device)
    ...
    with torch.inference_mode():
        ...
        if args.qformat == 'int4_awq' and model_type == 'qwen':
            ...
            export_model_config(model, model_type, getattr(torch, args.dtype), export_dir=export_path, inference_tensor_parallel=args.tp_size, inference_pipeline_parallel=args.pp_size, export_tensorrt_llm_config=False, export_npz=export_npz)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--qformat', help='Quantization format.', default='full_prec', choices=['fp8', 'int8_sq', 'int4_awq', 'w4a8_awq', 'int8_wo', 'int4_wo', 'full_prec'])
