...
class ConfigVersion:
    ...
    def version_hash(self) -> str:
        content = json.dumps({k: v.config_hash for k, v in self.config_entries.items()}, sort_keys=True)
        return hashlib.sha256(content.encode()).hexdigest()[:16]
class ConfigurationManager:
    def __init__(self, service_name: str, redis_url: Optional[str]=None, config_file: Optional[str]=None, auto_reload: bool=True, validation_enabled: bool=True):
    ...
    async def set_config(self, key: str, value: Any, source: ConfigSource=ConfigSource.API, description: str='', validate: bool=True) -> bool:
        try:
            if validate and self.validation_enabled and (key in self.validators):
                ...
                if not await self._validate_value(validator, value):
                    ...
                    return False
            entry = ConfigEntry(key=key, value=value, source=source, timestamp=datetime.now().timestamp(), description=description)
    ...
    async def delete_config(self, key: str) -> bool:
        try:
            ...
            return False
    async def bulk_update(self, config_dict: Dict[str, Any], source: ConfigSource=ConfigSource.API, description: str='Bulk update') -> Dict[str, bool]:
        results = {}
    ...
    async def list_versions(self) -> List[Dict[str, Any]]:
        return [{'version_id': version.version_id, 'timestamp': version.timestamp, 'description': version.description, 'created_by': version.created_by, 'version_hash': version.version_hash, 'config_count': len(version.config_entries)} for version in self.config_versions]
    ...
    async def get_config_summary(self) -> Dict[str, Any]:
        return {'service_name': self.service_name, 'total_entries': len(self.config_entries), 'sources': list({entry.source.value for entry in self.config_entries.values()}), 'versions': len(self.config_versions), 'current_version': self.current_version.version_id if self.current_version else None, 'last_updated': max((entry.timestamp for entry in self.config_entries.values()), default=0)}
    async def export_configuration(self, format: str='json') -> str:
        config_data = {'service_name': self.service_name, 'export_timestamp': datetime.now().isoformat(), 'configuration': {key: {'value': entry.value, 'source': entry.source.value, 'timestamp': entry.timestamp, 'description': entry.description} for key, entry in self.config_entries.items()}}
        ...
        try:
            ...
            for key, value in data.items():
                if key not in self.config_entries:
                    entry = ConfigEntry(key=key, value=value, source=ConfigSource.FILE, timestamp=datetime.now().timestamp(), description=f'Loaded from {self.config_file}')
                    self.config_entries[key] = entry
        ...
        try:
            ...
            for key, value_json in config_data.items():
                if key not in self.config_entries:
                    ...
                    entry = ConfigEntry(key=key, value=entry_data['value'], source=ConfigSource.REDIS, timestamp=entry_data.get('timestamp', datetime.now().timestamp()), description=entry_data.get('description', 'Loaded from Redis'))
                    self.config_entries[key] = entry
    ...
    async def _load_from_environment(self) -> None:
        ...
        for key, value in os.environ.items():
            if key.startswith(prefix):
                ...
                if config_key not in self.config_entries:
                    try:
                        parsed_value = json.loads(value)
                    except (json.JSONDecodeError, ValueError):
                        parsed_value = value
                    entry = ConfigEntry(key=config_key, value=parsed_value, source=ConfigSource.ENVIRONMENT, timestamp=datetime.now().timestamp(), description=f'Loaded from environment variable {key}')
    ...
    async def _save_config_entry(self, entry: ConfigEntry) -> None:
        if self.redis:
            try:
                config_key = f'config:{self.service_name}'
                entry_data = {'value': entry.value, 'source': entry.source.value, 'timestamp': entry.timestamp, 'description': entry.description}
    ...
    async def _create_version_snapshot(self, description: str) -> str:
        ...
        version = ConfigVersion(version_id=version_id, timestamp=datetime.now().timestamp(), config_entries=dict(self.config_entries), description=description)
        ...
        while True:
            try:
                ...
                if self.config_file and os.path.exists(self.config_file):
                    ...
                    last_load_time = max((entry.timestamp for entry in self.config_entries.values() if entry.source == ConfigSource.FILE), default=0)
    ...
    async def _check_redis_updates(self) -> None:
        try:
            ...
            if update_timestamp:
                ...
                last_local_update = max((entry.timestamp for entry in self.config_entries.values()), default=0)
