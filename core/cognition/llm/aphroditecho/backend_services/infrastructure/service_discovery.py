...
class ServiceDiscovery:
    def __init__(self, redis_url: str='redis://localhost:6379', health_check_interval: float=30.0, health_check_timeout: float=5.0, max_consecutive_failures: int=3, service_ttl: int=300):
    ...
    async def register_service(self, endpoint: ServiceEndpoint) -> bool:
        try:
            ...
            self.service_health[endpoint.service_id] = ServiceHealth(service_id=endpoint.service_id, status=ServiceStatus.STARTING, last_check=time.time(), response_time_ms=0.0)
            if self.redis:
                service_data = {**asdict(endpoint), 'service_type': endpoint.service_type.value, 'registered_at': time.time()}
                await self.redis.hset(f'services:{endpoint.service_id}', mapping=service_data)
                ...
                try:
                    ...
                    for service_id in service_ids:
                        ...
                        if service_data:
                            ...
                            endpoint = ServiceEndpoint(**{k: v for k, v in service_data.items() if k in ServiceEndpoint.__annotations__})
            ...
            if not services:
                for endpoint in self.services.values():
                    if service_type is None or endpoint.service_type == service_type:
                        ...
                        if health and health.status == ServiceStatus.HEALTHY:
                            services.append(endpoint)
            logger.debug(f'🔍 Discovered {len(services)} services' + (f' of type {service_type.value}' if service_type else ''))
            return services
        except Exception as e:
        ...
        try:
            ...
            if self.redis:
                ...
                if service_data:
                    service_data['service_type'] = ServiceType(service_data['service_type'])
                    service_data['metadata'] = json.loads(service_data.get('metadata', '{}'))
                    return ServiceEndpoint(**{k: v for k, v in service_data.items() if k in ServiceEndpoint.__annotations__})
            return None
        except Exception as e:
