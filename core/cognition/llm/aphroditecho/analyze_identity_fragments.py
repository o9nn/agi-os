...
def analyze_hypergraph_file(filepath):
    with open(filepath, 'r') as f:
        data = json.load(f)
    analysis = {'file': str(filepath), 'hypernode_count': len(data.get('hypernodes', {})), 'hyperedge_count': len(data.get('hyperedges', {})), 'identity_seeds': [], 'cognitive_functions': set(), 'domains': set(), 'specializations': set(), 'persona_traits': set(), 'memory_types': defaultdict(int)}
    for node_id, node_data in data.get('hypernodes', {}).items():
        identity_seed = node_data.get('identity_seed', {})
        analysis['identity_seeds'].append({'id': node_id, 'name': identity_seed.get('name'), 'domain': identity_seed.get('domain'), 'specialization': identity_seed.get('specialization'), 'persona_trait': identity_seed.get('persona_trait'), 'cognitive_function': identity_seed.get('cognitive_function')})
...
def main():
    ...
    summary = {'total_files_analyzed': len(all_analyses), 'total_hypernodes': total_hypernodes, 'total_hyperedges': total_hyperedges, 'unique_domains': sorted(list(all_domains)), 'unique_cognitive_functions': sorted(list(all_cognitive_functions)), 'unique_specializations': sorted(list(all_specializations)), 'unique_persona_traits': sorted(list(all_persona_traits)), 'detailed_analyses': all_analyses}
