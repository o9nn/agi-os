...
def run(agent_name, agent_path, agent_func, agent_data):
    spec = importlib.util.spec_from_file_location(os.path.basename(agent_path), agent_path)
    ...
    return_to_llm(value)
    dump_result(f'{agent_name}:{agent_func}')
...
def dump_result(name):
    ...
    show_result = False
    try:
        if re.search(f"\\b({os.environ['LLM_DUMP_RESULTS']})\\b", name):
            show_result = True
    except:
        pass
    if not show_result:
