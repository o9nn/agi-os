...
from .models import AsyncConversation, AsyncKeyModel, AsyncModel, AsyncResponse, Attachment, CancelToolCall, Conversation, EmbeddingModel, EmbeddingModelWithAliases, KeyModel, Model, ModelWithAliases, Options, Prompt, Response, Tool, Toolbox, ToolCall, ToolOutput, ToolResult
...
import pathlib
import struct
__all__ = ['AsyncConversation', 'AsyncKeyModel', 'AsyncResponse', 'Attachment', 'CancelToolCall', 'Collection', 'Conversation', 'Fragment', 'get_async_model', 'get_key', 'get_model', 'hookimpl', 'KeyModel', 'Model', 'ModelError', 'NeedsKeyException', 'Options', 'Prompt', 'Response', 'Template', 'Tool', 'Toolbox', 'ToolCall', 'ToolOutput', 'ToolResult', 'user_dir', 'schema_dsl']
...
def get_plugins(all=False):
    ...
    for plugin in pm.get_plugins():
        if not all and plugin.__name__.startswith('llm.default_plugins.'):
            continue
        plugin_info = {'name': plugin.__name__, 'hooks': [h.name for h in pm.get_hookcallers(plugin)]}
...
def get_fragment_loaders() -> Dict[str, Callable[[str], Union[Fragment, Attachment, List[Union[Fragment, Attachment]]]]]:
...
def get_tools() -> Dict[str, Union[Tool, Type[Toolbox]]]:
    ...
    def register(tool_or_function: Union[Tool, Type[Toolbox], Callable[..., Any]], name: Optional[str]=None) -> None:
        ...
        if inspect.isclass(tool_or_function):
            if issubclass(tool_or_function, Toolbox):
                ...
                raise TypeError('Toolbox classes must inherit from llm.Toolbox, {} does not.'.format(tool_or_function.__name__))
    ...
    for plugin in pm.get_plugins():
        ...
        plugin_impls = [impl for impl in hook_caller.get_hookimpls() if impl.plugin is plugin]
...
def get_model(name: Optional[str]=None, _skip_async: bool=False) -> Model:
    ...
    try:
        ...
        if async_model:
            ...
            raise UnknownModelError('Unknown model: ' + name)
def get_key(explicit_key: Optional[str]=None, key_alias: Optional[str]=None, env_var: Optional[str]=None, *, alias: Optional[str]=None, env: Optional[str]=None, input: Optional[str]=None) -> Optional[str]:
    if alias:
        key_alias = alias
    if env:
        env_var = env
