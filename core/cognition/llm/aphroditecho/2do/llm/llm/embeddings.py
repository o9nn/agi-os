...
class Collection:
    class DoesNotExist(Exception):
        pass
    def __init__(self, name: str, db: Optional[Database]=None, *, model: Optional[EmbeddingModel]=None, model_id: Optional[str]=None, create: bool=True) -> None:
        import llm
        ...
        if rows:
            ...
            self.id = cast(Table, self.db['collections']).insert({'name': self.name, 'model': model_id}).last_pk
        else:
    ...
    def count(self) -> int:
        return next(self.db.query('\n            select count(*) as c from embeddings where collection_id = (\n                select id from collections where name = ?\n            )\n            ', (self.name,)))['c']
    def embed(self, id: str, value: Union[str, bytes], metadata: Optional[Dict[str, Any]]=None, store: bool=False) -> None:
        from llm import encode
        content_hash = self.content_hash(value)
        if self.db['embeddings'].count_where('content_hash = ? and collection_id = ?', [content_hash, self.id]):
            return
        embedding = self.model().embed(value)
        cast(Table, self.db['embeddings']).insert({'collection_id': self.id, 'id': id, 'embedding': encode(embedding), 'content': value if store and isinstance(value, str) else None, 'content_blob': value if store and isinstance(value, bytes) else None, 'content_hash': content_hash, 'metadata': json.dumps(metadata) if metadata else None, 'updated': int(time.time())}, replace=True)
    def embed_multi(self, entries: Iterable[Tuple[str, Union[str, bytes]]], store: bool=False, batch_size: int=100) -> None:
        self.embed_multi_with_metadata(((id, value, None) for id, value in entries), store=store, batch_size=batch_size)
    def embed_multi_with_metadata(self, entries: Iterable[Tuple[str, Union[str, bytes], Optional[Dict[str, Any]]]], store: bool=False, batch_size: int=100) -> None:
        import llm
        ...
        while True:
            ...
            existing_ids = [row['id'] for row in self.db.query('\n                    select id from embeddings\n                    where collection_id = ? and content_hash in ({})\n                    '.format(','.join(('?' for _ in items_and_hashes))), [collection_id] + [item_and_hash[1] for item_and_hash in items_and_hashes])]
            ...
            with self.db.conn:
                cast(Table, self.db['embeddings']).insert_all(({'collection_id': collection_id, 'id': id, 'embedding': llm.encode(embedding), 'content': value if store and isinstance(value, str) else None, 'content_blob': value if store and isinstance(value, bytes) else None, 'content_hash': self.content_hash(value), 'metadata': json.dumps(metadata) if metadata else None, 'updated': int(time.time())} for embedding, (id, value, metadata) in zip(embeddings, filtered_batch)), replace=True)
    def similar_by_vector(self, vector: List[float], number: int=10, skip_id: Optional[str]=None, prefix: Optional[str]=None) -> List[Entry]:
        import llm
        ...
        return [Entry(id=row['id'], score=row['score'], content=row['content'], metadata=json.loads(row['metadata']) if row['metadata'] else None) for row in self.db.query('\n            select id, content, metadata, distance_score(embedding) as score\n            from embeddings\n            where {where}\n            order by score desc limit {number}\n        '.format(where=' and '.join(where_bits), number=number), where_args)]
    def similar_by_id(self, id: str, number: int=10, prefix: Optional[str]=None) -> List[Entry]:
        import llm
        matches = list(self.db['embeddings'].rows_where('collection_id = ? and id = ?', (self.id, id)))
        if not matches:
            raise self.DoesNotExist('ID not found')
        embedding = matches[0]['embedding']
        comparison_vector = llm.decode(embedding)
        return self.similar_by_vector(comparison_vector, number, skip_id=id, prefix=prefix)
    def similar(self, value: Union[str, bytes], number: int=10, prefix: Optional[str]=None) -> List[Entry]:
