...
DEFAULT_PLUGINS = ('llm.default_plugins.openai_models', 'llm.default_plugins.default_tools')
    ...
    if LLM_LOAD_PLUGINS is not None:
        for package_name in [name for name in LLM_LOAD_PLUGINS.split(',') if name.strip()]:
            try:
                ...
                llm_entry_points = [ep for ep in distribution.entry_points if ep.group == 'llm']
