...
from typing import Any, AsyncGenerator, AsyncIterator, Awaitable, Callable, Dict, Iterable, Iterator, List, Optional, Set, Union, get_type_hints
from .utils import ensure_fragment, ensure_tool, make_schema_id, mimetype_from_path, mimetype_from_string, token_usage_string, monotonic_ulid
    ...
    def id(self):
        if self._id is None:
            if self.content:
                ...
                self._id = hashlib.sha256(json.dumps({'url': self.url}).encode('utf-8')).hexdigest()
        return self._id
...
class Tool:
    ...
    def hash(self):
        to_hash = {'name': self.name, 'description': self.description, 'input_schema': self.input_schema}
        if self.plugin:
    ...
    def function(cls, function, name=None, description=None):
        ...
        return cls(name=name or function.__name__, description=description or function.__doc__ or None, input_schema=_get_arguments_input_schema(function, name), implementation=function)
...
class Toolbox:
    ...
    def __init_subclass__(cls, **kwargs):
        ...
        def wrapped_init(self, *args, **kwargs):
            ...
            self._config = {name: value for name, value in bound.arguments.items() if name != 'self' and sig.parameters[name].kind not in (inspect.Parameter.VAR_POSITIONAL, inspect.Parameter.VAR_KEYWORD)}
            self._extra_tools = []
    ...
    def method_tools(cls) -> List[Tool]:
        ...
        for method_name in dir(cls):
            ...
            if callable(method):
                tool = Tool.function(method, name='{}_{}'.format(cls.__name__, method_name))
                tools.append(tool)
        return tools
    ...
    def add_tool(self, tool_or_function: Union[Tool, Callable[..., Any]], pass_self: bool=False):
        def _upgrade(fn):
            if pass_self:
...
class Prompt:
    ...
    options: 'Options'
    def __init__(self, prompt, model, *, fragments=None, attachments=None, system=None, system_fragments=None, prompt_json=None, options=None, schema=None, tools=None, tool_results=None):
        self._prompt = prompt
        self.model = model
...
class Conversation(_BaseConversation):
    ...
    def prompt(self, prompt: Optional[str]=None, *, fragments: Optional[List[str]]=None, attachments: Optional[List[Attachment]]=None, system: Optional[str]=None, schema: Optional[Union[dict, type[BaseModel]]]=None, tools: Optional[List[ToolDef]]=None, tool_results: Optional[List[ToolResult]]=None, system_fragments: Optional[List[str]]=None, stream: bool=True, key: Optional[str]=None, **options) -> 'Response':
        return Response(Prompt(prompt, model=self.model, fragments=fragments, attachments=attachments, system=system, schema=schema, tools=tools or self.tools, tool_results=tool_results, system_fragments=system_fragments, options=self.model.Options(**options)), self.model, stream, conversation=self, key=key)
    def chain(self, prompt: Optional[str]=None, *, fragments: Optional[List[str]]=None, attachments: Optional[List[Attachment]]=None, system: Optional[str]=None, system_fragments: Optional[List[str]]=None, stream: bool=True, schema: Optional[Union[dict, type[BaseModel]]]=None, tools: Optional[List[ToolDef]]=None, tool_results: Optional[List[ToolResult]]=None, chain_limit: Optional[int]=None, before_call: Optional[BeforeCallSync]=None, after_call: Optional[AfterCallSync]=None, key: Optional[str]=None, options: Optional[dict]=None) -> 'ChainResponse':
        self.model._validate_attachments(attachments)
        return ChainResponse(Prompt(prompt, fragments=fragments, attachments=attachments, system=system, schema=schema, tools=tools or self.tools, tool_results=tool_results, system_fragments=system_fragments, model=self.model, options=self.model.Options(**options or {})), model=self.model, stream=stream, conversation=self, key=key, before_call=before_call or self.before_call, after_call=after_call or self.after_call, chain_limit=chain_limit if chain_limit is not None else self.chain_limit)
    @classmethod
    def from_row(cls, row):
        from llm import get_model
        return cls(model=get_model(row['model']), id=row['id'], name=row['name'])
    def __repr__(self):
...
class AsyncConversation(_BaseConversation):
    ...
    def chain(self, prompt: Optional[str]=None, *, fragments: Optional[List[str]]=None, attachments: Optional[List[Attachment]]=None, system: Optional[str]=None, system_fragments: Optional[List[str]]=None, stream: bool=True, schema: Optional[Union[dict, type[BaseModel]]]=None, tools: Optional[List[ToolDef]]=None, tool_results: Optional[List[ToolResult]]=None, chain_limit: Optional[int]=None, before_call: Optional[BeforeCallAsync]=None, after_call: Optional[AfterCallAsync]=None, key: Optional[str]=None, options: Optional[dict]=None) -> 'AsyncChainResponse':
        self.model._validate_attachments(attachments)
        return AsyncChainResponse(Prompt(prompt, fragments=fragments, attachments=attachments, system=system, schema=schema, tools=tools or self.tools, tool_results=tool_results, system_fragments=system_fragments, model=self.model, options=self.model.Options(**options or {})), model=self.model, stream=stream, conversation=self, key=key, before_call=before_call or self.before_call, after_call=after_call or self.after_call, chain_limit=chain_limit if chain_limit is not None else self.chain_limit)
    def prompt(self, prompt: Optional[str]=None, *, fragments: Optional[List[str]]=None, attachments: Optional[List[Attachment]]=None, system: Optional[str]=None, schema: Optional[Union[dict, type[BaseModel]]]=None, tools: Optional[List[ToolDef]]=None, tool_results: Optional[List[ToolResult]]=None, system_fragments: Optional[List[str]]=None, stream: bool=True, key: Optional[str]=None, **options) -> 'AsyncResponse':
        return AsyncResponse(Prompt(prompt, model=self.model, fragments=fragments, attachments=attachments, system=system, schema=schema, tools=tools, tool_results=tool_results, system_fragments=system_fragments, options=self.model.Options(**options)), self.model, stream, conversation=self, key=key)
    def to_sync_conversation(self):
        return Conversation(model=self.model, id=self.id, name=self.name, responses=[], tools=self.tools, chain_limit=self.chain_limit)
    @classmethod
    def from_row(cls, row):
        from llm import get_async_model
        return cls(model=get_async_model(row['model']), id=row['id'], name=row['name'])
    def __repr__(self):
        count = len(self.responses)
        s = 's' if count == 1 else ''
        return f'<{self.__class__.__name__}: {self.id} - {count} response{s}'
...
class _BaseResponse:
    ...
    def __init__(self, prompt: Prompt, model: '_BaseModel', stream: bool, conversation: Optional[_BaseConversation]=None, key: Optional[str]=None):
    ...
    def set_usage(self, *, input: Optional[int]=None, output: Optional[int]=None, details: Optional[dict]=None):
    ...
    def from_row(cls, db, row, _async=False):
        ...
        tools = [Tool(name=tool_row['name'], description=tool_row['description'], input_schema=json.loads(tool_row['input_schema']), implementation=None, plugin=tool_row['plugin']) for tool_row in db.query('\n                select tools.* from tools\n                join tool_responses on tools.id = tool_responses.tool_id\n                where tool_responses.response_id = ?\n            ', [row['id']])]
        tool_results = [ToolResult(name=tool_results_row['name'], output=tool_results_row['output'], tool_call_id=tool_results_row['tool_call_id']) for tool_results_row in db.query('\n                select * from tool_results\n                where response_id = ?\n            ', [row['id']])]
        all_fragments = list(db.query(FRAGMENT_SQL, {'response_id': row['id']}))
        fragments = [row['content'] for row in all_fragments if row['fragment_type'] == 'prompt']
        system_fragments = [row['content'] for row in all_fragments if row['fragment_type'] == 'system']
        response = cls(model=model, prompt=Prompt(prompt=row['prompt'], model=model, fragments=fragments, attachments=[], system=row['system'], schema=schema, tools=tools, tool_results=tool_results, system_fragments=system_fragments, options=model.Options(**json.loads(row['options_json']))), stream=False)
        ...
        response.attachments = [Attachment.from_row(attachment_row) for attachment_row in db.query('\n                select attachments.* from attachments\n                join prompt_attachments on attachments.id = prompt_attachments.attachment_id\n                where prompt_attachments.response_id = ?\n                order by prompt_attachments."order"\n            ', [row['id']])]
        response._tool_calls = [ToolCall(name=tool_row['name'], arguments=json.loads(tool_row['arguments']), tool_call_id=tool_row['tool_call_id']) for tool_row in db.query('\n                select * from tool_calls\n                where response_id = ?\n                order by tool_call_id\n            ', [row['id']])]
        return response
    def token_usage(self) -> str:
        return token_usage_string(self.input_tokens, self.output_tokens, self.token_details)
    def log_to_db(self, db):
        conversation = self.conversation
        if not conversation:
            conversation = Conversation(model=self.model)
        db['conversations'].insert({'id': conversation.id, 'name': _conversation_name(self.prompt.prompt or self.prompt.system or ''), 'model': conversation.model.model_id}, ignore=True)
        schema_id = None
        ...
        for previous_response in conversation.responses[:-1]:
            for fragment in (previous_response.prompt.fragments or []) + (previous_response.prompt.system_fragments or []):
                fragment_id = ensure_fragment(db, fragment)
                replacements[f'f:{fragment_id}'] = fragment
                replacements[f'r:{previous_response.id}'] = previous_response.text_or_raise()
        for i, fragment in enumerate(self.prompt.fragments):
            fragment_id = ensure_fragment(db, fragment)
            replacements[f'f{fragment_id}'] = fragment
            db['prompt_fragments'].insert({'response_id': response_id, 'fragment_id': fragment_id, 'order': i})
        for i, fragment in enumerate(self.prompt.system_fragments):
            fragment_id = ensure_fragment(db, fragment)
            replacements[f'f{fragment_id}'] = fragment
            db['system_fragments'].insert({'response_id': response_id, 'fragment_id': fragment_id, 'order': i})
        response_text = self.text_or_raise()
        replacements[f'r:{response_id}'] = response_text
        json_data = self.json()
        response = {'id': response_id, 'model': self.model.model_id, 'prompt': self.prompt._prompt, 'system': self.prompt._system, 'prompt_json': condense_json(self._prompt_json, replacements), 'options_json': {key: value for key, value in dict(self.prompt.options).items() if value is not None}, 'response': response_text, 'response_json': condense_json(json_data, replacements), 'conversation_id': conversation.id, 'duration_ms': self.duration_ms(), 'datetime_utc': self.datetime_utc(), 'input_tokens': self.input_tokens, 'output_tokens': self.output_tokens, 'token_details': json.dumps(self.token_details) if self.token_details else None, 'schema_id': schema_id, 'resolved_model': self.resolved_model}
        db['responses'].insert(response)
        for index, attachment in enumerate(self.prompt.attachments):
            attachment_id = attachment.id()
            db['attachments'].insert({'id': attachment_id, 'type': attachment.resolve_type(), 'path': attachment.path, 'url': attachment.url, 'content': attachment.content}, replace=True)
            db['prompt_attachments'].insert({'response_id': response_id, 'attachment_id': attachment_id, 'order': index})
        tool_ids_by_name = {}
        for tool in self.prompt.tools:
            tool_id = ensure_tool(db, tool)
            tool_ids_by_name[tool.name] = tool_id
            db['tool_responses'].insert({'tool_id': tool_id, 'response_id': response_id})
        for tool_call in self.tool_calls():
            db['tool_calls'].insert({'response_id': response_id, 'tool_id': tool_ids_by_name.get(tool_call.name) or None, 'name': tool_call.name, 'arguments': json.dumps(tool_call.arguments), 'tool_call_id': tool_call.tool_call_id})
        for tool_result in self.prompt.tool_results:
            instance_id = None
            if tool_result.instance:
                try:
                    if not tool_result.instance.instance_id:
                        tool_result.instance.instance_id = db['tool_instances'].insert({'plugin': tool.plugin, 'name': tool.name.split('_')[0], 'arguments': json.dumps(tool_result.instance._config)}).last_pk
                    instance_id = tool_result.instance.instance_id
                except AttributeError:
                    pass
            tool_result_id = db['tool_results'].insert({'response_id': response_id, 'tool_id': tool_ids_by_name.get(tool_result.name) or None, 'name': tool_result.name, 'output': tool_result.output, 'tool_call_id': tool_result.tool_call_id, 'instance_id': instance_id, 'exception': '{}: {}'.format(tool_result.exception.__class__.__name__, str(tool_result.exception)) if tool_result.exception else None}).last_pk
            for index, attachment in enumerate(tool_result.attachments):
                attachment_id = attachment.id()
                db['attachments'].insert({'id': attachment_id, 'type': attachment.resolve_type(), 'path': attachment.path, 'url': attachment.url, 'content': attachment.content}, replace=True)
                db['tool_results_attachments'].insert({'tool_result_id': tool_result_id, 'attachment_id': attachment_id, 'order': index})
class Response(_BaseResponse):
    ...
    def text_or_raise(self) -> str:
        return self.text()
    def execute_tool_calls(self, *, before_call: Optional[BeforeCallSync]=None, after_call: Optional[AfterCallSync]=None) -> List[ToolResult]:
        tool_results = []
            ...
            if before_call:
                try:
                    ...
                    if inspect.isawaitable(cb_result):
                        raise TypeError("Asynchronous 'before_call' callback provided to a synchronous tool execution context. Please use an async chain/response or a synchronous callback.")
                except CancelToolCall as ex:
                    tool_results.append(ToolResult(name=tool_call.name, output='Cancelled: ' + str(ex), tool_call_id=tool_call.tool_call_id, exception=ex))
                    continue
            if tool is None:
                msg = 'tool "{}" does not exist'.format(tool_call.name)
                tool_results.append(ToolResult(name=tool_call.name, output='Error: ' + msg, tool_call_id=tool_call.tool_call_id, exception=KeyError(msg)))
                continue
            if not tool.implementation:
                raise ValueError('No implementation available for tool: {}'.format(tool_call.name))
            attachments = []
            exception = None
            try:
                ...
                result = f'Error: {ex}'
                exception = ex
            tool_result_obj = ToolResult(name=tool_call.name, output=result, attachments=attachments, tool_call_id=tool_call.tool_call_id, instance=_get_instance(tool.implementation), exception=exception)
            if after_call:
                cb_result = after_call(tool, tool_call, tool_result_obj)
                if inspect.isawaitable(cb_result):
                    raise TypeError("Asynchronous 'after_call' callback provided to a synchronous tool execution context. Please use an async chain/response or a synchronous callback.")
    ...
    def usage(self) -> Usage:
        self._force()
        return Usage(input=self.input_tokens, output=self.output_tokens, details=self.token_details)
    def __iter__(self) -> Iterator[str]:
        ...
        if isinstance(self.model, Model):
            for chunk in self.model.execute(self.prompt, stream=self.stream, response=self, conversation=self.conversation):
                assert chunk is not None
                yield chunk
                self._chunks.append(chunk)
        elif isinstance(self.model, KeyModel):
            for chunk in self.model.execute(self.prompt, stream=self.stream, response=self, conversation=self.conversation, key=self.model.get_key(self._key)):
                assert chunk is not None
                yield chunk
                self._chunks.append(chunk)
        else:
...
class AsyncResponse(_BaseResponse):
    ...
    async def _on_done(self):
        for callback_func in self.done_callbacks:
            if callable(callback_func):
                ...
                await callback_func
    async def execute_tool_calls(self, *, before_call: Optional[BeforeCallAsync]=None, after_call: Optional[AfterCallAsync]=None) -> List[ToolResult]:
        tool_calls_list = await self.tool_calls()
        tools_by_name = {tool.name: tool for tool in self.prompt.tools}
        instances_to_prepare: list[Toolbox] = []
        for tool_to_prep in tools_by_name.values():
            inst = _get_instance(tool_to_prep.implementation)
            if isinstance(inst, Toolbox) and (not getattr(inst, '_async_prepared', False)):
                instances_to_prepare.append(inst)
        for inst in instances_to_prepare:
            ...
            if tool is None:
                ...
                async def run_async(tc=tc, tool=tool, idx=idx):
                    if before_call:
                        try:
                            ...
                            return (idx, ToolResult(name=tc.name, output='Cancelled: ' + str(ex), tool_call_id=tc.tool_call_id, exception=ex))
                    exception = None
                    attachments = []
                    try:
                        ...
                        if isinstance(result, ToolOutput):
                            ...
                            result = result.output
                        output = result if isinstance(result, str) else json.dumps(result, default=repr)
                    except Exception as ex:
                        output = f'Error: {ex}'
                        exception = ex
                    tr = ToolResult(name=tc.name, output=output, attachments=attachments, tool_call_id=tc.tool_call_id, instance=_get_instance(tool.implementation), exception=exception)
                ...
                if before_call:
                    try:
                        ...
                        indexed_results.append((idx, ToolResult(name=tc.name, output='Cancelled: ' + str(ex), tool_call_id=tc.tool_call_id, exception=ex)))
                        continue
                exception = None
                attachments = []
                if tool is None:
                    ...
                    try:
                        ...
                        if isinstance(res, ToolOutput):
                            ...
                            res = res.output
                        output = res if isinstance(res, str) else json.dumps(res, default=repr)
                    except Exception as ex:
                        output = f'Error: {ex}'
                        exception = ex
                    tr = ToolResult(name=tc.name, output=output, attachments=attachments, tool_call_id=tc.tool_call_id, instance=_get_instance(tool.implementation), exception=exception)
    ...
    async def __anext__(self) -> str:
        ...
        if not hasattr(self, '_generator'):
            if isinstance(self.model, AsyncModel):
                self._generator = self.model.execute(self.prompt, stream=self.stream, response=self, conversation=self.conversation)
            elif isinstance(self.model, AsyncKeyModel):
                self._generator = self.model.execute(self.prompt, stream=self.stream, response=self, conversation=self.conversation, key=self.model.get_key(self._key))
            else:
    ...
    async def to_sync_response(self) -> Response:
        ...
        if not isinstance(self.model, (Model, KeyModel)):
            pass
        response = Response(self.prompt, sync_model, self.stream, conversation=self.conversation.to_sync_conversation() if self.conversation else None)
        response.id = self.id
    ...
    def fake(cls, model: 'AsyncModel', prompt: str, *attachments: List[Attachment], system: str, response: str):
        response_obj = cls(model=model, prompt=Prompt(prompt, model=model, attachments=attachments, system=system), stream=False)
...
class _BaseChainResponse:
    ...
    conversation: Optional['_BaseConversation'] = None
    _key: Optional[str] = None
    def __init__(self, prompt: Prompt, model: '_BaseModel', stream: bool, conversation: _BaseConversation, key: Optional[str]=None, chain_limit: Optional[int]=10, before_call: Optional[Union[BeforeCallSync, BeforeCallAsync]]=None, after_call: Optional[Union[AfterCallSync, AfterCallAsync]]=None):
        self.prompt = prompt
        self.model = model
        self.stream = stream
        self._key = key
...
class ChainResponse(_BaseChainResponse):
    ...
    def responses(self) -> Iterator[Response]:
        prompt = self.prompt
        count = 0
        current_response: Optional[Response] = Response(prompt, self.model, self.stream, key=self._key, conversation=self.conversation)
        while current_response:
            ...
            tool_results = current_response.execute_tool_calls(before_call=self.before_call, after_call=self.after_call)
            attachments = []
            for tool_result in tool_results:
                attachments.extend(tool_result.attachments)
            if tool_results:
                current_response = Response(Prompt('', self.model, tools=current_response.prompt.tools, tool_results=tool_results, options=self.prompt.options, attachments=attachments), self.model, stream=self.stream, key=self._key, conversation=self.conversation)
            else:
...
class AsyncChainResponse(_BaseChainResponse):
    ...
    async def responses(self) -> AsyncIterator[AsyncResponse]:
        prompt = self.prompt
        count = 0
        current_response: Optional[AsyncResponse] = AsyncResponse(prompt, self.model, self.stream, key=self._key, conversation=self.conversation)
        while current_response:
            count += 1
            yield current_response
            self._responses.append(current_response)
            if self.chain_limit and count >= self.chain_limit:
                raise ValueError(f'Chain limit of {self.chain_limit} exceeded.')
            tool_results = await current_response.execute_tool_calls(before_call=self.before_call, after_call=self.after_call)
            if tool_results:
                attachments = []
                for tool_result in tool_results:
                    attachments.extend(tool_result.attachments)
                prompt = Prompt('', self.model, tools=current_response.prompt.tools, tool_results=tool_results, options=self.prompt.options, attachments=attachments)
                current_response = AsyncResponse(prompt, self.model, stream=self.stream, key=self._key, conversation=self.conversation)
            else:
                current_response = None
                break
...
class _get_key_mixin:
    ...
    def get_key(self, explicit_key: Optional[str]=None) -> Optional[str]:
        ...
        if self.key is not None:
            return self.key
        key_value = get_key(explicit_key=explicit_key, key_alias=self.needs_key, env_var=self.key_env_var)
        if key_value:
            return key_value
...
class _BaseModel(ABC, _get_key_mixin):
    ...
    class Options(_Options):
        pass
    def _validate_attachments(self, attachments: Optional[List[Attachment]]=None) -> None:
        ...
        for attachment in attachments or []:
            ...
            if attachment_type not in self.attachment_types:
                raise ValueError(f"This model does not support attachments of type '{attachment_type}', only {', '.join(self.attachment_types)}")
    def __str__(self) -> str:
        return '{}{}: {}'.format(self.__class__.__name__, ' (async)' if isinstance(self, (AsyncModel, AsyncKeyModel)) else '', self.model_id)
    def __repr__(self) -> str:
        return f'<{str(self)}>'
class _Model(_BaseModel):
    def conversation(self, tools: Optional[List[ToolDef]]=None, before_call: Optional[BeforeCallSync]=None, after_call: Optional[AfterCallSync]=None, chain_limit: Optional[int]=None) -> Conversation:
        return Conversation(model=self, tools=tools, before_call=before_call, after_call=after_call, chain_limit=chain_limit)
    def prompt(self, prompt: Optional[str]=None, *, fragments: Optional[List[str]]=None, attachments: Optional[List[Attachment]]=None, system: Optional[str]=None, system_fragments: Optional[List[str]]=None, stream: bool=True, schema: Optional[Union[dict, type[BaseModel]]]=None, tools: Optional[List[ToolDef]]=None, tool_results: Optional[List[ToolResult]]=None, **options) -> Response:
        key_value = options.pop('key', None)
        self._validate_attachments(attachments)
        return Response(Prompt(prompt, fragments=fragments, attachments=attachments, system=system, schema=schema, tools=tools, tool_results=tool_results, system_fragments=system_fragments, model=self, options=self.Options(**options)), self, stream, key=key_value)
    def chain(self, prompt: Optional[str]=None, *, fragments: Optional[List[str]]=None, attachments: Optional[List[Attachment]]=None, system: Optional[str]=None, system_fragments: Optional[List[str]]=None, stream: bool=True, schema: Optional[Union[dict, type[BaseModel]]]=None, tools: Optional[List[ToolDef]]=None, tool_results: Optional[List[ToolResult]]=None, before_call: Optional[BeforeCallSync]=None, after_call: Optional[AfterCallSync]=None, key: Optional[str]=None, options: Optional[dict]=None) -> ChainResponse:
        return self.conversation().chain(prompt=prompt, fragments=fragments, attachments=attachments, system=system, system_fragments=system_fragments, stream=stream, schema=schema, tools=tools, tool_results=tool_results, before_call=before_call, after_call=after_call, key=key, options=options)
class Model(_Model):
    @abstractmethod
...
class KeyModel(_Model):
    @abstractmethod
    def execute(self, prompt: Prompt, stream: bool, response: Response, conversation: Optional[Conversation], key: Optional[str]) -> Iterator[str]:
        pass
class _AsyncModel(_BaseModel):
    def conversation(self, tools: Optional[List[ToolDef]]=None, before_call: Optional[BeforeCallAsync]=None, after_call: Optional[AfterCallAsync]=None, chain_limit: Optional[int]=None) -> AsyncConversation:
        return AsyncConversation(model=self, tools=tools, before_call=before_call, after_call=after_call, chain_limit=chain_limit)
    def prompt(self, prompt: Optional[str]=None, *, fragments: Optional[List[str]]=None, attachments: Optional[List[Attachment]]=None, system: Optional[str]=None, schema: Optional[Union[dict, type[BaseModel]]]=None, tools: Optional[List[ToolDef]]=None, tool_results: Optional[List[ToolResult]]=None, system_fragments: Optional[List[str]]=None, stream: bool=True, **options) -> AsyncResponse:
        key_value = options.pop('key', None)
        self._validate_attachments(attachments)
        return AsyncResponse(Prompt(prompt, fragments=fragments, attachments=attachments, system=system, schema=schema, tools=tools, tool_results=tool_results, system_fragments=system_fragments, model=self, options=self.Options(**options)), self, stream, key=key_value)
...
class AsyncKeyModel(_AsyncModel):
    @abstractmethod
    async def execute(self, prompt: Prompt, stream: bool, response: AsyncResponse, conversation: Optional[AsyncConversation], key: Optional[str]) -> AsyncGenerator[str, None]:
        if False:
            yield ''
        pass
