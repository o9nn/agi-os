...
@pytest.mark.parametrize('attachment_type,attachment_content', [('image/png', TINY_PNG), ('audio/wav', TINY_WAV)])
def test_prompt_attachment(mock_model, logs_db, attachment_type, attachment_content):
    runner = CliRunner()
    mock_model.enqueue(['two boxes'])
    result = runner.invoke(cli.cli, ['prompt', '-m', 'mock', 'describe file', '-a', '-'], input=attachment_content, catch_exceptions=False)
    assert result.exit_code == 0, result.output
    assert result.output == 'two boxes\n'
    assert mock_model.history[0][0].attachments[0] == llm.Attachment(type=attachment_type, path=None, url=None, content=attachment_content, _id=ANY)
    ...
    assert attachment == {'id': ANY, 'type': attachment_type, 'path': None, 'url': None, 'content': attachment_content}
