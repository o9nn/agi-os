...
def test_keys_set(monkeypatch, tmpdir):
    ...
    assert json.loads(content) == {'// Note': 'This file stores secret API credentials. Do not share!', 'openai': 'foo'}
@pytest.mark.xfail(sys.platform == 'win32', reason='Expected to fail on Windows')
...
@pytest.mark.httpx_mock(assert_all_requests_were_expected=False, can_send_already_matched_responses=True)
def test_uses_correct_key(mocked_openai_chat, monkeypatch, tmpdir):
    ...
    KEYS = {'openai': 'from-keys-file', 'other': 'other-key'}
    ...
    assert_key('from-env')
    keys_path.write_text(json.dumps(KEYS), 'utf-8')
    result3 = runner.invoke(cli, ['hello', '--key', 'other', '--no-stream'], catch_exceptions=False)
    assert result3.exit_code == 0
    assert_key('other-key')
    result4 = runner.invoke(cli, ['hello', '--key', 'custom-key', '--no-stream'], catch_exceptions=False)
