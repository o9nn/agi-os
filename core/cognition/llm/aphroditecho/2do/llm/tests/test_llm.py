...
def test_llm_prompt_creates_log_database(mocked_openai_chat, tmpdir, monkeypatch, custom_database_path):
...
@pytest.mark.parametrize('logs_off,logs_args,should_log', ((True, [], False), (False, [], True), (False, ['--no-log'], False), (False, ['--log'], True), (True, ['-n'], False), (True, ['--log'], True)))
def test_llm_default_prompt(mocked_openai_chat, use_stdin, user_path, logs_off, logs_args, should_log):
    ...
    if not should_log:
        ...
        return
    assert len(rows) == 1
    expected = {'model': 'gpt-4o-mini', 'prompt': 'three names \nfor a pet pelican', 'system': None, 'options_json': '{}', 'response': 'Bob, Alice, Eve'}
    row = rows[0]
    assert expected.items() <= row.items()
    assert isinstance(row['duration_ms'], int)
    assert isinstance(row['datetime_utc'], str)
    assert json.loads(row['prompt_json']) == {'messages': [{'role': 'user', 'content': 'three names \nfor a pet pelican'}]}
    assert json.loads(row['response_json']) == {'choices': [{'message': {'content': {'$': f"r:{row['id']}"}}}], 'model': 'gpt-4o-mini'}
    log_result = runner.invoke(cli, ['logs', '-n', '1', '--json'], catch_exceptions=False)
    log_json = json.loads(log_result.output)
    assert log_json[0].items() >= {'model': 'gpt-4o-mini', 'prompt': 'three names \nfor a pet pelican', 'system': None, 'prompt_json': {'messages': [{'role': 'user', 'content': 'three names \nfor a pet pelican'}]}, 'options_json': {}, 'response': 'Bob, Alice, Eve', 'response_json': {'model': 'gpt-4o-mini', 'choices': [{'message': {'content': {'$': f"r:{row['id']}"}}}]}, 'conversation_name': 'three names for a pet pelican', 'conversation_model': 'gpt-4o-mini'}.items()
@mock.patch.dict(os.environ, {'OPENAI_API_KEY': 'X'})
@pytest.mark.parametrize('async_', (False, True))
def test_llm_prompt_continue(httpx_mock, user_path, async_):
    httpx_mock.add_response(method='POST', url='https://api.openai.com/v1/chat/completions', json={'model': 'gpt-4o-mini', 'usage': {}, 'choices': [{'message': {'content': 'Bob, Alice, Eve'}}]}, headers={'Content-Type': 'application/json'})
    httpx_mock.add_response(method='POST', url='https://api.openai.com/v1/chat/completions', json={'model': 'gpt-4o-mini', 'usage': {}, 'choices': [{'message': {'content': 'Terry'}}]}, headers={'Content-Type': 'application/json'})
    log_path = user_path / 'logs.db'
    log_db = sqlite_utils.Database(str(log_path))
    log_db['responses'].delete_where()
    runner = CliRunner()
    args = ['three names \nfor a pet pelican', '--no-stream'] + (['--async'] if async_ else [])
    result = runner.invoke(cli, args, catch_exceptions=False)
    ...
    rows = list(log_db['responses'].rows)
    assert len(rows) == 2
@pytest.mark.parametrize('args,expect_just_code', ((['-x'], True), (['--extract'], True), (['-x', '--async'], True), (['--extract', '--async'], True), (['--no-stream'], False)))
def test_extract_fenced_code(mocked_openai_chat_returning_fenced_code, args, expect_just_code):
    runner = CliRunner()
    result = runner.invoke(cli, ['-m', 'gpt-4o-mini', '--key', 'x', 'Write code'] + args, catch_exceptions=False)
...
def test_openai_completion(mocked_openai_completion, user_path):
    ...
    log_db['responses'].delete_where()
    runner = CliRunner()
    result = runner.invoke(cli, ['-m', 'gpt-3.5-turbo-instruct', 'Say this is a test', '--no-stream', '--key', 'x'], catch_exceptions=False)
    assert result.exit_code == 0
    assert result.output == '\n\nThis is indeed a test\n'
    last_request = mocked_openai_completion.get_requests()[-1]
    assert json.loads(last_request.content) == {'model': 'gpt-3.5-turbo-instruct', 'prompt': 'Say this is a test', 'stream': False, 'max_tokens': 256}
    rows = list(log_db['responses'].rows)
    assert len(rows) == 1
    expected = {'model': 'gpt-3.5-turbo-instruct', 'prompt': 'Say this is a test', 'system': None, 'prompt_json': '{"messages": ["Say this is a test"]}', 'options_json': '{}', 'response': '\n\nThis is indeed a test'}
    row = rows[0]
    assert expected.items() <= row.items()
def test_openai_completion_system_prompt_error():
    runner = CliRunner()
    result = runner.invoke(cli, ['-m', 'gpt-3.5-turbo-instruct', 'Say this is a test', '--no-stream', '--key', 'x', '--system', 'system prompts not allowed'])
    assert result.exit_code == 1
    assert 'System prompts are not supported for OpenAI completion models' in result.output
def test_openai_completion_logprobs_stream(mocked_openai_completion_logprobs_stream, user_path):
    ...
    assert len(rows) == 1
    row = rows[0]
    assert json.loads(row['response_json']) == {'content': {'$': f"r:{row['id']}"}, 'logprobs': [{'text': '\n\n', 'top_logprobs': [{'\n\n': -0.6, '\n': -1.9}]}, {'text': 'Hi', 'top_logprobs': [{'Hi': -1.1, 'Hello': -0.7}]}, {'text': '.', 'top_logprobs': [{'.': -1.1, '!': -0.9}]}, {'text': '', 'top_logprobs': []}], 'id': 'cmpl-80MdSaou7NnPuff5ZyRMysWBmgSPS', 'object': 'text_completion', 'model': 'gpt-3.5-turbo-instruct', 'created': 1695097702}
def test_openai_completion_logprobs_nostream(mocked_openai_completion_logprobs, user_path):
    log_path = user_path / 'logs.db'
    log_db = sqlite_utils.Database(str(log_path))
    log_db['responses'].delete_where()
    runner = CliRunner()
    args = ['-m', 'gpt-3.5-turbo-instruct', 'Say hi', '-o', 'logprobs', '2', '--key', 'x', '--no-stream']
    result = runner.invoke(cli, args, catch_exceptions=False)
    assert result.exit_code == 0
    assert result.output == '\n\nHi.\n'
    rows = list(log_db['responses'].rows)
    assert len(rows) == 1
    row = rows[0]
    assert json.loads(row['response_json']) == {'choices': [{'finish_reason': 'stop', 'index': 0, 'logprobs': {'text_offset': [16, 18, 20], 'token_logprobs': [-0.6, -1.1, -0.9], 'tokens': ['\n\n', 'Hi', '1'], 'top_logprobs': [{'\n': -1.9, '\n\n': -0.6}, {'Hello': -0.7, 'Hi': -1.1}, {'!': -1.1, '.': -0.9}]}, 'text': {'$': f"r:{row['id']}"}}], 'created': 1695097747, 'id': 'cmpl-80MeBfKJutM0uMNJkRrebJLeP3bxL', 'model': 'gpt-3.5-turbo-instruct', 'object': 'text_completion', 'usage': {'completion_tokens': 3, 'prompt_tokens': 5, 'total_tokens': 8}}
...
def test_openai_localai_configuration(mocked_localai, user_path):
    ...
    assert json.loads(last_request.content) == {'model': 'orca-mini-3b', 'messages': [{'role': 'user', 'content': 'three names \nfor a pet pelican'}], 'stream': False}
    ...
    assert json.loads(last_request2.content) == {'model': 'babbage', 'prompt': 'hi', 'stream': False}
@pytest.mark.parametrize('args,exit_code', ((['-q', 'mo', '-q', 'ck'], 0), (['-q', 'mock'], 0), (['-q', 'badmodel'], 1), (['-q', 'mock', '-q', 'badmodel'], 1)))
def test_prompt_select_model_with_queries(mock_model, user_path, args, exit_code):
    runner = CliRunner()
    result = runner.invoke(cli, args + ['hello'], catch_exceptions=False)
...
@pytest.mark.parametrize('args,expected_model_ids,unexpected_model_ids', ((['-q', 'gpt-4o'], ['OpenAI Chat: gpt-4o'], None), (['-q', 'mock'], ['MockModel: mock'], None), (['--query', 'mock'], ['MockModel: mock'], None), (['-q', '4o', '-q', 'mini'], ['OpenAI Chat: gpt-4o-mini'], ['OpenAI Chat: gpt-4o ']), (['-m', 'gpt-4o-mini', '-m', 'gpt-4.5'], ['OpenAI Chat: gpt-4o-mini', 'OpenAI Chat: gpt-4.5'], ['OpenAI Chat: gpt-4o '])))
...
def test_get_async_models():
    ...
    assert all((isinstance(model, (llm.AsyncModel, llm.AsyncKeyModel)) for model in models))
    model_ids = [model.model_id for model in models]
    assert 'gpt-4o-mini' in model_ids
...
class Dog(BaseModel):
    name: str
    age: int
dog_schema = {'properties': {'name': {'title': 'Name', 'type': 'string'}, 'age': {'title': 'Age', 'type': 'integer'}}, 'required': ['name', 'age'], 'title': 'Dog', 'type': 'object'}
...
def test_schema(mock_model, use_pydantic):
    ...
    response = mock_model.prompt('invent a dog', schema=Dog if use_pydantic else dog_schema)
...
def test_model_environment_variable(monkeypatch):
    ...
    runner = CliRunner()
    result = runner.invoke(cli, ['--no-stream', 'hello', '-s', 'sys'], catch_exceptions=False)
    assert result.exit_code == 0
    assert json.loads(result.output) == {'prompt': 'hello', 'system': 'sys', 'attachments': [], 'stream': False, 'previous': []}
...
def test_schema_via_cli(mock_model, tmpdir, monkeypatch, use_filename):
    ...
    runner = CliRunner()
    result = runner.invoke(cli, ['--schema', schema_value, 'prompt', '-m', 'mock'], catch_exceptions=False)
    assert result.exit_code == 0
    assert result.output == '{"name": "Cleo", "age": 10}\n'
    assert (user_path / 'logs.db').exists()
    rows = list(sqlite_utils.Database(str(user_path / 'logs.db'))['schemas'].rows)
    assert rows == [{'id': '9a8ed2c9b17203f6d8905147234475b5', 'content': '{"schema":"one"}'}]
    if use_filename:
        result2 = runner.invoke(cli, ['--schema', '9a8ed2c9b17203f6d8905147234475b5', 'prompt', '-m', 'mock'], catch_exceptions=False)
        assert result2.exit_code == 0
@pytest.mark.parametrize('args,expected', ((['--schema', 'name, age int'], {'type': 'object', 'properties': {'name': {'type': 'string'}, 'age': {'type': 'integer'}}, 'required': ['name', 'age']}), (['--schema-multi', 'name, age int'], {'type': 'object', 'properties': {'items': {'type': 'array', 'items': {'type': 'object', 'properties': {'name': {'type': 'string'}, 'age': {'type': 'integer'}}, 'required': ['name', 'age']}}}, 'required': ['items']})))
def test_schema_using_dsl(mock_model, tmpdir, monkeypatch, args, expected):
    user_path = tmpdir / 'user'
    mock_model.enqueue([json.dumps(dog)])
    monkeypatch.setenv('LLM_USER_PATH', str(user_path))
    runner = CliRunner()
    result = runner.invoke(cli, ['prompt', '-m', 'mock'] + args, catch_exceptions=False)
    assert result.exit_code == 0
    assert result.output == '{"name": "Cleo", "age": 10}\n'
    rows = list(sqlite_utils.Database(str(user_path / 'logs.db'))['schemas'].rows)
...
async def test_schema_async(async_mock_model, use_pydantic):
    ...
    response = async_mock_model.prompt('invent a dog', schema=Dog if use_pydantic else dog_schema)
...
def test_schemas_dsl():
    ...
    assert result.exit_code == 0
    assert json.loads(result.output) == {'type': 'object', 'properties': {'name': {'type': 'string'}, 'age': {'type': 'integer'}, 'bio': {'type': 'string', 'description': 'short bio'}}, 'required': ['name', 'age', 'bio']}
    result2 = runner.invoke(cli, ['schemas', 'dsl', 'name, age int', '--multi'])
    assert result2.exit_code == 0
    assert json.loads(result2.output) == {'type': 'object', 'properties': {'items': {'type': 'array', 'items': {'type': 'object', 'properties': {'name': {'type': 'string'}, 'age': {'type': 'integer'}}, 'required': ['name', 'age']}}}, 'required': ['items']}
@mock.patch.dict(os.environ, {'OPENAI_API_KEY': 'X'})
@pytest.mark.parametrize('custom_database_path', (False, True))
