import json
import pytest
from llm.utils import extract_fenced_code_block, instantiate_from_spec, maybe_fenced_code, schema_dsl, simplify_usage_dict, truncate_string, monotonic_ulid
...
@pytest.mark.parametrize('input,last,expected', [['This is a sample text without any code blocks.', False, None], ["Here is some text.\n\n```\ndef foo():\n    return 'bar'\n```\n\nMore text.", False, "def foo():\n    return 'bar'\n"], ["Here is some text.\n\n```python\ndef foo():\n    return 'bar'\n```\n\nMore text.", False, "def foo():\n    return 'bar'\n"], ["Here is some text.\n\n````\ndef foo():\n    return 'bar'\n````\n\nMore text.", False, "def foo():\n    return 'bar'\n"], ["Here is some text.\n\n````javascript\nfunction foo() {\n    return 'bar';\n}\n````\n\nMore text.", False, "function foo() {\n    return 'bar';\n}\n"], ["Here is some text.\n\n```python\ndef foo():\n    return 'bar'\n````\n\nMore text.", False, None], ["First code block:\n\n```python\ndef foo():\n    return 'bar'\n```\n\nSecond code block:\n\n```javascript\nfunction foo() {\n    return 'bar';\n}\n```", False, "def foo():\n    return 'bar'\n"], ["First code block:\n\n```python\ndef foo():\n    return 'bar'\n```\n\nSecond code block:\n\n```javascript\nfunction foo() {\n    return 'bar';\n}\n```", True, "function foo() {\n    return 'bar';\n}\n"], ["First code block:\n\n```python\ndef foo():\n    return 'bar'\n```\n\nSecond code block:\n\n```javascript\nfunction foo() {\n    return 'bar';\n}\n``` ", True, "function foo() {\n    return 'bar';\n}\n"], ['Here is some text.\n\n```python\ndef foo():\n    return `bar`\n```\n\nMore text.', False, 'def foo():\n    return `bar`\n']])
def test_extract_fenced_code_block(input, last, expected):
    actual = extract_fenced_code_block(input, last=last)
    assert actual == expected
@pytest.mark.parametrize('schema, expected', [('name, bio', {'type': 'object', 'properties': {'name': {'type': 'string'}, 'bio': {'type': 'string'}}, 'required': ['name', 'bio']}), ('name, age int, balance float, active bool', {'type': 'object', 'properties': {'name': {'type': 'string'}, 'age': {'type': 'integer'}, 'balance': {'type': 'number'}, 'active': {'type': 'boolean'}}, 'required': ['name', 'age', 'balance', 'active']}), ('name: full name, age int: years old', {'type': 'object', 'properties': {'name': {'type': 'string', 'description': 'full name'}, 'age': {'type': 'integer', 'description': 'years old'}}, 'required': ['name', 'age']}), ('\n        name\n        bio\n        age int\n        ', {'type': 'object', 'properties': {'name': {'type': 'string'}, 'bio': {'type': 'string'}, 'age': {'type': 'integer'}}, 'required': ['name', 'bio', 'age']}), ("\n        name: the person's name\n        age int: their age in years, must be positive\n        bio: a short bio, no more than three sentences\n        ", {'type': 'object', 'properties': {'name': {'type': 'string', 'description': "the person's name"}, 'age': {'type': 'integer', 'description': 'their age in years, must be positive'}, 'bio': {'type': 'string', 'description': 'a short bio, no more than three sentences'}}, 'required': ['name', 'age', 'bio']}), ('', {'type': 'object', 'properties': {}, 'required': []}), ('name str, description str', {'type': 'object', 'properties': {'name': {'type': 'string'}, 'description': {'type': 'string'}}, 'required': ['name', 'description']}), ("  name  ,  age   int  :  person's age  ", {'type': 'object', 'properties': {'name': {'type': 'string'}, 'age': {'type': 'integer', 'description': "person's age"}}, 'required': ['name', 'age']})])
...
def test_schema_dsl_multi():
    ...
    assert result == {'type': 'object', 'properties': {'items': {'type': 'array', 'items': {'type': 'object', 'properties': {'name': {'type': 'string'}, 'age': {'type': 'integer', 'description': 'The age'}}, 'required': ['name', 'age']}}}, 'required': ['items']}
@pytest.mark.parametrize('text, max_length, normalize_whitespace, keep_end, expected', [('Hello, world!', 100, False, False, 'Hello, world!'), ('Hello, world!', 5, False, False, 'He...'), ('', 10, False, False, ''), (None, 10, False, False, None), ('Hello   world!', 100, True, False, 'Hello world!'), ('Hello \n\t world!', 100, True, False, 'Hello world!'), ('Hello   world!', 5, True, False, 'He...'), ('Hello, world!', 10, False, True, 'He... d!'), ('Hello, world!', 7, False, False, 'Hell...'), ('1234567890', 7, False, False, '1234...'), ('Hello   world!', 10, True, True, 'He... d!'), ('Hello \n\t world!', 12, True, True, 'Hello world!'), ('12345', 5, False, False, '12345'), ('123456', 5, False, False, '12...'), ('12345', 5, False, True, '12345'), ('123456', 5, False, False, '12...'), ('A' * 200, 10, False, False, 'AAAAAAA...'), ('A' * 200, 10, False, True, 'AA... AA'), ('123456789', 9, False, False, '123456789'), ('1234567890', 9, False, False, '123456...'), ('123456789', 9, False, True, '123456789'), ('1234567890', 9, False, True, '12... 90'), ('1234567890', 8, False, True, '12345...'), ('1234567890', 9, False, True, '12... 90')])
def test_truncate_string(text, max_length, normalize_whitespace, keep_end, expected):
    result = truncate_string(text=text, max_length=max_length, normalize_whitespace=normalize_whitespace, keep_end=keep_end)
    assert result == expected
@pytest.mark.parametrize('text, max_length, keep_end, prefix_len, expected_full', [('0123456789', 10, True, None, '0123456789'), ('012345678901234', 14, True, 4, '0123... 1234'), ('abcdefghijklmnopqrstuvwxyz', 10, True, 2, 'ab... yz'), ('abcdefghijklmnopqrstuvwxyz', 12, True, 3, 'abc... xyz'), ('abcdefghijklmnopqrstuvwxyz', 8, True, None, 'abcde...')])
def test_test_truncate_string_keep_end(text, max_length, keep_end, prefix_len, expected_full):
    result = truncate_string(text=text, max_length=max_length, keep_end=keep_end)
    ...
    if prefix_len is not None and len(text) > max_length and (max_length >= 9):
        ...
        assert result[-prefix_len:] == text[-prefix_len:]
        assert '... ' in result
@pytest.mark.parametrize('content,expected_fenced', [('<div><p>Test</p><span>Test</span><a>Test</a><b>Test</b><i>Test</i><u>Test</u>', True), ('<p>Just a paragraph</p>', False), ('line1\nline2\nline3\nline4\nline5', True), ('x' * 130 + '\n' + 'x' * 130 + '\n' + 'x' * 130 + '\n' + 'x' * 50, False), ('<div>\n<p>Line 1</p>\n<p>Line 2</p>\n<p>Line 3</p>\n</div>', True), ('<div><p>Only two</p></div>', False), ('', False), ('```\ndef test():\n    pass\n```', True)])
def test_maybe_fenced_code(content: str, expected_fenced: bool):
    ...
    if expected_fenced:
        ...
        assert result == content
@pytest.mark.parametrize('content,backtick_count', [('def test():\n    pass', 3), ('```\ndef test():\n    pass\n```', 4), ('````\ndef test():\n    pass\n````', 5)])
...
class ValueFlag:
    def __init__(self, value=None, flag=False):
        self.value = value
        self.flag = flag
@pytest.mark.parametrize('spec, expected_cls, expected_attrs', [('Files', Files, {'dir': '.'}), ('Files()', Files, {'dir': '.'}), ('Files("tmp")', Files, {'dir': 'tmp'}), ('Files({"dir": "/tmp"})', Files, {'dir': '/tmp'}), ('Files(dir="/data")', Files, {'dir': '/data'}), ('ValueFlag({"value": 123, "flag": true})', ValueFlag, {'value': 123, 'flag': True}), ('ValueFlag(flag=true)', ValueFlag, {'flag': True}), ('ValueFlag(value=123, flag=false)', ValueFlag, {'value': 123, 'flag': False})])
...
@pytest.mark.parametrize('spec', ['Files({"dir":})', 'Files(', 'Files(dir=)', 'Files({"dir": [})', 'Files(.)', 'Files(this is invalid)', 'ValueFlag(value=123, flag=falseTypo)'])
