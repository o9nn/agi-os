...
def test_cli_aliases_list(args):
    ...
    for line in '3.5         : gpt-3.5-turbo\nchatgpt     : gpt-3.5-turbo\nchatgpt-16k : gpt-3.5-turbo-16k\n3.5-16k     : gpt-3.5-turbo-16k\n4           : gpt-4\ngpt4        : gpt-4\n4-32k       : gpt-4-32k\ne-demo      : embed-demo (embedding)\nada         : text-embedding-ada-002 (embedding)\n'.split('\n'):
        line = line.strip()
        if not line:
            continue
        regex = '\\s+'.join((re.escape(part) for part in line.split()))
...
def test_cli_aliases_list_json(args):
    ...
    assert result.exit_code == 0
    assert json.loads(result.output).items() >= {'3.5': 'gpt-3.5-turbo', 'chatgpt': 'gpt-3.5-turbo', 'chatgpt-16k': 'gpt-3.5-turbo-16k', '3.5-16k': 'gpt-3.5-turbo-16k', '4': 'gpt-4', 'gpt4': 'gpt-4', '4-32k': 'gpt-4-32k', 'ada': 'text-embedding-ada-002', 'e-demo': 'embed-demo'}.items()
@pytest.mark.parametrize('args,expected,expected_error', ((['foo', 'bar'], {'foo': 'bar'}, None), (['foo', '-q', 'mo'], {'foo': 'mock'}, None), (['foo', '-q', 'mog'], None, 'No model found matching query: mog')))
...
def test_cli_aliases_are_registered(user_path, args):
    (user_path / 'aliases.json').write_text(json.dumps({'foo': 'bar', 'turbo': 'gpt-3.5-turbo'}), 'utf-8')
    runner = CliRunner()
    result = runner.invoke(cli, args)
    assert result.exit_code == 0
