...
def test_chat_basic(mock_model, logs_db):
    ...
    result = runner.invoke(llm.cli.cli, ['chat', '-m', 'mock'], input='Hi\nHi two\nquit\n', catch_exceptions=False)
    assert result.exit_code == 0
    assert result.output == "Chatting with mock\nType 'exit' or 'quit' to exit\nType '!multi' to enter multiple lines, then '!end' to finish\nType '!edit' to open your default editor and modify the prompt\nType '!fragment <my_fragment> [<another_fragment> ...]' to insert one or more fragments\n> Hi\none world\n> Hi two\none again\n> quit\n"
    conversations = list(logs_db['conversations'].rows)
    assert conversations[0] == {'id': ANY, 'name': 'Hi', 'model': 'mock'}
    conversation_id = conversations[0]['id']
    responses = list(logs_db['responses'].rows)
    assert responses == [{'id': ANY, 'model': 'mock', 'resolved_model': None, 'prompt': 'Hi', 'system': None, 'prompt_json': None, 'options_json': '{}', 'response': 'one world', 'response_json': None, 'conversation_id': conversation_id, 'duration_ms': ANY, 'datetime_utc': ANY, 'input_tokens': 1, 'output_tokens': 1, 'token_details': None, 'schema_id': None}, {'id': ANY, 'model': 'mock', 'resolved_model': None, 'prompt': 'Hi two', 'system': None, 'prompt_json': None, 'options_json': '{}', 'response': 'one again', 'response_json': None, 'conversation_id': conversation_id, 'duration_ms': ANY, 'datetime_utc': ANY, 'input_tokens': 2, 'output_tokens': 1, 'token_details': None, 'schema_id': None}]
    mock_model.enqueue(['continued'])
    result2 = runner.invoke(llm.cli.cli, ['chat', '-m', 'mock', '-c'], input='Continue\nquit\n', catch_exceptions=False)
    assert result2.exit_code == 0
    assert result2.output == "Chatting with mock\nType 'exit' or 'quit' to exit\nType '!multi' to enter multiple lines, then '!end' to finish\nType '!edit' to open your default editor and modify the prompt\nType '!fragment <my_fragment> [<another_fragment> ...]' to insert one or more fragments\n> Continue\ncontinued\n> quit\n"
    new_responses = list(logs_db.query('select * from responses where id not in ({})'.format(', '.join(('?' for _ in responses))), [r['id'] for r in responses]))
    assert new_responses == [{'id': ANY, 'model': 'mock', 'resolved_model': None, 'prompt': 'Continue', 'system': None, 'prompt_json': None, 'options_json': '{}', 'response': 'continued', 'response_json': None, 'conversation_id': conversation_id, 'duration_ms': ANY, 'datetime_utc': ANY, 'input_tokens': 1, 'output_tokens': 1, 'token_details': None, 'schema_id': None}]
@pytest.mark.xfail(sys.platform == 'win32', reason='Expected to fail on Windows')
def test_chat_system(mock_model, logs_db):
    runner = CliRunner()
    mock_model.enqueue(['I am mean'])
    result = runner.invoke(llm.cli.cli, ['chat', '-m', 'mock', '--system', 'You are mean'], input='Hi\nquit\n')
    assert result.exit_code == 0
    assert result.output == "Chatting with mock\nType 'exit' or 'quit' to exit\nType '!multi' to enter multiple lines, then '!end' to finish\nType '!edit' to open your default editor and modify the prompt\nType '!fragment <my_fragment> [<another_fragment> ...]' to insert one or more fragments\n> Hi\nI am mean\n> quit\n"
    responses = list(logs_db['responses'].rows)
    assert responses == [{'id': ANY, 'model': 'mock', 'resolved_model': None, 'prompt': 'Hi', 'system': 'You are mean', 'prompt_json': None, 'options_json': '{}', 'response': 'I am mean', 'response_json': None, 'conversation_id': ANY, 'duration_ms': ANY, 'datetime_utc': ANY, 'input_tokens': 1, 'output_tokens': 1, 'token_details': None, 'schema_id': None}]
@pytest.mark.xfail(sys.platform == 'win32', reason='Expected to fail on Windows')
def test_chat_options(mock_model, logs_db, user_path):
    ...
    result = runner.invoke(llm.cli.cli, ['chat', '-m', 'mock', '--option', 'max_tokens', '10'], input='Hi with override\nquit\n')
    assert result.exit_code == 0
    responses = list(logs_db['responses'].rows)
...
def test_chat_tools(logs_db):
    ...
    result = runner.invoke(llm.cli.cli, ['chat', '-m', 'echo', '--functions', functions], input='\n'.join([json.dumps({'prompt': 'Convert hello to uppercase', 'tool_calls': [{'name': 'upper', 'arguments': {'text': 'hello'}}]}), 'quit']), catch_exceptions=False)
    assert result.exit_code == 0
    assert result.output == 'Chatting with echo\nType \'exit\' or \'quit\' to exit\nType \'!multi\' to enter multiple lines, then \'!end\' to finish\nType \'!edit\' to open your default editor and modify the prompt\nType \'!fragment <my_fragment> [<another_fragment> ...]\' to insert one or more fragments\n> {"prompt": "Convert hello to uppercase", "tool_calls": [{"name": "upper", "arguments": {"text": "hello"}}]}\n{\n  "prompt": "Convert hello to uppercase",\n  "system": "",\n  "attachments": [],\n  "stream": true,\n  "previous": []\n}{\n  "prompt": "",\n  "system": "",\n  "attachments": [],\n  "stream": true,\n  "previous": [\n    {\n      "prompt": "{\\"prompt\\": \\"Convert hello to uppercase\\", \\"tool_calls\\": [{\\"name\\": \\"upper\\", \\"arguments\\": {\\"text\\": \\"hello\\"}}]}"\n    }\n  ],\n  "tool_results": [\n    {\n      "name": "upper",\n      "output": "HELLO",\n      "tool_call_id": null\n    }\n  ]\n}\n> quit\n'
@pytest.mark.xfail(sys.platform == 'win32', reason='Expected to fail on Windows')
def test_chat_fragments(tmpdir):
    ...
    with open(path2, 'w') as fp:
        fp.write('two')
    runner = CliRunner()
    output = runner.invoke(llm.cli.cli, ['chat', '-m', 'echo', '-f', path1], input='hi\n!fragment {}\nquit\n'.format(path2)).output
