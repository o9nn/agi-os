...
def test_tool_use_chain_of_two_calls(vcr):
    ...
    chain_response = model.chain('Can the country of Crumpet have dragons? Answer with only YES or NO', tools=[lookup_population, can_have_dragons], stream=False, key=API_KEY)
...
def test_tool_use_async_tool_function():
    ...
    chain_response = model.chain(json.dumps({'tool_calls': [{'name': 'hello'}]}), tools=[hello])
    output = chain_response.text()
    bits = output.split('\n}{\n')
    assert len(bits) == 2
    objects = [json.loads(bits[0] + '}'), json.loads('{' + bits[1])]
    assert objects == [{'prompt': '', 'system': '', 'attachments': [], 'stream': True, 'previous': []}, {'prompt': '', 'system': '', 'attachments': [], 'stream': True, 'previous': [{'prompt': '{"tool_calls": [{"name": "hello"}]}'}], 'tool_results': [{'name': 'hello', 'output': 'world', 'tool_call_id': None}]}]
@pytest.mark.asyncio
async def test_async_tools_run_tools_in_parallel():
    ...
    chain_response = model.chain(json.dumps({'tool_calls': [{'name': 'hello'}, {'name': 'hello2'}]}), tools=[hello, hello2])
    output = await chain_response.text()
    bits = output.split('\n}{\n')
    assert len(bits) == 2
    objects = [json.loads(bits[0] + '}'), json.loads('{' + bits[1])]
    assert objects == [{'prompt': '', 'system': '', 'attachments': [], 'stream': True, 'previous': []}, {'prompt': '', 'system': '', 'attachments': [], 'stream': True, 'previous': [{'prompt': '{"tool_calls": [{"name": "hello"}, {"name": "hello2"}]}'}], 'tool_results': [{'name': 'hello', 'output': 'world', 'tool_call_id': None}, {'name': 'hello2', 'output': 'world2', 'tool_call_id': None}]}]
    delta_ns = start_timestamps[1][1] - start_timestamps[0][1]
    assert delta_ns < 100000000 * 0.2
@pytest.mark.asyncio
async def test_async_toolbox():
    ...
    model = llm.get_async_model('echo')
    chain_response = model.chain(json.dumps({'tool_calls': [{'name': 'Tools_go'}]}), tools=[instance])
    output = await chain_response.text()
...
def test_toolbox_add_tool():
    ...
    chain_response = model.chain(json.dumps({'tool_calls': [{'name': 'new_method'}]}), tools=[tools])
    output = chain_response.text()
...
def test_conversation_with_tools(vcr):
    ...
    output1 = conversation.chain(json.dumps({'tool_calls': [{'name': 'multiply', 'arguments': {'a': 5324, 'b': 23233}}]})).text()
    assert '123692492' in output1
    output2 = conversation.chain(json.dumps({'tool_calls': [{'name': 'add', 'arguments': {'a': 841758375, 'b': 123123}}]})).text()
    assert '841881498' in output2
def test_default_tool_llm_version():
    runner = CliRunner()
    result = runner.invoke(cli.cli, ['-m', 'echo', '-T', 'llm_version', json.dumps({'tool_calls': [{'name': 'llm_version'}]})])
    assert result.exit_code == 0
    assert '"output": "{}"'.format(version('llm')) in result.output
def test_cli_tools_with_options():
    runner = CliRunner()
    result = runner.invoke(cli.cli, ['-m', 'mock', '-o', 'max_tokens', '10', '-T', 'llm_version', json.dumps({'tool_calls': [{'name': 'llm_version'}]})], catch_exceptions=False)
    assert result.exit_code == 0
def test_functions_tool_locals():
    runner = CliRunner()
    result = runner.invoke(cli.cli, ['-m', 'echo', '--functions', 'my_locals = locals', '-T', 'llm_version', json.dumps({'tool_calls': [{'name': 'locals'}]})])
    assert result.exit_code == 0
def test_default_tool_llm_time():
    runner = CliRunner()
    result = runner.invoke(cli.cli, ['-m', 'echo', '-T', 'llm_time', json.dumps({'tool_calls': [{'name': 'llm_time'}]})])
    assert result.exit_code == 0
    assert 'timezone_offset' in result.output
    info = llm_time()
    assert set(info.keys()) == {'timezone_offset', 'utc_time_iso', 'local_time', 'local_timezone', 'utc_time', 'is_dst'}
def test_incorrect_tool_usage():
    model = llm.get_model('echo')
    def simple(name: str):
        return name
    chain_response = model.chain(json.dumps({'tool_calls': [{'name': 'bad_tool'}]}), tools=[simple])
    output = chain_response.text()
...
def test_tool_returning_attachment():
    ...
    def return_attachment() -> llm.Attachment:
        return llm.ToolOutput('Output', attachments=[llm.Attachment(content=b'This is a test attachment', type='image/png')])
    chain_response = model.chain(json.dumps({'tool_calls': [{'name': 'return_attachment'}]}), tools=[return_attachment])
    output = chain_response.text()
...
def test_tool_conversation_settings():
    ...
    conversation = model.conversation(tools=[llm_time], before_call=before, after_call=after)
...
def test_tool_errors(async_):
    runner = CliRunner()
    result = runner.invoke(cli.cli, ['-m', 'echo', '--functions', ERROR_FUNCTION, json.dumps({'tool_calls': [{'name': 'trigger_error', 'arguments': {'msg': 'Error!'}}]})] + (['--async'] if async_ else []))
    assert result.exit_code == 0
