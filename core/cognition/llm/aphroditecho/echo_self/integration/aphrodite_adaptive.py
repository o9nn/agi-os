...
try:
    ...
    from .adaptive_architecture import AdaptiveArchitectureFramework, PerformanceMetrics, ArchitectureMutation
    from ..integration.aphrodite_bridge import AphroditeBridge
if TYPE_CHECKING:
    pass
...
class AdaptiveModelLoader:
    def __init__(self, integration: AphroditeAdaptiveIntegration):
        ...
        try:
            ...
            self.dynamic_loader = DynamicModelLoader(max_models=5, memory_limit_gb=16.0, eviction_policy='lru')
            ...
            try:
                if self.dynamic_loader:
                    if isinstance(model_config, dict):
                        ...
                        aphrodite_model_config = ModelConfig(model=model_config.get('model', model_name), tokenizer=model_config.get('tokenizer'), tokenizer_mode=model_config.get('tokenizer_mode', 'auto'), trust_remote_code=model_config.get('trust_remote_code', False), dtype=model_config.get('dtype', 'auto'), max_model_len=model_config.get('max_model_len'), quantization=model_config.get('quantization'), quantization_param_path=model_config.get('quantization_param_path'), seed=model_config.get('seed', 0))
                    else:
                        aphrodite_model_config = model_config
                    success = await self.dynamic_loader.load_model(model_name=model_name, model_config=aphrodite_model_config, force_reload=False)
                    if success:
                        await self.dynamic_loader.switch_active_model(model_name)
                        self.loaded_models[model_name] = {'config': model_config if isinstance(model_config, dict) else model_config.__dict__, 'loaded_at': time.time(), 'adaptation_enabled': enable_adaptation, 'modification_count': 0, 'dynamic_loader_managed': True}
                        ...
                        return False
                self.loaded_models[model_name] = {'config': model_config.copy() if isinstance(model_config, dict) else model_config.__dict__.copy(), 'loaded_at': time.time(), 'adaptation_enabled': enable_adaptation, 'modification_count': 0, 'dynamic_loader_managed': False}
                if enable_adaptation:
                    integration_success = await self.integration.integrate_with_aphrodite(model_config)
                    if not integration_success:
                        logger.warning(f'Adaptation integration failed for model {model_name}')
    ...
    def get_resource_usage(self) -> Dict[str, Any]:
        with self._loader_lock:
            base_info = {'total_models': len(self.loaded_models), 'models': {}}
            if self.dynamic_loader:
                dynamic_usage = self.dynamic_loader.get_resource_usage()
                base_info.update({'dynamic_loader_stats': dynamic_usage, 'memory_utilization': dynamic_usage.get('memory_utilization', 0.0)})
            for model_name, model_info in self.loaded_models.items():
                base_info['models'][model_name] = {'loaded_at': model_info['loaded_at'], 'modification_count': model_info['modification_count'], 'adaptation_enabled': model_info['adaptation_enabled'], 'dynamic_loader_managed': model_info.get('dynamic_loader_managed', False)}
            return base_info