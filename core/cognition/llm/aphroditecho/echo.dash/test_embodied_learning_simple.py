...
def test_basic_functionality():
    ...
    try:
        from embodied_learning import BodyState, MotorAction, SensorimotorExperience, create_embodied_learning_system
        ...
        motor_action = MotorAction(joint_targets={'shoulder': 1.0, 'elbow': 0.5}, muscle_commands={'bicep': 0.8}, duration=1.0)
        print('✓ MotorAction creation successful')
        experience = SensorimotorExperience(initial_body_state=body_state, motor_action=motor_action, resulting_body_state=body_state, sensory_feedback={'touch': 'object_contact', 'proprioception': 'joint_feedback'}, reward=1.0, success=True)
        ...
        motor_action, skill_metrics = system.learn_motor_skill('test_skill', {'target': 'reach_position'}, body_state)
        ...
        for i in range(5):
            varied_state = BodyState(position=(i * 0.1, 0.0, 1.0), joint_angles={'shoulder': float(i * 0.2), 'elbow': float(i * 0.15)})
            varied_action = MotorAction(joint_targets={'shoulder': float(i * 0.2 + 0.1), 'elbow': float(i * 0.15 + 0.05)}, duration=1.0 + i * 0.1)
            varied_experience = SensorimotorExperience(initial_body_state=body_state, motor_action=varied_action, resulting_body_state=varied_state, sensory_feedback={'proprioception': f'experience_{i}', 'touch': f'contact_{i}'}, success=i % 2 == 0, reward=1.0 if i % 2 == 0 else 0.0)
...
def test_performance():
    ...
    try:
        ...
        for i in range(num_experiences):
            experience = SensorimotorExperience(initial_body_state=BodyState(), motor_action=MotorAction(), resulting_body_state=BodyState(), sensory_feedback={'test': f'feedback_{i}'}, success=True)
            system.process_embodied_experience(experience)
        processing_time = time.time() - start_time
        avg_time = processing_time / num_experiences * 1000
