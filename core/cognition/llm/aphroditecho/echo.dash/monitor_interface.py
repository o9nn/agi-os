...
class MonitorInterface:
    ...
    def update_system_stats(self):
        try:
            self.system_stats = {'cpu': psutil.cpu_percent(interval=0.1), 'memory': psutil.virtual_memory().percent, 'disk': psutil.disk_usage('/').percent}
        except Exception as e:
    ...
    def draw_header(self):
        try:
            ...
            self.screen.addstr(1, 0, f"CPU: {self.system_stats.get('cpu', 0):.1f}% | Memory: {self.system_stats.get('memory', 0):.1f}% | Disk: {self.system_stats.get('disk', 0):.1f}%")
            last_distress = self.status.get('last_distress')
            if last_distress:
                self.screen.addstr(2, 0, 'Last Distress: ', curses.A_BOLD)
                self.screen.addstr(2, 14, f"{datetime.fromtimestamp(last_distress['time']).isoformat()} - {last_distress['reason']}", curses.color_pair(3))
        except Exception as e:
