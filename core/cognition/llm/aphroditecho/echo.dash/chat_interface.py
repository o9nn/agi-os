...
class ChatInterface:
    def __init__(self):
        ...
        if SESSION_MANAGER_AVAILABLE:
            try:
                ...
                self.chat_session_id = create_chat_session('chatgpt', f"ChatGPT Session - {time.strftime('%Y-%m-%d %H:%M')}")
        ...
        logging.basicConfig(level=logging.DEBUG)
        self.logger = logging.getLogger(__name__)
        self.session.headers.update({'authority': 'chat.openai.com', 'accept': '*/*', 'accept-encoding': 'gzip, deflate, br', 'accept-language': 'en-US,en;q=0.9', 'cache-control': 'no-cache', 'pragma': 'no-cache', 'referer': 'https://chat.openai.com/', 'sec-ch-ua': '"Not_A Brand";v="8", "Chromium";v="120", "Google Chrome";v="120"', 'sec-ch-ua-mobile': '?0', 'sec-ch-ua-platform': '"Windows"', 'sec-fetch-dest': 'empty', 'sec-fetch-mode': 'cors', 'sec-fetch-site': 'same-origin', 'user-agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36'})
    ...
    def authenticate(self, session_token):
        try:
            ...
            self.session.cookies.set('__Secure-next-auth.session-token', session_token, domain='chat.openai.com', path='/', secure=True, rest={'HttpOnly': True})
            self.logger.info('Getting CSRF token...')
            response = self._make_request('GET', '/', headers={'Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7'})
            self.logger.info('Getting session info...')
            self.session.headers.update({'accept': '*/*', 'authorization': '', 'cookie': f'__Secure-next-auth.session-token={session_token}'})
            ...
            if 'accessToken' in data:
                ...
                conv_response = self._make_request('GET', '/backend-api/conversations', params={'offset': 0, 'limit': 1})
    ...
    def send_query(self, query):
        try:
            ...
            if self.auto_save_enabled and self.chat_session_id:
                try:
                    log_chat_message(self.chat_session_id, 'user', query, platform=ChatPlatform.CHATGPT, metadata={'conversation_id': self.conversation_id, 'timestamp': time.time()})
                except Exception as e:
            ...
            payload = {'action': 'next', 'messages': [{'id': message_id, 'author': {'role': 'user'}, 'content': {'content_type': 'text', 'parts': [query]}, 'metadata': {}}], 'model': 'text-davinci-002-render-sha', 'parent_message_id': parent_id, 'timezone_offset_min': -120, 'suggestions': [], 'history_and_training_disabled': False, 'conversation_mode': {'kind': 'primary_assistant'}, 'force_paragen': False, 'force_rate_limit': False}
            if self.conversation_id:
                payload['conversation_id'] = self.conversation_id
            self.logger.info(f'Sending query: {query}')
            response = self._make_request('POST', '/backend-api/conversation', json=payload, stream=True, timeout=30)
            if response.status_code == 200:
                ...
                if self.auto_save_enabled and self.chat_session_id and full_response:
                    try:
                        log_chat_message(self.chat_session_id, 'assistant', full_response, platform=ChatPlatform.CHATGPT, metadata={'conversation_id': self.conversation_id, 'parent_message_id': self.parent_message_id, 'timestamp': time.time()})
                    except Exception as e:
