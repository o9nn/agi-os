...
try:
    ...
    def demonstrate_embodied_learning():
        ...
        print('=' * 60)
        body_dimensions = {'arm_length': 0.75, 'leg_length': 0.95, 'torso_height': 0.65, 'head_size': 0.25, 'hand_reach': 0.2}
        ...
        exploration_positions = [(0.0, 0.0, 1.0), (0.2, 0.0, 1.0), (0.1, 0.3, 1.0), (0.0, 0.0, 1.2), (-0.1, 0.0, 0.8), (0.15, 0.15, 1.1)]
        for i, target_pos in enumerate(exploration_positions):
            print(f'Exploring position {i + 1}: {target_pos}')
            body_state = BodyState(position=(0.0, 0.0, 1.0), joint_angles={'shoulder': float(i * 0.3), 'elbow': float(i * 0.2), 'wrist': float(i * 0.1)}, joint_velocities={'shoulder': 0.1, 'elbow': 0.05, 'wrist': 0.02})
            motor_action = system.plan_spatial_movement(target_pos, body_state)
            if motor_action:
                success = i < 4
                reward = 1.0 if success else 0.3
                resulting_state = BodyState(position=target_pos if success else (0.05, 0.05, 1.0), joint_angles=motor_action.joint_targets, muscle_activations={'bicep': 0.7, 'tricep': 0.3})
                experience = SensorimotorExperience(initial_body_state=body_state, motor_action=motor_action, resulting_body_state=resulting_state, sensory_feedback={'proprioception': f'joint_feedback_{i}', 'touch': 'contact' if success else 'miss', 'visual': f'target_at_{target_pos}'}, success=success, reward=reward)
        ...
        print('-' * 40)
        spatial_targets = [(0.3, 0.2, 1.1, 'Near target'), (0.5, 0.0, 1.0, 'Forward reach'), (0.0, 0.4, 1.2, 'Side reach'), (0.8, 0.6, 1.3, 'Far target')]
        ...
        print('-' * 40)
        skills_to_learn = [('reach_precise', {'accuracy': 0.95, 'position': (0.3, 0.2, 1.1)}), ('grasp_object', {'grip_strength': 0.8, 'precision': 0.9}), ('lift_weight', {'force': 0.7, 'stability': 0.85}), ('place_accurate', {'accuracy': 0.9, 'gentleness': 0.8})]
        for skill_name, target_outcome in skills_to_learn:
            print(f'\nLearning skill: {skill_name}')
            for attempt in range(5):
                motor_action, skill_metrics = system.learn_motor_skill(skill_name, target_outcome, current_body_state)
                min(0.9, 0.2 + attempt * 0.15)
                success = attempt >= 2 and attempt / 5.0 > 0.4
                reward = 1.0 if success else 0.2
                outcome = SensorimotorExperience(initial_body_state=current_body_state, motor_action=motor_action, resulting_body_state=BodyState(position=(0.2 + attempt * 0.02, 0.1, 1.05), joint_angles=motor_action.joint_targets), sensory_feedback={'skill_feedback': f'{skill_name}_attempt_{attempt}', 'performance': 'good' if success else 'needs_work'}, success=success, reward=reward)
                system.update_from_skill_outcome(skill_name, outcome)
                print(f"  Attempt {attempt + 1}: {('✓' if success else '✗')} (reward: {reward:.1f})")
    ...
    def save_demonstration_results(metrics):
        ...
        with open(results_file, 'w') as f:
            json.dump({'timestamp': time.time(), 'demonstration': 'embodied_learning_task_2.1.2', 'metrics': serializable_metrics, 'status': 'completed_successfully'}, f, indent=2)
