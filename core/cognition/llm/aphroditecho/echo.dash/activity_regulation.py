...
class ActivityRegulator:
            ...
            try:
                ...
                for task_id, task in self.periodic_tasks.items():
                    if task.last_run:
                        ...
                        if time_since_last > task.interval * 2:
                            self._log_activity('emergency', f'Task {task_id} delayed by {time_since_last:.1f}s')
    ...
    def add_task(self, task_id: str, callback: Callable, priority: TaskPriority=TaskPriority.MEDIUM, interval: Optional[float]=None):
        task = ScheduledTask(priority=priority, scheduled_time=time.time(), task_id=task_id, callback=callback, interval=interval)
        if interval:
                ...
                while not self.task_queue.empty():
                    ...
                    if current_time >= task.scheduled_time:
                        try:
                            ...
                            self._log_activity('emergency', f'Task {task.task_id} failed: {str(e)}')
                    else:
                        self.task_queue.put(task)
                        break
