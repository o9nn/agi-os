...
class ActivityStream:
    def __init__(self, screen, stream_type: StreamType):
        ...
        self.paths = {'cognitive': self.echo_dir / 'cognitive' / 'activity.json', 'sensory': self.echo_dir / 'sensory' / 'activity.json', 'ml': self.echo_dir / 'ml' / 'activity.json', 'browser': self.echo_dir / 'browser' / 'activity.json', 'terminal': self.echo_dir / 'terminal' / 'activity.json', 'personality': self.echo_dir / 'personality' / 'activity.json', 'emergency': self.echo_dir / 'emergency' / 'activity.json'}
        self.activities = {k: [] for k in self.paths}
        self.max_items = 100
        self.system_stats = {}
        self.last_screen_state = {}
    def _screen_state_changed(self) -> bool:
        current_state = {'activities': self.activities, 'stats': self.system_stats}
                ...
                if path.exists():
                    try:
                        with open(path) as f:
                            ...
                            print(f'Updated {activity_type} activities: {len(self.activities[activity_type])} entries')
                    except Exception as e:
                        print(f'Error updating {activity_type} activities: {str(e)}')
        except Exception as e:
            print(f'Error in update_activities: {str(e)}')
            self.activities['emergency'].append({'time': time.time(), 'error': f'Error updating activities: {str(e)}'})
    def update_system_stats(self):
        try:
            self.system_stats = {'cpu': psutil.cpu_percent(interval=0.1), 'memory': psutil.virtual_memory().percent, 'disk': psutil.disk_usage('/').percent, 'time': datetime.now().isoformat()}
        except Exception as e:
            self.activities['emergency'].append({'time': time.time(), 'error': f'Error updating stats: {str(e)}'})
    def get_activity_color(self, activity_type: str) -> int:
        colors = {'cognitive': curses.color_pair(1), 'sensory': curses.color_pair(4), 'ml': curses.color_pair(5), 'browser': curses.color_pair(6), 'terminal': curses.color_pair(2), 'personality': curses.color_pair(5), 'emergency': curses.color_pair(3)}
    ...
    def draw_header(self):
        try:
            ...
            stats = f"CPU: {self.system_stats.get('cpu', 0):.1f}% | Memory: {self.system_stats.get('memory', 0):.1f}% | Disk: {self.system_stats.get('disk', 0):.1f}% | Time: {self.system_stats.get('time', '')}"
    ...
    def _draw_activity(self, activity: Dict, activity_type: str, line: int):
        try:
            ...
            remaining_width = width - timestamp_width - 2
            ...
            if line < height - 1:
                ...
                self.screen.addstr(line, timestamp_width + 1, message, self.get_activity_color(activity_type))
        except curses.error:
            pass
                ...
                for activity_type, activities in self.activities.items():
                    if activities:
                        ...
                        if current_line < height - 1:
                            self.screen.addstr(current_line, 0, header, curses.A_BOLD | self.get_activity_color(activity_type))
                            current_line += 2
...
def main(stream_type: StreamType=StreamType.ALL, verbose: bool=False):
    if verbose:
        ...
        paths = {'cognitive': echo_dir / 'cognitive' / 'activity.json', 'sensory': echo_dir / 'sensory' / 'activity.json', 'ml': echo_dir / 'ml' / 'activity.json', 'browser': echo_dir / 'browser' / 'activity.json', 'terminal': echo_dir / 'terminal' / 'activity.json', 'personality': echo_dir / 'personality' / 'activity.json', 'emergency': echo_dir / 'emergency' / 'activity.json'}
                                ...
                                if isinstance(activities, list):
                                    ...
                                    for activity in new_activities:
                                        ...
                                        if activity_type == 'emergency':
                                            print(f'\x1b[91m[{timestamp}] !!! {activity_type}: {msg}\x1b[0m')
                                        else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--type', choices=[t.value for t in StreamType], default=StreamType.ALL.value, help='Type of activities to display')
    parser.add_argument('--verbose', '-v', action='store_true', help='Use verbose logging mode instead of UI')
