...
from deep_tree_echo import DeepTreeEcho, Membrane, CognitiveMembrane, ExtensionMembrane, SecurityMembrane, MembraneManager, MembraneMessage
class TestPSystemMembranes(unittest.TestCase):
    ...
    def test_message_passing(self):
        success = self.echo.send_membrane_message('cognitive', 'extension', 'test_message', {'data': 'test'})
        self.assertTrue(success)
        results = self.echo.process_membrane_messages()
        self.assertIsInstance(results, dict)
    def test_extension_loading(self):
        success = self.echo.load_extension_to_membrane('test_extension', {'version': '1.0', 'description': 'Test extension'})
        self.assertTrue(success)
    ...
    def test_membrane_message_structure(self):
        message = MembraneMessage(source_membrane='test_source', target_membrane='test_target', message_type='test_type', data={'test': 'data'})
