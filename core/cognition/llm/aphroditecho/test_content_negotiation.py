...
try:
    from aphrodite.endpoints.deep_tree_echo.content_negotiation import ContentNegotiator, AcceptedType, XMLResponseGenerator, MultiFormatResponse, wants_html, wants_xml, create_negotiated_response
...
class TestContentNegotiation:
    ...
    def test_xml_generation(self):
        ...
        data = {'service': 'API', 'stats': {'users': 100, 'active': True}, 'endpoints': ['/api/v1', '/api/v2']}
    ...
    def test_multi_format_response_xml(self):
        ...
        request = self.create_mock_request('application/xml')
        response = self.multi_format.create_response(data, request, xml_root='test_response')
    ...
    def test_create_negotiated_response_function(self):
        ...
        assert hasattr(response, 'body')
        request = self.create_mock_request('application/xml')
        response = create_negotiated_response(data, request, xml_root='api_response')
        assert response.media_type == 'application/xml'
        content = response.body.decode()
