...
from hypergraph_service import get_hypergraph_service, HypergraphService, IdentityFragment, InteractionData, EchoPropagationResult
...
async def propagate_echo(request: EchoPropagationRequest, hypergraph: HypergraphService=Depends(get_hypergraph_service)):
    start_time = datetime.now()
    try:
        all_results = []
        for start_node in request.start_nodes:
            results = await hypergraph.propagate_activation(start_node, request.max_depth, request.min_weight)
        ...
        propagation_paths = [r.path for r in final_results[:10]]
        propagated_nodes = [{'node_id': r.node_id, 'depth': r.depth, 'accumulated_weight': r.accumulated_weight, 'path': r.path} for r in final_results]
        execution_time = (datetime.now() - start_time).total_seconds()
        return EchoPropagationResponse(propagated_nodes=propagated_nodes, activation_strength=total_activation, propagation_paths=propagation_paths, total_nodes=len(final_results), execution_time=execution_time)
...
async def get_identity_context(context: str, limit: int=10, hypergraph: HypergraphService=Depends(get_hypergraph_service)):
    try:
        ...
        for fragment in limited_fragments:
            fragment_dict = {'id': fragment.id, 'type': fragment.type, 'data': fragment.data, 'metadata': fragment.metadata, 'activation_strength': fragment.activation_strength}
        ...
        return IdentityContextResponse(context=context, active_fragments=fragment_dicts, dominant_persona=dominant_persona, confidence_score=confidence_score, total_fragments=len(fragments))
    except Exception as e:
...
async def update_hypergraph(request: HypergraphUpdateRequest, background_tasks: BackgroundTasks, hypergraph: HypergraphService=Depends(get_hypergraph_service)):
    try:
        interaction_data = InteractionData(trigger_id=request.interaction_data.get('trigger_id', ''), affected_nodes=request.interaction_data.get('affected_nodes', []), strength=request.interaction_data.get('strength', 0.5), context=request.interaction_data.get('context', {}), timestamp=datetime.now())
        background_tasks.add_task(hypergraph.update_from_interaction, interaction_data)
...
async def create_identity_fragment(request: CreateFragmentRequest, hypergraph: HypergraphService=Depends(get_hypergraph_service)):
    try:
        valid_types = ['persona', 'skill', 'memory', 'belief', 'value', 'trait', 'goal', 'context']
        if request.type not in valid_types:
            raise HTTPException(status_code=400, detail=f'Invalid fragment type. Must be one of: {valid_types}')
        fragment_id = await hypergraph.create_identity_fragment(request.type, request.data, request.metadata)
        return {'fragment_id': fragment_id, 'type': request.type, 'status': 'created'}
...
async def create_relationship(request: CreateRelationshipRequest, hypergraph: HypergraphService=Depends(get_hypergraph_service)):
    try:
        ...
        if request.relationship_type not in valid_types:
            raise HTTPException(status_code=400, detail=f'Invalid relationship type. Must be one of: {valid_types}')
        relationship_id = await hypergraph.create_relationship(request.node_ids, request.relationship_type, request.weight, request.metadata)
        return {'relationship_id': relationship_id, 'type': request.relationship_type, 'status': 'created'}
    except HTTPException:
        raise
    except Exception as e:
...
async def get_fragment_by_id(fragment_id: str, hypergraph: HypergraphService=Depends(get_hypergraph_service)):
    try:
        fragment = await hypergraph.get_node_by_id(fragment_id)
        if not fragment:
            raise HTTPException(status_code=404, detail='Fragment not found')
        return {'id': fragment.id, 'type': fragment.type, 'data': fragment.data, 'metadata': fragment.metadata, 'activation_strength': fragment.activation_strength}
    except HTTPException:
        raise
    except Exception as e:
...
async def get_fragment_relationships(fragment_id: str, hypergraph: HypergraphService=Depends(get_hypergraph_service)):
    try:
        relationships = await hypergraph.get_node_relationships(fragment_id)
        return {'fragment_id': fragment_id, 'relationships': relationships, 'total_relationships': len(relationships)}
    except Exception as e:
...
async def get_fragments_by_type(fragment_type: str, limit: int=50, hypergraph: HypergraphService=Depends(get_hypergraph_service)):
    try:
        ...
        for fragment in fragments:
            fragment_dict = {'id': fragment.id, 'type': fragment.type, 'data': fragment.data, 'metadata': fragment.metadata, 'activation_strength': fragment.activation_strength}
            fragment_dicts.append(fragment_dict)
        return {'fragment_type': fragment_type, 'fragments': fragment_dicts, 'total_fragments': len(fragment_dicts)}
    except Exception as e:
...
async def get_hypergraph_statistics(hypergraph: HypergraphService=Depends(get_hypergraph_service)):
    try:
        stats = await hypergraph.get_statistics()
        return HypergraphStatsResponse(total_nodes=stats['total_nodes'], total_edges=stats['total_edges'], total_events=stats['total_events'], average_edge_weight=stats['average_edge_weight'], node_types=stats['node_types'], edge_types=stats['edge_types'], cache_stats=stats['cache_stats'])
    except Exception as e:
        logger.error(f'Error getting hypergraph statistics: {e}')
        raise HTTPException(status_code=500, detail=f'Statistics retrieval failed: {str(e)}')
@hypergraph_router.get('/health')
async def hypergraph_health_check(hypergraph: HypergraphService=Depends(get_hypergraph_service)):
    try:
        stats = await hypergraph.get_statistics()
        return {'status': 'healthy', 'timestamp': datetime.now().isoformat(), 'database_connected': True, 'total_nodes': stats['total_nodes'], 'total_edges': stats['total_edges']}
    except Exception as e:
        logger.error(f'Hypergraph health check failed: {e}')
        return JSONResponse(status_code=503, content={'status': 'unhealthy', 'timestamp': datetime.now().isoformat(), 'error': str(e)})
@hypergraph_router.post('/cache/clear')
async def clear_hypergraph_cache(hypergraph: HypergraphService=Depends(get_hypergraph_service)):
    try:
...
async def get_cache_statistics(hypergraph: HypergraphService=Depends(get_hypergraph_service)):
    try:
        return {'propagation_cache_size': len(hypergraph.propagation_cache), 'propagation_cache_maxsize': hypergraph.propagation_cache.maxsize, 'identity_cache_size': len(hypergraph.identity_cache), 'identity_cache_maxsize': hypergraph.identity_cache.maxsize, 'timestamp': datetime.now().isoformat()}
    except Exception as e:
