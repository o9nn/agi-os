...
def get_memory_cycles():
    ...
    result = []
    for cycle in cycles:
        result.append({'id': cycle.id, 'name': cycle.name, 'cycle_type': cycle.cycle_type, 'duration_ms': cycle.duration_ms, 'variance_percent': cycle.variance_percent, 'last_execution': cycle.last_execution.isoformat() if cycle.last_execution else None, 'next_scheduled': cycle.next_scheduled.isoformat() if cycle.next_scheduled else None, 'enabled': cycle.enabled, 'description': cycle.description, 'execution_count': cycle.execution_count, 'avg_execution_time_ms': cycle.avg_execution_time_ms})
...
def create_memory_cycle():
    ...
    try:
        cycle = MemoryCycle(name=data['name'], cycle_type=data['cycle_type'], duration_ms=data['duration_ms'], variance_percent=data.get('variance_percent', 10.0), priority=data.get('priority', 5), enabled=data.get('enabled', True), description=data.get('description'), function_code=data.get('function_code'), user_id=current_user.id if current_user.is_authenticated else None)
        db.session.add(cycle)
        db.session.commit()
        cycle.calculate_next_execution()
        db.session.commit()
        return (jsonify({'id': cycle.id, 'name': cycle.name, 'cycle_type': cycle.cycle_type, 'duration_ms': cycle.duration_ms, 'next_scheduled': cycle.next_scheduled.isoformat() if cycle.next_scheduled else None}), 201)
...
def get_memory_cycle(cycle_id):
    ...
    return jsonify({'id': cycle.id, 'name': cycle.name, 'cycle_type': cycle.cycle_type, 'duration_ms': cycle.duration_ms, 'variance_percent': cycle.variance_percent, 'last_execution': cycle.last_execution.isoformat() if cycle.last_execution else None, 'next_scheduled': cycle.next_scheduled.isoformat() if cycle.next_scheduled else None, 'priority': cycle.priority, 'enabled': cycle.enabled, 'description': cycle.description, 'function_code': cycle.function_code, 'execution_count': cycle.execution_count, 'avg_execution_time_ms': cycle.avg_execution_time_ms, 'last_result': last_result, 'created_at': cycle.created_at.isoformat(), 'updated_at': cycle.updated_at.isoformat(), 'user_id': cycle.user_id})
...
def update_memory_cycle(cycle_id):
    ...
    try:
        ...
        if 'duration_ms' in data or 'variance_percent' in data:
            cycle.calculate_next_execution()
        db.session.commit()
        return jsonify({'id': cycle.id, 'name': cycle.name, 'cycle_type': cycle.cycle_type, 'duration_ms': cycle.duration_ms, 'next_scheduled': cycle.next_scheduled.isoformat() if cycle.next_scheduled else None, 'enabled': cycle.enabled, 'updated_at': cycle.updated_at.isoformat()})
    except Exception as e:
...
def trigger_memory_cycle(cycle_id):
    ...
    try:
        ...
        db.session.commit()
        return jsonify({'success': True, 'cycle_id': cycle.id, 'name': cycle.name, 'last_execution': cycle.last_execution.isoformat(), 'next_scheduled': cycle.next_scheduled.isoformat() if cycle.next_scheduled else None, 'result': result})
    except Exception as e:
...
def get_memory_nodes():
    ...
    result = []
    for node in nodes:
        result.append({'id': node.id, 'node_id': node.node_id, 'memory_type': node.memory_type, 'activation_level': node.activation_level, 'decay_rate': node.decay_rate, 'consolidation_stage': node.consolidation_stage, 'emotional_valence': node.emotional_valence, 'emotional_arousal': node.emotional_arousal, 'salience': node.salience, 'timestamp': node.timestamp.isoformat(), 'source': node.source, 'context': node.get_context(), 'last_activated': node.last_activated.isoformat() if node.last_activated else None, 'activation_count': node.activation_count})
    return jsonify(result)
...
def create_memory_node():
    ...
    try:
        ...
        node = MemoryNode(node_id=data['node_id'], memory_type=data['memory_type'], activation_level=data.get('activation_level', 0.5), decay_rate=data.get('decay_rate', 0.05), consolidation_stage=data.get('consolidation_stage', 0), emotional_valence=data.get('emotional_valence', 0.0), emotional_arousal=data.get('emotional_arousal', 0.0), salience=data.get('salience', 0.5), context=context_json, source=data.get('source', 'user'))
        db.session.add(node)
        db.session.commit()
        return (jsonify({'id': node.id, 'node_id': node.node_id, 'memory_type': node.memory_type, 'activation_level': node.activation_level, 'timestamp': node.timestamp.isoformat()}), 201)
    except Exception as e:
...
def get_memory_node(node_id):
    ...
    base_node_details = {'id': base_node.id, 'name': base_node.name, 'node_type': base_node.node_type, 'expression': base_node.expression} if base_node else None
    associations = []
    for assoc in MemoryAssociation.query.filter((MemoryAssociation.source_id == node_id) | (MemoryAssociation.target_id == node_id)).all():
        other_id = assoc.target_id if assoc.source_id == node_id else assoc.source_id
        other_node = MemoryNode.query.get(other_id)
        if other_node:
            associations.append({'id': assoc.id, 'other_node_id': other_id, 'other_node_type': other_node.memory_type, 'association_type': assoc.association_type, 'strength': assoc.strength, 'bidirectional': assoc.bidirectional, 'created_at': assoc.created_at.isoformat(), 'metadata': assoc.get_metadata() if hasattr(assoc, 'get_metadata') else {}})
    return jsonify({'id': node.id, 'node_id': node.node_id, 'memory_type': node.memory_type, 'activation_level': node.activation_level, 'decay_rate': node.decay_rate, 'consolidation_stage': node.consolidation_stage, 'emotional_valence': node.emotional_valence, 'emotional_arousal': node.emotional_arousal, 'salience': node.salience, 'timestamp': node.timestamp.isoformat(), 'source': node.source, 'context': node.get_context(), 'created_at': node.created_at.isoformat(), 'updated_at': node.updated_at.isoformat(), 'last_activated': node.last_activated.isoformat() if node.last_activated else None, 'activation_count': node.activation_count, 'base_node': base_node_details, 'associations': associations})
...
def update_memory_node(node_id):
    ...
    try:
        ...
        db.session.commit()
        return jsonify({'id': node.id, 'memory_type': node.memory_type, 'activation_level': node.activation_level, 'updated_at': node.updated_at.isoformat()})
    except Exception as e:
...
def delete_memory_node(node_id):
    node = MemoryNode.query.get_or_404(node_id)
    try:
        MemoryAssociation.query.filter((MemoryAssociation.source_id == node_id) | (MemoryAssociation.target_id == node_id)).delete()
...
def activate_memory_node(node_id):
    ...
    try:
        ...
        return jsonify({'success': True, 'node_id': node_id, 'previous_activation': node.activation_level, 'new_activation': new_activation, 'timestamp': datetime.utcnow().isoformat()})
    except Exception as e:
...
def get_memory_associations():
    ...
    for assoc in associations:
        result.append({'id': assoc.id, 'source_id': assoc.source_id, 'target_id': assoc.target_id, 'association_type': assoc.association_type, 'strength': assoc.strength, 'bidirectional': assoc.bidirectional, 'created_at': assoc.created_at.isoformat(), 'updated_at': assoc.updated_at.isoformat(), 'metadata': assoc.get_metadata() if hasattr(assoc, 'get_metadata') else {}})
    return jsonify(result)
...
def create_memory_association():
    ...
    try:
        ...
        existing = MemoryAssociation.query.filter_by(source_id=data['source_id'], target_id=data['target_id']).first()
        if existing:
        ...
        assoc = MemoryAssociation(source_id=data['source_id'], target_id=data['target_id'], association_type=data.get('association_type', 'default'), strength=data.get('strength', 0.5), bidirectional=data.get('bidirectional', True), metadata=metadata_json)
        db.session.add(assoc)
        db.session.commit()
        return (jsonify({'id': assoc.id, 'source_id': assoc.source_id, 'target_id': assoc.target_id, 'association_type': assoc.association_type, 'strength': assoc.strength, 'bidirectional': assoc.bidirectional, 'created_at': assoc.created_at.isoformat()}), 201)
    except Exception as e:
...
def get_memory_association(assoc_id):
    ...
    return jsonify({'id': assoc.id, 'source_id': assoc.source_id, 'target_id': assoc.target_id, 'association_type': assoc.association_type, 'strength': assoc.strength, 'bidirectional': assoc.bidirectional, 'created_at': assoc.created_at.isoformat(), 'updated_at': assoc.updated_at.isoformat(), 'metadata': assoc.get_metadata() if hasattr(assoc, 'get_metadata') else {}})
...
def update_memory_association(assoc_id):
    ...
    try:
        ...
        db.session.commit()
        return jsonify({'id': assoc.id, 'association_type': assoc.association_type, 'strength': assoc.strength, 'updated_at': assoc.updated_at.isoformat()})
    except Exception as e:
...
def get_pattern_templates():
    ...
    for pattern in patterns:
        result.append({'id': pattern.id, 'name': pattern.name, 'pattern_type': pattern.pattern_type, 'structure': pattern.get_structure(), 'rules': pattern.get_rules(), 'activation_threshold': pattern.activation_threshold, 'description': pattern.description, 'created_at': pattern.created_at.isoformat(), 'user_id': pattern.user_id})
    return jsonify(result)
...
def create_pattern_template():
    ...
    try:
        pattern = PatternTemplate(name=data['name'], pattern_type=data['pattern_type'], activation_threshold=data.get('activation_threshold', 0.7), description=data.get('description'), user_id=current_user.id if current_user.is_authenticated else None)
        ...
        db.session.commit()
        return (jsonify({'id': pattern.id, 'name': pattern.name, 'pattern_type': pattern.pattern_type, 'created_at': pattern.created_at.isoformat()}), 201)
    except Exception as e:
...
def get_pattern_template(pattern_id):
    ...
    for node in pattern.memory_nodes:
        memory_nodes.append({'id': node.id, 'memory_type': node.memory_type, 'activation_level': node.activation_level})
    return jsonify({'id': pattern.id, 'name': pattern.name, 'pattern_type': pattern.pattern_type, 'structure': pattern.get_structure(), 'rules': pattern.get_rules(), 'activation_threshold': pattern.activation_threshold, 'description': pattern.description, 'created_at': pattern.created_at.isoformat(), 'updated_at': pattern.updated_at.isoformat(), 'user_id': pattern.user_id, 'memory_nodes': memory_nodes})
...
def update_pattern_template(pattern_id):
    ...
    try:
        ...
        db.session.commit()
        return jsonify({'id': pattern.id, 'name': pattern.name, 'pattern_type': pattern.pattern_type, 'updated_at': pattern.updated_at.isoformat()})
    except Exception as e:
...
def match_pattern_template(pattern_id):
    ...
    try:
        ...
        return jsonify({'pattern_id': pattern_id, 'pattern_name': pattern.name, 'pattern_type': pattern.pattern_type, 'match_score': match_score, 'threshold': pattern.activation_threshold, 'matched': match_score >= pattern.activation_threshold, 'node_count': len(nodes)})
    except Exception as e:
...
def find_similar_memories(node_id):
    ...
    try:
        ...
        for sim_node, score in similar:
            result.append({'node_id': sim_node.id, 'memory_type': sim_node.memory_type, 'similarity_score': score, 'activation_level': sim_node.activation_level, 'emotional_valence': sim_node.emotional_valence, 'emotional_arousal': sim_node.emotional_arousal})
        return jsonify({'base_node_id': node_id, 'similar_nodes': result, 'count': len(result)})
    except Exception as e:
...
def get_dream_states():
    ...
    result = []
    for dream in dreams:
        result.append({'id': dream.id, 'title': dream.title, 'start_time': dream.start_time.isoformat() if dream.start_time else None, 'end_time': dream.end_time.isoformat() if dream.end_time else None, 'duration_seconds': dream.duration_seconds, 'dream_type': dream.dream_type, 'emotional_tone': dream.emotional_tone, 'coherence': dream.coherence, 'insights_count': len(dream.get_insights()) if hasattr(dream, 'get_insights') else 0, 'new_associations_count': len(dream.get_new_associations()) if hasattr(dream, 'get_new_associations') else 0})
    return jsonify(result)
...
def get_dream_state(dream_id):
    ...
    return jsonify({'id': dream.id, 'title': dream.title, 'start_time': dream.start_time.isoformat() if dream.start_time else None, 'end_time': dream.end_time.isoformat() if dream.end_time else None, 'duration_seconds': dream.duration_seconds, 'content': dream.get_content() if hasattr(dream, 'get_content') else {}, 'source_memories': dream.get_source_memories() if hasattr(dream, 'get_source_memories') else [], 'pattern_activations': dream.get_pattern_activations() if hasattr(dream, 'get_pattern_activations') else {}, 'insights': dream.get_insights() if hasattr(dream, 'get_insights') else [], 'new_associations': dream.get_new_associations() if hasattr(dream, 'get_new_associations') else [], 'dream_type': dream.dream_type, 'emotional_tone': dream.emotional_tone, 'coherence': dream.coherence, 'user_id': dream.user_id})
...
def start_memory_system():
    if not temporal_processor.running:
        ...
        return jsonify({'success': True, 'message': 'Memory system started', 'state': temporal_processor.get_system_state()})
    else:
        return jsonify({'success': False, 'message': 'Memory system is already running', 'state': temporal_processor.get_system_state()})
@app.route('/api/memory/system/stop', methods=['POST'])
@login_required
def stop_memory_system():
    if temporal_processor.running:
        temporal_processor.stop()
        return jsonify({'success': True, 'message': 'Memory system stopped', 'state': temporal_processor.get_system_state()})
    else:
        return jsonify({'success': False, 'message': 'Memory system is not running', 'state': temporal_processor.get_system_state()})
