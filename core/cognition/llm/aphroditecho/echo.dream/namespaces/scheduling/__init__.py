...
class SchedulingNamespace:
    ...
    def create_schedule(self, name, description, duration_days):
        ...
        self.schedules[schedule_id] = {'id': schedule_id, 'name': name, 'description': description, 'duration_days': duration_days, 'milestones': [], 'created': datetime.now().isoformat(), 'modified': datetime.now().isoformat()}
        self._save_schedules()
        return self.schedules[schedule_id]
    def add_milestone(self, schedule_id, milestone_name, day_offset, estimated_hours):
        if schedule_id not in self.schedules:
            return False
        milestone = {'name': milestone_name, 'day_offset': day_offset, 'estimated_hours': estimated_hours, 'added': datetime.now().isoformat()}
    ...
    def schedule_project(self, project_id, schedule_id, start_date):
        ...
        for milestone in schedule['milestones']:
            ...
            self.calendar[date_key].append({'project_id': project_id, 'milestone': milestone['name'], 'hours': milestone['estimated_hours'], 'scheduled_at': datetime.now().isoformat()})
        self._save_calendar()
        return True
