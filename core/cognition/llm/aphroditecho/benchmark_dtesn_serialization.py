...
from test_serialization_simple import MockDTESNResult, OptimizedJSONSerializer, DeterministicSerializer, SerializationConfig, SerializationFormat
def benchmark_serialization_methods(result_sizes: List[int]=[1, 2, 5, 10], iterations: int=1000) -> Dict[str, Dict[str, Any]]:
    results = {}
    for size in result_sizes:
        ...
        size_results['naive_json'] = {'avg_time_ms': statistics.mean(times), 'median_time_ms': statistics.median(times), 'min_time_ms': min(times), 'max_time_ms': max(times), 'std_dev_ms': statistics.stdev(times) if len(times) > 1 else 0, 'size_bytes': len(json_data), 'method': 'Standard JSON serialization'}
        ...
        for _ in range(iterations):
            ...
            times.append((time.time() - start) * 1000)
        size_results['optimized_json'] = {'avg_time_ms': statistics.mean(times), 'median_time_ms': statistics.median(times), 'min_time_ms': min(times), 'max_time_ms': max(times), 'std_dev_ms': statistics.stdev(times) if len(times) > 1 else 0, 'size_bytes': len(optimized_data), 'method': 'Optimized JSON serialization'}
        ...
        for _ in range(iterations):
            ...
            times.append((time.time() - start) * 1000)
        size_results['deterministic'] = {'avg_time_ms': statistics.mean(times), 'median_time_ms': statistics.median(times), 'min_time_ms': min(times), 'max_time_ms': max(times), 'std_dev_ms': statistics.stdev(times) if len(times) > 1 else 0, 'size_bytes': len(deterministic_data), 'method': 'Deterministic serialization'}
...
def calculate_improvements(results: Dict[str, Dict[str, Any]]) -> Dict[str, Dict[str, float]]:
    ...
    for size_key, size_data in results.items():
        ...
        for method_key, method_data in size_data.items():
            ...
            size_improvements[method_key] = {'time_improvement': time_improvement, 'size_improvement': size_improvement, 'size_ratio': method_data['size_bytes'] / baseline['size_bytes']}
...
def main():
    ...
    results = benchmark_serialization_methods(result_sizes=[1, 2, 5, 10], iterations=1000)
