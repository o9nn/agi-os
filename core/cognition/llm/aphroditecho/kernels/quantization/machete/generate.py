...
import jinja2
from aphrodite_cutlass_library_extension import DataType, EpilogueScheduleTag, EpilogueScheduleType, MixedInputKernelScheduleType, TileSchedulerTag, TileSchedulerType, APHRODITEDataType, APHRODITEDataTypeNames, APHRODITEDataTypeSize, APHRODITEDataTypeTag, APHRODITEDataTypeTorchDataTypeTag, APHRODITEDataTypeAPHRODITEScalarTypeTag, APHRODITEKernelScheduleTag
...
def generate_type_option_name(kernel_types: TypeConfig):
    return ', '.join([f"{field.name.replace('b_', 'with_') + '_type'}=" + APHRODITEDataTypeNames[getattr(kernel_types, field.name)] for field in fields(TypeConfig)])
...
template_globals = {'void': DataType.void, 'DataTypeTag': APHRODITEDataTypeTag, 'APHRODITEScalarTypeTag': APHRODITEDataTypeAPHRODITEScalarTypeTag, 'TorchTypeTag': APHRODITEDataTypeTorchDataTypeTag, 'KernelScheduleTag': APHRODITEKernelScheduleTag, 'EpilogueScheduleTag': EpilogueScheduleTag, 'TileSchedulerTag': TileSchedulerTag, 'to_cute_constant': to_cute_constant, 'gen_sch_sig': generate_terse_sch_sig, 'gen_type_sig': generate_type_signature, 'unique_schedules': unique_schedules, 'unsigned_type_with_bitwidth': unsigned_type_with_bitwidth, 'gen_type_option_name': generate_type_option_name}
...
def create_sources(impl_configs: list[ImplConfig], num_impl_files=8):
    ...
    for impl_config in impl_configs:
        ...
        prepack_types.append(PrepackTypeConfig(a=impl_config.types.a, b_num_bits=APHRODITEDataTypeSize[impl_config.types.b], convert=convert_type, accumulator=impl_config.types.accumulator))
...
def generate():
    ...
    def get_unique_schedules(heuristic: dict[str, ScheduleConfig]):
        ...
        return schedules
    impl_configs = []
    GPTQ_kernel_type_configs = list((TypeConfig(a=a, b=b, b_group_scale=a, b_group_zeropoint=DataType.void, b_channel_scale=DataType.void, a_token_scale=DataType.void, out=a, accumulator=DataType.f32) for b in (APHRODITEDataType.u4b8, APHRODITEDataType.u8b128) for a in (DataType.f16, DataType.bf16)))
    ...
    QQQ_kernel_types = [*(TypeConfig(a=DataType.s8, b=APHRODITEDataType.u4b8, b_group_scale=b_group_scale, b_group_zeropoint=DataType.void, b_channel_scale=DataType.f32, a_token_scale=DataType.f32, out=DataType.f16, accumulator=DataType.s32) for b_group_scale in (DataType.f16, DataType.void)), *(TypeConfig(a=DataType.e4m3, b=APHRODITEDataType.u4b8, b_group_scale=b_group_scale, b_group_zeropoint=DataType.void, b_channel_scale=DataType.f32, a_token_scale=DataType.f32, out=DataType.f16, accumulator=DataType.f32) for b_group_scale in (DataType.f16, DataType.void))]
