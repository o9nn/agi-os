...
std::vector<at::Tensor> get_mla_metadata(at::Tensor& seqlens_k,
...
cutlass::ceil_div(num_heads_per_head_k, block_size_m);
auto tile_scheduler_metadata =
torch::empty({num_sm_parts, TileSchedulerMetaDataSize}, options);
...
auto opts = q.options();
at::Tensor out =
torch::empty({batch_size, seqlen_q, num_heads, head_size_v}, opts);
...
torch::empty({batch_size + params.num_sm_parts, num_heads, seqlen_q},
opts.dtype(at::kFloat));
at::Tensor out_accum = torch::empty(
{batch_size + params.num_sm_parts, num_heads, seqlen_q, head_size_v},
opts.dtype(at::kFloat));
...
softmax_lse =
softmax_lse.view({batch_size, num_heads_k, seqlen_q_ori, ngroups})
.transpose(2, 3)
...
m.impl("get_mla_metadata", torch::kCUDA,
make_pytorch_shim(&get_mla_metadata));
m.def("fwd_kvcache_mla", make_pytorch_shim(&mha_fwd_kvcache_mla));
m.impl("fwd_kvcache_mla", torch::kCUDA,
make_pytorch_shim(&mha_fwd_kvcache_mla));
}
PyMODINIT_FUNC PyInit__flashmla_C() {
static struct PyModuleDef module = {PyModuleDef_HEAD_INIT, "_flashmla_C",
nullptr, 0, nullptr};
