...
async def test_interaction_tracking():
    ...
    config = DTESNTrainingConfiguration(training_config=TrainingConfiguration(population_size=8, training_mode=TrainingMode.HYBRID, episode_batch_size=6), population_config=PopulationConfig(population_size=8), learning_config=LearningConfiguration(), enable_dtesn_monitoring=False, enable_aar_orchestration=False)
