...
export class LlamaContext {
     ...
     public readonly _ctx: AddonContext;
     public readonly _onReclaimUnusedSequenceId = new EventRelay<void>();
     public readonly _backendContextDisposeGuard: DisposeGuard;
     private readonly _model: LlamaModel;
     private readonly _contextSize: number;
     private readonly _batchSize: number;
     private readonly _flashAttention: boolean;
     private readonly _idealThreads: number;
     private readonly _minThreads: number;
     private readonly _performanceTracking: boolean;
     private readonly _totalSequences: number;
     private readonly _unusedSequenceIds: number[] = [];
     private readonly _batchingOptions: Required<BatchingOptions>;
     private readonly _swaFullCache: boolean = false;
     private readonly _queuedDecodeSequenceIds = new Set<number>();
     private readonly _queuedDecodes: InternalQueuedDecode[] = [];
     private readonly _disposeAggregator = new AsyncDisposeAggregator();
     private readonly _modelPreventDisposalHandle: DisposalPreventionHandle;
     private readonly _loraAdapters = new Set<AddonModelLora>();
     private readonly _gcRegistry: FinalizationRegistry<Set<AddonModelLora>>;
     private _nextGeneratedSequenceId = 0;
     private _dispatchDecodeScheduled = false;
     private _batchDispatchPending = false;
     private _threadSplitterConsumer?: ThreadsSplitterConsumer;
     private _freeReservedThreadsTimeout?: ReturnType<typeof setTimeout>;
     private _currentDispatchBatchHandle: object = {};
     private _allocatedContextSize?: number;
...
export class LlamaContextSequence {
     ...
     private readonly _gcRegistry: FinalizationRegistry<number>;
     private readonly _context: LlamaContext;
     private readonly _contextShift: Required<ContextShiftOptions>;
     private readonly _tokenPredictor?: TokenPredictor;
     private readonly _tokenMeter: TokenMeter;
     private readonly _disposeAggregator = new DisposeAggregator();
     private readonly _lock = {};
     private _resetTokenPredictor: boolean = false;
     private _tokenPredictorOwner: {} = {};
     public _contextTokens: Token[] = [];
     private _nextTokenIndex: number = 0;
     private _loadedTokenPredictions: Array<[
        input: Token,
        output: [token: Token, probabilities: (Token | number)[] | undefined, confidence: number | undefined]
    ]> = [];
     private _usedTokenPredictions: number = 0;
     private _unusedTokenPredictions: number = 0;
     private _validatedTokenPredictions: number = 0;
     private _refutedTokenPredictions: number = 0;
