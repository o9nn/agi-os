...
class BCTest(TestCase):
    ...
    def init(self):
        ...
        scheme_eval(self.atomspace, '(add-to-load-path "{0}/{1}")'.format(project_source_dir, 'tests/ure/backwardchainer/scm'))
    def test_bc_deduction(self):
        ...
        chainer = BackwardChainer(self.atomspace, ConceptNode('URE'), InheritanceLink(VariableNode('$who'), C), TypedVariableLink(VariableNode('$who'), TypeNode('ConceptNode')))
