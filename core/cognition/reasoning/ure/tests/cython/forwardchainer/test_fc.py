...
class FCTest(TestCase):
    ...
    def init(self):
        ...
        scheme_eval(self.atomspace, '(add-to-load-path "{0}/{1}")'.format(project_source_dir, 'tests/ure/forwardchainer/scm'))
        scheme_eval(self.atomspace, '(add-to-load-path "{0}/{1}")'.format(project_source_dir, 'opencog/scm/opencog/ure'))
    def test_forward_chainer_instantiation(self):
        chainer = ForwardChainer(self.atomspace, ConceptNode('PLN'), SetLink())
    ...
    def test_fc_deduction(self):
        ...
        chainer = ForwardChainer(self.atomspace, ConceptNode('fc-deduction-rule-base'), InheritanceLink(VariableNode('$who'), C), TypedVariableLink(VariableNode('$who'), TypeNode('ConceptNode')))
        chainer.do_chain()
