...
dyn_print_connector_list(l, dj->left, 0, flags);
dyn_strcat(l, " <> ");
dyn_print_connector_list(l, dj->right, 1, flags);
...
{
prt_error("Valid flags for the \"!!word
"(show disjuncts):\n"
...
const void *carg[3] = { NULL, NULL, opts };
Regex_node *rn = NULL;
if (arg != NULL)
{
if (NULL != arg[1])
...
Pool_desc *Exp_pool = pool_new(__func__, "Exp", 256,
sizeof(Exp), false,
false, false);
dyn_str *s = dyn_str_new();
