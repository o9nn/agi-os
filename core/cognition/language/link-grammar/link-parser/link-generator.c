...
static char short_options[2*ARGP_OPTION_ARRAY_SIZE +2];
...
if (*p == '\0') col = (end_col - start_col) + 1;
}
else
{
...
if (*end_blank == '\0') break;
p = line_start = end_blank + 1;
if (p[strspn(p, " ")] == '\0') break;
end_blank = NULL;
last_is_nonblank = true;
col = 0;
continue;
...
assert(linkages_valid == num_linkages );
