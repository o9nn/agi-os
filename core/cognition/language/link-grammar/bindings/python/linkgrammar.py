...
Clinkgrammar = clg
__all__ = ['ParseOptions', 'Dictionary', 'Link', 'Linkage', 'Sentence', 'LG_Error', 'LG_DictionaryError', 'LG_TimerExhausted', 'Clinkgrammar']
def kwargs_only(init):
    def new_init(self_, *args, **kwargs):
        if args:
            raise TypeError('{}: Positional arguments are not allowed'.format(self_.__class__.__name__))
        return init(self_, **kwargs)
    return new_init
class ParseOptions(object):
    @kwargs_only
    def __init__(self, verbosity=0, linkage_limit=1000, min_null_count=0, max_null_count=0, islands_ok=False, short_length=16, all_short_connectors=False, display_morphology=True, spell_guess=False, use_sat=False, max_parse_time=-1, disjunct_cost=None, repeatable_rand=True, test='', debug='', dialect=''):
...
class Link(object):
    def __init__(self, linkage, index, left_word, left_label, right_label, right_word):
        ...
        self.left_word, self.right_word, self.left_label, self.right_label = (left_word, right_word, left_label, right_label)
    def __eq__(self, other):
        return self.left_word == other.left_word and self.left_label == other.left_label and (self.right_word == other.right_word) and (self.right_label == other.right_label)
    def __str__(self):
...
class Linkage(object):
    ...
    def link(self, i):
        return Link(self, i, self.word(clg.linkage_get_link_lword(self._obj, i)), clg.linkage_get_link_llabel(self._obj, i), clg.linkage_get_link_rlabel(self._obj, i), self.word(clg.linkage_get_link_rword(self._obj, i)))
    def links(self):
