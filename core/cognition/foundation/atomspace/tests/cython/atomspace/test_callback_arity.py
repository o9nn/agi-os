...
class TestExecutionOutputLink(unittest.TestCase):
    ...
    def test_correct_argcount(self):
        ...
        exec_link = ExecutionOutputLink(GroundedSchemaNode('py:return_concept'), ListLink(atom1))
        result = exec_link.execute()
        self.assertTrue(result.name == 'test')
    def test_too_many_args(self):
        atom1 = ConceptNode('atom1')
        exec_link = ExecutionOutputLink(GroundedSchemaNode('py:return_concept'), ListLink(atom1, atom1))
        try:
    ...
    def test_too_few_args(self):
        atom1 = ConceptNode('atom1')
        exec_link = ExecutionOutputLink(GroundedSchemaNode('py:return_concept'), ListLink())
        try:
            result = self.space.execute(exec_link)
            self.assertFalse('call should fail')
        except RuntimeError as e:
