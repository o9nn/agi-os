...
class DoExecuteTest(unittest.TestCase):
    ...
    def test_do_execute_atom(self):
        DefineLink(DefinedSchemaNode('add'), LambdaLink(VariableList(VariableNode('$X'), VariableNode('$Y')), PlusLink(VariableNode('$X'), VariableNode('$Y'))))
        res = ExecutionOutputLink(DefinedSchemaNode('add'), ListLink(NumberNode('3'), NumberNode('4'))).execute()
    ...
    def test_add_atom_from_grounded_schema_node(self):
        ...
        test_as.execute(ExecutionOutputLink(GroundedSchemaNode('py:add_new_link'), ListLink()))
        self.assertTrue(test_as.is_link_in_atomspace(types.InheritanceLink, [test_as.add_node(types.ConceptNode, 'cat'), test_as.add_node(types.ConceptNode, 'animal')]))
