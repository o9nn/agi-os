...
class LinkValueTest(unittest.TestCase):
    ...
    def test_value_equals(self):
        ...
        self.assertEqual(LinkValue([FloatValue(42), StringValue('foo')]), LinkValue([FloatValue(42), StringValue('foo')]))
        self.assertNotEqual(LinkValue(StringValue('foo')), LinkValue(StringValue('bar')))
        self.assertNotEqual(LinkValue([FloatValue(1), StringValue('foo')]), LinkValue([StringValue('foo'), FloatValue(1)]))
    def test_add_value_to_atom(self):
        ...
        self.assertEqual(LinkValue([StringValue('a'), FloatValue(42)]), atom.get_value(key))
    def test_get_list_of_items_from_value(self):
        value = LinkValue([FloatValue(42), StringValue('foo')])
        self.assertEqual([FloatValue(42), StringValue('foo')], value.to_list())
    def test_str(self):
        value = LinkValue([FloatValue(42), StringValue('foo')])
