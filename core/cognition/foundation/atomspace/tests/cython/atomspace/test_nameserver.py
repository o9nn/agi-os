import unittest
from opencog.atomspace import types, decl_type, type_decl_context, AtomSpace, get_type_name
...
class NameserverTest(unittest.TestCase):
    ...
    def _get_atoms_by_type(self, type):
        ...
        return Get(VariableList(TypedVariable(Variable('X'), Type(type_name))), And(Variable('X'))).execute()
