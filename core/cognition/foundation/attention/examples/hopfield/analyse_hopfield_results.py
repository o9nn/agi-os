...
def loadAllResults(path='./'):
    ...
    for r in result_files:
        print('Loading ' + r)
        pattern = 'results_n_(\\d+)_d_(\\d\\.\\d+)_(\\w+).csv'
        m = re.search(pattern, r)
        g = m.groups()
        configs[g[0], g[1]] = 0
    configkeys = configs.keys()
    for r in configkeys:
        configs[r] = loadResultOfOneRun(os.path.join(path, 'results_n_' + r[0] + '_d_' + r[1] + '_'))
    return configs
...
def plotAfterAndDiff(results, size, density, path='./'):
    ...
    f = p.figure(1)
    f.clear()
    f.text(0.5, 0.94, 'Results for size ' + size + ', link density ' + density, horizontalalignment='center')
    p.subplot(211)
...
def plotDistOfCycle(results, size, path='./', cycle=-1):
    ...
    for d in densities:
        ...
        p.figure(0).text(0.01, 0.9 - offset * 0.05, '%d%%>=0' % p_gt_one, horizontalalignment='left', color=my_color, size='smaller')
