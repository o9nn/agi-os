...
class MorkClient:
    ...
    def _poll(self, status_loc: str, max_attempts: int=16, initial_delay: float=0.005, backoff: float=2.0) -> dict:
...
def copy_to_temp(args: argparse.Namespace) -> Path:
    try:
        ...
        print(f'Error: {e}')
        raise e
def load_s_expressions(uri: str, mork_server_address: str=MORK_SERVER_ADDRESS) -> dict:
