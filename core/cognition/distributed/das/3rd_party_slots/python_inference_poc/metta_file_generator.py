...
class NodeLinkGenerator:
    debug = False
    def __init__(self, sentence_node_count: str, word_count: str, word_length: str, alphabet_range: str, debug: bool) -> None:
    ...
    @staticmethod
    def add_links(writer: 'NodeLinkWriter', links: dict[str, Any], node_list: list[dict[str, Any]], link_type: str, keyword_gen: Callable, strength_divisor: int=1) -> None:
        for k, v in links.items():
            targets = [node_list[int(i)] for i in k.split('->')]
            keyword = keyword_gen()
            link = {'type': link_type, 'targets': targets, 'strength': v / strength_divisor, 'keyword': keyword, 'indexed_keyword': keyword}
...
def main():
    ...
    parser.add_argument('--sentence-node-count', default=3, help='Number of Sentence nodes to be generated, eg: 100')
    ...
    generator = NodeLinkGenerator(int(args.sentence_node_count), int(args.word_count), int(args.word_length), args.alphabet_range, debug=True)
