...
class AtomDecoder(HandleDecoder):
    ...
    def setup_mongo_db(self, hostname: str=None, port: int=None, username: str=None, password: str=None) -> None:
    ...
    def build_node(self, doc) -> Node:
        return Node(type=doc['named_type'], name=doc['name'], custom_attributes=Properties(doc.get('custom_attributes', {})))
    def build_link(self, doc) -> Link:
        return Link(type=doc['named_type'], targets=doc['targets'], is_toplevel=doc['is_toplevel'], custom_attributes=Properties(doc.get('custom_attributes', {})))