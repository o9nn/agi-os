...
class DistributedAlgorithmNodeManager:
    ...
    def start(self):
        ...
        distributed__algorithm__node__pb2__grpc.add_DistributedAlgorithmNodeServicer_to_server(self.servicer, self.server)
    ...
    def to_remote_peer(self, command: str, args: list[str]) -> None:
        ...
        with grpc.insecure_channel(self.peer_id) as channel:
            ...
            message = distributed__algorithm__node__pb2.MessageData(command=self.PROXY_COMMAND, args=new_args, sender=self.node_id, is_broadcast=False, visited_recipients=[])
            try:
...
class DistributedAlgorithmNodeServicer(distributed__algorithm__node__pb2__grpc.DistributedAlgorithmNodeServicer):
    ...
    def execute_message(self, request: distributed__algorithm__node__pb2.MessageData, context=None):
        log.info(f'Remote command: <{request.command}> arrived at DistributedAlgorithmNodeServicer {self.proxy.proxy_node.node_id}')
