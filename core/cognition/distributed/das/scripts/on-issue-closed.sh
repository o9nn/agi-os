...
local BODY="This issue was automatically created from issue
...
if [ -z "$ITEM_ID" ]; then
echo "Failed to create project card for issue
return 1
fi
echo "$CARD_RESPONSE"
}
...
main() {
echo "Checking labels for issue
...
echo "No assignees found for issue
fi
fi
if [ -z "$ISSUE_ASSIGNEES" ]; then
echo "No assignees found for issue
...
echo "Label '$LABEL_NAME' not found on issue
fi
}
main