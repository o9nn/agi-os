...
class TestMOSES(unittest.TestCase):
    ...
    def test_float_score(self):
        input_data = [[0, 0, 0], [0.2, 0.2, 0.4], [1, 1, 2], [1, 0, 1], [2.0, 1, 3]]
        output = self.moses.run(input=input_data, python=True, args='--max-time=60 --balance=1')
        self.assertTrue(any([isinstance(o.score, float) for o in output]), 'Test only valid when there is a float score')
        model = output[0].eval
        print(model([0, 1]))
        print(model([1, 1]))