...
def parse_log(logFileName, options):
    ...
    info_re = '\\[INFO\\]'
    header_re = '{}: # (\\w+(?:\\t\\w+)+)'.format(prefix)
    content_re = '{}: ([^#\\t]+(?:\\t.+))+'.format(prefix)
    all_header_re = '(?:{} {} )?{}'.format(timestamp_re, info_re, header_re)
    all_content_re = '({}) {} {}'.format(timestamp_re, info_re, content_re)
...
if __name__ == '__main__':
    usage = 'usage: %prog MOSES_LOG [options]\n' + 'Parse a log file MOSES_LOG and outputs a CSV file' + ' describing the learning performance.'
    parser = OptionParser(usage)
    parser.add_option('-p', '--prefix', default='Demes', help="Prefix of statistic to gather (possible prefixes are 'Demes' and 'Stats'). [default = %default]")
    parser.add_option('-o', '--output-file', default='', help='File where to write CSV file. If none is provided it writes in the stdout.')
