...
ctlr->tdpool = mmucacheinhib(xspanalloc(128*sizeof(TD), CACHELINESZ, 0), 128*sizeof(TD));
for(i=128; --i>=0;){
ctlr->tdpool[i].next = ctlr->freetd;
ctlr->freetd = &ctlr->tdpool[i];
}
ctlr->qhpool = mmucacheinhib(xspanalloc(64*sizeof(QH), CACHELINESZ, 0), 64*sizeof(QH));
for(i=64; --i>=0;){
