...
setbt812regpair(0x16, (int)(subcarrier & 0xFFFF));
...
setreg(VideoBufferLayoutControl, 0);
if (vsync)
slreg1 |= VSyncLow;
if (!hsync) {
slreg1 ^= HSyncLow;
...
filter = scaley > 512 ? (ScreenWidth == 640 ? 0x44 : 0xC5) : 0x46;
...
setreg(CapturePixelBufLow, 0x04);
setreg(CapturePixelBufHigh, 0x0E);
setreg(CaptureMultiBufDepthA, 0xFF);
setreg(CaptureMultiBufDepthB, 0x03);
updateshadowregs();
setreg(CaptureControl, 0x90);
val = getreg(InputFieldPixelBufStatus);
USED(val);
toggle = !(getreg(OutputProcControlA) & 0x01) ? 0x8000 : 0x0000;
setreg(CaptureControl, 0xC0);
