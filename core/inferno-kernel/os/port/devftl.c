...
error(up->env->errstr);
old = 1;
statfd = ftlkopen(fname, "stat", OREAD);
if(statfd == nil){
statfd = ftl->flashctl;
old = 0;
}
...
nov = ((ftl->segsize/Bsize)*4 + BAMoffset + Bsize - 1)/Bsize;
ftl->fstart = nov;
...
ftl->nblock -= segblocks;
if(ftl->nblock >= 1000)
ftl->rwlimit = ftl->nblock-100;
else
...
putflash(ftl, to->bamoffset, bam, nb);
for(i=0; i<from->nbam; i++){
uchar *p = (uchar*)&bam[i];
v = bam[i];
PUT4(p, v);
