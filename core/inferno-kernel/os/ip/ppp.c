...
kclose(cfd);
kchanio(ppp->cchan, "m1", 2, OWRITE);
kchanio(ppp->cchan, "q64000", 6, OWRITE);
kchanio(ppp->cchan, "n1", 2, OWRITE);
kchanio(ppp->cchan, "r1", 2, OWRITE);
kchanio(ppp->cchan, "d1", 2, OWRITE);
}
ppp->pppup = 1;
init(ppp);
return ppp;
...
hnputl(p->peeroutbytes, ppp->out.bytes+53);
