...
g->gedr = g->gedr;
g->gpcr = ~0;
g->gpdr = GPIO_CLK_SET0_o | GPIO_CLK_SET1_o;
g->gafr |= GPIO_SYS_CLK_i;
p = PPCREG;
p->ppdr |= PPC_TXD4 | PPC_SCLK | PPC_SFRM;
...
if(0)
strcpy(opt, "mode=0 essid=Limbo station=ipaq1 crypt=off");
else
