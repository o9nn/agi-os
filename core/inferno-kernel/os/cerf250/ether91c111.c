...
static void
pointtotxpacket(Ctlr *ctlr, int pkt, int read)
{
ushort junk;
pkt &= 0x3F;
...
regwdatam(ctlr, (ushort*)b->rp, lenw);
lastw = 0x1000;
if(odd){
lastw |= 0x2000;
lastw |= b->rp[lenb - 1];
...
break;
n = (BLEN(ctlr->waiting) & ~1) + Framectlsize;
...
regw(ctlr, Rcr, RcrRxEn | RcrAbortEnb | RcrStripCRC);
if(rev == 0x90){
int reg2, reg3;
