...
if(n->op == OCONST && n->left != Z){
n->left->type = n->type;
n = n->left;
}
if((n->op == OSTRING || n->op == OLSTRING) && n->left != Z)
n = n->left;
...
prkeywd("if");
egen(n->left, OBRACKET, PRE);
if(n->right->left->op == OIF && n->right->left->right->right == Z && n->right->right != Z)
sgen(n->right->left, 1|YESBR, ln);
else
sgen(n->right->left, 1, ln);
if(n->right->right != Z){
prdelim("else");
if(n->right->right->op == OIF){
prdelim(" ");
