...
o1 = OP_U(p->to.reg, -v);
v &= 0xFFF;
o2 = OP_FI(4, p->to.reg, p->to.reg, v);
break;
}
if(v & 0x800)
v += 0x1000;
...
if(thechar == 'j')
v += 32;
o1 = OP_FI(1, p->from.reg, p->to.reg, v & 0x3F);
o2 = OP_I(p->to.reg, p->to.reg, v);
break;
case 11:
v = regoff(&p->from);
...
if (v > 12)
o2 = OP_FI(1, p->to.reg, p->to.reg, v - 12);
else
o2 = OP_I(p->to.reg, p->to.reg, 12 - v);
break;
case 22:
