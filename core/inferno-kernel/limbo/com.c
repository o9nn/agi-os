...
patch(genrawop(&lastinst->src, IJMP, nil, nil, nil), pp);
popblock();
patch(p, nextinst());
...
case Oexstmt:
pushblock();
pp = genrawop(&n->right->src, IEXC0, nil, nil, nil);
p1 = nextinst();
scom(n->left);
p2 = nextinst();
p3 = genrawop(&n->right->src, IJMP, nil, nil, nil);
p = genrawop(&n->right->src, IEXC, nil, nil, nil);
