...
IRR(Osw, O(Frame,mr),Ro1, Ro3);
op3(i, Olw, Ri, 1);
ml = nil;
IRR(Olw, (ulong)&ml->links[i->reg].u.pc,Ri, Rj);
...
IRR(Olw, O(Modlink,compiled),Ro1, Ro2);
...
IRR(Olw, O(Array,data),Ro1, Ro1);
IRR(Olw, O(Type,size),Ro2, Ro2);
delay();
RRR(Omul, Ro3,Ro2,Rzero);
RRR(Omflo, Rzero,Rzero,Ro2);
JR(Ojr, Rlink);
RRR(Oaddu, Ro1,Ro2,Ro2);
}
static void
maccase(void)
{
...
IRR(Oaddui, 0, Ri, Ro2);
*cp3 |= (code - cp3) - 1;
IRR(Oaddui, 12, Rj, Ro3);
IRR(Oaddui, 1, Ri, Rpic);
BRRI(Obeq, Rzero,Rzero, (cp1-code)-1);
RRR(Osubu, Rpic, Ro2, Ro2);
*cp1 |= (code - cp1) - 1;
IRR(Olw, 0,Rlink, Ro2);
delay();
SRR(Osll, 1, Ro2, Ro3);
...
IRR(Osw, O(Frame,lr),Ro1, Rlink);
cp1 = code;
BRRI(Obne, Ro2, Rzero, 0);
IRR(Oaddui, 0,Ro1, Rfp);
IRR(Osw, O(REG,st),Rreg, Rlink);
...
IRR(Olw, O(Type,size),Ri, Ro3);
IRR(Olw, O(REG,SP),Rreg, Ro2);
IRR(Olw, O(REG,TS),Rreg, Ro1);
RRR(Oaddu,Ro3,Ro2, Ro2);
RRR(Osltu, Ro1,Ro2, Ro3);
cp = code;
BRI(Obne,Ro3,0);
delay();
IRR(Osw, O(REG,s),Rreg, Ri);
