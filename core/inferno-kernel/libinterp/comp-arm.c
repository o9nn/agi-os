...
mem(Ldw, Blo, RA0, RA1);
mem(Ldw, Blo, RA3, RA2);
DP(AL, op, RA2, RA2, 0, RA1) | SBIT;
mem(Ldw, Bhi, RA0, RA1);
mem(Ldw, Bhi, RA3, RA0);
DP(AL, opc, RA0, RA0, 0, RA1);
if((i->add&ARM) != AXNON)
opwst(i, Lea, RA3);
mem(Stw, Blo, RA3, RA2);
mem(Stw, Bhi, RA3, RA0);
...
BRA(LE, 0);
inner = code;
DP(AL, Mov, 0, RA0, (1<<3)|2, RA2);
DP(AL, Add, RA0, RCON, (1<<3), RA0);
DP(AL, Add, RA3, RCON, (2<<3), RCON);
LDW(AL, RCON, RTA, 4);
CMP(AL, RA1, 0, 0, RTA);
DP(LT, Mov, 0, RA2, 0, RA0);
BRANCH(LT, loop);
LDW(AL, RCON, RTA, 8);
CMP(AL, RA1, 0, 0, RTA);
LDW(LT, RCON, R15, 12);
DPI(AL, Add, RCON, RA3, 0, 12);
DPI(AL, Add, RA0, RTA, 0, 1);
DP(AL, Sub, RA2, RA2, 0, RTA) | SBIT;
BRANCH(GT, inner);
PATCH(cp1);
LDW(AL, RLINK, RA2, 0);
DP(AL, Add, RA2, RA2, (1<<3), RA2);
DP(AL, Add, RLINK, RLINK, (2<<3), RA2);
LDW(AL, RLINK, R15, 4);
}
static void
macfrp(void)
