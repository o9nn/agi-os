...
return 0;
p = buf;
PBIT16(p, len - BIT16SZ); p += BIT16SZ;
p += BIT16SZ;
p += BIT32SZ;
*p++ = e->qid.type;
PBIT32(p, e->qid.vers); p += BIT32SZ;
PBIT64(p, e->qid.path); p+= 8;
PBIT32(p, e->perm); p+= BIT32SZ;
PBIT32(p, e->mtime); p+= BIT32SZ;
PBIT32(p, e->mtime); p+= BIT32SZ;
if(e->qid.type & QTDIR) {
PBIT64(p, 0);
p += 8;
...
PBIT16(p, namelen); p += BIT16SZ; memmove(p, e->name, namelen); p+= namelen;
PBIT16(p, unamelen + unamebad); p += BIT16SZ;
if(unamebad)
*p++ = '(';
memmove(p, uname, unamelen + unamebad); p+= unamelen;
if(unamebad)
*p++ = ')';
PBIT16(p, gnamelen + gnamebad); p += BIT16SZ;
if(gnamebad)
*p++ = '(';
memmove(p, gname, gnamelen); p+= gnamelen;
if(gnamebad)
*p++ = ')';
PBIT16(p, munamelen + munamebad); p += BIT16SZ;
if(munamebad)
*p++ = '(';
