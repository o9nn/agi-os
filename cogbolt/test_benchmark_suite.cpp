...
BOLT_ASSERT_TRUE(result.averageDurationMs >= 10.0);
BOLT_ASSERT_TRUE(result.averageDurationMs < 100.0);
...
BOLT_ASSERT_TRUE(results.size() >= 3);
...
BOLT_ASSERT_TRUE(result.isValid());
BOLT_ASSERT_TRUE(result.standardDeviationMs > 0.0);
...
current.averageDurationMs = 80.0;
...
BOLT_ASSERT_TRUE(comparisons[0].durationChangePercent < 0);
