...
BOLT_ASSERT_TRUE(!entry.file.empty() == false);
...
BOLT_ASSERT_TRUE(formatted.find("T:") != std::string::npos);
}
...
{
bolt::RotatingFileSink sink(baseFile, 50, 3);
BOLT_ASSERT_TRUE(sink.isOpen());
for (int i = 0; i < 5; ++i) {
...
bolt::CategoryFilter filter(bolt::LogLevel::INFO);
...
BOLT_ASSERT_TRUE(filter.shouldLog(memoryDebug));
BOLT_ASSERT_TRUE(filter.shouldLog(memoryInfo));
BOLT_ASSERT_FALSE(filter.shouldLog(editorInfo));
BOLT_ASSERT_TRUE(filter.shouldLog(editorError));
BOLT_ASSERT_TRUE(filter.shouldLog(coreInfo));
BOLT_ASSERT_FALSE(filter.shouldLog(coreDebug));
}
...
bolt::Logger logger(bolt::LogLevel::TRACE);
...
BOLT_ASSERT_TRUE(bolt::Logger::stringToLevel("UNKNOWN") == bolt::LogLevel::INFO);
}
...
BOLT_ASSERT_TRUE(logger.getDroppedMessages() == numMessages / 2);
}
