...
EXPECT_LT(compressed.size(), originalMessage.size());
...
EXPECT_EQ(compressedShort.size(), shortMessage.size());
std::string longMessage(200, 'A');
auto compressedLong = compressor.compress(longMessage);
EXPECT_LT(compressedLong.size(), longMessage.size());
}
...
metrics.recordLatency(endpoint, 1500);
