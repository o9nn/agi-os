...
def extract_version(content):
    major = re.search('^set *\\( *(\\w+)_VERSION_MAJOR +(\\d+) ', content, re.M).group(2)
    minor = re.search('^set *\\( *(\\w+)_VERSION_MINOR +(\\d+) ', content, re.M).group(2)
    sub = re.search('^set *\\( *(\\w+)_VERSION_(?:SUB|PATCH|UPDATE) +(\\d+) ', content, re.M).group(2)
    ...
    with tarfile.open(fileobj=tar_gz_file, mode='r:gz') as tar:
        for member in tar.getmembers():
            if not member.isfile():
                continue
            if (m := re.fullmatch('SuiteSparse-[^/]+/(\\w+)/CMakeLists.txt', member.name)):
                name = m.group(1)
...
def update_portfile(pkg_name, new_version, new_hash):
    ...
    content, n = re.subn('\\bREF v\\S+', f'REF v{new_version}', content, re.M)
    if n != 1:
        raise Exception(f'Updating {pkg_name} portfile ref failed!')
    content, n = re.subn('\\bSHA512 \\S+', f'SHA512 {new_hash}', content, re.M)
    if n != 1:
...
def main(suitesparse_version):
    suitesparse_url = f'https://github.com/DrTimothyAldenDavis/SuiteSparse/archive/refs/tags/v{suitesparse_version}.tar.gz'
