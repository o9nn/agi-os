...
def generate_baseline():
    ...
    for i, port_name in enumerate(port_names, 1):
        port_file_path = os.path.join(VERSIONS_DB_DIRECTORY, f'{port_name[0]}-', f'{port_name}.json')
        ...
        with open(port_file_path, 'r') as db_file:
            try:
                ...
                if versions_object['versions']:
                    ...
                    baseline_entries[port_name] = {'baseline': get_version_tag(last_version), 'port-version': get_version_port_version(last_version)}
