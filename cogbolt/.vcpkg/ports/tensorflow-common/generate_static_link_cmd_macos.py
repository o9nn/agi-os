...
with open(sys.argv[1], 'r') as f_in:
    with open('static_link.sh', 'w') as f_out:
        if os.path.isfile(f'{params_path}/libtensorflow_framework.{version}.dylib-2.params'):
            p_cd = re.compile('^\\((cd .*) && \\\\$')
            p_linker = re.compile(f'^\\s*.+cc_wrapper.sh.+(@bazel-out\\S+libtensorflow{lib_suffix}\\.\\d+\\.\\d+\\.\\d+\\.dylib-2\\.params).*')
            ...
            p_cd = re.compile('^\\((cd .*) && \\\\$')
            p_linker1 = re.compile(f'^.*cc_wrapper.sh.+-shared.+-o (bazel-out\\S+libtensorflow{lib_suffix}\\.\\d+\\.\\d+\\.\\d+\\.dylib)')
