...
BOLT_ASSERT_TRUE(actualDuration < workDuration + 20);
...
BOLT_ASSERT_TRUE(avgDuration >= 25 && avgDuration <= 35);
BOLT_ASSERT_TRUE(minDuration >= 8);
BOLT_ASSERT_TRUE(maxDuration >= 45);
...
auto endCount = profiler.getTotalMetricsCount();
BOLT_ASSERT_EQ(endCount, startCount);
profiler.enable();
...
BOLT_ASSERT_EQ(finalCount, endCount + 1);
}
...
BOLT_ASSERT_TRUE(overhead < 100.0);
