...
class Model:
    ...
    def __init__(self, model_path: Path, state_dict: Any, vocab_path: Path, ftype: gguf.LlamaFileType, fname_out: Path, is_big_endian: bool=False, use_temp_file: bool=False, eager: bool=False, metadata_override: Path | None=None, model_name: str | None=None, split_max_tensors: int=0, split_max_size: int=0, dry_run: bool=False, small_first_shard: bool=False, hparams: dict[str, Any] | None=None):
    ...
    def prepare_tensors(self):
        ...
        for name, data_torch in chain(self.generate_extra_tensors(), self.get_tensors()):
            ...
            for new_name, data_torch in self.modify_tensors(data_torch, name, bid):
                ...
                if data_qtype is False and (any((self.match_model_tensor_name(new_name, key, bid) for key in (gguf.MODEL_TENSOR.TIME_MIX_FIRST, gguf.MODEL_TENSOR.TIME_MIX_W1, gguf.MODEL_TENSOR.TIME_MIX_W2, gguf.MODEL_TENSOR.TIME_MIX_DECAY_W1, gguf.MODEL_TENSOR.TIME_MIX_DECAY_W2))) or not new_name.endswith('.weight') or 'lerp' in new_name):
                    data_qtype = gguf.GGMLQuantizationType.F32
                if data_qtype is False and any((self.match_model_tensor_name(new_name, key, bid) for key in (gguf.MODEL_TENSOR.TOKEN_EMBD, gguf.MODEL_TENSOR.OUTPUT))):
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('vocab', type=Path, help='Path to rwkv_vocab_v20230424.txt')
...
def main() -> None:
    ...
    hparams = {'num_hidden_layers': n_layer, 'head_size': n_embd // n_head, 'hidden_size': n_embd, 'layer_norm_epsilon': 1e-05, 'intermediate_size': None}
    if int(version) == 6:
    ...
    with torch.inference_mode():
        ...
        try:
            ...
            sys.exit(1)
        model_instance = model_class(model_path=model_path, state_dict=state_dict, vocab_path=args.vocab, ftype=output_type, fname_out=fname_out, is_big_endian=args.bigendian, use_temp_file=args.use_temp_file, eager=args.no_lazy, hparams=hparams, metadata_override=args.metadata, model_name=args.model_name, split_max_tensors=args.split_max_tensors, split_max_size=split_str_to_n_bytes(args.split_max_size), dry_run=args.dry_run, small_first_shard=args.no_tensor_first_split)
        if args.vocab_only:
