...
class ModelBase:
    _model_classes: dict[ModelType, dict[str, type[ModelBase]]] = {ModelType.TEXT: {}, ModelType.MMPROJ: {}}
    dir_model: Path
    ...
    def __init__(self, dir_model: Path, ftype: gguf.LlamaFileType, fname_out: Path, *, is_big_endian: bool=False, use_temp_file: bool=False, eager: bool=False, metadata_override: Path | None=None, model_name: str | None=None, split_max_tensors: int=0, split_max_size: int=0, dry_run: bool=False, small_first_shard: bool=False, hparams: dict[str, Any] | None=None, remote_hf_model_id: str | None=None):
        ...
        self.gguf_writer = gguf.GGUFWriter(path=None, arch=gguf.MODEL_ARCH_NAMES[self.model_arch], endianess=self.endianess, use_temp_file=self.use_temp_file, split_max_tensors=split_max_tensors, split_max_size=split_max_size, dry_run=dry_run, small_first_shard=small_first_shard)
    @classmethod
    ...
    def get_tensors(self) -> Iterator[tuple[str, Tensor]]:
        ...
        if len(tensor_names_from_parts.symmetric_difference(self.tensor_names)) > 0:
            ...
            if len(extra) == 0 and len(missing_files) > 0:
                ...
                raise ValueError(f'Mismatch between weight map and model parts for tensor names:\nMissing tensors: {missing}\nExtra tensors: {extra}')
        ...
        for name, data_torch in chain(self.generate_extra_tensors(), self.get_tensors()):
            ...
            for new_name, data_torch in self.modify_tensors(data_torch, name, bid):
                ...
                if data_qtype is False and (any((self.match_model_tensor_name(new_name, key, bid) for key in (gguf.MODEL_TENSOR.FFN_GATE_INP, gguf.MODEL_TENSOR.POS_EMBD, gguf.MODEL_TENSOR.TOKEN_TYPES, gguf.MODEL_TENSOR.SSM_CONV1D, gguf.MODEL_TENSOR.SHORTCONV_CONV, gguf.MODEL_TENSOR.TIME_MIX_FIRST, gguf.MODEL_TENSOR.TIME_MIX_W1, gguf.MODEL_TENSOR.TIME_MIX_W2, gguf.MODEL_TENSOR.TIME_MIX_DECAY_W1, gguf.MODEL_TENSOR.TIME_MIX_DECAY_W2, gguf.MODEL_TENSOR.TIME_MIX_LERP_FUSED, gguf.MODEL_TENSOR.POSNET_NORM1, gguf.MODEL_TENSOR.POSNET_NORM2, gguf.MODEL_TENSOR.V_ENC_EMBD_POS, gguf.MODEL_TENSOR.A_ENC_EMBD_POS, gguf.MODEL_TENSOR.ALTUP_CORRECT_COEF, gguf.MODEL_TENSOR.ALTUP_PREDICT_COEF))) or not new_name.endswith('.weight')):
                    data_qtype = gguf.GGMLQuantizationType.F32
                if data_qtype is False and any((self.match_model_tensor_name(new_name, key, bid) for key in (gguf.MODEL_TENSOR.TOKEN_EMBD, gguf.MODEL_TENSOR.PER_LAYER_TOKEN_EMBD, gguf.MODEL_TENSOR.OUTPUT, gguf.MODEL_TENSOR.ALTUP_ROUTER, gguf.MODEL_TENSOR.LAUREL_L, gguf.MODEL_TENSOR.LAUREL_R))):
...
class TextModel(ModelBase):
    ...
    def _create_vocab_sentencepiece(self):
        ...
        vocab_size = self.find_hparam(['vocab_size_per_layer_input', 'vocab_size'], optional=True) or tokenizer.vocab_size()
...
class GPTNeoXModel(TextModel):
    ...
    def modify_tensors(self, data_torch: Tensor, name: str, bid: int | None) -> Iterable[tuple[str, Tensor]]:
        ...
        if re.match('gpt_neox\\.layers\\.\\d+\\.attention\\.query_key_value\\.weight', name):
            qkv_weights = data_torch.reshape((n_head, 3, n_embed // n_head, n_embed))
            data_torch = torch.cat((qkv_weights[:, 0, :, :].reshape((-1, n_embed)), qkv_weights[:, 1, :, :].reshape((-1, n_embed)), qkv_weights[:, 2, :, :].reshape((-1, n_embed))), dim=0)
            logger.info('re-format attention.linear_qkv.weight')
        elif re.match('gpt_neox\\.layers\\.\\d+\\.attention\\.query_key_value\\.bias', name):
            qkv_bias = data_torch.reshape((n_head, 3, n_embed // n_head))
            data_torch = torch.cat((qkv_bias[:, 0, :].reshape((n_embed,)), qkv_bias[:, 1, :].reshape((n_embed,)), qkv_bias[:, 2, :].reshape((n_embed,))), dim=0)
...
class BloomModel(TextModel):
    ...
    def modify_tensors(self, data_torch: Tensor, name: str, bid: int | None) -> Iterable[tuple[str, Tensor]]:
        ...
        n_head = self.hparams.get('n_head', self.hparams.get('num_attention_heads'))
        n_embed = self.hparams.get('hidden_size', self.hparams.get('n_embed'))
        name = re.sub('transformer\\.', '', name)
        tensors: list[tuple[str, Tensor]] = []
        if re.match('h\\.\\d+\\.self_attention\\.query_key_value\\.weight', name):
            qkv_weights = data_torch.reshape((n_head, 3, n_embed // n_head, n_embed))
            data_torch = torch.cat((qkv_weights[:, 0, :, :].reshape((-1, n_embed)), qkv_weights[:, 1, :, :].reshape((-1, n_embed)), qkv_weights[:, 2, :, :].reshape((-1, n_embed))), dim=0)
            logger.info('re-format attention.linear_qkv.weight')
        elif re.match('h\\.\\d+\\.self_attention\\.query_key_value\\.bias', name):
            qkv_bias = data_torch.reshape((n_head, 3, n_embed // n_head))
            data_torch = torch.cat((qkv_bias[:, 0, :].reshape((n_embed,)), qkv_bias[:, 1, :].reshape((n_embed,)), qkv_bias[:, 2, :].reshape((n_embed,))), dim=0)
            logger.info('re-format attention.linear_qkv.bias')
        tensors.append((self.map_tensor_name(name), data_torch))
...
class BaichuanModel(TextModel):
    ...
    def modify_tensors(self, data_torch: Tensor, name: str, bid: int | None) -> Iterable[tuple[str, Tensor]]:
        ...
        if bid is not None and name == f'model.layers.{bid}.self_attn.W_pack.weight':
            ...
            tensors = [(self.format_tensor_name(gguf.MODEL_TENSOR.ATTN_Q, bid), self._reverse_hf_permute_part(data_torch, 0, head_count, head_count)), (self.format_tensor_name(gguf.MODEL_TENSOR.ATTN_K, bid), self._reverse_hf_permute_part(data_torch, 1, head_count, head_count_kv)), (self.format_tensor_name(gguf.MODEL_TENSOR.ATTN_V, bid), self._reverse_hf_part(data_torch, 2))]
        else:
...
class XverseModel(TextModel):
    ...
    def set_vocab(self):
        ...
        for token_id in range(vocab_size):
            ...
            elif re.fullmatch(b'<0x[0-9A-Fa-f]{2}>', token_text):
                toktype = gguf.TokenType.BYTE
...
class RefactModel(TextModel):
    ...
    def set_vocab(self):
        super().set_vocab()
        special_vocab = gguf.SpecialVocab(self.dir_model, load_merges=False, special_token_types=['prefix', 'suffix', 'middle', 'eot'])
...
class StableLMModel(TextModel):
    ...
    def prepare_tensors(self):
        ...
        if self._q_norms is not None or self._k_norms is not None:
            norms = ([k for d in self._q_norms for k in d.keys()] if self._q_norms is not None else []) + ([k for d in self._k_norms for k in d.keys()] if self._k_norms is not None else [])
            if len(norms) > 0:
...
class LlamaModel(TextModel):
    ...
    def set_vocab_tekken(self):
        ...
        logger.info(f'Setting bos, eos, unk and pad token IDs to {vocab.bos_id}, {vocab.eos_id}, {vocab.unk_id}, {vocab.pad_id}.')
    ...
    def modify_tensors(self, data_torch: Tensor, name: str, bid: int | None) -> Iterable[tuple[str, Tensor]]:
        ...
        is_multimodal_tensor = 'vision_tower' in name or 'vision_model' in name or 'audio_tower' in name or ('model.connector' in name) or ('multi_modal_projector' in name)
...
class LlavaVisionModel(MmprojModel):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.hparams['model_type'] == 'pixtral':
            self.hparams['layer_norm_eps'] = self.hparams.get('layer_norm_eps', 1e-05)
...
class SmolVLMModel(MmprojModel):
    ...
    def set_gguf_parameters(self):
        ...
        self.gguf_writer.add_vision_attention_layernorm_eps(self.hparams.get('layer_norm_eps', 1e-05))
        self.gguf_writer.add_vision_projector_scale_factor(self.global_config.get('scale_factor', 2))
        self.gguf_writer.add_vision_use_gelu(True)
    def tensor_force_quant(self, name, new_name, bid, n_dims):
        del bid, new_name, n_dims
...
class Llama4Model(LlamaModel):
    ...
    def modify_tensors(self, data_torch: Tensor, name: str, bid: int | None):
        ...
        if 'gate_up_proj' in name:
            ...
            return [(self.map_tensor_name(name_gate), gate_proj_weight), (self.map_tensor_name(name_up), up_proj_weight)]
...
class DeciModel(TextModel):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if 'block_configs' in self.hparams:
            ...
            self._ffn_dims: list[int] = [DeciModel._ffn_mult_to_intermediate_size(multiplier, self.hparams['hidden_size']) for multiplier in _ffn_multipliers]
    def set_vocab(self):
...
class BitnetModel(TextModel):
    ...
    def weight_quant(self, weight: Tensor) -> Tensor:
        dtype = weight.dtype
        weight = weight.float()
        scale = weight.abs().mean().clamp(min=1e-05)
        iscale = 1 / scale
        result = (weight * iscale).round().clamp(-1, 1) / iscale
        return result.type(dtype)
    def modify_tensors(self, data_torch: Tensor, name: str, bid: int | None) -> Iterable[tuple[str, Tensor]]:
        new_name = self.map_tensor_name(name)
        if any((self.match_model_tensor_name(new_name, key, bid) for key in [gguf.MODEL_TENSOR.ATTN_Q, gguf.MODEL_TENSOR.ATTN_K, gguf.MODEL_TENSOR.ATTN_V, gguf.MODEL_TENSOR.ATTN_OUT, gguf.MODEL_TENSOR.FFN_UP, gguf.MODEL_TENSOR.FFN_DOWN, gguf.MODEL_TENSOR.FFN_GATE])):
...
class Qwen2Model(TextModel):
    ...
    def modify_tensors(self, data_torch: Tensor, name: str, bid: int | None) -> Iterable[tuple[str, Tensor]]:
        ...
        if 'language_model.' in name:
            name = name.replace('language_model.', '')
        if name.startswith('mlp') or name.startswith('multi_modal_projector') or name.startswith('vision_model') or name.startswith('audio_tower'):
            return []
...
class Ernie4_5Model(TextModel):
    ...
    def modify_tensors(self, data_torch: Tensor, name: str, bid: int | None) -> Iterable[tuple[str, Tensor]]:
        ...
        if 'qkv_proj' in name:
            ...
            return [(self.map_tensor_name(name_q), q_proj_weight), (self.map_tensor_name(name_k), k_proj_weight), (self.map_tensor_name(name_v), v_proj_weight)]
...
class Ernie4_5MoeModel(Ernie4_5Model):
    ...
    def modify_tensors(self, data_torch: Tensor, name: str, bid: int | None) -> Iterable[tuple[str, Tensor]]:
        ...
        match = re.match('model.mtp_block.(\\d+)', name)
        if match:
            return []
        match = re.match('model.mtp_emb_norm.(\\d+)', name)
        if match:
            return []
        match = re.match('model.mtp_hidden_norm.(\\d+)', name)
        if match:
            return []
        match = re.match('model.mtp_linear_proj.(\\d+)', name)
        if match:
            return []
...
class Qwen2VLVisionModel(MmprojModel):
    ...
    def set_gguf_parameters(self):
        ...
        self.gguf_writer.add_vision_attention_layernorm_eps(self.global_config.get('rms_norm_eps', 1e-06))
    def tensor_force_quant(self, name, new_name, bid, n_dims):
        del bid, name, n_dims
    ...
    def modify_tensors(self, data_torch: Tensor, name: str, bid: int | None) -> Iterable[tuple[str, Tensor]]:
        ...
        if name.startswith('visual.'):
            if '.qkv.' in name:
                ...
                wk = data_torch[c:c * 2]
                wv = data_torch[c * 2:]
                return [(self.map_tensor_name(name.replace('qkv', 'q')), wq), (self.map_tensor_name(name.replace('qkv', 'k')), wk), (self.map_tensor_name(name.replace('qkv', 'v')), wv)]
            elif 'patch_embed.proj.weight' in name:
                c1, c2, kt, kh, kw = data_torch.shape
                del c1, c2, kh, kw
                assert kt == 2, 'Current implmentation only support temporal_patch_size of 2'
                return [(gguf.TENSOR_NAMES[gguf.MODEL_TENSOR.V_ENC_EMBD_PATCH] + '.weight', data_torch[:, :, 0, ...]), (gguf.TENSOR_NAMES[gguf.MODEL_TENSOR.V_ENC_EMBD_PATCH] + '.weight.1', data_torch[:, :, 1, ...])]
            else:
                return [(self.map_tensor_name(name), data_torch)]
        return []
...
class Qwen25OmniModel(Qwen2VLVisionModel):
    ...
    def set_gguf_parameters(self):
        ...
        self.gguf_writer.add_audio_attention_layernorm_eps(self.hparams_audio.get('layer_norm_eps', 1e-05))
    def get_vision_config(self) -> dict[str, Any] | None:
    ...
    def modify_tensors(self, data_torch: Tensor, name: str, bid: int | None) -> Iterable[tuple[str, Tensor]]:
        ...
        if any([name.startswith(prefix) for prefix in vision_prefix]):
            ...
            if '.qkv.' in name:
                ...
                wk = data_torch[c:c * 2]
                wv = data_torch[c * 2:]
                return [(self.map_tensor_name(name.replace('attn.qkv', 'self_attn.q_proj')), wq), (self.map_tensor_name(name.replace('attn.qkv', 'self_attn.k_proj')), wk), (self.map_tensor_name(name.replace('attn.qkv', 'self_attn.v_proj')), wv)]
            return [(self.map_tensor_name(name), data_torch)]
        return []
...
class InternLM2Model(TextModel):
    ...
    def set_vocab(self):
        ...
        if chat_eos_token_id is not None:
            ...
            logger.warning(f'Replace eos:{old_eos} with a special token:{chat_eos_token_id} in chat mode so that the conversation can end normally.')
        special_vocab.add_to_gguf(self.gguf_writer)
    def set_gguf_parameters(self):
        self.gguf_writer.add_context_length(self.hparams['max_position_embeddings'])
        self.gguf_writer.add_block_count(self.hparams['num_hidden_layers'])
    ...
    def modify_tensors(self, data_torch: Tensor, name: str, bid: int | None) -> Iterable[tuple[str, Tensor]]:
        ...
        if bid is not None and f'model.layers.{bid}.attention.wqkv' in name:
            ...
            return [(self.format_tensor_name(gguf.MODEL_TENSOR.ATTN_Q, bid), q), (self.format_tensor_name(gguf.MODEL_TENSOR.ATTN_K, bid), k), (self.format_tensor_name(gguf.MODEL_TENSOR.ATTN_V, bid), v)]
        else:
            return [(self.map_tensor_name(name), data_torch)]
...
class BertModel(TextModel):
    ...
    def _xlmroberta_set_vocab(self) -> None:
        ...
        if isinstance(tokenizer, SentencePieceProcessor):
            ...
            toktypes = [SentencePieceTokenTypes.CONTROL, SentencePieceTokenTypes.CONTROL, SentencePieceTokenTypes.CONTROL, SentencePieceTokenTypes.UNKNOWN] + toktypes[3:-1]
...
class NeoBert(BertModel):
    ...
    def set_gguf_parameters(self):
        ...
        f_rms_eps = self.hparams.get('norm_eps', 1e-06)
        self.gguf_writer.add_layer_norm_rms_eps(f_rms_eps)
        logger.info(f'gguf: rms norm epsilon = {f_rms_eps}')
...
class GemmaModel(TextModel):
    ...
    def set_vocab(self):
        self._set_vocab_sentencepiece()
        special_vocab = gguf.SpecialVocab(self.dir_model, load_merges=False, special_token_types=['prefix', 'suffix', 'middle', 'fsep', 'eot'])
...
class Gemma3Model(TextModel):
    ...
    def set_gguf_parameters(self):
        ...
        self.gguf_writer.add_layer_norm_rms_eps(self.hparams.get('rms_norm_eps', 1e-06))
...
class Gemma3VisionModel(MmprojModel):
    def set_gguf_parameters(self):
        ...
        self.gguf_writer.add_vision_attention_layernorm_eps(hparams.get('layer_norm_eps', 1e-06))
        self.gguf_writer.add_vision_use_gelu(True)
...
class Gemma3NModel(Gemma3Model):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._altup_proj = [torch.Tensor(), torch.Tensor(), torch.Tensor()]
        self._altup_unembd = [torch.Tensor(), torch.Tensor(), torch.Tensor()]
    def set_vocab(self):
        super().set_vocab()
    def set_gguf_parameters(self):
        super().set_gguf_parameters()
...
class Rwkv7Model(TextModel):
    ...
    def modify_tensors(self, data_torch: Tensor, name: str, bid: int | None) -> Iterable[tuple[str, Tensor]]:
        ...
        if bid is not None and 'attention.x_' in name:
            ...
            if not (new_name.endswith('.weight') or new_name.endswith('.bias')):
                new_name += '.weight'
            if self.lora_needs_transpose and any((new_name.endswith(t) for t in ['time_mix_w1.weight', 'time_mix_w2.weight', 'time_mix_a1.weight', 'time_mix_a2.weight', 'time_mix_v1.weight', 'time_mix_v2.weight', 'time_mix_g1.weight', 'time_mix_g2.weight'])):
                data_torch = data_torch.transpose(0, 1)
            if 'r_k' in new_name:
...
class MambaModel(TextModel):
    ...
    def set_gguf_parameters(self):
        ...
        rms_norm_eps = self.find_hparam(['layer_norm_epsilon', 'rms_norm_eps'], optional=True) or 1e-05
...
class Mamba2Model(TextModel):
    ...
    def modify_tensors(self, data_torch: Tensor, name: str, bid: int | None) -> Iterable[tuple[str, Tensor]]:
        ...
        if self.match_model_tensor_name(new_name, gguf.MODEL_TENSOR.SSM_CONV1D, bid):
            data_torch = data_torch.squeeze()
        elif any((self.match_model_tensor_name(new_name, t, bid, suffix='') for t in [gguf.MODEL_TENSOR.SSM_A, gguf.MODEL_TENSOR.SSM_D])):
...
class JambaModel(TextModel):
    ...
    def set_gguf_parameters(self):
        ...
        rms_norm_eps = self.find_hparam(['layer_norm_epsilon', 'rms_norm_eps'], optional=True) or 1e-06
        n_kv_head = self.hparams['num_key_value_heads']
        attn_offset = self.hparams['attn_layer_offset']
        attn_period = self.hparams['attn_layer_period']
        n_kv_vec = [0 for _ in range(attn_offset)] + [n_kv_head if (i - attn_offset) % attn_period == 0 else 0 for i in range(attn_offset, self.block_count)]
        self.gguf_writer.add_block_count(self.block_count)
...
class OpenELMModel(TextModel):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._ffn_dims: list[int] = [OpenELMModel._make_divisible(multiplier * self._n_embd, ffn_dim_divisor) for multiplier in ffn_multipliers]
        assert isinstance(self._num_kv_heads, list) and isinstance(self._num_kv_heads[0], int)
...
class DeepseekV2Model(TextModel):
    ...
    def modify_tensors(self, data_torch: Tensor, name: str, bid: int | None) -> Iterable[tuple[str, Tensor]]:
        ...
        if name.endswith('e_score_correction_bias'):
            name = name.replace('e_score_correction_bias', 'e_score_correction.bias')
        block_count = self.hparams['num_hidden_layers']
        match = re.match('model.layers.(\\d+)', name)
...
class ChatGLMModel(TextModel):
    ...
    def set_gguf_parameters(self):
        ...
        self.gguf_writer.add_head_count(n_head)
        self.gguf_writer.add_head_count_kv(n_head_kv)
        self.gguf_writer.add_layer_norm_rms_eps(self.hparams.get('layernorm_epsilon', 1e-05))
        self.gguf_writer.add_file_type(self.ftype)
...
class GraniteMoeModel(GraniteModel):
    ...
    def modify_tensors(self, data_torch: Tensor, name: str, bid: int | None) -> Iterable[tuple[str, Tensor]]:
        if name.endswith('block_sparse_moe.input_linear.weight'):
            ...
            return [(self.format_tensor_name(gguf.MODEL_TENSOR.FFN_GATE_EXP, bid), gate), (self.format_tensor_name(gguf.MODEL_TENSOR.FFN_UP_EXP, bid), up)]
        ...
        if name.endswith('shared_mlp.input_linear.weight'):
            ...
            gate, up = data_torch.split(ffn_dim, dim=-2)
            if has_experts:
                return [(self.format_tensor_name(gguf.MODEL_TENSOR.FFN_GATE_SHEXP, bid), gate), (self.format_tensor_name(gguf.MODEL_TENSOR.FFN_UP_SHEXP, bid), up)]
            return [(self.format_tensor_name(gguf.MODEL_TENSOR.FFN_GATE, bid), gate), (self.format_tensor_name(gguf.MODEL_TENSOR.FFN_UP, bid), up)]
...
class BailingMoeModel(TextModel):
    ...
    def modify_tensors(self, data_torch: Tensor, name: str, bid: int | None) -> Iterable[tuple[str, Tensor]]:
        ...
        if name.endswith('attention.dense.weight'):
            ...
            return [(self.format_tensor_name(gguf.MODEL_TENSOR.ATTN_Q, bid), BailingMoeModel.permute(q, n_head, n_head)), (self.format_tensor_name(gguf.MODEL_TENSOR.ATTN_K, bid), BailingMoeModel.permute(k, n_head, n_kv_head)), (self.format_tensor_name(gguf.MODEL_TENSOR.ATTN_V, bid), v)]
        ...
        if new_name == output_name and self.hparams.get('norm_head'):
            ...
            data_torch /= torch.norm(data_torch, p=2, dim=0, keepdim=True) + 1e-07
        return [(new_name, data_torch)]
    def prepare_tensors(self):
        super().prepare_tensors()
        if self._experts is not None:
...
class WhisperEncoderModel(MmprojModel):
    ...
    def set_gguf_parameters(self):
        ...
        self.gguf_writer.add_audio_attention_layernorm_eps(self.hparams.get('layer_norm_eps', 1e-05))
    def tensor_force_quant(self, name, new_name, bid, n_dims):
        del bid, new_name, n_dims
        if '.conv' in name and '.weight' in name:
            return gguf.GGMLQuantizationType.F16
    ...
    def modify_tensors(self, data_torch: Tensor, name: str, bid: int | None) -> Iterable[tuple[str, Tensor]]:
        ...
        if 'gate_up_proj' in name:
            if name.endswith('_bias'):
                ...
                return [(self.map_tensor_name(name_gate), gate_proj_bias), (self.map_tensor_name(name_up), up_proj_bias)]
            elif '_blocks' not in name and '_scales' not in name:
                logger.warning(f'{name} is not in MXFP4, performance may be degraded')
                name_up = name.replace('gate_up_proj', 'up_proj.weight')
                name_gate = name.replace('gate_up_proj', 'gate_proj.weight')
...
class LFM2Model(TextModel):
    ...
    def _add_feed_forward_length(self):
        ...
        self.gguf_writer.add_feed_forward_length(ff_dim)
    def set_gguf_parameters(self):
        self.hparams['num_key_value_heads'] = [self.hparams['num_key_value_heads'] if layer_type == 'full_attention' else 0 for layer_type in self.hparams['layer_types']]
        super().set_gguf_parameters()
        self.gguf_writer.add_vocab_size(self.hparams['vocab_size'])
...
class LazyTorchTensor(gguf.LazyBase):
    ...
    dtype: torch.dtype
    shape: torch.Size
    _dtype_map: dict[torch.dtype, type] = {torch.float16: np.float16, torch.float32: np.float32, torch.uint8: np.uint8}
    _dtype_str_map: dict[str, torch.dtype] = {'F64': torch.float64, 'F32': torch.float32, 'BF16': torch.bfloat16, 'F16': torch.float16, 'I64': torch.int64, 'I32': torch.int32, 'I16': torch.int16, 'U8': torch.uint8, 'I8': torch.int8, 'BOOL': torch.bool, 'F8_E4M3': torch.float8_e4m3fn, 'F8_E5M2': torch.float8_e5m2}
    def numpy(self) -> gguf.LazyNumpyTensor:
        dtype = self._dtype_map[self.dtype]
        return gguf.LazyNumpyTensor(meta=gguf.LazyNumpyTensor.meta_with_dtype_and_shape(dtype, self.shape), args=(self,), func=lambda s: s.numpy())
    @classmethod
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--outfile', type=Path, help='path to write to; default: based on input. {ftype} will be replaced by the outtype.')
    parser.add_argument('--outtype', type=str, choices=['f32', 'f16', 'bf16', 'q8_0', 'tq1_0', 'tq2_0', 'auto'], default='f16', help='output format - use f32 for float32, f16 for float16, bf16 for bfloat16, q8_0 for Q8_0, tq1_0 or tq2_0 for ternary, and auto for the highest-fidelity 16-bit float type depending on the first loaded tensor type')
    parser.add_argument('--bigendian', action='store_true', help='model is executed on big endian machine')
    parser.add_argument('model', type=str, help='directory containing model file or huggingface repository ID (if --remote)', nargs='?')
    parser.add_argument('--use-temp-file', action='store_true', help='use the tempfile library while processing (helpful when running out of memory, process killed)')
    parser.add_argument('--no-lazy', action='store_true', help='use more RAM by computing all outputs before writing (use in case lazy evaluation is broken)')
    ...
    parser.add_argument('--no-tensor-first-split', action='store_true', help='do not add tensors to the first split (disabled by default)')
    ...
    parser.add_argument('--mmproj', action='store_true', help="(Experimental) Export multimodal projector (mmproj) for vision models. This will only work on some vision models. A prefix 'mmproj-' will be added to the output file name.")
...
def main() -> None:
    ...
    if args.remote:
        ...
        local_dir = snapshot_download(repo_id=hf_repo_id, allow_patterns=['LICENSE', '*.json', '*.md', '*.txt', 'tokenizer.model'])
    ...
    if not dir_model.is_dir():
        ...
        sys.exit(1)
    ftype_map: dict[str, gguf.LlamaFileType] = {'f32': gguf.LlamaFileType.ALL_F32, 'f16': gguf.LlamaFileType.MOSTLY_F16, 'bf16': gguf.LlamaFileType.MOSTLY_BF16, 'q8_0': gguf.LlamaFileType.MOSTLY_Q8_0, 'tq1_0': gguf.LlamaFileType.MOSTLY_TQ1_0, 'tq2_0': gguf.LlamaFileType.MOSTLY_TQ2_0, 'auto': gguf.LlamaFileType.GUESSED}
