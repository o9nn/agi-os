...
class Hyperparameters:
    ...
    def load(self, data, offset):
        self.n_vocab, self.n_embd, self.n_mult, self.n_head, self.n_layer, self.n_rot, ftype = struct.unpack('<7I', data[offset:offset + 4 * 7])
        try:
...
def handle_metadata(cfg, hp):
    ...
    fakemodel = {'tok_embeddings.weight': convert.LazyTensor.__new__(convert.LazyTensor), 'layers.0.feed_forward.w1.weight': convert.LazyTensor.__new__(convert.LazyTensor)}
...
def handle_args():
    ...
    parser.add_argument('--vocabtype', default='spm,hfft', help='vocab format - only meaningful with --model-metadata-dir and/or --vocab-dir (default: spm,hfft)')
