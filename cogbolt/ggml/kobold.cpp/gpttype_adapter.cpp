...
static clip_image_u8 * clp_img_data = nullptr;
static clip_ctx * clp_ctx_a = nullptr;
static whisper_preprocessor::whisper_filters w_filters;
static std::vector<media_object> media_objects;
static std::vector<int> last_media_mem;
static std::string media_composite_image_signature = "";
...
static std::vector<int> special_stop_sequence;
...
static std::unordered_multimap<gpt_vocab::id, std::vector<gpt_vocab::id>> dry_sequence_breakers;
static std::vector<int> dry_repeat_count;
...
static std::string concat_output_reader_copy_poll = "";
static std::string concat_output_reader_copy_res = "";
static std::vector<logit_bias> logit_biases;
static bool add_bos_token = true;
static bool load_guidance = false;
static bool check_slowness = false;
static bool highpriority = false;
static int delayed_generated_tokens_limit = 0;
std::deque<std::string> delayed_generated_tokens;
static std::map<int,std::vector<int>> antislop_banned_token_ids;
...
static std::string RemoveBell(const std::string & input)
{
std::string word2;
...
draft_model_params.n_gpu_layers = draft_gpulayers;
...
auto draftok = (llama_decode(main_ctx, batch2.batch)==0);
if(!draftok)
{
...
float threshold = a * maxprob * maxprob;
...
const float min_logit = cur_p->data[0].logit + logf(p);
size_t i = 1;
for (; i < cur_p->size; ++i) {
...
float h = candidates_p->data[0].logit;
for (size_t i = 0; i < candidates_p->size; ++i)
{
...
std::vector<llama_model_kv_override> kvos;
std::vector<llama_model_tensor_buft_override> tenos;
std::vector<std::string> temp_tensor_names;
...
n_vocab = vocab.id_to_token.size();
if (file_format == FileFormat::RWKV_1)
{
auto padding = 512u;
...
{
media_composite_image_signature = "";
...
if (temp.find("[gMASK]<sop>") != std::string::npos) {
if (addedmemory == "") {
if (!kcpp_data->prompt.empty() && kcpp_data->prompt.rfind("[gMASK]", 0) == 0) {
kcpp_data->prompt.erase(0, 7);
}
if (!kcpp_data->prompt.empty() && kcpp_data->prompt.rfind("<sop>", 0) == 0) {
kcpp_data->prompt.erase(0, 5);
}
if (!kcpp_data->prompt.empty() && kcpp_data->prompt[0] == ' ') {
kcpp_data->prompt.erase(0, 1);
}
addedmemory = "[gMASK]<sop> ";
} else {
if (!addedmemory.empty() && addedmemory.rfind("[gMASK]", 0) == 0) {
addedmemory.erase(0, 7);
}
if (!addedmemory.empty() && addedmemory.rfind("<sop>", 0) == 0) {
addedmemory.erase(0, 5);
}
if (!addedmemory.empty() && addedmemory[0] == ' ') {
...
{
if(grammarstr=="" || current_grammar!=grammarstr)
{
...
std::vector<int> embd_inp_mem;
std::vector<int> media_intro;
std::vector<int> guidance_embd;
...
{
embd_inp_mem.insert(embd_inp_mem.begin(), bos[0]);
}
...
bool v3_use_scratch = true;
speculative_draft_result draft_results;
...
if(embd.size()!=1 || draft_ctx==nullptr || remaining_tokens<=speculative_chunk_amt || grammar!=nullptr || startedsampling==false)
{
draft_used = false;
...
n_past += embd.size();
embd.clear();
if (!early_abort && (int)embd_inp.size() <= input_consumed)
{
...
if(!inputs.render_special && (found_eog || VecContainsIntVal(special_stop_sequence,id)))
{
tokenizedstr = "";
}
...
if(currtoken==MEDIA_TOKEN_IDENTIFIER_A || currtoken==MEDIA_TOKEN_IDENTIFIER_B)
{
if(!media_embds_built)
{
PrepareMediaEmbds(nctx, media_intro);
media_embds_built = true;
