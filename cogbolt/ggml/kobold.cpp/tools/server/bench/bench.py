...
def main(args_in: list[str] | None=None) -> None:
    ...
    parser.add_argument('--n-prompts', type=int, help='SERVER_BENCH_N_PROMPTS: total prompts to randomly select in the benchmark', required=True)
    parser.add_argument('--max-prompt-tokens', type=int, help='SERVER_BENCH_MAX_PROMPT_TOKENS: maximum prompt tokens to filter out in the dataset', required=True)
    parser.add_argument('--max-tokens', type=int, help='SERVER_BENCH_MAX_CONTEXT: maximum context size of the completions request to filter out in the dataset: prompt + predicted tokens', required=True)
                ...
                for metric_name in data['metrics']:
                    for metric_metric in data['metrics'][metric_name]:
                        ...
                        if isinstance(value, float) or isinstance(value, int):
                            ...
                            github_env.write(f'{escape_metric_name(metric_name)}_{escape_metric_name(metric_metric)}={value}\n')
    ...
    if server_process:
        ...
        while is_server_listening(args.host, args.port):
            time.sleep(0.1)
    title = f'llama.cpp {args.name} on {args.runner_label}\n duration={args.duration} {iterations} iterations'
    xlabel = f'{args.hf_repo}/{args.hf_file}\nparallel={args.parallel} ctx-size={args.ctx_size} ngl={args.n_gpu_layers} batch-size={args.batch_size} ubatch-size={args.ubatch_size} pp={args.max_prompt_tokens} pp+tg={args.max_tokens}\nbranch={args.branch} commit={args.commit}'
    end_time = time.time()
    prometheus_metrics = {}
    if is_server_listening('0.0.0.0', 9090):
        metrics = ['prompt_tokens_seconds', 'predicted_tokens_seconds', 'kv_cache_usage_ratio', 'requests_processing', 'requests_deferred']
        for metric in metrics:
            resp = requests.get(f'http://localhost:9090/api/v1/query_range', params={'query': 'llamacpp:' + metric, 'start': start_time, 'end': end_time, 'step': 2})
            ...
            if resp.status_code != 200:
                ...
                plt.xticks(rotation=0, fontsize=14, horizontalalignment='center', alpha=0.7)
    ...
    bench_results = {'i': iterations, 'req': {'p95': round(data['metrics']['http_req_duration']['p(95)'], 2), 'avg': round(data['metrics']['http_req_duration']['avg'], 2)}, 'pp': {'p95': round(data['metrics']['llamacpp_prompt_processing_second']['p(95)'], 2), 'avg': round(data['metrics']['llamacpp_prompt_processing_second']['avg'], 2), '0': round(mean(prometheus_metrics['prompt_tokens_seconds']), 2) if 'prompt_tokens_seconds' in prometheus_metrics else 0}, 'tg': {'p95': round(data['metrics']['llamacpp_tokens_second']['p(95)'], 2), 'avg': round(data['metrics']['llamacpp_tokens_second']['avg'], 2), '0': round(mean(prometheus_metrics['predicted_tokens_seconds']), 2) if 'predicted_tokens_seconds' in prometheus_metrics else 0}}
...
def start_benchmark(args):
    ...
    k6_args = ['run', args.scenario, '--no-color', '--no-connection-reuse', '--no-vu-connection-reuse']
...
def start_server_background(args):
    ...
    server_args = ['--host', args.host, '--port', args.port]
    ...
    server_process = subprocess.Popen(args, **pkwargs)
