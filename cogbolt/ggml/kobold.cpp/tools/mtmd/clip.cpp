...
ggml_tensor * patch_embeddings_1 = nullptr;
...
{
embeddings = ggml_concat(ctx0, model.mm_glm_tok_boi, embeddings, 1);
embeddings = ggml_concat(ctx0, embeddings, model.mm_glm_tok_eoi, 1);
}
}
else {
...
GGML_ASSERT(!model.layers[0].k_b);
...
int new_image_size = (target_width * target_height * nc) + 512;
...
memset(letterboxed_image, 0, new_image_size);
...
n_patches_sq = n_patches_y * n_patches_x + n_patches_y - 1;
} break;
case PROJECTOR_TYPE_LLAMA4:
{
...
assert(embed_dim % 2 == 0);
std::vector<std::vector<std::vector<float>>> emb_h = get_1d_sincos_pos_embed_from_grid_new(embed_dim / 2, grid[0]);
std::vector<std::vector<std::vector<float>>> emb_w = get_1d_sincos_pos_embed_from_grid_new(embed_dim / 2, grid[1]);
int H = emb_h.size();
int W = emb_h[0].size();
std::vector<std::vector<std::vector<float>>> emb(H, std::vector<std::vector<float>>(W, std::vector<float>(embed_dim)));
for (int h = 0; h < H; ++h) {
