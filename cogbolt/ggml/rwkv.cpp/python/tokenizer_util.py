...
def add_tokenizer_argument(parser) -> None:
    parser.add_argument('tokenizer', help='Tokenizer to use; supported tokenizers: auto (guess from n_vocab), 20B, world', nargs='?', type=str, default='auto')
def get_tokenizer(tokenizer_name: str, n_vocab: int) -> Tuple[Callable[[List[int]], str], Callable[[str], List[int]]]:
