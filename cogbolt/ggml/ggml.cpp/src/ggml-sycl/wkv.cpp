...
scope_op_debug_print scope_dbg_print(__func__, dst, 6);
...
const size_t shared_mem_size = C / H * 4 * sizeof(float);
...
scope_op_debug_print scope_dbg_print(__func__, dst, 7);
...
GGML_ASSERT(C / H == WKV_BLOCK_SIZE || C / H == WKV_BLOCK_SIZE * 2);
dpct::queue_ptr stream = ctx.stream();
const size_t shared_mem_size = C / H * 5 * sizeof(float);
sycl::range<3> block_dims(1, 1, C / H);
sycl::range<3> grid_dims(1, 1, B * H);
if (C / H == WKV_BLOCK_SIZE) {
sycl_launch(stream, [&](sycl::handler & cgh) {
