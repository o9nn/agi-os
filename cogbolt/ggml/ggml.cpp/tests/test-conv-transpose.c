...
struct ggml_tensor * t = ggml_new_tensor_2d(ctx, GGML_TYPE_F32, 3, 2);
memcpy(t->data, buf_f32, ggml_nbytes(t));
struct ggml_tensor * k = ggml_new_tensor_3d(ctx, GGML_TYPE_F16, 2, 3, 2);
memcpy(k->data, buf_f16, ggml_nbytes(k));
struct ggml_tensor * out_1 = ggml_conv_transpose_1d(ctx, k, t, 1 , 0 , 1 );
struct ggml_tensor * out_2 = ggml_conv_transpose_1d(ctx, k, t, 2 , 0 , 1 );
struct ggml_tensor * out_3 = ggml_conv_transpose_1d(ctx, k, t, 3 , 0 , 1 );
...
{
struct ggml_context * ctx = make_ctx();
struct ggml_tensor * t = ggml_new_tensor_4d(ctx, GGML_TYPE_F32, 3, 2, 2, 1);
memcpy(t->data, buf_f32, ggml_nbytes(t));
struct ggml_tensor * k = ggml_new_tensor_4d(ctx, GGML_TYPE_F16, 2, 2, 3, 2);
memcpy(k->data, buf_f16, ggml_nbytes(k));
