...
__c_type_to_python_type = {'void': 'None', '_Bool': 'bool', 'char': 'int', 'short': 'int', 'int': 'int', 'long': 'int', 'ptrdiff_t': 'int', 'size_t': 'int', 'int8_t': 'int', 'uint8_t': 'int', 'int16_t': 'int', 'uint16_t': 'int', 'int32_t': 'int', 'uint32_t': 'int', 'int64_t': 'int', 'uint64_t': 'int', 'float': 'float', 'double': 'float', 'ggml_fp16_t': 'np.float16'}
...
class PythonStubFuncDeclVisitor(c_ast.NodeVisitor):
    ...
    def get_source_snippet_lines(self, coord: pycparser.plyparser.Coord) -> Tuple[list[str], list[str]]:
        ...
        ncomment_lines = len(list(itertools.takewhile(lambda i: re.search('^\\s*(//|/\\*)', source_lines[i]), range(coord.line - 2, -1, -1))))
