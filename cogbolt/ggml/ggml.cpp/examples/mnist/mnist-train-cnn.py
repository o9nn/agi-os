...
def train(model_path):
    ...
    model = keras.Sequential([keras.Input(shape=input_shape, dtype=tf.float32), layers.Conv2D(8, kernel_size=(3, 3), padding='same', activation='relu', dtype=tf.float32), layers.MaxPooling2D(pool_size=(2, 2)), layers.Conv2D(16, kernel_size=(3, 3), padding='same', activation='relu', dtype=tf.float32), layers.MaxPooling2D(pool_size=(2, 2)), layers.Flatten(), layers.Dense(num_classes, activation='softmax', dtype=tf.float32)])
    model.summary()
    batch_size = 1000
    epochs = 30
