...
from typing import Any, Callable, ContextManager, Iterable, Iterator, List, Literal, Tuple, Set
...
class ServerProcess:
    ...
    def start(self, timeout_seconds: int | None=DEFAULT_HTTP_TIMEOUT) -> None:
        ...
        server_args = ['--host', self.server_host, '--port', self.server_port, '--temp', self.temperature, '--seed', self.seed]
        if self.model_file:
        ...
        self.process = subprocess.Popen([str(arg) for arg in [server_path, *server_args]], creationflags=flags, stdout=sys.stdout, stderr=sys.stdout, env={**os.environ, 'LLAMA_CACHE': 'tmp'} if 'LLAMA_CACHE' not in os.environ else None)
        server_instances.add(self)
        print(f'server pid={self.process.pid}, pytest pid={os.getpid()}')
        start_time = time.time()
        while time.time() - start_time < timeout_seconds:
            try:
                response = self.make_request('GET', '/health', headers={'Authorization': f'Bearer {self.api_key}' if self.api_key else None})
    ...
    def stop(self) -> None:
        ...
        if self.process:
            ...
            self.process.kill()
            self.process = None
    def make_request(self, method: str, path: str, data: dict | Any | None=None, headers: dict | None=None, timeout: float | None=None) -> ServerResponse:
        ...
        return result
    def make_stream_request(self, method: str, path: str, data: dict | None=None, headers: dict | None=None) -> Iterator[dict]:
        url = f'http://{self.server_host}:{self.server_port}{path}'
        if method == 'POST':
        ...
        for line_bytes in response.iter_lines():
            ...
            if '[DONE]' in line:
                ...
                yield data
    def make_any_request(self, method: str, path: str, data: dict | None=None, headers: dict | None=None, timeout: float | None=None) -> dict:
        ...
        if stream:
            ...
            for chunk in self.make_stream_request(method, path, data, headers):
                ...
                for tc in choice['delta'].get('tool_calls', []):
                    ...
                    if tc['index'] >= len(tool_calls):
                        ...
                        assert 'function' in tc and 'name' in tc['function'] and (len(tc['function']['name']) > 0), f"Expected function call with name, got {tc.get('function')}"
                        tool_calls.append(dict(id='', type='function', function=dict(name='', arguments='')))
            ...
            result = dict(choices=[dict(index=0, finish_reason=finish_reason, message=dict(role='assistant', content=''.join(content) if content else None, reasoning_content=''.join(reasoning_content) if reasoning_content else None, tool_calls=tool_calls if tool_calls else None))])
...
def match_regex(regex: str, text: str) -> bool:
    return re.compile(regex, flags=RegexFlag.IGNORECASE | RegexFlag.MULTILINE | RegexFlag.DOTALL).search(text) is not None
