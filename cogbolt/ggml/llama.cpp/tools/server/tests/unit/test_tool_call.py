...
class CompletionMode(Enum):
    NORMAL = 'normal'
    STREAMED = 'streamed'
TEST_TOOL = {'type': 'function', 'function': {'name': 'test', 'description': '', 'parameters': {'type': 'object', 'properties': {'success': {'type': 'boolean', 'const': True}}, 'required': ['success']}}}
PYTHON_TOOL = {'type': 'function', 'function': {'name': 'python', 'description': 'Runs code in an ipython interpreter and returns the result of the execution after 60 seconds.', 'parameters': {'type': 'object', 'properties': {'code': {'type': 'string', 'description': 'The code to run in the ipython interpreter.'}}, 'required': ['code']}}}
WEATHER_TOOL = {'type': 'function', 'function': {'name': 'get_current_weather', 'description': 'Get the current weather in a given location', 'parameters': {'type': 'object', 'properties': {'location': {'type': 'string', 'description': "The city and country/state, e.g. 'San Francisco, CA', or 'Paris, France'"}}, 'required': ['location']}}}
def do_test_completion_with_required_tool_tiny(server: ServerProcess, tool: dict, argument_key: str | None, n_predict, **kwargs):
    body = server.make_any_request('POST', '/v1/chat/completions', data={'max_tokens': n_predict, 'messages': [{'role': 'system', 'content': 'You are a coding assistant.'}, {'role': 'user', 'content': 'Write an example'}], 'tool_choice': 'required', 'tools': [tool], 'parallel_tool_calls': False, **kwargs})
...
def do_test_completion_without_tool_call(server: ServerProcess, n_predict: int, tools: list[dict], tool_choice: str | None, **kwargs):
    body = server.make_any_request('POST', '/v1/chat/completions', data={'max_tokens': n_predict, 'messages': [{'role': 'system', 'content': 'You are a coding assistant.'}, {'role': 'user', 'content': 'say hello world with python'}], 'tools': tools if tools else None, 'tool_choice': tool_choice, **kwargs}, timeout=TIMEOUT_HTTP_REQUEST)
    choice = body['choices'][0]
    assert choice['message'].get('tool_calls') is None, f"Expected no tool call in {choice['message']}"
@pytest.mark.parametrize('stream', [CompletionMode.NORMAL, CompletionMode.STREAMED])
@pytest.mark.parametrize('template_name,n_predict,tools,tool_choice', [('meta-llama-Llama-3.3-70B-Instruct', 128, [], None), ('meta-llama-Llama-3.3-70B-Instruct', 128, [TEST_TOOL], None), ('meta-llama-Llama-3.3-70B-Instruct', 128, [PYTHON_TOOL], 'none')])
...
@pytest.mark.slow
@pytest.mark.parametrize('stream', [CompletionMode.NORMAL, CompletionMode.STREAMED])
@pytest.mark.parametrize('template_name,n_predict,tools,tool_choice', [('meetkai-functionary-medium-v3.2', 256, [], None), ('meetkai-functionary-medium-v3.2', 256, [TEST_TOOL], None), ('meetkai-functionary-medium-v3.2', 256, [PYTHON_TOOL], 'none'), ('meetkai-functionary-medium-v3.1', 256, [], None), ('meetkai-functionary-medium-v3.1', 256, [TEST_TOOL], None), ('meetkai-functionary-medium-v3.1', 256, [PYTHON_TOOL], 'none'), ('meta-llama-Llama-3.2-3B-Instruct', 256, [], None), ('meta-llama-Llama-3.2-3B-Instruct', 256, [TEST_TOOL], None), ('meta-llama-Llama-3.2-3B-Instruct', 256, [PYTHON_TOOL], 'none')])
...
@pytest.mark.slow
@pytest.mark.parametrize('stream', [CompletionMode.NORMAL, CompletionMode.STREAMED])
@pytest.mark.parametrize('hf_repo,template_override', [('bartowski/Meta-Llama-3.1-8B-Instruct-GGUF:Q4_K_M', None), ('bartowski/Meta-Llama-3.1-8B-Instruct-GGUF:Q4_K_M', 'chatml'), ('bartowski/Phi-3.5-mini-instruct-GGUF:Q4_K_M', None), ('bartowski/Phi-3.5-mini-instruct-GGUF:Q4_K_M', 'chatml'), ('bartowski/Qwen2.5-1.5B-Instruct-GGUF:Q4_K_M', None), ('bartowski/Qwen2.5-1.5B-Instruct-GGUF:Q4_K_M', 'chatml'), ('bartowski/Qwen2.5-Coder-3B-Instruct-GGUF:Q4_K_M', None), ('bartowski/Qwen2.5-Coder-3B-Instruct-GGUF:Q4_K_M', 'chatml'), ('bartowski/Qwen2.5-7B-Instruct-GGUF:Q4_K_M', None), ('bartowski/Qwen2.5-7B-Instruct-GGUF:Q4_K_M', 'chatml'), ('bartowski/Hermes-2-Pro-Llama-3-8B-GGUF:Q4_K_M', ('NousResearch/Hermes-2-Pro-Llama-3-8B', 'tool_use')), ('bartowski/Hermes-2-Pro-Llama-3-8B-GGUF:Q4_K_M', 'chatml'), ('bartowski/Hermes-3-Llama-3.1-8B-GGUF:Q4_K_M', ('NousResearch/Hermes-3-Llama-3.1-8B', 'tool_use')), ('bartowski/Hermes-3-Llama-3.1-8B-GGUF:Q4_K_M', 'chatml'), ('bartowski/Llama-3.2-3B-Instruct-GGUF:Q4_K_M', ('meta-llama/Llama-3.2-3B-Instruct', None)), ('bartowski/Llama-3.2-3B-Instruct-GGUF:Q4_K_M', 'chatml'), ('bartowski/c4ai-command-r7b-12-2024-GGUF:Q6_K_L', ('CohereForAI/c4ai-command-r7b-12-2024', 'tool_use')), ('bartowski/DeepSeek-R1-Distill-Qwen-7B-GGUF:Q4_K_M', None), ('bartowski/gemma-2-2b-it-GGUF:Q4_K_M', None)])
...
def do_test_weather(server: ServerProcess, **kwargs):
    body = server.make_any_request('POST', '/v1/chat/completions', data={'messages': [{'role': 'system', 'content': 'You are a chatbot that uses tools/functions. Dont overthink things.'}, {'role': 'user', 'content': 'What is the weather in Istanbul?'}], 'tools': [WEATHER_TOOL], **kwargs}, timeout=TIMEOUT_HTTP_REQUEST)
    choice = body['choices'][0]
    tool_calls = choice['message'].get('tool_calls')
    assert tool_calls and len(tool_calls) == 1, f"Expected 1 tool call in {choice['message']}"
    tool_call = tool_calls[0]
...
def do_test_calc_result(server: ServerProcess, result_override: str | None, n_predict: int, **kwargs):
    ...
    if result_override is not None:
        ...
        assert re.match("^[\\s\\S]*?((That's|\\bis) (approximately )?)?\\b0\\.(5\\b|56\\b|556)", content), f'Expected something like "The y coordinate is 0.56.", got {content}'
@pytest.mark.slow
@pytest.mark.parametrize('stream', [CompletionMode.NORMAL, CompletionMode.STREAMED])
@pytest.mark.parametrize('n_predict,reasoning_format,expect_reasoning_content,expect_content,hf_repo,template_override', [(128, 'deepseek', None, '^The sum of 102 and 7 is 109[\\s\\S]*', 'bartowski/Phi-3.5-mini-instruct-GGUF:Q4_K_M', None), (128, None, None, '^The sum of 102 and 7 is 109[\\s\\S]*', 'bartowski/Phi-3.5-mini-instruct-GGUF:Q4_K_M', None), (1024, 'deepseek', 'I need to calculate the sum of 102 and 7[\\s\\S]*', 'To find the sum of[\\s\\S]*', 'bartowski/DeepSeek-R1-Distill-Qwen-7B-GGUF:Q4_K_M', None), (1024, 'deepseek', 'First, I [\\s\\S]*', 'To find the sum of[\\s\\S]*', 'bartowski/DeepSeek-R1-Distill-Qwen-7B-GGUF:Q4_K_M', ('llama-cpp-deepseek-r1', None))])
def test_thoughts(n_predict: int, reasoning_format: Literal['deepseek', 'none'] | None, expect_content: str | None, expect_reasoning_content: str | None, hf_repo: str, template_override: str | Tuple[str, str | None] | None, stream: CompletionMode):
    ...
    if isinstance(template_override, tuple):
        ...
        server.chat_template = template_override
    server.start(timeout_seconds=TIMEOUT_SERVER_START)
    body = server.make_any_request('POST', '/v1/chat/completions', data={'max_tokens': n_predict, 'messages': [{'role': 'user', 'content': "What's the sum of 102 and 7?"}], 'stream': stream == CompletionMode.STREAMED}, timeout=TIMEOUT_HTTP_REQUEST)
    choice = body['choices'][0]
    assert choice['message'].get('tool_calls') is None, f"Expected no tool call in {choice['message']}"
    content = choice['message'].get('content')
...
def do_test_hello_world(server: ServerProcess, **kwargs):
    body = server.make_any_request('POST', '/v1/chat/completions', data={'messages': [{'role': 'system', 'content': 'You are a tool-calling agent.'}, {'role': 'user', 'content': 'say hello world with python'}], 'tools': [PYTHON_TOOL], **kwargs}, timeout=TIMEOUT_HTTP_REQUEST)
    choice = body['choices'][0]
    tool_calls = choice['message'].get('tool_calls')
    assert tool_calls and len(tool_calls) == 1, f"Expected 1 tool call in {choice['message']}"
    tool_call = tool_calls[0]
    ...
    assert re.match('print\\(("[Hh]ello,? [Ww]orld!?"|\'[Hh]ello,? [Ww]orld!?\')\\)', re.sub('#.*\\n?', '', code)), f'Expected hello world, got {code}'