...
class DocsGenerator:
        ...
        try:
            with open(file_path, 'r', newline='') as f:
                ...
                for row in reader:
                    ...
                    supported_str = row.get('error_message', '').strip()
                    backend_reg_name = row.get('backend_reg_name', '').strip()
                    if not operation or not backend_name or operation in ['CONTEXT_ERROR', 'BUILD_ERROR']:
                        continue
    ...
    def run(self) -> None:
        ...
        if not self.all_operations:
            self.logger.error('No operations found. Make sure to run test-backend-ops support --output csv > docs/ops/file.csv first.')
            return
