...
LLAMA_BENCH_DB_FIELDS = ['build_commit', 'build_number', 'cpu_info', 'gpu_info', 'backends', 'model_filename', 'model_type', 'model_size', 'model_n_params', 'n_batch', 'n_ubatch', 'n_threads', 'cpu_mask', 'cpu_strict', 'poll', 'type_k', 'type_v', 'n_gpu_layers', 'split_mode', 'main_gpu', 'no_kv_offload', 'flash_attn', 'tensor_split', 'tensor_buft_overrides', 'defrag_thold', 'use_mmap', 'embeddings', 'no_op_offload', 'n_prompt', 'n_gen', 'n_depth', 'test_time', 'avg_ns', 'stddev_ns', 'avg_ts', 'stddev_ts']
LLAMA_BENCH_DB_TYPES = ['TEXT', 'INTEGER', 'TEXT', 'TEXT', 'TEXT', 'TEXT', 'TEXT', 'INTEGER', 'INTEGER', 'INTEGER', 'INTEGER', 'INTEGER', 'TEXT', 'INTEGER', 'INTEGER', 'TEXT', 'TEXT', 'INTEGER', 'TEXT', 'INTEGER', 'INTEGER', 'INTEGER', 'TEXT', 'TEXT', 'REAL', 'INTEGER', 'INTEGER', 'INTEGER', 'INTEGER', 'INTEGER', 'INTEGER', 'TEXT', 'INTEGER', 'INTEGER', 'REAL', 'REAL']
TEST_BACKEND_OPS_DB_FIELDS = ['test_time', 'build_commit', 'backend_name', 'op_name', 'op_params', 'test_mode', 'supported', 'passed', 'error_message', 'time_us', 'flops', 'bandwidth_gb_s', 'memory_kb', 'n_runs']
TEST_BACKEND_OPS_DB_TYPES = ['TEXT', 'TEXT', 'TEXT', 'TEXT', 'TEXT', 'TEXT', 'INTEGER', 'INTEGER', 'TEXT', 'REAL', 'REAL', 'REAL', 'INTEGER', 'INTEGER']
...
LLAMA_BENCH_KEY_PROPERTIES = ['cpu_info', 'gpu_info', 'backends', 'n_gpu_layers', 'tensor_buft_overrides', 'model_filename', 'model_type', 'n_batch', 'n_ubatch', 'embeddings', 'cpu_mask', 'cpu_strict', 'poll', 'n_threads', 'type_k', 'type_v', 'use_mmap', 'no_kv_offload', 'split_mode', 'main_gpu', 'tensor_split', 'flash_attn', 'n_prompt', 'n_gen', 'n_depth']
...
LLAMA_BENCH_PRETTY_NAMES = {'cpu_info': 'CPU', 'gpu_info': 'GPU', 'backends': 'Backends', 'n_gpu_layers': 'GPU layers', 'tensor_buft_overrides': 'Tensor overrides', 'model_filename': 'File', 'model_type': 'Model', 'model_size': 'Model size [GiB]', 'model_n_params': 'Num. of par.', 'n_batch': 'Batch size', 'n_ubatch': 'Microbatch size', 'embeddings': 'Embeddings', 'cpu_mask': 'CPU mask', 'cpu_strict': 'CPU strict', 'poll': 'Poll', 'n_threads': 'Threads', 'type_k': 'K type', 'type_v': 'V type', 'use_mmap': 'Use mmap', 'no_kv_offload': 'NKVO', 'split_mode': 'Split mode', 'main_gpu': 'Main GPU', 'tensor_split': 'Tensor split', 'flash_attn': 'FlashAttention'}
TEST_BACKEND_OPS_PRETTY_NAMES = {'backend_name': 'Backend', 'op_name': 'GGML op', 'op_params': 'Op parameters', 'test_mode': 'Mode', 'supported': 'Supported', 'passed': 'Passed', 'error_message': 'Error', 'flops': 'FLOPS', 'bandwidth_gb_s': 'Bandwidth (GB/s)', 'memory_kb': 'Memory (KB)', 'n_runs': 'Runs'}
...
help_b = 'The baseline commit to compare performance to. Accepts either a branch name, tag name, or commit hash. Defaults to latest master commit with data.'
parser.add_argument('-b', '--baseline', help=help_b)
help_c = 'The commit whose performance is to be compared to the baseline. Accepts either a branch name, tag name, or commit hash. Defaults to the non-master commit for which llama-bench was run most recently.'
parser.add_argument('-c', '--compare', help=help_c)
help_t = "The tool whose data is being compared. Either 'llama-bench' or 'test-backend-ops'. This determines the database schema and comparison logic used. If left unspecified, try to determine from the input file."
parser.add_argument('-t', '--tool', help=help_t, default=None, choices=[None, 'llama-bench', 'test-backend-ops'])
help_i = "JSON/JSONL/SQLite/CSV files for comparing commits. Specify multiple times to use multiple input files (JSON/CSV only). Defaults to 'llama-bench.sqlite' in the current working directory. If no such file is found and there is exactly one .sqlite file in the current directory, that file is instead used as input."
parser.add_argument('-i', '--input', action='append', help=help_i)
help_o = "Output format for the table. Defaults to 'pipe' (GitHub compatible). Also supports e.g. 'latex' or 'mediawiki'. See tabulate documentation for full list."
parser.add_argument('-o', '--output', help=help_o, default='pipe')
help_s = f"Columns to add to the table. Accepts a comma-separated list of values. Legal values for test-backend-ops: {', '.join(TEST_BACKEND_OPS_KEY_PROPERTIES)}. Legal values for llama-bench: {', '.join(LLAMA_BENCH_KEY_PROPERTIES[:-3])}. Defaults to model name (model_type) and CPU and/or GPU name (cpu_info, gpu_info) plus any column where not all data points are the same. If the columns are manually specified, then the results for each unique combination of the specified values are averaged WITHOUT weighing by the --repetitions parameter of llama-bench."
...
class LlamaBenchDataSQLite3(LlamaBenchData):
    ...
    def _builds_init(self):
        if self.connection:
            ...
            if self.build_len_min != self.build_len_max:
                logger.warning("Data contains commit hashes of differing lengths. It's possible that the wrong commits will be compared. Try purging the the database of old commits.")
        ...
        super()._builds_init()
    def builds_timestamp(self, reverse: bool=False) -> Union[Iterator[tuple], Sequence[tuple]]:
        data = self.cursor.execute(f'SELECT build_commit, test_time FROM {self.table_name} ORDER BY test_time;').fetchall()
    ...
    def _get_rows_llama_bench(self, properties: list[str], hexsha8_baseline: str, hexsha8_compare: str) -> Sequence[tuple]:
        select_string = ', '.join([f'tb.{p}' for p in properties] + ['tb.n_prompt', 'tb.n_gen', 'tb.n_depth', 'AVG(tb.avg_ts)', 'AVG(tc.avg_ts)'])
        equal_string = ' AND '.join([f'tb.{p} = tc.{p}' for p in LLAMA_BENCH_KEY_PROPERTIES] + [f"tb.build_commit = '{hexsha8_baseline}'", f"tc.build_commit = '{hexsha8_compare}'"])
        group_order_string = ', '.join([f'tb.{p}' for p in properties] + ['tb.n_gen', 'tb.n_prompt', 'tb.n_depth'])
        query = f'SELECT {select_string} FROM {self.table_name} tb JOIN {self.table_name} tc ON {equal_string} GROUP BY {group_order_string} ORDER BY {group_order_string};'
        return self.cursor.execute(query).fetchall()
    def _get_rows_test_backend_ops(self, properties: list[str], hexsha8_baseline: str, hexsha8_compare: str) -> Sequence[tuple]:
        select_string = ', '.join([f'tb.{p}' for p in properties] + ['AVG(tb.flops)', 'AVG(tc.flops)', 'AVG(tb.bandwidth_gb_s)', 'AVG(tc.bandwidth_gb_s)'])
        equal_string = ' AND '.join([f'tb.{p} = tc.{p}' for p in TEST_BACKEND_OPS_KEY_PROPERTIES] + [f"tb.build_commit = '{hexsha8_baseline}'", f"tc.build_commit = '{hexsha8_compare}'", 'tb.supported = 1', 'tc.supported = 1', 'tb.passed = 1', 'tc.passed = 1'])
...
def format_flops(flops_value: float) -> str:
    if flops_value == 0:
        return '0.00'
    units = [(1000000000000.0, 'T'), (1000000000.0, 'G'), (1000000.0, 'M'), (1000.0, 'k'), (1, '')]
...
def format_flops_for_table(flops_value: float, target_unit: str) -> str:
    if flops_value == 0:
        return '0.00'
    unit_divisors = {'TFLOPS': 1000000000000.0, 'GFLOPS': 1000000000.0, 'MFLOPS': 1000000.0, 'kFLOPS': 1000.0, 'FLOPS': 1}
...
def get_flops_unit_name(flops_values: list) -> str:
    ...
    if max_flops >= 1000000000000.0:
        return 'TFLOPS'
    elif max_flops >= 1000000000.0:
        return 'GFLOPS'
    elif max_flops >= 1000000.0:
        return 'MFLOPS'
    elif max_flops >= 1000.0:
        return 'kFLOPS'
    else:
        return 'FLOPS'
bench_data = None
...
if known_args.plot:
    def create_performance_plot(table_data: list[list[str]], headers: list[str], baseline_name: str, compare_name: str, output_file: str, plot_x_param: str, log_scale: bool=False, tool_type: str='llama-bench', metric_name: str='t/s'):
        ...
        for i, row in enumerate(table_data):
            ...
            grouped_data[group_key].append({'x_value': x_value, 'baseline': float(row[-3]), 'compare': float(row[-2]), 'speedup': float(row[-1])})
        if not grouped_data:
        ...
        def make_axes(num_groups, max_cols=2, base_size=(8, 4)):
            ...
            w, h = base_size
            fig, ax_arr = plt.subplots(rows, cols, figsize=(w * cols, h * rows), squeeze=False)
        ...
        for group_key, points in grouped_data.items():
            ...
            ax.plot(x_values, baseline_vals, 'o-', color='skyblue', label=f'{baseline_name}', linewidth=2, markersize=6)
            ax.plot(x_values, compare_vals, 's--', color='lightcoral', alpha=0.8, label=f'{compare_name}', linewidth=2, markersize=6)
            if log_scale:
        ...
        fig.suptitle(f'Performance comparison: {compare_name} vs. {baseline_name}', fontsize=14, fontweight='bold')
...
print(tabulate(table, headers=headers, floatfmt='.2f', tablefmt=known_args.output))