...
def generate_list_rule(element_type):
    ...
    list_rule = f'{rule_name} ::= "["  {element_rule} (","  {element_rule})* "]"'
    return list_rule
def get_members_structure(cls, rule_name):
    ...
    if cls.__annotations__ and cls.__annotations__ != {}:
        result = f'{rule_name} ::= "{{"'
        members = [f'  "\\"{name}\\"" ":"  {map_pydantic_type_to_gbnf(param_type)}' for name, param_type in get_type_hints(cls).items() if name != 'self']
        result += '"," '.join(members)
        result += '  "}"'
        return result
    ...
    result = f'{rule_name} ::=  "{{"'
    members = [f'  "\\"{name}\\"" ":"  {map_pydantic_type_to_gbnf(param.annotation)}' for name, param in parameters.items() if name != 'self' and param.annotation != inspect.Parameter.empty]
    result += '", "'.join(members)
    result += '  "}"'
    return result
...
def generate_gbnf_rule_for_type(model_name, field_name, field_type, is_optional, processed_models, created_rules, field_info=None) -> tuple[str, list[str]]:
    rules = []
    ...
    if isclass(origin_type) and issubclass(origin_type, BaseModel):
        ...
        element_rule_name, additional_rules = generate_gbnf_rule_for_type(model_name, f'{field_name}-element', element_type, is_optional, processed_models, created_rules)
        ...
        additional_key_type, additional_key_rules = generate_gbnf_rule_for_type(model_name, f'{field_name}-key-type', key_type, is_optional, processed_models, created_rules)
        additional_value_type, additional_value_rules = generate_gbnf_rule_for_type(model_name, f'{field_name}-value-type', value_type, is_optional, processed_models, created_rules)
        gbnf_type = f'{gbnf_type} ::= "{{"  ( {additional_key_type} ": "  {additional_value_type} ("," "\\n" ws {additional_key_type} ":"  {additional_value_type})*  )? "}}" '
        ...
        for union_type in union_types:
            if isinstance(union_type, GenericAlias):
                union_gbnf_type, union_rules_list = generate_gbnf_rule_for_type(model_name, field_name, union_type, False, processed_models, created_rules)
    ...
    elif isclass(origin_type) and issubclass(origin_type, float) and field_info and hasattr(field_info, 'json_schema_extra') and (field_info.json_schema_extra is not None):
        max_precision = field_info.json_schema_extra.get('max_precision') if field_info and hasattr(field_info, 'json_schema_extra') else None
        min_precision = field_info.json_schema_extra.get('min_precision') if field_info and hasattr(field_info, 'json_schema_extra') else None
        ...
        gbnf_type, rules = generate_gbnf_float_rules(max_digit=max_digits, min_digit=min_digits, max_precision=max_precision, min_precision=min_precision)
    elif isclass(origin_type) and issubclass(origin_type, int) and field_info and hasattr(field_info, 'json_schema_extra') and (field_info.json_schema_extra is not None):
        max_digits = field_info.json_schema_extra.get('max_digit') if field_info and hasattr(field_info, 'json_schema_extra') else None
        min_digits = field_info.json_schema_extra.get('min_digit') if field_info and hasattr(field_info, 'json_schema_extra') else None
...
def generate_gbnf_grammar(model: type[BaseModel], processed_models: set[type[BaseModel]], created_rules: dict[str, list[str]]) -> tuple[list[str], bool]:
    ...
    for field_name, field_info in model_fields.items():
        ...
        rule_name, additional_rules = generate_gbnf_rule_for_type(model_name, format_model_and_field_name(field_name), field_type, is_optional, processed_models, created_rules, field_info)
...
def generate_gbnf_grammar_from_pydantic_models(models: list[type[BaseModel]], outer_object_name: str | None=None, outer_object_content: str | None=None, list_of_outputs: bool=False) -> str:
    ...
    if outer_object_name is None:
        ...
        model_rule = f'{format_model_and_field_name(outer_object_name)} ::= (" "| "\\n") "{{" ws "\\"{outer_object_name}\\""  ":" ws grammar-models'
        fields_joined = ' | '.join([f'{format_model_and_field_name(model.__name__)}-grammar-model' for model in models])
        grammar_model_rules = f'\ngrammar-models ::= {fields_joined}'
        mod_rules = []
        for model in models:
            mod_rule = f'{format_model_and_field_name(model.__name__)}-grammar-model ::= '
            mod_rule += f'"\\"{model.__name__}\\"" "," ws "\\"{outer_object_content}\\"" ":" ws {format_model_and_field_name(model.__name__)}' + '\n'
...
def generate_markdown_documentation(pydantic_models: list[type[BaseModel]], model_prefix='Model', fields_prefix='Fields', documentation_with_field_description=True) -> str:
    documentation = ''
            ...
            for name, field_type in get_type_hints(model).items():
                ...
                if get_origin(field_type) == Union:
                    ...
                    for element_type in element_types:
                        if isclass(element_type) and issubclass(element_type, BaseModel):
                            pyd_models.append((element_type, False))
                documentation += generate_field_markdown(name, field_type, model, documentation_with_field_description=documentation_with_field_description)
            documentation += '\n'
...
def generate_field_markdown(field_name: str, field_type: type[Any], model: type[BaseModel], depth=1, documentation_with_field_description=True) -> str:
    indent = '    ' * depth
...
def generate_text_documentation(pydantic_models: list[type[BaseModel]], model_prefix='Model', fields_prefix='Fields', documentation_with_field_description=True) -> str:
    documentation = ''
    pyd_models: list[tuple[type[BaseModel], bool]] = [(model, True) for model in pydantic_models]
    for model, add_prefix in pyd_models:
        if add_prefix:
            ...
            for name, field_type in get_type_hints(model).items():
                ...
                if get_origin(field_type) == Union:
                    ...
                    for element_type in element_types:
                        if isclass(element_type) and issubclass(element_type, BaseModel):
                            pyd_models.append((element_type, False))
                documentation_fields += generate_field_text(name, field_type, model, documentation_with_field_description=documentation_with_field_description)
        ...
        if hasattr(model, 'Config') and hasattr(model.Config, 'json_schema_extra') and ('example' in model.Config.json_schema_extra):
            ...
            documentation += format_multiline_description(json_example, 2) + '\n'
    return documentation
def generate_field_text(field_name: str, field_type: type[Any], model: type[BaseModel], depth=1, documentation_with_field_description=True) -> str:
    indent = '    ' * depth
    field_info = model.model_fields.get(field_name)
    field_description = field_info.description if field_info and field_info.description else ''
    if get_origin(field_type) == list:
...
def save_gbnf_grammar_and_documentation(grammar, documentation, grammar_file_path='./grammar.gbnf', documentation_file_path='./grammar_documentation.md'):
    try:
...
def generate_and_save_gbnf_grammar_and_documentation(pydantic_model_list, grammar_file_path='./generated_grammar.gbnf', documentation_file_path='./generated_grammar_documentation.md', outer_object_name: str | None=None, outer_object_content: str | None=None, model_prefix: str='Output Model', fields_prefix: str='Output Fields', list_of_outputs: bool=False, documentation_with_field_description=True):
    documentation = generate_markdown_documentation(pydantic_model_list, model_prefix, fields_prefix, documentation_with_field_description=documentation_with_field_description)
...
def generate_gbnf_grammar_and_documentation(pydantic_model_list, outer_object_name: str | None=None, outer_object_content: str | None=None, model_prefix: str='Output Model', fields_prefix: str='Output Fields', list_of_outputs: bool=False, documentation_with_field_description=True):
    documentation = generate_markdown_documentation(copy(pydantic_model_list), model_prefix, fields_prefix, documentation_with_field_description=documentation_with_field_description)
    grammar = generate_gbnf_grammar_from_pydantic_models(pydantic_model_list, outer_object_name, outer_object_content, list_of_outputs)
    grammar = remove_empty_lines(grammar + get_primitive_grammar(grammar))
    return (grammar, documentation)
def generate_gbnf_grammar_and_documentation_from_dictionaries(dictionaries: list[dict[str, Any]], outer_object_name: str | None=None, outer_object_content: str | None=None, model_prefix: str='Output Model', fields_prefix: str='Output Fields', list_of_outputs: bool=False, documentation_with_field_description=True):
...
def json_schema_to_python_types(schema):
    type_map = {'any': Any, 'string': str, 'number': float, 'integer': int, 'boolean': bool, 'array': list}
