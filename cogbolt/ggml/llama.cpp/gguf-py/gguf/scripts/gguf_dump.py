...
def dump_metadata_json(reader: GGUFReader, args: argparse.Namespace) -> None:
    ...
    for idx, field in enumerate(reader.fields.values()):
        curr: dict[str, Any] = {'index': idx, 'type': field.types[0].name if field.types else 'UNKNOWN', 'offset': field.offset}
    ...
    if not args.no_tensors:
        for idx, tensor in enumerate(reader.tensors):
            tensors[tensor.name] = {'index': idx, 'shape': tensor.shape.tolist(), 'type': tensor.tensor_type.name, 'offset': tensor.field.offset}
...
def dump_markdown_metadata(reader: GGUFReader, args: argparse.Namespace) -> None:
    ...
    for n, field in enumerate(reader.fields.values(), 1):
        ...
        def escape_markdown_inline_code(value_string):
            max_backticks = max((len(match.group(0)) for match in re.finditer('`+', value_string)), default=0)
    ...
    kv_dump_table_header_map = [{'key_name': 'n', 'header_name': 'POS', 'align': 'right'}, {'key_name': 'pretty_type', 'header_name': 'TYPE', 'align': 'left'}, {'key_name': 'total_elements', 'header_name': 'Count', 'align': 'right'}, {'key_name': 'field_name', 'header_name': 'Key', 'align': 'left'}, {'key_name': 'value', 'header_name': 'Value', 'align': 'left'}]
    ...
    if not args.no_tensors:
        ...
        tensors_mapping_table_header_map = [{'key_name': 't_id', 'header_name': 'T_ID', 'align': 'right'}, {'key_name': 'layer_name', 'header_name': 'Tensor Layer Name', 'align': 'left'}, {'key_name': 'data_offset', 'header_name': 'Data Offset (B)', 'align': 'right'}, {'key_name': 'data_size', 'header_name': 'Data Size (B)', 'align': 'right'}]
