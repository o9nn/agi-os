...
import numpy as np
from .constants import GGUF_DEFAULT_ALIGNMENT, GGUF_MAGIC, GGUF_VERSION, GGMLQuantizationType, GGUFEndian, GGUFValueType, Keys, RopeScalingType, PoolingType, TokenType, ExpertGatingFuncType
...
class GGUFWriter:
    ...
    state: WriterState
    _simple_value_packing = {GGUFValueType.UINT8: 'B', GGUFValueType.INT8: 'b', GGUFValueType.UINT16: 'H', GGUFValueType.INT16: 'h', GGUFValueType.UINT32: 'I', GGUFValueType.INT32: 'i', GGUFValueType.FLOAT32: 'f', GGUFValueType.UINT64: 'Q', GGUFValueType.INT64: 'q', GGUFValueType.FLOAT64: 'd', GGUFValueType.BOOL: '?'}
    def __init__(self, path: os.PathLike[str] | str | None, arch: str, use_temp_file: bool=False, endianess: GGUFEndian=GGUFEndian.LITTLE, split_max_tensors: int=0, split_max_size: int=0, dry_run: bool=False, small_first_shard: bool=False):
        self.fout = None
        ...
        logger.info('gguf: This GGUF file is for {0} Endian only'.format('Big' if self.endianess == GGUFEndian.BIG else 'Little'))
    ...
    def ggml_pad(x: int, n: int) -> int:
        return (x + n - 1) // n * n
    def add_tensor_info(self, name: str, tensor_shape: Sequence[int], tensor_dtype: np.dtype, tensor_nbytes: int, raw_dtype: GGMLQuantizationType | None=None) -> None:
        if self.state is not WriterState.NO_FILE:
            raise ValueError(f'Expected output file to be not yet opened, got {self.state}')
        ...
        if len(self.tensors[-1]) > 0:
            if self.split_max_tensors != 0 and len(self.tensors[-1]) >= self.split_max_tensors or (self.split_max_size != 0 and sum((ti.nbytes for ti in self.tensors[-1].values())) + tensor_nbytes > self.split_max_size):
                self.tensors.append({})
