...
>(clientCtx: EventContext<E, EO>, event: InvokeEventa<Res, Req, ResErr, ReqErr>) {
  return (req: Req) => {
    ...
    clientCtx.emit(event.sendEvent, { invokeId, content: req }) 
    return stream
  }
}
...
>(serverCtx: EventContext<E, EO>, event: InvokeEventa<Res, Req, ResErr, ReqErr>, fn: StreamHandler<Res, Req, EO>) {
  serverCtx.on(event.sendEvent, async (payload, options) => { 
    ...
    try {
      const generator = fn(payload.body.content as Req, options) 
      for await (const res of generator) {
        serverCtx.emit(invokeReceiveEvent, { ...payload.body, content: res }, options) 
      }
      serverCtx.emit(invokeReceiveEventStreamEnd, { ...payload.body, content: undefined }, options) 
    }
    catch (error) {
      serverCtx.emit(invokeReceiveEventError, { ...payload.body, content: error as any }, options) 
    }
  })
}
