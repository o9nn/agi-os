...
describe('event target', async () => {
  it('context should be able to on and emit events', async () => {
    ...
    ctx.emit(defineInboundEventa(eventa.id), { message: 'Hello, Event Target!' }, { raw: { ipcRendererEvent: {} as IpcRendererEvent, event: { message: 'Hello, Event Target!' } } }) 
    ...
    ctx.emit(eventa, { message: 'Hello, Eventa!' }) 
