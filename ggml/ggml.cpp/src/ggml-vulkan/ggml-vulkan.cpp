...
private:
std::map<vk::Buffer, size_t> allocations;
size_t total_device;
size_t total_host;
};
#else
...
duoni.objectHandle = (uint64_t)(static_cast<VkPipeline>(pipeline->pipeline));
...
const uint32_t mmid_row_ids = mul_mat_id ? (4096 * sizeof(uint32_t) + 4) : 0;
...
device->buffer_type = {
ggml_backend_vk_buffer_type_interface,
ggml_backend_reg_dev_get(ggml_backend_vk_reg(), idx),
...
GGML_ASSERT(!qx_needs_dequant || to_fp16_vk_0 != nullptr);
...
GGML_ASSERT(op == GGML_OP_GET_ROWS || op == GGML_OP_CPY || (!ggml_is_quantized(src0->type) && (src1 == nullptr || !ggml_is_quantized(src1->type))));
GGML_ASSERT(ggml_vk_op_supports_incontiguous(op) || ggml_vk_dim01_contiguous(src0));
...
case GGML_OP_CONV_TRANSPOSE_1D:
{
elements = {uint32_t(src0->ne[1]), 1, 1};
} break;
case GGML_OP_POOL_2D:
{
...
const uint32_t OH = is_2D ? dst->ne[2] : 1;
const uint32_t OW =         dst->ne[1];
const uint32_t offset_delta = src1->nb[is_2D ? 2 : 1] / 4;
const uint32_t batch_offset = src1->nb[is_2D ? 3 : 2] / 4;
...
ggml_backend_vk_buffer_free_buffer,
ggml_backend_vk_buffer_get_base,
ggml_backend_vk_buffer_init_tensor,
ggml_backend_vk_buffer_memset_tensor,
ggml_backend_vk_buffer_set_tensor,
ggml_backend_vk_buffer_get_tensor,
ggml_backend_vk_buffer_cpy_tensor,
...
bool first_node_in_batch = true;
int submit_node_idx = 0;
vk_context compute_ctx;
if (vk_perf_logger_enabled) {
...
ggml_backend_vk_name,
ggml_backend_vk_free,
NULL,
NULL,
NULL,
NULL,
...
ggml_backend_vk_interface,
ggml_backend_reg_dev_get(ggml_backend_vk_reg(), dev_num),
ctx,
};
return vk_backend;
}
...
ggml_backend_vk_device_get_description,
...
ggml_backend_vk_device_offload_op,
NULL,
NULL,
NULL,
};
