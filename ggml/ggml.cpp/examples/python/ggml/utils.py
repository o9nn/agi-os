...
def copy(from_tensor: TensorLike, to_tensor: TensorLike, allow_requantize: bool=True):
    ...
    if from_type == to_type:
        ...
        assert allow_requantize or not lib.ggml_is_quantized(from_type) or (not lib.ggml_is_quantized(to_type)), f'Requantizing from {__type_name(from_type)} to {__type_name(to_type)} is disabled. Force with allow_requantize=True'
...
__k_quant_types = set([lib.GGML_TYPE_Q2_K, lib.GGML_TYPE_Q3_K, lib.GGML_TYPE_Q4_K, lib.GGML_TYPE_Q5_K, lib.GGML_TYPE_Q6_K, lib.GGML_TYPE_Q8_K])
__type_to_dtype_dict = {lib.GGML_TYPE_I8: np.int8, lib.GGML_TYPE_I16: np.int16, lib.GGML_TYPE_I32: np.int32, lib.GGML_TYPE_F16: np.float16, lib.GGML_TYPE_F32: np.float32}
...
def __is_contiguous(tensor: TensorLike):
    return tensor.flags['C_CONTIGUOUS'] if isinstance(tensor, np.ndarray) else lib.ggml_is_contiguous(tensor)
