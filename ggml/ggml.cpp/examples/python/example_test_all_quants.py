...
n = 256
orig = np.array([[cos(j * 2 * pi / n) * sin(i * 2 * pi / n) for j in range(n)] for i in range(n)], np.float32)
orig_tensor = lib.ggml_new_tensor_2d(ctx, lib.GGML_TYPE_F32, n, n)
copy(orig, orig_tensor)
quants = [type for type in range(lib.GGML_TYPE_COUNT) if lib.ggml_is_quantized(type) and type not in [lib.GGML_TYPE_Q8_1, lib.GGML_TYPE_Q8_K]]
def get_name(type):
...
for i, type in enumerate(quants):
    ...
    try:
        if type == None:
            ...
            d = quantized - orig
            results = {'l2': np.linalg.norm(d, 2), 'linf': np.linalg.norm(d, np.inf), 'compression': round(lib.ggml_nbytes(orig_tensor) / lib.ggml_nbytes(quantized_tensor), 1)}
            name = get_name(type)
