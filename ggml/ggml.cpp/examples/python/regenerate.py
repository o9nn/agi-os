...
CPPFLAGS = ['-I', C_INCLUDE_DIR, '-D__fp16=uint16_t', '-D__attribute__(x)=', '-D_Static_assert(x, m)='] + [x for x in os.environ.get('CPPFLAGS', '').split(' ') if x != '']
try:
...
for expr in set(re.findall(f'(?<=\\[)[^\\]]+(?=])|sizeof\\s*\\([^()]+\\)', header)):
    if re.match('^(\\d+|\\s*)$', expr):
        continue
    subprocess.run([CC, '-o', 'eval_size_expr', *CPPFLAGS, '-x', 'c', '-'], text=True, check=True, input=f'#include <stdio.h>\n                             #include "{API}"\n                             int main() {{ printf("%lu", (size_t)({expr})); }}')
