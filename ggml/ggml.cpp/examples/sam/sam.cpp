...
std::string model     = "models/sam-vit-b/ggml-model-f16.bin";
...
{
ctx_size += n_enc_out_chans*ggml_type_size(GGML_TYPE_F16);
ctx_size += n_enc_out_chans*ggml_type_size(GGML_TYPE_F32);
...
upscaled_embedding = ggml_cont(ctx0, ggml_transpose(ctx0, upscaled_embedding));
}
...
masks = ggml_cont(ctx0, ggml_transpose(ctx0, masks));
...
masks->nb[1], masks->nb[2], masks->nb[3], masks->nb[2] );
...
const bool   multimask_output) {
struct ggml_init_params ggml_params = {
state.buf_compute_fast.size(),
state.buf_compute_fast.data(),
true,
};
struct ggml_context * ctx0   = ggml_init(ggml_params);
struct ggml_cgraph  * gf     = ggml_new_graph(ctx0);
