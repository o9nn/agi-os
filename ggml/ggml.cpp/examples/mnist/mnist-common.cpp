...
buf[iex*MNIST_NINPUT + i] = image[i] / 255.0f;
}
}
return true;
}
...
ggml_set_zero(ggml_opt_dataset_labels(dataset));
mnist_model model = mnist_model_init_from_file("mnist-f32.gguf", "CPU", 1, 1);
