...
struct ggml_tensor * input = ggml_new_tensor_3d(ctx, GGML_TYPE_F32, 257, 1536, n_files);
ggml_set_name(input, "input");
ggml_set_input(input);
struct ggml_tensor * cur;
cur = ggml_mul_mat(ctx, model.dense_w, input);
cur = ggml_add(ctx, cur, model.dense_b);
cur = ggml_gelu(ctx, cur);
cur = ggml_reshape_3d(ctx, cur, 512, 384, n_files);
cur = ggml_cont(ctx, ggml_transpose(ctx, cur));
cur = ggml_norm(ctx, cur, hparams.f_norm_eps);
cur = ggml_mul(ctx, cur, model.layer_norm_gamma);
cur = ggml_add(ctx, cur, model.layer_norm_beta);
cur = ggml_cont(ctx, ggml_transpose(ctx, cur));
cur = ggml_mul_mat(ctx, model.dense_1_w, cur);
cur = ggml_add(ctx, cur, model.dense_1_b);
cur = ggml_gelu(ctx, cur);
cur = ggml_mul_mat(ctx, model.dense_2_w, cur);
cur = ggml_add(ctx, cur, model.dense_2_b);
cur = ggml_gelu(ctx, cur);
cur = ggml_cont(ctx, ggml_transpose(ctx, cur));
cur = ggml_pool_1d(ctx, cur, GGML_OP_POOL_MAX, 384, 384, 0);
cur = ggml_reshape_2d(ctx, cur, 256, n_files);
cur = ggml_norm(ctx, cur, hparams.f_norm_eps);
cur = ggml_mul(ctx, cur, model.layer_norm_1_gamma);
cur = ggml_add(ctx, cur, model.layer_norm_1_beta);
cur = ggml_mul_mat(ctx, model.target_label_w, cur);
cur = ggml_add(ctx, cur, model.target_label_b);
