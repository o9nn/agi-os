...
def convert_byteorder(reader: gguf.GGUFReader, args: argparse.Namespace) -> None:
    ...
    for tensor in reader.tensors:
        if tensor.tensor_type not in (gguf.GGMLQuantizationType.F32, gguf.GGMLQuantizationType.F16, gguf.GGMLQuantizationType.Q8_0, gguf.GGMLQuantizationType.Q4_K, gguf.GGMLQuantizationType.Q6_K):
    ...
    for idx, tensor in enumerate((pbar := tqdm(reader.tensors, desc='Converting tensor'))):
        log_message = f'Converting tensor {repr(tensor.name)}, type={tensor.tensor_type.name}, elements={tensor.n_elements} '
