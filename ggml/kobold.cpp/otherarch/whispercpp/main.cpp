...
void whisper_print_usage(int , char ** argv, const whisper_params & params) {
fprintf(stderr, "\n");
...
char *escaped = (char *)calloc(escaped_length, 1);
if (escaped == NULL) {
return NULL;
}
size_t pos = 0;
...
bool output_score(struct whisper_context * ctx, const char * fname, const whisper_params & , std::vector<std::vector<float>> ) {
std::ofstream fout(fname);
fprintf(stderr, "%s: saving output to '%s'\n", __func__, fname);
const int n_segments = whisper_full_n_segments(ctx);
for (int i = 0; i < n_segments; ++i) {
