...
SIGLIP_PRETRAINED_MODEL_ARCHIVE_LIST = ['google/siglip-base-patch16-224']
...
def _trunc_normal_(tensor, mean, std, a, b):
    ...
    if tensor.dtype == torch.float16:
        ...
        tensor.clamp_(min=a, max=b)
def trunc_normal_tf_(tensor: torch.Tensor, mean: float=0.0, std: float=1.0, a: float=-2.0, b: float=2.0):
    with torch.no_grad():
...
def should_skip_tensor(name: str, has_text: bool, has_vision: bool, has_minicpmv: bool) -> bool:
    if name in ('logit_scale', 'text_model.embeddings.position_ids', 'vision_model.embeddings.position_ids'):
        return True
...
def get_tensor_name(name: str) -> str:
    ...
    if 'mm_projector' in name:
        ...
        name = re.sub('mm\\.mlp\\.mlp', 'mm.model.mlp', name, count=1)
        name = re.sub('mm\\.peg\\.peg', 'mm.model.peg', name, count=1)
        return name
...
ap.add_argument('--clip-model-is-vision', action='store_true', required=False, help='The clip model is a pure vision model (ShareGPT4V vision extract for example)')
ap.add_argument('--clip-model-is-openclip', action='store_true', required=False, help='The clip model is from openclip (for ViT-SO400M type))')
...
if minicpmv_version == 1:
    ...
    emb_dim = 2560
    block_count = 27
default_vision_config = {'hidden_size': 1152, 'image_size': 980, 'intermediate_size': 4304, 'model_type': 'idefics2', 'num_attention_heads': 16, 'num_hidden_layers': 27, 'patch_size': 14}
...
if has_vision_encoder:
    ...
    fout.add_float32(add_key_str(KEY_ATTENTION_LAYERNORM_EPS, VISION), 1e-06)
...
def _replace_name_resampler(s, v):
    if re.match('resampler.pos_embed', s):
        return {s: v, re.sub('pos_embed', 'pos_embed_k', s): torch.from_numpy(get_2d_sincos_pos_embed(emb_dim, (70, 70)))}
    if re.match('resampler.proj', s):
        return {re.sub('proj', 'pos_embed_k', s): torch.from_numpy(get_2d_sincos_pos_embed(emb_dim, (70, 70))), re.sub('proj', 'proj.weight', s): v.transpose(-1, -2).contiguous()}
    if re.match('resampler.attn.in_proj_.*', s):
        return {re.sub('attn.in_proj_', 'attn.q.', s): v.chunk(3, dim=0)[0], re.sub('attn.in_proj_', 'attn.k.', s): v.chunk(3, dim=0)[1], re.sub('attn.in_proj_', 'attn.v.', s): v.chunk(3, dim=0)[2]}
    return {s: v}
