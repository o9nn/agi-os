...
def test_infill_without_input_extra():
    global server
    server.start()
    res = server.make_request('POST', '/infill', data={'input_prefix': '#include <cstdio>\n#include "llama.h"\n\nint main() {\n', 'prompt': '    int n_threads = llama_', 'input_suffix': '}\n'})
...
@pytest.mark.parametrize('input_extra', [{}, {'filename': 'ok'}, {'filename': 123}, {'filename': 123, 'text': 'abc'}, {'filename': 123, 'text': 456}])
def test_invalid_input_extra_req(input_extra):
    global server
    server.start()
    res = server.make_request('POST', '/infill', data={'input_extra': [input_extra], 'input_prefix': '#include <cstdio>\n#include "llama.h"\n\nint main() {\n', 'prompt': '    int n_threads = llama_', 'input_suffix': '}\n'})
