...
def write_state_dict(state_dict: Dict[str, torch.Tensor], dest_path: str, data_type: str) -> None:
    ...
    if is_v7_0:
        ...
        for k in state_dict.keys():
            if 'att.x_' in k:
                ...
                try:
                    state_dict_new[f'blocks.{l}.att.x_rwkvag'] = torch.cat([state_dict_new[f'blocks.{l}.att.x_rwkvag'], state_dict[k]], dim=0)
                except KeyError:
    ...
    with open(dest_path, 'wb') as out_file:
        ...
        for k in state_dict.keys():
            ...
            if is_v7_0:
                if any((s in k for s in ['.w1', '.w2', '.a1', '.a2', '.v1', '.v2', '.g1', '.g2'])):
            ...
            if is_FP16 and len(tensor.shape) > 1 and all((s not in k for s in ['.time_', '.k_k', '.k_a', '.r_k', '.x_rwkvag', '.x_k', '.w0', '.a0', '.v0'])):
                tensor = tensor.half()
            shape = tensor.shape
