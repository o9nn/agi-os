...
args = parse_args()
print('Loading model')
model: rwkv_cpp_model.RWKVModel = rwkv_cpp_model.RWKVModel(rwkv_cpp_shared_library.load_rwkv_shared_library(), args.model_path)
print('Loading text')
...
for i in range(run_count):
    ...
    if args.ignore_first_n_tokens == 0 or i + 1 >= args.ignore_first_n_tokens:
        losses = torch.tensor([torch.nn.functional.cross_entropy(logits, torch.tensor(target, dtype=torch.long), reduction='none').item()])
        loss_sum += losses
        loss_count += 1
    if run_count <= 5 or i % (run_count // 10) == 0:
        ...
        print(f'Token #{i}/{token_count}, {int(100.0 * i / token_count)}%, ETA {duration_remaining // 60} m {duration_remaining % 60} s', end='')
        if loss_count > 0:
            print(f', averages so far: {format_loss_with_perplexity(avg_loss_so_far)}')
        else:
            print()
print()
print(f'Model: {os.path.basename(args.model_path)}, data: {os.path.basename(args.text_path)} with {token_count} tokens, skipped {args.ignore_first_n_tokens} tokens, averages: {format_loss_with_perplexity(loss_sum / loss_count)}, latency {int((time.time() - start) * 1000 / run_count)} ms per token')