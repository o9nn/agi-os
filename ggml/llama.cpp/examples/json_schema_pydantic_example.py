...
if True:
    def create_completion(*, response_model=None, endpoint='http://localhost:8080/v1/chat/completions', messages, **kwargs):
        ...
        if response_model:
            ...
            messages = [{'role': 'system', 'content': f'You respond in JSON format with the following schema: {json.dumps(schema, indent=2)}'}] + messages
            response_format = {'type': 'json_object', 'schema': schema}
        data = requests.post(endpoint, headers={'Content-Type': 'application/json'}, json=dict(messages=messages, response_format=response_format, **kwargs)).json()
        if 'error' in data:
    ...
    client = instructor.patch(openai.OpenAI(api_key='123', base_url='http://localhost:8080'), mode=instructor.Mode.JSON_SCHEMA)
