...
def example_rce(host):
    ...
    gbnf_grammar, documentation = generate_gbnf_grammar_and_documentation(pydantic_model_list=tools, outer_object_name='function', outer_object_content='function_parameters', model_prefix='Function', fields_prefix='Parameters')
...
def example_calculator(host):
    ...
    text = create_completion(host, prompt, gbnf_grammar)
    json_data = json.loads(text)
    expected = {'function': 'Calculator', 'function_parameters': {'number_one': 42, 'operation': 'multiply', 'number_two': 42}}
...
def example_concurrent(host):
    ...
    gbnf_grammar, documentation = generate_gbnf_grammar_and_documentation(pydantic_model_list=tools, outer_object_name='function', outer_object_content='params', model_prefix='Function', fields_prefix='Parameters', list_of_outputs=True)
    ...
    text = create_completion(host, prompt, gbnf_grammar)
    json_data = json.loads(text)
    expected = [{'function': 'get_current_datetime', 'params': {'output_format': '%Y-%m-%d %H:%M:%S'}}, {'function': 'get_current_weather', 'params': {'location': 'London', 'unit': 'celsius'}}, {'function': 'Calculator', 'params': {'number_one': 42, 'operation': 'multiply', 'number_two': 42}}]
    res = 0
    if json_data != expected:
        print('  Result is not as expected!')
