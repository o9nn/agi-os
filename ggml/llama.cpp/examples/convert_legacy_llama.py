...
DT_Q8_0 = Q8_0QuantizedDataType('Q8_0', dtype=np.dtype(np.float32), valid_conversions=[], ggml_type=gguf.GGMLQuantizationType.Q8_0, block_size=32, quantized_dtype=np.dtype([('d', '<f2'), ('qs', 'i1', (32,))]))
NUMPY_TYPE_TO_DATA_TYPE: dict[np.dtype[Any], DataType] = {}
for dt in (DT_BF16, DT_F16, DT_F32, DT_I32):
    if dt.dtype in NUMPY_TYPE_TO_DATA_TYPE:
        raise ValueError(f'Invalid duplicate data type {dt}')
    NUMPY_TYPE_TO_DATA_TYPE[dt.dtype] = dt
SAFETENSORS_DATA_TYPES: dict[str, DataType] = {'BF16': DT_BF16, 'F16': DT_F16, 'F32': DT_F32, 'I32': DT_I32}
...
GGML_FILE_TYPE_TO_DATA_TYPE: dict[GGMLFileType, DataType] = {GGMLFileType.AllF32: DT_F32, GGMLFileType.MostlyF16: DT_F16, GGMLFileType.MostlyQ8_0: DT_Q8_0}
@dataclass
class Params:
    n_vocab: int
    n_embd: int
    ...
    def guessed(model: LazyModel) -> Params:
        ...
        if n_layer < 1:
            msg = "                failed to guess 'n_layer'. This model is unknown or unsupported.\n                Suggestion: provide 'config.json' of the model in the same directory containing model files."
            raise KeyError(textwrap.dedent(msg))
        n_head = n_embd // 128
        n_mult = 256
        n_ff = int(2 * (4 * n_embd) / 3)
        n_ff = n_mult * ((n_ff + n_mult - 1) // n_mult)
        return Params(n_vocab=n_vocab, n_embd=n_embd, n_layer=n_layer, n_ctx=-1, n_ff=n_ff, n_head=n_head, n_head_kv=n_head, f_norm_eps=1e-05)
    @staticmethod
    def loadHFTransformerJson(model: LazyModel, config_path: Path) -> Params:
        ...
        if 'max_sequence_length' in config:
            ...
            msg = "                failed to guess 'n_ctx'. This model is unknown or unsupported.\n                Suggestion: provide 'config.json' of the model in the same directory containing model files."
            raise KeyError(textwrap.dedent(msg))
        n_experts = None
        n_experts_used = None
        ...
        return Params(n_vocab=config['vocab_size'], n_embd=config['hidden_size'], n_layer=config['num_hidden_layers'], n_ctx=n_ctx, n_ff=config['intermediate_size'], n_head=(n_head := config['num_attention_heads']), n_head_kv=config.get('num_key_value_heads', n_head), n_experts=n_experts, n_experts_used=n_experts_used, f_norm_eps=config['rms_norm_eps'], f_rope_freq_base=config.get('rope_theta'), rope_scaling_type=rope_scaling_type, f_rope_scale=f_rope_scale, n_ctx_orig=n_ctx_orig, rope_finetuned=rope_finetuned)
    @staticmethod
    def loadOriginalParamsJson(model: LazyModel, config_path: Path) -> Params:
        ...
        if config.get('moe'):
            ...
            f_rope_freq_base = 1000000.0
        assert n_ff is not None
        return Params(n_vocab=model['tok_embeddings.weight'].shape[0], n_embd=config['dim'], n_layer=config['n_layers'], n_ctx=n_ctx, n_ff=n_ff, n_head=(n_head := config['n_heads']), n_head_kv=config.get('n_kv_heads', n_head), n_experts=n_experts, n_experts_used=n_experts_used, f_norm_eps=config['norm_eps'], f_rope_freq_base=config.get('rope_theta', f_rope_freq_base))
    @staticmethod
...
class LazyTensor:
    ...
    def load(self) -> Tensor:
        ret = self._load()
        assert ret.data_type == self.data_type or self.data_type.dtype == ret.data_type.dtype, (self.data_type, ret.data_type, self.description)
        return ret
...
def check_vocab_size(params: Params, vocab: BaseVocab, pad_vocab: bool=False) -> None:
    if params.n_vocab == -1:
        raise ValueError("The model's vocab size is set to -1 in params.json. Please update it manually." + (f' Maybe {vocab.vocab_size}?' if isinstance(vocab, Vocab) else ''))
    ...
    if pad_vocab and params.n_vocab > vocab.vocab_size:
        ...
        logger.debug(f'Padding vocab with {pad_count} token(s) - <dummy00001> through <dummy{pad_count:05}>')
...
class OutputFile:
    ...
    def write_tensor_data(self, ftype: GGMLFileType, model: LazyModel, concurrency: int) -> None:
        ...
        for i, ((name, lazy_tensor), ndarray) in enumerate(zip(model.items(), ndarrays)):
            ...
            logger.info(f'[{i + 1:{padi}d}/{len(model)}] Writing tensor {name:38s} | size {size:16} | type {lazy_tensor.data_type.name:4} | T+{int(elapsed):4}')
            self.gguf.write_tensor_data(ndarray)
    def close(self) -> None:
        self.gguf.close()
    @staticmethod
    def write_vocab_only(fname_out: Path, params: Params, vocab: Vocab, svocab: gguf.SpecialVocab, endianess: gguf.GGUFEndian=gguf.GGUFEndian.LITTLE, pad_vocab: bool=False, metadata: gguf.Metadata | None=None) -> None:
    ...
    @staticmethod
    def write_all(fname_out: Path, ftype: GGMLFileType, params: Params, model: LazyModel, vocab: BaseVocab, svocab: gguf.SpecialVocab, concurrency: int=DEFAULT_CONCURRENCY, endianess: gguf.GGUFEndian=gguf.GGUFEndian.LITTLE, pad_vocab: bool=False, metadata: gguf.Metadata | None=None) -> None:
        check_vocab_size(params, vocab, pad_vocab=pad_vocab)
        of = OutputFile(fname_out, endianess=endianess)
        of.add_meta_model(params, metadata)
        of.add_meta_arch(params)
...
class VocabFactory:
    ...
    def _create_special_vocab(self, vocab: BaseVocab, model_parent_path: Path) -> gguf.SpecialVocab:
        ...
        return gguf.SpecialVocab(model_parent_path, load_merges=load_merges, special_token_types=None, n_vocab=n_vocab)
    ...
    def load_vocab(self, vocab_types: list[str] | None, model_parent_path: Path) -> tuple[BaseVocab, gguf.SpecialVocab]:
        ...
        special_vocab = self._create_special_vocab(vocab, model_parent_path)
...
def default_convention_outfile(file_type: GGMLFileType, expert_count: int | None, model_params_count: tuple[int, int, int], metadata: gguf.Metadata) -> str:
    ...
    output_type = {GGMLFileType.AllF32: 'F32', GGMLFileType.MostlyF16: 'F16', GGMLFileType.MostlyQ8_0: 'Q8_0'}[file_type]
...
def default_outfile(model_paths: list[Path], file_type: GGMLFileType, expert_count: int | None, model_params_count: tuple[int, int, int], metadata: gguf.Metadata) -> Path:
    ...
    if ret in model_paths:
        logger.error(f'Error: Default output path ({ret}) would overwrite the input. Please explicitly specify a path using --outfile.')
        sys.exit(1)
    return ret
...
def main(args_in: list[str] | None=None) -> None:
    ...
    if args.pad_vocab or not args.vocab_only:
        ...
        if params.n_ctx == -1:
            if args.ctx is None:
                msg = "                    The model doesn't have a context size, and you didn't specify one with --ctx\n                    Please specify one with --ctx:\n                     - LLaMA v1: --ctx 2048\n                     - LLaMA v2: --ctx 4096"
                parser.error(textwrap.dedent(msg))
            params.n_ctx = args.ctx
        if args.outtype:
            params.ftype = {'f32': GGMLFileType.AllF32, 'f16': GGMLFileType.MostlyF16, 'q8_0': GGMLFileType.MostlyQ8_0}[args.outtype]
    ...
    if args.vocab_only:
        ...
        if params is None:
            params = Params(n_vocab=vocab.vocab_size, n_embd=1, n_layer=1, n_ctx=1, n_ff=1, n_head=1, n_head_kv=1, f_norm_eps=1e-05)
        OutputFile.write_vocab_only(outfile, params, vocab, special_vocab, endianess=endianess, pad_vocab=args.pad_vocab, metadata=metadata)
    ...
    OutputFile.write_all(outfile, ftype, params, model, vocab, special_vocab, concurrency=args.concurrency, endianess=endianess, pad_vocab=args.pad_vocab, metadata=metadata)
    logger.info(f'Wrote {outfile}')
