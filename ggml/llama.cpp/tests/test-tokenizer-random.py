...
def generator_custom_text_edge_cases() -> Iterator[str]:
    yield from ['\x1f-a', '¼-a', '½-a', '¾-a', 'a 〇b', 'Ⅵ-a', '\ufeff//', 'Cửa Việt', '<s>a', '<unk><|endoftext|><s>', 'a\na', '"`', ' ⹎', '\n\x0b  ', 'a\xa0\xa0\x00b', 'one <mask>', 'a </s> b', 'a <mask> b', '\xa0aC', '\u2029 ꏤ', 'a ?', 'å', '\U000ac517', '\U000522f4', '<s><s><unk><s>a<s>b<s>c<unk>d<unk></s>', '<s> <s> <unk><s>a<s>b<s>c<unk>d<unk></s>']
...
def generator_ascii_lr_strip() -> Iterator[str]:
    WHITESPACES = ['', ' ', '  ']
    CHARACTERS = list((chr(i) for i in range(1, 128))) + ['']
...
def compare_tokenizers(tokenizer1: TokenizerGroundtruth, tokenizer2: TokenizerLlamaCpp, generator: Iterator[str]):
    ...
    for text in generator:
        ...
        if encode_errors < MAX_ERRORS and ids1 != ids2:
            ...
            encode_errors += 1
            logger.error(f' encode_errors={encode_errors!r}')
        if decode_errors < MAX_ERRORS and (not check_detokenizer(text, text1, text2)):
            ...
            decode_errors += 1
            logger.error(f' decode_errors={decode_errors!r}')
        if encode_errors >= MAX_ERRORS and decode_errors >= MAX_ERRORS:
            logger.error(f' EXIT: encode_errors={encode_errors!r} decode_errors={decode_errors!r}')
            break
    t_total = time.perf_counter() - t_start
    logger.info(f'{generator.__qualname__}: end,  t_encode1={t_encode1:.3f} t_encode2={t_encode2:.3f}  t_decode1={t_decode1:.3f} t_decode2={t_decode2:.3f}  t_total={t_total:.3f}')
...
if __name__ == '__main__':
    if True:
        logging.basicConfig(level=logging.DEBUG, format='%(asctime)s.%(msecs)03d %(name)s %(levelname)s %(message)s', datefmt='%Y-%m-%d %H:%M:%S', filename=logger.name + '.log', filemode='a')
    logging.basicConfig(level=logging.DEBUG, format='%(levelname)s %(message)s')
    path_tokenizers = Path('./models/tokenizers/')
    path_vocab_format = './models/ggml-vocab-%s.gguf'
    tokenizers = ['llama-spm', 'phi-3', 'gemma', 'gemma-2', 'baichuan', 'bert-bge', 'jina-v2-en', 'llama-bpe', 'phi-2', 'deepseek-llm', 'deepseek-coder', 'falcon', 'mpt', 'starcoder', 'gpt-2', 'stablelm2', 'refact', 'qwen2', 'olmo', 'jina-v2-es', 'jina-v2-de', 'smaug-bpe', 'poro-chat', 'jina-v2-code', 'viking', 'jais']
    logger.info('=' * 50)
