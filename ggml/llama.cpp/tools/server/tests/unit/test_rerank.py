...
def test_rerank():
    global server
    server.start()
    res = server.make_request('POST', '/rerank', data={'query': 'Machine learning is', 'documents': TEST_DOCUMENTS})
...
def test_rerank_tei_format():
    global server
    server.start()
    res = server.make_request('POST', '/rerank', data={'query': 'Machine learning is', 'texts': TEST_DOCUMENTS})
    assert res.status_code == 200
    ...
    assert most_relevant['index'] == 2
    assert least_relevant['index'] == 3
@pytest.mark.parametrize('documents', [[], None, 123, [1, 2, 3]])
def test_invalid_rerank_req(documents):
    global server
    server.start()
    res = server.make_request('POST', '/rerank', data={'query': 'Machine learning is', 'documents': documents})
    assert res.status_code == 400
    assert 'error' in res.body
@pytest.mark.parametrize('query,doc1,doc2,n_tokens', [('Machine learning is', 'A machine', 'Learning is', 19), ('Which city?', 'Machine learning is ', 'Paris, capitale de la', 26)])
def test_rerank_usage(query, doc1, doc2, n_tokens):
    global server
    server.start()
    res = server.make_request('POST', '/rerank', data={'query': query, 'documents': [doc1, doc2]})
    assert res.status_code == 200
