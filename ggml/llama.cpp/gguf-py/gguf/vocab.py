...
try:
    ...
    from mistral_common.tokens.tokenizers.utils import _filter_valid_tokenizer_files
...
class SpecialVocab:
    ...
    def __init__(self, path: str | os.PathLike[str], load_merges: bool=False, special_token_types: Iterable[str] | None=None, n_vocab: int | None=None):
    ...
    def __repr__(self) -> str:
        return '<SpecialVocab with {} merges, special tokens {}, add special tokens {}>'.format(len(self.merges), self.special_token_ids or 'unset', self.add_special_token or 'unset')
            ...
            if self.load_merges:
                ...
                if isinstance(merges, list) and merges:
                    if isinstance(merges[0], str):
                        ...
                        self.merges = [' '.join([''.join((chr(ord(c) + 256) if c == ' ' else c for c in part)) for part in pair]) for pair in merges]
                    else:
...
class BpeVocab(Vocab):
    ...
    def __init__(self, base_path: Path):
        ...
        if (fname_tokenizer := (base_path / 'vocab.json')).exists():
            ...
            if tokenizer_model['type'] != 'BPE' or tokenizer_model.get('byte_fallback', False) or tokenizer_json['decoder']['type'] != 'ByteLevel':
        ...
        if expected_ids != actual_ids:
            ...
            raise ValueError(f'Expected the {len(actual_ids)} added token ID(s) to be sequential in the range {vocab_size} - {expected_end_id}; got {actual_ids}')
...
class LlamaHfVocab(Vocab):
    ...
    def __init__(self, base_path: Path):
        ...
        with open(fname_tokenizer, encoding='utf-8') as f:
            tokenizer_json = json.load(f)
        tokenizer_model: dict[str, Any] = tokenizer_json['model']
        is_llama3 = tokenizer_model['type'] == 'BPE' and tokenizer_model.get('ignore_merges', False) and (not tokenizer_model.get('byte_fallback', True))
        if is_llama3:
            raise TypeError('Llama 3 must be converted with BpeVocab')
        if not is_llama3 and (tokenizer_model['type'] != 'BPE' or not tokenizer_model.get('byte_fallback', False) or tokenizer_json['decoder']['type'] != 'Sequence'):
            raise FileNotFoundError('Cannot find Llama BPE tokenizer')
        try:
            from transformers import AutoTokenizer
        except ImportError as e:
            raise ImportError('To use LlamaHfVocab, please install the `transformers` package. You can install it with `pip install transformers`.') from e
    ...
    def hf_tokens(self) -> Iterable[tuple[bytes, float, gguf.TokenType]]:
        ...
        for token_id in range(self.vocab_size_base):
            ...
            yield (token_text, self.get_token_score(token_id), self.get_token_type(token_id, token_text, self.special_ids))
...
class MistralVocab(Vocab):
    ...
    def __init__(self, base_path: Path):
        if not _mistral_common_installed:
            raise ImportError('To use MistralVocab, please install the `mistral-common` package. You can install it with `pip install mistral-common`.')
        ...
        if len(valid_tokenizer_files) > 1:
            ...
            logger.warning(f'Multiple tokenizer files found in {base_path}. Using {tokenizer_file}')
        else:
            tokenizer_file = valid_tokenizer_files[0]
        self.tokenizer = MistralTokenizer.from_file(base_path / tokenizer_file).instruct_tokenizer.tokenizer
        self.tokenizer_type = MistralTokenizerType.tekken if isinstance(self.tokenizer, Tekkenizer) else MistralTokenizerType.spm
        self.vocab_size = self.tokenizer.n_words
        self.fname_tokenizer = base_path / tokenizer_file
        self._name = 'mistral-' + self.tokenizer_type.value + '-' + self.tokenizer.version
    @property
    ...
    def _sentencepiece_tokens(self) -> Iterable[tuple[bytes, float, gguf.TokenType]]:
        ...
        assert isinstance(self.tokenizer, SentencePieceTokenizer), f'Expected SentencePieceTokenizer, got {type(self.tokenizer)}'
    ...
    def _tekken_tokens(self) -> Iterable[tuple[bytes, float, gguf.TokenType]]:
        ...
        for token_id in range(self.tokenizer.num_special_tokens):
            yield (self.tokenizer.id_to_piece(token_id).encode('utf-8'), 0, gguf.TokenType.CONTROL)
        for token in self.tokenizer._tekken_token2id_nospecial:
            yield (self.token_bytes_to_string(token, byte_encoder).encode('utf-8'), 0, gguf.TokenType.NORMAL)
    def get_token_id(self, token: str) -> int:
        ...
        if self.tokenizer_type == MistralTokenizerType.spm:
            ...
            return self.tokenizer._vocab.index(token) + self.tokenizer.num_special_tokens
        else:
    ...
    def extract_vocab_merges_from_model(self):
        assert Tekkenizer is not None and isinstance(self.tokenizer, Tekkenizer), f'Expected Tekkenizer, got {type(self.tokenizer)}'
        mergeable_ranks = self.tokenizer._model._mergeable_ranks
        token_bytes_map = {rank: token_bytes for token_bytes, rank in mergeable_ranks.items()}
        merge_pairs = []
        for i in range(256, self.vocab_size - self.tokenizer.num_special_tokens):
            ...
            local = sorted(local, key=lambda x: (mergeable_ranks[x[0]], mergeable_ranks[x[1]]), reverse=False)
            merge_pairs.extend(local)
        merge_pairs = sorted(merge_pairs, key=lambda val: val[2], reverse=False)
        byte_encoder = bytes_to_unicode()
        decoded_merge_pairs = [[self.token_bytes_to_string(val[0], byte_encoder), self.token_bytes_to_string(val[1], byte_encoder)] for val in merge_pairs]
        merges = [' '.join([''.join((chr(ord(c) + 256) if c == ' ' else c for c in part)) for part in pair]) for pair in decoded_merge_pairs]
        return merges