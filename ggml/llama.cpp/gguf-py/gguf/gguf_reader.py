...
from gguf.constants import GGML_QUANT_SIZES, GGUF_DEFAULT_ALIGNMENT, GGUF_MAGIC, GGUF_VERSION, GGMLQuantizationType, GGUFValueType, GGUFEndian
...
class GGUFReader:
    ...
    def _get_tensor_info_field(self, orig_offs: int) -> ReaderField:
        ...
        return ReaderField(orig_offs, str(bytes(name_data), encoding='utf-8'), [name_len, name_data, n_dims, dims, raw_dtype, offset_tensor], [1, 3, 4, 5])
    def _build_fields(self, offs: int, count: int) -> int:
        for _ in range(count):
            ...
            parts += field_parts
            self._push_field(ReaderField(orig_offs, str(bytes(kv_kdata), encoding='utf-8'), parts, [idx + idxs_offs for idx in field_idxs], field_types), skip_sum=True)
            offs += field_size
        return offs
    ...
    def _build_tensors(self, start_offs: int, fields: list[ReaderField]) -> None:
        ...
        for field in fields:
            ...
            tensors.append(ReaderTensor(name=tensor_name, tensor_type=ggml_type, shape=dims, n_elements=n_elems, n_bytes=n_bytes, data_offset=data_offs, data=self._get(data_offs, item_type, item_count).reshape(np_dims), field=field))
