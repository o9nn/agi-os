...
def plot(files: List[Path], output: Optional[Path]=None, test_regex: Optional[str]=None, server_regex: Optional[str]=None):
    ...
    for model in models:
        for temp in temps:
            ...
            row_vals = [data_dict.get((model, temp, server_name, test), np.nan) for server_name, test in all_cols]
    ...
    sns.heatmap(df, annot=True, cmap='RdYlGn', vmin=0.0, vmax=1.0, cbar=True, fmt='.2f', center=0.5, square=True, linewidths=0.5, cbar_kws={'label': 'Success Ratio'})
    ...
    with output.open('a' if append else 'w') as output_file:
        ...
        for t in [None] if temp is None else [t if t >= 0 else None for t in temp]:
                ...
                for server_name, server_path in servers:
                    ...
                    with scoped_server(server):
                        ...
                        for ignore_chat_grammar in [False]:
                            run(server, server_name=server_name, model_id=hf, temp=t, output_kwargs=dict(chat_template=chat_template, chat_template_file=chat_template_file), request_kwargs=dict(ignore_chat_grammar=ignore_chat_grammar))
            if ollama is not None:
                ...
                with scoped_server(server):
                    run(server, server_name='ollama', model_id=ollama, temp=t, output_kwargs=dict(chat_template=None, chat_template_file=None), request_kwargs=dict(model=ollama, max_tokens=n_predict, num_ctx=n_ctx))
