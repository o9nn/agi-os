...
def get_prompt_length(data: dict) -> int:
    ...
    response = session.post(f'{server_address}/apply-template', json={'messages': [{'role': 'user', 'content': data['prompt'], 'stream': True}]})
    response.raise_for_status()
    prompt: str = json.loads(response.text)['prompt']
    response = session.post(f'{server_address}/tokenize', json={'content': prompt, 'add_special': True})
    response.raise_for_status()
...
def send_prompt(data: dict) -> tuple[float, list[float]]:
    ...
    if data['external_server']:
        json_data: dict = {'prompt': data['prompt'], 'ignore_eos': True, 'seed': data['seed'], 'max_tokens': data['n_predict'], 'stream': True}
        response = session.post(f'{server_address}/v1/completions', json=json_data, stream=True)
    elif data['synthetic_prompt']:
        json_data: dict = {'prompt': data['prompt'], 'ignore_eos': True, 'cache_prompt': False, 'seed': data['seed'], 'n_predict': data['n_predict'], 'stream': True}
...
def benchmark(path_server: str, path_log: Optional[str], path_db: Optional[str], name: Optional[str], prompt_source: str, n_prompts: int, n_predict: int, n_predict_min: int, seed_offset: int):
    ...
    try:
        ...
        for i, p in enumerate(prompts):
            ...
            data.append({'session': session, 'server_address': server_address, 'external_server': external_server, 'prompt': p, 'synthetic_prompt': synthetic_prompts, 'n_predict': random.randint(n_predict_min, n_predict), 'seed': 3 * (seed_offset + 1000 * i) + 2 if seed_offset >= 0 else -1})
    ...
    logger.info(f'Average prompt latency:            {1000.0 * np.mean(prompt_t):.2f} ms')
    ...
    if path_db is not None:
        ...
        cursor = con.cursor()
        cursor.execute('CREATE TABLE IF NOT EXISTS server_bench(name TEXT, n_parallel INTEGER, prompt_source TEXT, n_prompts INTEGER, n_predict INTEGER, n_predict_min INTEGER, seed_offset INTEGER, runtime REAL);')
        cursor.execute('INSERT INTO server_bench VALUES (?, ?, ?, ?, ?, ?, ?, ?);', [name, parallel, prompt_source, n_prompts, n_predict, n_predict_min, seed_offset, token_t_last])
        con.commit()
    plt.figure()
    plt.scatter(prompt_n, 1000.0 * prompt_t, s=10.0, marker='.', alpha=0.25)
    plt.xlim(0, 1.05 * np.max(prompt_n))
    plt.ylim(0, 1050.0 * np.max(prompt_t))
    plt.title(name or '')
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Tool for benchmarking the throughput of the llama.cpp HTTP server. Results are printed to console and visualized as plots (saved to current working directory). To pass arguments such as the model path to the server, set the corresponding environment variables (see llama-server --help). The reported numbers are the speeds as observed by the Python script and may differ from the performance reported by the server, particularly when the server is fast vs. the network or Python script (e.g. when serving a very small model).')
    ...
    parser.add_argument('--prompt_source', type=str, default='rng-1024-2048', help="How to get the prompts for the benchmark, either 'mmlu' for MMLU questions or rng-MIN-MAX for synthetic prompts with random lengths in the interval [MIN, MAX]")
    ...
    parser.add_argument('--n_predict_min', type=int, default=1024, help='Min. number of tokens to predict per prompt (supported for synthetic prompts only)')
    parser.add_argument('--seed_offset', type=int, default=0, help='Offset for determining the seeds for pseudorandom prompt/generation lengths. Corelations between seeds can occur when set >= 1000. Negative values mean no seed.')
